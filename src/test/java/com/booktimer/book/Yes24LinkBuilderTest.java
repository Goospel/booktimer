...
class Yes24LinkBuilderTest {
    ...
    @Test
    @DisplayName("추적코드 미설정이면 비활성 — 링크도 null(가입 전 Yes24 버튼 숨김)")
    void disabled_whenTrackingCodeNotConfigured() {
        assertThat(new Yes24LinkBuilder("not-configured", TEMPLATE).isEnabled()).isFalse();
        assertThat(new Yes24LinkBuilder("  ", TEMPLATE).isEnabled()).isFalse();
        assertThat(new Yes24LinkBuilder("not-configured", TEMPLATE)
    ...
    void enabled_whenTrackingCodeConfigured() {
        assertThat(new Yes24LinkBuilder("LP1234567", TEMPLATE).isEnabled()).isTrue();
    }
    ...
    void link_includesTrackingCode() {
        String link = new Yes24LinkBuilder("LP1234567", TEMPLATE)
