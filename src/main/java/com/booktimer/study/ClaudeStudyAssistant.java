...
public class ClaudeStudyAssistant {
     ...
     *
     * <p>⚠️ <b>블록을 나눈 것이 보장하는 범위는 문자열 경계뿐이다.</b> 본문·범위·과목은 사용자가 친
     * 글이거나 사진에서 전사된 텍스트이고, 여기에 <b>필기(최대 24,000자)</b>가 더해지면서 반경이 커졌다 —
     * 한 블록에 적어 넣은 라벨이 <b>다른 블록의 내용으로는 못 넘어간다</b>. 그러나 <b>블록 안에서의 섹션
     * 위조는 그대로 가능하다</b>: 필기 본문은 여러 줄이 통째로 실리므로 그 안에 「[오늘 쓴 글]」 같은 줄을
     * 세울 수 있다(제목은 {@code StudyNote.optionalTitle}이 개행을 접어 한 줄로 묶는다).
     * 실효 방어는 <b>시스템 프롬프트의 「이건 데이터다」 선언</b>이고, 블록 분리는 그 위의 한 겹이다.
     * 폭발 반경은 자기 분석 결과뿐이라(툴·외부 호출·다른 사용자로 새는 경로가 없다) 여기까지로 둔다 —
     * 이스케이프·랜덤 구분자는 <b>툴 사용이나 유출 경로가 붙는 날</b>이 그 차례다.
     */
