 ...
 *
 * <p><b>진행 중 독서 세션이 있으면 공부 시작을 거부</b>한다 — 두 원장이 같은 시간을 이중으로 세지 않게
 * 하는 자리다. <b>역방향도 같다</b>({@link ReadingSessionService#start}) — 2026-09-01엔 웹이 공부를 몰라
 * 「역방향은 넣지 않는다」로 뒀지만, 웹 대시보드에 독서/공부 토글이 붙어 그 전제가 사라졌다. 이제 어느
 * 쪽으로 시작하든 서버가 막는다.
 */
@Service
@Transactional
