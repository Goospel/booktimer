...
public enum RateLimitAction {
    ...
    /**
     * 회원가입({@code POST /signup})의 <b>클라이언트 IP</b>별 상한. 정상 사용자는 평생 1회다.
     *
     * <p>겨누는 것은 계정 양산 자체보다 <b>이 폼이 임의 주소로 메일을 내보내는 문</b>이라는 점이다 —
     * 가입 성공이면 인증 메일이, 이미 있는 이메일이면 「이미 계정 있음」 통지가 그 주소로 나간다. 상한이
     * 없으면 남의 메일함에 우리 도메인 메일을 무제한으로 꽂을 수 있고, 그건 발신 평판(SES)까지 태운다.
     *
     * <p><b>시간당 10으로 넉넉히 잡은 이유</b>: 국내 이동통신은 CGNAT라 한 공인 IP를 다수가 공유한다.
     * 촘촘히 잡으면 공격자가 아니라 같은 NAT 뒤의 정상 신규 사용자가 가입을 못 한다. 목적은 정밀 차단이
     * 아니라 폭주의 상한이고, 고정 윈도우라 한 시간이면 저절로 풀린다.
     */
    SIGNUP(10, Duration.ofHours(1)),

    /**
     * 비밀번호 재설정 요청({@code POST /password/forgot})의 <b>클라이언트 IP</b>별 상한 —
     * 임의 주소로 메일을 내보내는 문이라는 점에서 {@link #SIGNUP}과 같은 성질이고 한도도 같다.
     */
    PASSWORD_FORGOT(10, Duration.ofHours(1)),

    /**
     * 비밀번호 재설정 요청의 <b>정규화한 이메일</b>별 상한 — <b>한 주소로 나가는 발송량 상한</b>이다.
     * IP 키({@link #PASSWORD_FORGOT})만으로는 출처를 분산하면 한 사람의 수신함에 재설정 메일을
     * 무제한으로 꽂을 수 있다. 피해자 수신함과 SES 발신 평판을 지키는 것이 이 키의 실익이다.
     *
     * <p>⚠️ <b>토큰 무효화로 인한 링크 봉쇄는 이 키로 끊지 못한다.</b> {@code EmailTokenService.issue}가
     * 직전 미사용 토큰을 죽이므로, 공격자는 시간당 3회만으로도 피해자의 재설정 링크를 계속 무효화할 수
     * 있다. 게다가 그 3회를 다 쓰면 <b>피해자 본인의 요청이 {@code ?limited}로 거부</b>돼 한 시간 동안
     * 재설정 자체가 잠긴다 — 상한이 없던 옛 코드에서는 시간당 3회짜리 공격자가 거의 무해했다.
     * 즉 이 키는 봉쇄를 <b>없앤 게 아니라 옮겼다</b>: 발송량 상한을 얻는 대신 저빈도 공격자에게 잠금
     * 수단을 준 <b>수용한 트레이드오프</b>다.
     *
     * <p><b>근본 해소는 상한이 아니다</b> — {@code issue}가 직전 토큰을 죽이는 것을 그만두는 것이다
     * (유효 토큰 복수 허용 + TTL 만료). 별도 결정으로 남긴다.
     *
     * <p>키는 {@code trim().toLowerCase(Locale.ROOT)}로 정규화한다 — 안 하면 대소문자·공백만 바꿔 상한을
     * 우회한다. 존재하지 않는 이메일도 <b>똑같이</b> 센다: 상한 반응이 계정 존재 여부에 따라 갈리면 그
     * 자체가 열거 채널이 된다.
     *
     * <p>시간당 3회 — 정상 사용자가 한 시간에 재설정 메일을 세 번 넘게 조를 일은 없다.
     */
    PASSWORD_FORGOT_EMAIL(3, Duration.ofHours(1));
