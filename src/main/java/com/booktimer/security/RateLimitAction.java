...
public enum RateLimitAction {
    ...
    /**
     * 비밀번호 재설정 요청의 <b>정규화한 이메일</b>별 상한 — IP 키({@link #PASSWORD_FORGOT})만으로는
     * 못 막는 <b>피해자 지목 공격</b>을 끊는다.
     *
     * <p>{@code EmailTokenService.issue}는 직전 미사용 토큰을 즉시 죽인다. 그래서 공격자가 IP를 바꿔
     * 가며 피해자 주소로 반복 요청하면, 피해자가 방금 받은 재설정 링크가 <b>열어 보기도 전에 계속
     * 무효화</b>된다 — 비밀번호를 잊은 사람이 영영 못 들어오는 봉쇄다. IP당 상한은 분산 출처 앞에서
     * 무력하므로, 셀 수 있는 다른 키인 <b>대상 이메일</b>로 한 겹 더 센다.
     *
     * <p>키는 {@code trim().toLowerCase()}로 정규화한다 — 안 하면 대소문자·공백만 바꿔 상한을 우회한다.
     * 존재하지 않는 이메일도 <b>똑같이</b> 센다: 상한 반응이 계정 존재 여부에 따라 갈리면 그 자체가
     * 열거 채널이 된다.
     *
     * <p>피해자 1명당 시간당 3회 — 정상 사용자가 한 시간에 재설정 메일을 세 번 넘게 조를 일은 없다.
     */
