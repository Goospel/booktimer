...
public enum RateLimitAction {
     ...
     *
     * <p><b>링크 봉쇄는 이 키가 아니라 토큰 쪽에서 끊었다.</b> 예전엔 {@code EmailTokenService.issue}가
     * 직전 미사용 토큰을 죽여서, 공격자가 시간당 3회만 요청해도 피해자가 손에 든 재설정 링크가 계속
     * 무효화됐다. 지금은 발급이 직전 토큰을 건드리지 않고(유효 토큰 복수 허용 + TTL 만료) 무효화는
     * <b>성공 소비 시점</b>에 하므로 그 봉쇄 자체가 없다.
     *
     * <p>⚠️ 남는 것은 <b>쿼터 선점</b>이다 — 공격자가 시간당 3회를 먼저 소진하면 피해자 본인의 요청이
     * 그 시간 동안 {@code ?limited}로 거부된다(재설정은 한 시간 뒤 또는 이미 받은 링크로 가능). 발송량
     * 상한을 얻는 대가이며 <b>수용한 트레이드오프</b>다.
     *
