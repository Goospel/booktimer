...
/**
 * 정원 꾸미기(배치) 유스케이스 — 보유 식물을 격자 캔버스에 놓고 저장한다.
 *
 ...
 *
 * <p><b>위조 방어</b>: {@link #save}는 미보유 식물·격자 범위 밖·중복 셀/식물 요청을 거부한다(설계 §4).
 */
@Service
public class GardenLayoutService {
    ...
    /** 격자 셀 총수(0..GRID_CELLS-1). */
    public static final int GRID_CELLS = GRID_COLUMNS * GRID_ROWS;
     ...
     * 식물 메타(이모지·이름)는 보유 식물 풀에서 결합하고, 결과는 셀 인덱스 오름차순으로 돌려준다.
     *
     ...
     * 저장 전 전수 검증: 미보유 식물(위조)·격자 범위 밖·중복 셀/식물을 거부한다(설계 §4). 검증 실패 시 저장 전체가 무산된다.
     */
    @Transactional
    public void save(User user, List<PlacementRequest> requests) {
        ...
        for (PlacementRequest r : requests) {
            ...
            if (r.cell() < 0 || r.cell() >= GRID_CELLS) {
                throw new IllegalArgumentException("격자 범위를 벗어난 셀입니다: " + r.cell());
            }
            ...
            if (!seenCells.add(r.cell())) {
                throw new IllegalArgumentException("한 셀에는 식물 하나만 놓을 수 있습니다: 셀 " + r.cell());
            }
        ...
        for (PlacementRequest r : requests) {
            placementRepository.save(GardenPlacement.of(user, r.axis(), r.code(), r.cell()));
        }
    }
