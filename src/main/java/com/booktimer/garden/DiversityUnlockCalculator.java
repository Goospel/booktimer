...
/**
 * 작가 다양성 식물 보유 유도 — 순수 계산(DB·Spring·시간 무관).
 *
 ...
 * 한다(부채 모델 N-058). 두 단계:
 * <ol>
 *   <li>{@link #distinctCount} — 완독책의 작가 목록에서 서로 다른 값의 수를 센다.
 *       {@code Book.author}는 적재 시 정규화되지 않으므로(category와 달리 blankToNull 없음)
 *       여기서 strip + 빈/null 제외를 직접 한다(미완성 메타 누수 가드 N-055).</li>
 *   <li>{@link #resolve} — 카탈로그 각 식물의 카운트로 임계 도달을 판정한다.</li>
 * </ol>
 */
public final class DiversityUnlockCalculator {
     ...
     *
     * @param values 완독책의 작가 목록(null 원소 허용 — 수동 등록 책은 작가가 null일 수 있음)
     ...
     *
     * @param catalog     작가 다양성 식물 카탈로그(진열 순서)
     * @param authorCount 완독 distinct 작가 수
     * @return 식물별 보유 상태. authorCount로 임계 판정.
     */
    public static List<DiversityPlantState> resolve(List<DiversityPlant> catalog, int authorCount) {
        return catalog.stream()
                .map(plant -> {
                    boolean owned = plant.getThresholdCount() >= 1 && plant.getThresholdCount() <= authorCount;
