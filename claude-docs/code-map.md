...
- **📌 책 고르기 하단 시트(§6.5, 발견 1 확장)**: 시작 진입은 드롭다운이 아니라 **표지 칩**(`BookPickForm.vue` — 기본=최근 읽은 책, "측정 시작"1탭 + "바꾸기"→시트 + "책 없이 시작"). 책 고르기·종료 후 태깅은 **하나의 시트** `BookPickSheet.vue`가 `mode`(`start`/`tag`)로 겸용 — 검색(돋보기)해서 담기(`/api/books/search`·`POST /api/books` 재사용, 담으면 `owned` 전환+`added` emit)·상태 필터칩·표지 목록(34×48). 시트는 열릴 때 `/api/books`로 표지·`searchEnabled` 로드(실패 시 props 제목-only 폴백). **백엔드 0**(기존 계약 재사용). 새 색은 시트 `#F6F2E9`·백드롭 `rgba(34,32,26,.34)`(설계 §6.5/§8 지정)만.
- **프론트**: `frontend/src/dashboard/`(`TimerCard.vue`, `BookPickForm.vue`(표지 칩), `BookPickSheet.vue`(통합 책 시트: 시작 고르기+종료 태깅·검색담기), `DashboardApp.vue`, `useReadingTimer.ts`, `timerProgress.ts`, `ContributionGraph.vue`) · 표지색·이니셜·상태배지는 `frontend/src/books/pure.ts` 재사용(교차 아일랜드)
