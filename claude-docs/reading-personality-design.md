...
| **출간연대 신호 제거** | `pubDecades` 필드 및 LLM 주입·시그니처에서 완전 제거 | ✅ 2026-06-09 | `pubDate`는 에디션 인쇄일 — 고전 재판이 현대작으로 오판, 저자·장르로 충분 (§8 참고) |
...
- ~~**신/구**: 알라딘 `pubDate` → 출간연도 분포(최신작↔고전)~~ **[제거됨 2026-06-09]** — 알라딘 `pubDate`는 에디션 인쇄일이지 작품 최초 출간일이 아니다. 고전 재판(죄와 벌 1866→2015 인쇄)이 "2010년대 출간작"으로 오판돼 LLM이 확신형 오서술을 생성. 저자·장르가 "고전 독자"를 이미 정확히 말해주므로 신호 제거. 작품 시대를 진짜 쓰려면 원작 출간연도를 새 소스로 보강해야 한다(별도 백로그).
...
- ~~출력 **태그 체계**(축·라벨 네이밍) — ②매칭 본격화 때 확정. v1은 LLM 자유 태그로 저장만.~~ → **2026-06-10 확정**: 알라딘 대분류 → 8종족(`ReadingTribe`) 닫힌 어휘. 3축 결정적(`ReadingTagger`): 장르 정체성(1~2개)·폭(외길형/잡식가/균형형)·저자 충성도(한우물형). LLM 태그 대신 온더플라이 계산(`PersonalityView.from`). 매핑표 초안 — 운영 distinct 대분류로 검증 예정. (Phase 6b 1단계, PR #281)
...
- ~~(후속) 공개용 결과 생성 트리거 — 책 공개 토글 시 재생성? "공개 성향 보기" opt-in?~~ → **해소**: §10 PR #241에서 공개/비공개 분기를 폐지하고 **공개+완독 책 기반 단일 생성·항상 노출**로 확정.
...
6. **(후속) 공개용 생성 + 사람 추천(매칭)** — PUBLIC 책 기반 결과 + 태그로 비슷/정반대 추천. §전략 밀도 신호 뒤.
   ...
   - 🔁 **분석 히스토리(최대 3개) + 대표 고르기(2026-06-08, PR #246)** — 사용자 피드백: "책 추가 후 다시 분석했는데 성향이 안 바뀐 것 같다. 비슷한 책이라 비슷한 값인지, 정말 안 바뀐 건지 비교가 안 된다. 하루 3번뿐이라 또 못 돌린다." 근본 원인은 Phase 4의 **단일 캐시(`reading_personality` user_id unique = 1행/유저)** — "다시 분석"이 같은 행을 덮어써 과거 서술이 소실돼 비교 자체가 불가능했다. **모델을 최대 3행/유저 + `selected`(대표) 플래그로 확장**(유저당 selected 0~1). 이로써 위 §126·Phase 4·6a의 "user_id unique·단일 캐시·1행" 서술은 **이 항목으로 대체**된다(근거는 보존). 대표 = **공개 책방 노출 + 교체에서 보호**. 사용자 결정 3: ①고른 게 책방 대표+삭제 보호 ②"다시 분석"은 후보로만 추가(자동 대표 X, 기존 선택 유지) ③새 행은 "다시 분석" 버튼에서만 쌓임(단순 GET 방문은 교체 안 함). 서비스 3진입점: `currentPersonality`(GET=대표 읽기, 히스토리 비고 책 충분하면 첫 1개 부트스트랩=자동 대표, **책장 바뀌어도 자동 재생성 안 함** — Phase 4의 "시그니처 변동이면 GET에서 재생성"을 폐기, 생성은 명시적 행동에서만) · `reanalyze`("다시 분석"=후보 추가 후 4행이면 대표 뺀 가장 오래된 후보 1개 교체, LLM 실패 시 stale 대표 유지·새 행 X = serve-stale 유지) · `select`(대표 변경, LLM 無, **본인 행만=IDOR 가드**). `history`는 최신순 + 대표 + **stale**(이 분석 이후 책장 바뀜) 표시로 "안 바뀐 건가?" 혼란을 분석 시각·stale 뱃지로 해소. 교체 정렬은 `generated_at` + **id tiebreak**(같은 초도 결정적). 공개 책방은 `findByUserAndSelectedTrue`로 대표만 읽는다. **V29**: user_id unique 드롭이 MySQL(DROP INDEX)/H2(DROP CONSTRAINT)로 갈리는 함정(V26 주석) 회피 위해 CREATE+INSERT…SELECT+DROP+RENAME로 테이블 재생성(기존 1행→`selected=true` 이관). TDD Red→Green: 도메인 히스토리 10·GET경로 6·뷰 1·컨트롤러 3 + 전체 그린(FlywayMigrationTest로 V29 H2 검증).
   - ✅ **Phase 6b 2단계 책방 헤더 태그 노출(2026-06-10, PR #282)** — #281 결정적 태거 출력을 본인 화면(`/personality`) 외에도 책방 `/u/{loginId}` 상단 박스(닉네임/@핸들/팔로우 카운트 묶음)에 칩 행으로 노출. `ProfileView`에 `personalityTags` 필드 추가, `ProfileService`에 `ReadingProfileService` 주입 후 `ReadingTagger.tagsOf(publicProfileOf(target))` 호출(입력은 공개 책만 — §7 누출 차단), 컨트롤러 모델 적재, `profile.html`은 기존 `.tag-chip-row`/`.tag-chip` 재사용(CSS 신규 0). 서술 캐시와 독립이라 LLM 서술이 없어도(미부트스트랩·LLM 실패) 태그는 사실에서 직접 뜬다(견고성↑). 빈 목록이면 행 자체 숨김(`th:if`로 공개 완독 0권/매핑 장르 없음 콜드스타트 가림). 친추 매칭(태그 집합 겹침)은 다음 단계로 분리.
   - ✅ **Phase 6b 3단계 태그 클릭 → 근거 책 드릴다운(2026-06-10, PR #283)** — 책방 헤더의 칩을 **클릭하면 그 태그를 만든 공개+완독 책 목록**을 칩 아래에 htmx로 인라인 펼친다. "왜 이 태그를 받았는가"를 방문자가 직접 확인 → 태그의 신뢰·재미 동시 강화. 클릭 가능 분류: **장르 종족 + 한우물형** 칩만(폭 태그=외길형·잡식가·균형형은 부분집합 정의 안 됨 → 비클릭, span 유지). 신규 순수 도메인 `PersonalityTagAttribution.booksForTag`(공개 책 입력 → 완독만 추리고 장르 라벨→`ReadingTribe.ofLabel`→그 종족 대분류 책 / `AUTHOR_LOYAL_TAG`→최다독 저자 책 동률 시 저자명 오름차순으로 단일 선택). 도메인 노출: `ReadingTribe.ofLabel` 역매핑, `ReadingTagger.AUTHOR_LOYAL_TAG` 상수화(분류·귀속이 같은 상수 참조), `ReadingProfileAggregator.primaryGenre` public화(대분류=2번째 세그먼트 단일 소스 공유). 배선: `profile/ProfileTag(label, clickable)` record, `ProfileView.personalityTags`가 `List<String>`→`List<ProfileTag>`로(#282 후속), `ProfileService.resolveVisibleTarget`로 가드 체인(운영자·차단·없는 아이디) 추출해 `profileOf`와 신규 `booksForPersonalityTag(viewer, loginId, tag)`가 **공유** — 같은 404 보장 + PRIVATE 책 누출 차단(공개 책만 입력 → 결과에 안 샘) 일관. 핸들러 `GET /u/{loginId}/personality-tag?tag=…` → `profile :: tagBooksPanel` 프래그먼트, `profile.html` 칩이 clickable이면 `<a hx-get>`로 `#tag-books-panel` swap. **범위**: 책방(`/u/{loginId}`)만 — `/personality` 본인 화면 클릭은 후속. **불변**: 공개+완독만 노출 / 새 경로도 책방과 동일 가드 재사용(차단·404·ADMIN). Flyway·태거 규칙·LLM 무관. TDD Red→Green: `PersonalityTagAttributionTest`(장르 합산·완독 필터·미매핑 제외·한우물형 동률 결정성·null/blank·알 수 없는 태그·빈 입력) + `ProfileControllerTest`(공개 완독→근거 책·PRIVATE 누출 차단=N-055·운영자/차단/없는 아이디 404·비로그인 리다이렉트·한우물형 라우팅·알 수 없는 태그→빈 목록 200·칩 clickable 분류).
