...
| 공허한 테스트 — 통과하는데 겨눈 규칙을 지워도 안 깨진다(검사 키가 남의 문자열과 겹치거나, 매처가 그 실패를 안 매칭하거나, 하니스가 그 동작을 아예 안 돌림) | T-144(정적 마크업 `toContain` × TDS 주입 CSS) · T-145(Mockito `never()`+`anyString()`이 null 미매칭) · T-149(정적 렌더가 effect 미실행 → 부정 카운트 단언 상시 통과) · T-161(테스트 스키마에 그 DB 제약이 아예 없음) · T-180(목 모드가 fetch를 안 태워 네트워크 탭 요청 계수가 항상 0) · T-181(`indexOf` 순서 단언이 대상 부재 시 `-1`로 상시 참) | 6 | ⚠️ **prose 승격 유지**(글로벌 CLAUDE.md·implementer 규약의 「돌연변이 확인」이 담당 — T-144·T-145는 돌연변이 확인이 잡아냈고, **T-149(3회차)는 같은 규약 덕에 작성 전에 사전 회피**됐다. 규칙이 세 번 연속 작동해 새 계단은 여전히 불필요). **T-161(4회차)은 탐지 경로가 다르다** — 돌연변이 확인이 아니라 **설계 문서가 깐 전제("H2가 실 DB 제약을 탄다")를 구현자가 의심해** 작성 전에 회피했다. 즉 이 군의 방어선은 돌연변이 확인 하나가 아니라 「계측기가 그 실패를 볼 수 있는가」를 **쓰기 전에 묻는 습관**이고, 그건 정규식으로 잡을 수 없어 계단을 올릴 대상이 아니다. **T-181(6회차)은 세 번째 탐지 경로다** — RED 단계에서 「예상 실패 수 ≠ 실제 실패 수」라는 산술이 잡았다(돌연변이 확인은 GREEN 이후라 이보다 늦다). 「RED를 봤다」가 아니라 「예상한 것이 전부 죽었나」를 세는 것이 이 군의 가장 이른 방어선이다. **hookify warn 후보** `never()`+`anyString()`/`anyLong()` 동시 등장 감지는 보류 유지 — 3회차(T-149)는 그 조합이 아니라 별개 메커니즘(정적 렌더 × 부정 단언)이라 정규식 한 줄로 못 잡는다; Mockito 조합이 재출현하면 그때 승격. ⚠️ 묶는 근거는 도구(vitest/Mockito)가 아니라 **"통과가 증거가 아닌 상태"**와 그 유일한 탐지 수단이 **돌연변이 확인**이라는 점이다 |
