...
| 브라우저 자동화가 읽기(스크린샷·접근성 트리)는 되는데 **입력만** 타임아웃 — 원인이 코드가 아니라 그 탭·패널의 상태다 | T-154(클로드 내부 패널 미개방) · T-179(데스크톱 크롬 기기 모드) | 2 | ⚠️ prose(T-179 재발방지) — **새 탭으로 갈아타면 즉시 풀린다**는 진단·해결이 한 수라 하드픽스 이득이 작다. 3회차면 「검증 시작 시 전용 탭을 새로 연다」를 절차로 승격 |
