...
| 테스트 스키마 출처 착각 — 마이그레이션에만 있는 테이블·제약은 메인 스위트(Hibernate 생성)에 **존재하지 않아 위반될 수도 없다**(무성 그린 / 영영 안 되는 RED) | T-168(테이블·FK) · T-169(제약·UNIQUE) | 2 | ⚠️ **미승격** — T-168은 자기 결함에 하드픽스(FK 집합 양방향 대조)를 뒀지만 그건 *그 결함* 전용이고, 「검증을 어느 스위트에 둘 것인가」라는 **자리 선택 자체를 강제하는 가드는 없다**(그래서 같은 날 제약 면에서 또 밟았다). 승격 후보 = 마이그레이션 파일 스테이징 시 hookify **warn** 룰(① 검증은 `FlywayMigrationTest`에 ② 버전 번호는 `origin/main` 기준 — T-170도 같은 트리거라 한 룰이 둘을 덮는다). 사용자 승인 대기 |
