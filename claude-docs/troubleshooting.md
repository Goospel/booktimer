...
| 2026-09-16 | T-251 (**브라우저 실측에서 서비스워커가 옛 번들을 먹여 「고친 게 안 먹었다」는 거짓 판정을 만든다 — 차단은 첫 이동이 아니라 매 이동에 건다** / **1회차** / 증상: #1147의 첫 페인트 수정을 확인하는데 20ms 간격 샘플링이 고치기 전과 똑같은 구간(112~354ms `data-mode=reading`)을 그대로 냈다. 수정은 실제로 코드에 들어가 있었고 서버도 새 파일을 서빙하고 있었다. / 원인: `navigate_page`의 `initScript`는 **그 내비게이션에만** 적용된다. 첫 이동에서만 서비스워커 등록을 막아 두면 이후 이동에서 페이지가 `sw.js`를 다시 등록하고, 그 캐시(`shell-v8`)가 옛 `dashboard/dashboard.js`를 돌려준다. 페이지가 받은 번들은 **613,132자**, 레포 파일은 **613,143자**(11자 차이). / 감별에서 두 번째로 틀릴 뻔한 것: `fetch(...).then(r => r.text()).length`는 **문자 수**이고 `wc -c`·파일 크기는 **바이트 수**다(한글이 있어 617,744바이트 = 613,143자). 바이트와 문자를 맞대면 멀쩡한 번들도 「다르다」로 보인다 — 같은 단위로 비교해야 한다. / 해결: unregister + `caches.delete` 후 측정 경로의 **모든 내비게이션**에 서비스워커 차단 `initScript`를 붙이고 `ignoreCache: true`로 다시 쟀다. `reading` 샘플이 인라인 부트 실행 전(26·41ms) 2건뿐이고 그 뒤로는 끝까지 `study` — 수정이 실제로 먹은 것이 확인됐다. / 재발방지: ① 측정 세션의 서비스워커 차단은 첫 이동이 아니라 **매 이동**에 건다. ② 「고쳤는데 그대로다」가 나오면 앱을 의심하기 전에 **지금 브라우저가 받은 자산이 저장소 파일과 같은지**부터 같은 단위로 대조한다. / **관련**: T-071·T-075·T-080(SW stale 캐시 군 — 그 셋은 배포 후 서빙, 이 항목은 **실측 세션의 관측 경로**) · 글로벌 `browser-verify` 스킬 「관측 경로를 먼저 의심한다」) |
