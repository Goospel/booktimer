...
| TDS 컴포넌트가 넘긴 `style`의 키를 걸러낸다 — 소스엔 있는데 DOM엔 없어 정렬·표시가 조용히 죽는다 | T-198(기록 화면 `textAlign: 'right'`) · **T-216(빈 상태 안내 5곳 `textAlign: 'center'`)** | **2** | ⚠️ **1회차 등재가 2회차를 못 막았다** — T-198은 자기 한 곳만 고치고 **같은 패턴의 나머지를 훑지 않아** 다섯 자리가 그대로 남았다. 2회차 처방은 둘: ① 이 함정을 만나면 **리포 전체에서 `<Text` + `textAlign`을 훑는다**(자리 하나가 아니라 군으로 닫는다) ② T-198이 「단위 테스트가 원리상 못 잡는다」고 적은 것을 **가드로 뒤집었다** — 걸러진 키는 못 세도 **해결 형태(래퍼 div의 `text-align:center`)는 정적 마크업에 남아** 계측된다. ⚠️ 단 창을 「가장 가까이 감싼 div」로 좁혀야 한다(통짜 `toContain`은 격자·탭에 걸려 상시 통과, 글자 수 창은 TDS `<style>` 블록에 먹힌다). 훅 승격 불가 — 부모 상속까지 봐야 죽은 키인지 정해져 정적 판정이 안 된다 |
