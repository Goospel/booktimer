...
| 검증 환경이 실환경보다 관대해 그 클래스가 가짜 green으로 통과(헤드리스 로드순서 → 데스크톱 크롬 ↔ 실기기 → 정적 렌더 ↔ 실 브라우저) | T-053 · T-054(헤드리스가 `defer` 단순화) · T-176(합성·성능) · T-177(iOS UA 기본 글자색) · T-182(정적 렌더에 effect·레이아웃이 없다) | 5 | ✅ CLAUDE.md 「🖥️ 프론트 검증」 — 2회차에 **실 브라우저 게이트**(N-082·N-083), 4회차에 **미니앱 실기기 게이트 + 「미검증 가정은 머지 전 체크리스트」**로 확장. ⚠️ 3·4회차가 말하는 건 축이 하나 더 있다는 것 — 「헤드리스 vs 실 브라우저」로 닫았더니 **「데스크톱 크롬 vs 실기기」**에서 두 번 샜다(성능 여유 / 엔진별 UA 기본값). **5회차(T-182)는 승격이 작동한 사례다** — 미니앱 하니스는 jsdom이 없어 effect·레이아웃·스크롤이 아예 없는데(그 클래스는 단위로 못 잡는다), 이미 세워 둔 실 브라우저 게이트가 오버레이 결함 2건을 잡아 머지 전에 고쳤다. 훅 승격 불가(정적 감지 대상이 아님) — 판단 규칙이 유일한 계측기라, 그 환경에서만 갈리는 축(합성·메모리·UA 기본값·safe-area·effect·레이아웃)을 만나면 그 자리에서 실 브라우저·실기기 확인을 게이트로 세운다 |
