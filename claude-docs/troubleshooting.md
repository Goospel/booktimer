...
| 2026-08-13 | T-153 (**`deploy.sh --expect`에 코드 조각을 주면 minify가 변형해 검증이 헛돈다 — 마커는 압축 불변 문자열 리터럴이어야 한다** / 증상: 잔디 방향 핫픽스 배포에서 `--expect "slice(0,15)"`로 걸었더니 클린 빌드가 성공했는데도 "번들에 없다"로 배포가 차단됐다 — 소스엔 그 코드가 분명 들어 있다. / 원인: vite minify가 숫자 상수를 호이스팅·개명해 `slice(0,15)`가 번들에서 `slice(0,EGe)` 형태로 변형됐다. `--expect`는 dist 산출물을 그대로 검색하므로 식별자·숫자 인자·공백을 포함한 **코드 조각은 압축 후 형태가 보존되지 않아** 소스 기준 문자열과 어긋난다. / 해결: dist를 직접 열어 변형된 실제 형태를 확인한 뒤, 압축이 건드리지 않는 마커(사용자 노출 한글 UI 문구)로 바꿔 재배포. / 재발방지: `--expect` 마커는 **문자열 리터럴(한글 UI 문구)**로 고른다 — minify는 문자열 리터럴의 내용은 변형하지 않는다. 코드 조각은 금지. CLAUDE.md 「미니앱 개발 루프」 절에 한 줄 승격 기재. 1회차 신규 — "검증기가 신호를 못 보는" 결이 T-150(비변별 마커)과 닿지만 방향이 반대다(저쪽은 어느 번들에나 있어 통과, 이쪽은 새 번들에 있는데 실패). 미니앱 잔디 핫픽스 배포 중 발생) |
