...
| 도구 재생성 시 BOM/EOL 미보존 → phantom diff(첫 줄·전체 줄끝) | T-093 · T-103 · T-172 · **T-209 곁다리** | **4** | ⚠️ **기존 승격(코드 패턴)이 3회차를 못 막았다** — 그건 「우리가 짜는 PowerShell 재생성기」에만 적용되는 규약이라, **남이 만든 도구**(`sed -i`)에는 걸 자리가 없다. T-172는 `find -exec sed -i`가 **패턴이 안 맞는 파일까지 전부 다시 써서** 42개 템플릿의 EOL이 뒤집힌 것(git 정규화가 흡수해 무해했지만, `.gitattributes`가 없는 레포였다면 42파일 phantom diff). 승격 후보 = 「`sed -i`를 `find -exec`/글롭으로 다중 파일에 돌리면 경고」 hookify warn 룰(대상 한정 치환은 Edit 도구나 파일별 루프로). ⚠️ **4회차(T-209 곁다리)는 그 승격 후보가 안 잡는 자리에서 났다** — `sed -i`가 아니라 **세션이 그 자리에서 짠 python 일회용 스크립트**가 `io.open(...,'w')`로 다시 쓰며 CRLF를 LF로 눕혔다. 즉 이 군의 뿌리는 도구 이름이 아니라 **「읽고 다시 쓰는 모든 경로」**다. 그래서 hookify 룰은 여전히 보류하고(일회용 스크립트의 파일 쓰기는 정규식으로 못 겨눈다) 규약을 한 줄로 좁힌다: **파일을 통째로 다시 쓸 땐 `newline=''`로 읽고 같은 개행으로 쓴다.** 곁다리 피해가 phantom diff에 그치지 않은 것도 이번이 처음이다 — 개행이 바뀌자 **뒤이은 앵커 매칭이 조용히 전부 빗나가** 원인 추적이 길어졌다 |
