...
| 트랩군 | 발생 T-### | 회차 | 승격 상태 |
|---|---|---|---|
| PR 머지 자동화(`pr-merge.sh`) — 백그라운드 hang·미완료·헛폴링 | T-083 · T-088 · T-091 · T-094 | 4 | ✅ 하드픽스(`pr-merge.sh`: gh API 브랜치삭제 + `timeout`, T-094) + 절차(머지=완료확인) + CLAUDE.md Git워크플로 DIRTY 진단 |
...
| CSS 주석 속 `*/`(특히 wildcard-slash `.foo-*/.bar-*`)가 주석 조기 종료 → 다음 규칙 침묵 드랍 | T-087 | 2 (#522대 .dash-card → 이 PR .book-*/.record-*) | ✅ 예방 규칙(주석에 wildcard-slash 금지, 슬래시→`·`/`과`) + §11 실 브라우저/static-preview 게이트가 침묵 드랍 포착(N-118). 하드픽스 훅은 tight 주석 `/*x*/`·인라인 `/* c */color` false-positive로 보류 |
