...
| 도구 재생성 시 BOM/EOL 미보존 → phantom diff(첫 줄·전체 줄끝) | T-093 · T-103 · T-172 | 3 | ⚠️ **기존 승격(코드 패턴)이 3회차를 못 막았다** — 그건 「우리가 짜는 PowerShell 재생성기」에만 적용되는 규약이라, **남이 만든 도구**(`sed -i`)에는 걸 자리가 없다. T-172는 `find -exec sed -i`가 **패턴이 안 맞는 파일까지 전부 다시 써서** 42개 템플릿의 EOL이 뒤집힌 것(git 정규화가 흡수해 무해했지만, `.gitattributes`가 없는 레포였다면 42파일 phantom diff). 승격 후보 = 「`sed -i`를 `find -exec`/글롭으로 다중 파일에 돌리면 경고」 hookify warn 룰(대상 한정 치환은 Edit 도구나 파일별 루프로). 4회차 나오면 확정 |
