...
| 규칙을 **설명하는 주석**이 그 규칙의 **소스 문자열 계측**을 깨뜨린다(구현은 옳은데 테스트가 붉어짐) | T-203(css 전환 규칙 → 여백 진입 게이트) | 2 (같은 세션 안, 2026-08-22) | ✅ T-203 등재 + 계측 전 주석 제거를 절차로. **3회차면 `readSourceWithoutComments()` 공용 헬퍼로 승격**(지금은 두 곳뿐이라 각자 처리) |
