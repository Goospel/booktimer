...
| **bootRun이 서빙하는 정적 리소스가 소스보다 낡다 — 실체는 `build/resources/main` 복사본** | T-187(① 복사본 미갱신 · ② 해시 URL) · **2026-09-11 #1098 재발(①, 가짜 RED)** | **2** | ⏸ **미승격(2026-09-12)** — 후보는 「e2e 스펙이 서빙 CSS와 소스를 대조해 낡으면 실패」인데 전제(`processResources`가 바이트 그대로 복사)를 **Docker 미기동으로 못 재서** 보류(T-187에 CHECK/EXPECT/EVIDENCE로 원장화). 그 전까지 감별은 `curl /css/app.css`에 방금 넣은 문자열 유무. |
