...
| **미니앱 번들이 이번 소스·lock이 아닌 옛 입력으로 조용히 빌드된다 — 산출물 검사로는 신·구를 못 가른다** | T-150(① 빌드 무성 no-op → 옛 dist) · **T-244(② 정션이 가리키는 main의 옛 `node_modules` → 옛 SDK)** | **2** | ✅ **하드픽스** — `miniapp/deploy.sh` ① 클린 빌드 + 이번 릴리스 마커(T-150) · ⓪ **빌드 전에** 직접 의존성 설치본을 `package-lock.json`과 대조해 다르면 차단(T-244, 2026-09-14). 스모크 `miniapp/tests/test-deploy-gate.sh` Case 6·8. ⚠️ 두 회차 모두 번들 **내용** 검사(마커·env·해시)는 통과하는 모양이었다 — 옛 입력으로 만들어도 문구는 그대로다. 3회차가 나오면 「빌드에 들어간 입력」 중 아직 대조하지 않은 것(env 파일·전역 도구 버전)부터 본다. |
