...
| 트랩군 | 발생 T-### | 회차 | 승격 상태 |
|---|---|---|---|
| changelog 행이 rebase에서 중복 생성 — `merge=union`이 「한 브랜치의 추가→수정」을 양쪽 다 남긴다 | **T-210** | **2** | ✅ **하드픽스**(`require-changelog-no-dup.ps1` — `git push` 게이트, 같은 날짜+같은 굵은 제목 행이 둘 이상이면 차단. 테스트 8건, 비인접 중복 포함). ⚠️ 2회 모두 **같은 세션 안에서** 났다 — 리뷰 반영이 있는 PR은 「기능 커밋에서 행 추가 → 리뷰 커밋에서 그 행 보강」이 되어 이 구조에 자동으로 들어간다. 원천인 T-098(merge=union)은 **여전히 옳다**(두 세션이 각자 행을 붙이는 원래 시나리오엔 정답) — 고칠 것은 union이 아니라 「한 브랜치가 자기 행을 두 번 건드리는」 쪽이다. ⚠️ PowerShell 훅이라 **원격(웹) 세션엔 안 붙는다**(T-208) — 그 환경에선 rebase 직후 `git diff origin/main -- claude-docs/changelog.md`로 자기 행이 1줄인지 손으로 확인 |
