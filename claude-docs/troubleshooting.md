...
| 브라우저 자동화의 CDP 명령이 타임아웃 — 원인이 코드가 아니라 그 탭·패널의 상태다 | T-154(클로드 내부 패널 미개방) · T-179(데스크톱 크롬 기기 모드) · T-184(같은 탭 재사용, 입력에 더해 **스크린샷까지**) · 4회차(2026-08-19, T-184 후일담에 병합 — 새 탭으로 시작했는데도 도중에 상함) | 4 | ✅ **예고대로 절차 승격**(CLAUDE.md 「🖥️ 프론트 검증」) — 검증 시작 시 `tabs_create_mcp`로 **전용 탭을 새로 연다**. ⚠️ 4회차가 그 절차의 한계를 실측했다: 새 탭으로 시작해도 **세션 도중에 죽는다**. 절차는 유지하되(비용 0) 「한 번 열면 안전」이라는 기대는 버리고, 죽으면 그 자리에서 또 갈아탄다 — 승격 단계를 더 올릴 자리는 아니다(훅으로 감지할 수 있는 대상이 아니고, 재시도 비용이 이미 최소다). ⚠️ 3회차가 군 정의를 넓혔다: 「읽기는 되는데 **입력만**」인 줄 알았는데 `Page.captureScreenshot`도 같이 죽었다 — 안 죽는 건 **CDP를 안 쓰는 경로**(`javascript_tool`·콘솔 읽기)라, 판별 기준은 「읽기냐 입력이냐」가 아니라 **「CDP를 타느냐」**다 |
