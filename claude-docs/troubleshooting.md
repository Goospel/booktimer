...
| **커밋 테스트 게이트 8분 상한을 정상 스위트가 넘긴다 — 경합이 아닌데 훅 메시지는 경합이라 한다** | **T-235** | **2** (2026-09-09 9분 6초 → 09-11 8분 19초, 둘 다 `BUILD SUCCESSFUL`) | ⏸ **미승격(2026-09-11)** — 처방은 T-235에 있다(게이트 밖 `./gradlew test` 1회 완주 → up-to-date → 커밋). 1회차 대조군은 3분 23초라 스위트 자체가 8분을 **상시** 넘는 것은 아니고 조건부다(무엇이 조건인지 미확정). 3회째면 `BOOKTIMER_TEST_GATE_TIMEOUT_MS` 상향 + 훅 메시지 「원인이 둘」 수정을 훅 변경 PR로 낸다. |
