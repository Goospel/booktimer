...
1. ✅ 이 워크플로 + 데이터 계약 문서화.
2. ✅ **랜딩 리프레시** — 종이톤(#F3EEE4 배경·#FCFAF5 카드) + 세이지 그린(#6E8A6A 악센트) 토큰으로 `:root` 전역 교체(이름 유지=무파괴) + 본문 폰트 '고운돋움'(둥근 한글 산세리프) 전역 적용 + landing 히어로 위계 강화. 토큰 이름 유지라 25개 화면 전역 리프레시(bootRun·정적 목업 스폿체크로 회귀 0 확인).
