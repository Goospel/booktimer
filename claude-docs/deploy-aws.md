# AWS 배포 가이드 — ECS Fargate + ALB + RDS (구 아키텍처)

> ⚠️ **이 문서는 구 아키텍처다.** 2026-07-28부터 **EC2 단일 인스턴스**로 이전 중이며,
> 현행 정본은 **[deploy-ec2.md](deploy-ec2.md)** 다.
> 🛑 **2026-08-03: ALB·타깃그룹·ECS 서비스/클러스터·RDS를 전부 삭제했다.** 아래 리소스 생성·운영
> 절차는 **더 이상 실재하는 인프라를 설명하지 않으니 그대로 따라 하지 말 것**(롤백 경로도 함께 사라졌다).
> RDS는 최종 스냅샷 `booktimer-db-final-20260803` 으로만 남아 있다.
> 그럼에도 이 문서를 지우지 않는 이유는 SSM 파라미터 목록·SES 설정처럼 **지금도 유효한 지식**이
> 섞여 있어서다 — 읽을 때 "ECS/ALB/RDS 부분은 역사"라고 구분해서 볼 것.
> 배경: 크레딧 고갈로 원가 월 ~$100이 드러남 → EC2 1대(월 ~$30)로 전환. `plan.md` §ECS→EC2 이전.
...
> `GeminiStudyPlanner.errorReasonOf`). **실제로 429가 오는지 403이 오는지는 미확인이다**(일부러 소진시킬 수
> 없어 그날에만 관측된다). 소진 속도 가늠(실측 토큰 기준): 일정 ≈ 11.96원/회, 성향 ≈ 7.42원/회 →
...
⚠️ **후불이 함께 켜져 있다** — 선불이 마르면 후불(카드)로 넘어갈 가능성이 있다. 화면 문구만으론 확정하지
못했고, 넘어간다면 「잔액이 마르면 알아서 멈춘다」는 전제가 깨진다. **예산 알림이 그걸 잡을 유일한 신호다.**
