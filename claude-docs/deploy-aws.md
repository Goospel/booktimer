...
>
> ⚠️ **자동 충전은 꺼 두었다 — 잔액이 마르면 조용히 429가 된다.** 그때 어댑터는 `RATE_LIMITED`로 매핑해
> 「잠시 후 다시 시도해 주세요」를 보여주는데, 충전 전까지 그 안내는 지킬 수 없는 말이 된다. 자동 충전을
> 끈 이상 **GCP 예산 알림(50%·90%)이 유일한 조기 경보**이므로 25,000원 기준으로 걸어 둔다.
> 소진 속도 가늠(실측 토큰 기준): 일정 ≈ 11.96원/회, 성향 ≈ 7.42원/회 → 전역 상한 50회/일을 다 쓰면 약 41일,
> 현실적인 5회/일이면 400일대.
