...
> ## 🗄️ 이 문서는 아카이브다 (2026-08-18 폐기)
>
> **새 `N-###` 을 추가하지 않는다.** 개념("왜 이렇게 동작하는가")은 이제
> [troubleshooting.md](troubleshooting.md) 의 `T-###` **원인 필드**가 담고, 개념만 있고 함정이 없는 것은
> 아예 남기지 않는다. 실제 운영도 그렇게 흘러왔다 — 마지막 항목 **N-149** 이후 사실상 정지 상태였고,
> 그 사이 CLAUDE.md 만 계속 「learning-notes 후보를 제안하라」고 지시해 세션이 헛수고를 했다(그 문구를 걷었다).
>
> **기존 N-### 는 그대로 살아 있다.** CLAUDE.md·plan.md·코드 주석에 남은 `N-###` 링크는 낡은 참조가 아니라
> **의도된 포인터**다(N-032 멀티세션 · N-055 null-state 경계 · N-077 CSRF 세션 lazy 생성 · N-082·N-083
> 로드순서 · N-127 E2E 실행법 등). 읽는 건 계속 권장한다 — 막을 이유가 없다.
...
- [N-148. 열거완화·비동기 발송 설계에선 프론트로 실발송을 검증 못 한다 — 받은편지함·발송자 통계가 유일한 확증](#n-148-열거완화비동기-발송-설계에선-프론트로-실발송을-검증-못-한다--받은편지함발송자-통계가-유일한-확증)
- [N-149. 서점 출간일은 번역판 날짜다 — 원작 연도와 작가 국적은 Wikidata work로 얻고 ISBN 아닌 원제와 저자로 잇는다](#n-149-서점-출간일은-번역판-날짜다--원작-연도와-작가-국적은-wikidata-work로-얻고-isbn-아닌-원제와-저자로-잇는다)
...
**후속 (2026-06-24): 그 상세 재요청(2차)도 거부됨.** 4가지를 충실히 담았는데도 또 거부 → "정보 부족이 거부의 큰 원인"이라는 ③의 진단이 **부분적이었다**(아래 일반화 갱신). 신규·저트래픽 서비스라 **실발송 실적·발신 평판이 0**인 게 더 근본 사유로 보인다(AWS는 책임 발송자임을 *실적*으로 보고 싶어 하는데, 샌드박스라 실적을 못 쌓음 = 닭-달걀). 결론: 상세를 더 담아 또 재요청하기보다 **실사용·트래픽이 쌓일 때까지 이메일 의존 기능을 보류**하고 다른 작업을 우선하는 게 합리적(plan.md retention 레버 ① 보류 박스에 결정 박음).

**후속2 — 반전 (2026-07-06): 4차 재요청으로 결국 승인.** 위 "실적 0이라 무망" 결론은 성급했다. 거부 사유(deliverability·sender reputation)를 **코드로 검증한 사실로 조목조목 반박**한 4차 appeal(6/29 제출)이 2026-07-06 승인됨 — 서울 리전 샌드박스 해제, 발신 한도 50,000통/일·14통/초. 트래픽·실적이 0인 상태 그대로였는데도 통했다. 즉 **근본 사유는 "실적 부재"가 아니라 "요청의 구체성·신뢰성 부족"이었다** — 무엇을·누구에게·어떻게(1:1 트랜잭션·본인 입력 주소·기본 OFF 마케팅·서명 토큰 수신거부·first-send 바운스 완화·검증 도메인 DKIM/SPF 정렬)를 실제 구현 근거로 적고, 없는 기능(SNS 바운스 핸들러 등)은 **정직하게 빼자** 사람 리뷰로 넘어가 뒤집혔다. 교훈: **AWS SES 거부는 거부 사유를 정면으로 반박하는 상세·정직 + 끈기(칠전팔기)면 실적 0에서도 뒤집을 수 있다** — 단, 검증 가능한 사실만 주장하라(거짓 주장은 AWS가 검증 가능해 역효과).
...
- **거부 가능한 외부 심사는 "상세·정직"이 통과율 — 단 사유가 "정보 부족"일 때만**: SES도 AdSense([[n-088]])도 빈약하면 거부·구체적이면 통과가 *기본*이나, **근본 사유가 정보 부족이 아니라 실적/평판/콘텐츠 부재면 상세로도 안 뚫린다.** BookTimer는 4가지를 충실히 담은 2차 재요청도 거부됨(2026-06-24) → 사유는 정보가 아니라 *신규·저트래픽의 발송 실적 0*(닭-달걀). AdSense도 "콘텐츠 부족"이 사유라 콘텐츠를 안 늘리면 재심사 동일 거부. **일반 교훈: 외부 관문이 '실적·실체(발송 이력·트래픽·공개 콘텐츠)'를 보는 종류면, 서류를 다듬는 것보다 그 실체를 쌓는 게 먼저고, 그때까진 그 기능을 보류하고 다른 일을 하는 게 맞다.** 토글·연동 코드는 둬도 외부 관문은 시간·실체가 약이다. **⚠️ 단서 (2026-07-06 반전): 이 "실체가 약" 결론은 SES엔 틀렸다** — BookTimer는 실적 0 그대로 4차 재요청으로 승인됐다(위 후속2). SES는 AdSense와 달리 "발송 실적"보다 **요청의 구체성·발송자 책임 증빙**을 보는 관문이었고, 거부 사유를 코드 근거로 정면 반박하니 실적 없이 뚫렸다. 그러니 관문마다 "무엇을 보는가"를 구분하라 — AdSense=공개 콘텐츠 실체(실체가 약), SES=발송자 책임·요청 구체성(서류·정직·끈기로 가능). 관문을 뭉뚱그려 "다 실체가 약"이라 단정하지 말 것.
...
## N-104. Thymeleaf 표현식의 `${}` 범위가 산술 의미를 바꾼다 — 밖은 Thymeleaf 산술(소수), 안은 SpEL(타입대로)

**한 줄 요약**: Thymeleaf에서 `${x} / n`처럼 `${}` **밖**에서 나누면 Thymeleaf 자체 산술이 적용돼 소수가 나오고, `${x / n}`처럼 `${}` **안**에서 나누면 SpEL이 피연산자 타입대로 계산한다(`long / long` = 정수 나눗셈). 같은 값·같은 나눗셈이라도 `${}`의 경계가 결과 타입을 바꾼다.

### 무슨 일이 있었나

책 상세 누적 시간이 운영에서 `· 누적 11.0944…시간 5.666…분`으로 샜다. 정작 같은 페이지의 일자별 행과 `/history`는 정수로 멀쩡했다. 코드를 보니 누적만 산술이 `${}` 밖에 있었다.

```html
<!-- 누적(버그): ${totalSeconds}만 변수로 꺼내고 / 는 ${} 밖 → Thymeleaf 산술 -->
'· 누적 ' + (${totalSeconds} / 3600) + '시간 ...'   <!-- 5400 / 3600 = 1.5 -->

<!-- 일자별(정상): 산술까지 ${} 안 → SpEL -->
${(r.totalSeconds() / 3600) + '시간 ...'}            <!-- 5400L / 3600 = 1 -->
```

### 왜 차이가 나는가

- `${...}`는 "표현식 평가 시작/끝" 경계다. 그 **안**의 산술은 표현식 언어(Spring 환경이면 SpEL)가 처리한다. SpEL은 `5400L / 3600`을 자바처럼 **정수 나눗셈** → 1.
- `${}` **밖**의 `+`·`/`는 Thymeleaf가 토큰을 이어 붙이며 처리하는 **Thymeleaf 산술**이다. 여기선 숫자 나눗셈을 소수로 평가 → 1.5.
- 그래서 `'누적 ' + (${x} / 3600)`은 (문자열 연결 + Thymeleaf 나눗셈)이라 1.5가, `${'누적 ' + (x / 3600)}`은 (SpEL 전체 평가)라 1이 나온다.

### 비유

`${}`는 계산기의 괄호다. 식을 통째로 괄호 안에 넣어야 계산기(SpEL)가 "이건 정수끼리 나눗셈"이라고 타입을 보고 푼다. 괄호 밖으로 숫자를 흘리면 Thymeleaf가 자기 방식(소수)으로 대충 이어 계산해 버린다.

### Q&A 대비

- **Q. 왜 일자별 행은 멀쩡했나?** → 거긴 산술이 `${}` 안이라 SpEL 정수 나눗셈을 했다. 같은 long 값이라도 경계 위치만 달랐다.
- **Q. 소수가 보이면 어떻게 빨리 잡나?** → 그 표현식에서 산술(`/`·`%`)이 `${}` 안에 있는지 밖에 있는지부터 본다. 밖이면 안으로 옮긴다.
- **Q. 정수로 강제하는 다른 방법은?** → `${}` 안에서 long끼리 나누면 자동 정수다. `#numbers.formatInteger(x, 1)`는 **반올림**이라 1.5→2가 되어 오답 — 내림이 필요한 시·분 환산엔 쓰면 안 된다.
- **Q. 운영에선 보였는데 코드는 멀쩡해 보였던 이유?** → `${}` 밖 나눗셈은 컴파일·렌더 에러 없이 조용히 소수를 내므로, 테스트(MockMvc content 단언)나 실제 데이터가 있어야 드러난다.

### 관련

- **T-074** — 이 함정의 재발 방지 절차(증상→해결).
- **T-031/T-032/T-059** — 다른 Thymeleaf 함정들(null SpEL·우선순위·인라인 `[[`).
- `src/main/resources/templates/book-detail.html` — 누적 시간 표현(수정본).

---

## N-105. 사파리/iOS 캐시 3계층 — HTTP 캐시·SW Cache Storage·SW 스크립트 갱신은 수명이 다르다

**한 줄 요약**: "브라우저 캐시"는 수명이 완전히 다른 3계층(① HTTP 디스크 캐시 ② Service Worker Cache Storage ③ SW 스크립트 자체의 갱신 체크)으로 나뉜다. 특히 ②는 **시간 만료가 없고**(코드의 `caches.delete`·사이트 데이터 삭제·스토리지 압박 eviction 전까지 영구), ③은 iOS Safari에서 **게을러서**(탭 완전 종료 전까지 며칠도 옛 SW 유지) "배포했는데 사파리만 옛 버전"이 난다. iOS는 추가로 7일 미사용 시 ②③을 자동 삭제하되 **홈 화면 PWA(standalone)는 면제·탭과 스토리지 분리**라 "Safari 탭은 옛 버전, 홈 PWA는 최신"으로 갈린다.

### 무슨 일이 있었나

PWA 셸 자산을 고친 #430을 배포했는데, 같은 기기에서 **Safari 탭은 옛 레이아웃(알림 토글 없음), 홈 화면 PWA는 최신**으로 보였다. "캐시 때문이냐"는 질문에서 출발해, "사파리 캐시 생명주기가 얼마냐"를 정확히 답하려면 캐시를 한 덩어리로 보면 안 되고 3계층으로 쪼개야 한다는 게 핵심이었다.

### 3계층과 각각의 수명

| 계층 | 무엇 | 수명 |
|---|---|---|
| **① HTTP 캐시** (디스크/메모리) | 일반 리소스 | `Cache-Control` 헤더 그대로. 없으면 휴리스틱(Last-Modified 경과의 ~10%). `no-store`면 0 |
| **② SW Cache Storage** (`caches` API) | SW가 `c.put`으로 담은 셸 자산 | **시간 만료 없음.** 코드(`caches.delete`)·사이트 데이터 삭제·스토리지 eviction 전까지 영구 |
| **③ SW 스크립트 갱신** (sw.js 재확인) | 브라우저가 sw.js를 다시 받아 새 버전 감지 | 스펙상 sw.js의 HTTP `max-age` **상한 24h**. iOS Safari는 체크가 게을러 탭 종료 전까지 며칠도 옛 SW 유지 |

### 왜 계층마다 수명이 다른가

- **①은 HTTP 표준**이라 서버 헤더가 수명을 정한다. 그래서 끄려면 `no-store`/`max-age=0`로 서버에서 통제 가능.
- **②는 앱(SW) 코드가 명령형으로 관리**한다. Cache API엔 TTL 개념이 아예 없다 — `caches.match`가 히트하면 서버에 묻지 않고 즉시 반환(cache-first). 그래서 파일명이 고정(해시 없음)이면 버전을 안 올리는 한 영원히 stale(→ T-071·T-075). 우리 해법은 핵심 자산을 network-first로 두고(`NETWORK_FIRST` 배열) `CACHE` 상수를 올려 activate에서 옛 키 삭제.
- **③은 새 SW로 교체되는 관문**이다. ②를 고쳐도 **새 sw.js가 설치·activate돼야** 그 수정이 적용된다. 그런데 iOS Safari는 ③ 체크가 게을러, 옛 SW가 계속 ②를 cache-first로 서빙한다 → "코드는 옳은데 화면은 옛 버전".

### iOS 특이 정책 두 개

- **7일 미사용 자동 삭제**: Safari는 사이트를 7일간 안 열면 ②③(SW 등록·Cache Storage·localStorage·IndexedDB)을 통째로 지운다(WebKit ITP). 어쩌다 들르는 사용자는 오히려 항상 fresh.
- **홈 화면 PWA(standalone)는 면제** + 탭과 **스토리지 분리**. 그래서 "Safari 탭 옛 버전 / 홈 PWA 최신"의 분리가 정확히 이 때문이다.

### 비유

캐시는 한 통이 아니라 냉장고 3칸이다. ①은 **유통기한 스티커**(헤더)대로 비는 칸, ②는 **유통기한 없는 냉동칸**(내가 직접 버려야 빔), ③은 **냉장고 자체를 새 모델로 바꾸는 일**(바꿔야 안의 정책이 갱신됨). iOS Safari는 이 "새 모델 교체"를 미루는 게으른 주인이고, 홈 PWA는 아예 별도 냉장고다.

### Q&A 대비

- **Q. "사파리 캐시 며칠 가요?"에 한 숫자로 답하면?** → 틀린다. ①은 헤더 나름, ②는 무기한, ③은 상한 24h지만 iOS는 사실상 탭 종료까지. 계층을 먼저 나눠야 한다.
- **Q. 가만 두면 언제 최신 되나?** → 보장 없음. iOS는 ③이 게을러 탭을 백그라운드에 살려두면 며칠도 옛 SW. 탭 완전 종료 후 재방문이 보통의 트리거.
- **Q. 즉시 확실히 당기려면?** → 사이트 데이터 삭제(설정→Safari→고급→웹 사이트 데이터). ②③ 한 번에 초기화.
- **Q. 코드로 영구 박는 법은?** → 파일명 고정 자산은 SW에서 network-first로, 셸 버전 상수(`CACHE`)를 배포마다 올려 activate에서 옛 캐시 삭제. ①은 핵심 자산에 `no-store`.
- **Q. 왜 홈 PWA만 최신이었나?** → 탭과 스토리지가 분리돼 마침 새 SW를 먼저 잡았고, 7일 캡 면제라 더 오래 유지. 분리가 우연이 아니라 정책.

### 관련

- **N-101** — PWA 전반(manifest·SW·iOS 홈화면 설치). 이 노트는 그 SW의 캐시 수명 디테일.
- **T-071 / T-075** — 파일명 고정 자산 × SW cache-first → 배포해도 stale(②의 재발 방지 절차). #430이 NETWORK_FIRST 일반화로 해결.

---

## N-106. SSR은 즉시 반영, SPA는 캐시에 갇힌다 — 변경분이 '동적 HTML'에 담기냐 '고정명 정적 번들'에 담기냐

**한 줄 요약**: "SSR일 땐 배포하면 바로 보였는데 Vue로 바꾸니 캐시 먹는다"의 정체는 **바뀐 코드가 어디에 담기느냐**의 차이다. SSR은 변경분이 **매 요청 서버가 새로 그리는 HTML**(비캐시 대상)에 담겨 즉시 반영되고, SPA는 변경분이 **고정 파일명의 정적 JS 번들**(`garden.js`, 캐시 대상)로 옮겨가 브라우저·SW가 "URL 같으니 안 바뀐 것"으로 보고 옛 캐시를 재사용한다. 정석 해법은 **콘텐츠 해시 파일명(cache busting)**이고, 이 프로젝트가 그걸 끈 건 Thymeleaf가 파일명을 하드코딩 참조하기 때문이다.

### 무슨 일이 있었나

마을/정원 프론트를 SSR(Thymeleaf)에서 Vue SPA로 옮긴 뒤, "예전엔 배포하면 즉각 화면에 보였는데 왜 이제 캐시로 안 보이냐"는 질문에서 출발. 같은 배포 행위인데 결과가 달라진 이유를 "어디가 캐시되느냐"로 쪼개야 답이 나왔다.

### SSR vs SPA — 변경분의 거처

| | SSR (Thymeleaf) | SPA (Vue 번들) |
|---|---|---|
| **화면 로직이 사는 곳** | 서버가 그리는 **HTML 문서** | 정적 파일 **`garden.js`** |
| **그 자원의 성질** | 동적 — 매 요청 새로 렌더 | 정적 — 한 번 받으면 끝 |
| **캐시 여부** | 안 함(SW도 navigate=network-first) | 함(HTTP 캐시·SW Cache Storage) |
| **배포 후 반영** | 다음 요청에 서버가 새 HTML → **즉시** | URL 같아 캐시 재사용 → **stale** |

핵심은 SPA로 가면서 "바뀌는 알맹이"가 **비캐시 자원(HTML)에서 캐시 자원(정적 JS)으로 이사**했다는 것. 브라우저가 정적 자산을 캐싱하는 건 정상·의도된 성능 최적화고, 함정은 **파일명이 고정**이라 "바뀌었다"는 신호가 없다는 점이다.

```ts
// frontend/vite.config.ts — 해시 없는 고정 이름
output: { entryFileNames: 'garden.js' }
```
```html
<!-- garden.html:31 — 이름을 그대로 박아 참조 -->
<script type="module" th:src="@{/garden/garden.js}"></script>
```

### 정석 해법 — 콘텐츠 해시 파일명(cache busting)

```
garden.js          ❌ 내용 바뀌어도 이름 동일 → URL 같음 → 캐시 재사용
garden.a3f9c1.js   ✅ 내용 바뀌면 해시 바뀜 → URL 달라짐 → 무조건 새로 받음
```

해시를 쓰면 URL이 바뀌므로 브라우저는 선택의 여지 없이 새 파일을 받고, 옛 파일은 캐시에 남아도 아무도 참조 안 해 무해하다. Vite가 **원래 기본으로 해주는** 동작인데 이 프로젝트는 일부러 껐다.

### 왜 껐나 — Thymeleaf 하드코딩 참조

템플릿이 `@{/garden/garden.js}`로 파일명을 박아 쓰기 때문. 해시 파일명을 쓰면 빌드마다 이름이 바뀌어 템플릿이 그 이름을 알 길이 없다. 해시를 도입하려면 "빌드가 생성한 실제 파일명 → 템플릿 주입" 배선(Vite `manifest.json` → Thymeleaf 주입, 또는 `?v=빌드버전` 쿼리스트링)이 필요하다. 지금은 그 배선이 없어 **고정 파일명 + SW network-first**(N-105 ②계층 대응, #430)로 우회하는 상태다. 단, SW의 `fetch()`도 결국 HTTP 캐시(N-105 ①계층)를 거치므로 그쪽 `Cache-Control` 수명이 길면 stale이 남을 수 있다.

### 비유

SSR은 **주문마다 새로 요리해 내오는 음식**, SPA 고정 번들은 **"3번 도시락"이라는 이름표가 붙은 도시락**이다. 도시락 속 레시피를 바꿔도 이름표가 같으니 손님은 냉장고의 옛 "3번 도시락"을 그냥 꺼내 먹는다. 해시 파일명은 레시피를 바꿀 때마다 **이름표 번호까지 새로 매기는 것** — 손님이 옛 도시락과 헷갈릴 수 없다.

### Q&A 대비

- **Q. 왜 SSR은 캐시가 안 끼었나?** → 변경분이 동적 HTML에 있고, HTML은 매 요청 새로 그려지며 캐시 대상이 아니라서(SW도 navigate는 network-first).
- **Q. SPA가 캐시 먹는 건 버그인가?** → 아니다. 정적 자산 캐싱은 정상·성능 최적화. 버그는 "고정 파일명이라 바뀐 걸 알릴 신호가 없는 것".
- **Q. 진짜 정석 해결은?** → 콘텐츠 해시 파일명. URL이 바뀌어 캐시를 우회한다. SW network-first는 한 계층만 막는 우회책.
- **Q. 왜 이 프로젝트는 해시를 안 쓰나?** → Thymeleaf가 파일명을 하드코딩 참조해서. 해시 도입엔 manifest 주입·`?v=` 같은 배선이 선행돼야 한다.

### 관련

- **N-098** — SSR 섬 아키텍처(Vite 번들을 static 자원으로 커밋해 bootJar 통합). 이 노트는 그 섬 번들이 왜 캐시 문제를 새로 만드는지.
...
## N-115. 전역 클래스+margin-collapse 레이아웃을 wrapper 없는 컴포넌트로 쪼개면 '바깥 간격 책임'이 흩어진다 — 간격은 컨테이너 gap으로

> **한 줄 요약**: 카드 사이 간격을 각 블록의 `margin-bottom` 하나에 맡긴 SSR 레이아웃을, scoped 스타일 없는
> Vue 컴포넌트(특히 wrapper div 없는 fragment)로 쪼개면 "바깥 간격을 누가 갖나"가 컴포넌트 경계로 흩어져
> 누락·중복·0px 붙음이 난다. 간격 책임을 자식 마진이 아니라 **부모 컨테이너의 `gap`**으로 옮기면 사라진다.

### 문제 / 배경

대시보드(`#dashboard-app`) 세로 스택에서 `바로가기 그리드(.quick-actions)`와 `독서 잔디 카드(.card)`가 0px로 붙고,
전체 리듬이 20·16·14·0·16·18로 들쭉날쭉했다. 카드 간격은 `.card{margin-bottom:16px}` 단일 규칙에 의존했는데,
카드가 아닌 요소(`.greeting` 20·`.highlight-actions` 14·`.quick-actions` 0)가 흐름에 섞이며 각자의 개별 마진이
리듬을 깼다. `.quick-actions`는 아래 마진이 없고 다음 `.card`도 위 마진이 없어 둘 사이가 0px가 됐다.

### 해법 / 개념

- **왜 SSR에선 안 보였나**: 한 템플릿에 전부 펼쳐 쓰던 시절엔 "이 블록 다음엔 16 띄운다"를 한곳에서 직접 조율했다.
  Vue 컴포넌트로 쪼개면 각 컴포넌트가 자기 루트의 마진을 들고 다니는데, 그게 전역 공유 클래스(`.card`·`.greeting`)라
  컴포넌트마다 제각각이고 카드 아닌 블록이 끼면 리듬이 깨진다 — "바깥 간격 책임"의 분산.
- **Vue3 fragment의 평탄화**: `<template>`에 루트가 여럿이면 wrapper div 없이 마운트 요소의 직계 자식으로 평탄히
  놓인다. 그래서 `#dashboard-app` 직계 = 모든 블록 → 부모에 `display:flex;flex-direction:column;gap:16px` 한 줄이
  사이 간격을 전부 균일하게 책임진다(자식이 몇 개든·무엇이든).
- **margin → gap 이전**: gap을 켜면서 자식의 개별 수직 마진을 `#dashboard-app > *{margin:0}`으로 리셋해야 한다.
  안 그러면 gap + margin이 더해진 이중 간격이 난다. 간격을 자식이 아니라 컨테이너 한 곳이 소유하게 만드는 게 핵심.
- **스코프로 회귀 차단**: 마운트 id(`#dashboard-app`)에만 걸면 같은 공유 클래스를 쓰는 다른 페이지(books·profile…)는
  그 id가 없어 0 영향. 전역 `.container`에 걸면 모든 페이지가 영향권이라 회귀 면적이 커진다.

### 예시

| | SSR 한 템플릿 | scoped 없는 컴포넌트 분할 |
|---|---|---|
| 간격 소유자 | 한곳에서 직접 조율 | 각 자식 마진(흩어짐) |
| 카드 아닌 블록 끼면 | 그 자리서 마진 조정 | 개별 마진이 리듬 깸(0px·비균일) |
| 처방 | — | 부모 `gap` + 자식 `margin:0`(스코프) |

> 일반화: 컴포넌트 간 간격은 "자식이 바깥 여백을 갖는다"보다 "부모가 자식 사이를 띄운다"(flex/grid `gap`)가 견고하다.
> 자식은 자기 안쪽(패딩)만 책임지고 바깥 간격은 부모가 소유 — 컴포넌트 재배치·추가/삭제에 안 깨진다.

### 관련

- **N-113·N-114** — 같은 SSR→Vue 전환에서 "구조(form 래퍼·float clear) 소실이 잠복 가정을 폭로"한 자매 사례. 본 노트는 그 간격판.

---

## N-116. standalone PWA 콜드 런치는 마지막 라우트가 아니라 manifest start_url부터 다시 로딩한다 — start_url은 실재하는 라우트여야

> **한 줄 요약**: `display: standalone` PWA를 OS가 메모리에서 내린 뒤 다시 열면, 마지막에 보던 페이지가 아니라
> manifest의 `start_url`부터 콜드 로딩한다. 이 값이 서버에 매핑 없는 경로면 **콜드 런치마다 404**가 난다 —
> 인증·캐시와 무관한 "시작점 오기" 버그. start_url은 반드시 실재하는 라우트여야 한다.

### 문제 / 배경

설치한 PWA를 "어느 시간 지나고" 다시 열면 404 에러 페이지("대시보드로" 링크)가 뜨고, 그 링크를 누르면
로그인 절차 없이 바로 대시보드가 떴다. 로그인은 멀쩡한데(세션 쿠키 생존) 왜 404인가가 수수께끼였다.

원인은 `manifest.json`의 `start_url: "/dashboard"`였는데, 서버엔 `/dashboard`라는 HTML 라우트가 없었다 —
대시보드는 `DashboardController.@GetMapping("/")`(루트)에 매핑돼 있었다(`/dashboard`는 `/api/dashboard` API뿐).
즉 시작점이 존재하지 않는 경로라 콜드 런치 때마다 Spring이 404를 냈다.

### 해법 / 개념

- **왜 "어느 시간 지나고"부터인가**: standalone PWA가 백그라운드에 살아 있는 동안엔 마지막 라우트를 복원해
  404가 안 보인다. OS가 메모리에서 앱을 내린 뒤(= 시간이 지난 뒤) 다시 열면 **콜드 런치**라 `start_url`부터
  새로 로딩한다 — 이때 비로소 잘못된 시작점이 폭로된다. "가끔만 재현"이 이 메모리 수명 차이 때문.
- **왜 로그인은 멀쩡한가**: 세션 쿠키는 그대로라, 404 페이지의 "대시보드로"(→ `/`) 링크를 누르면
  컨트롤러가 `principal`을 보고 즉시 대시보드를 렌더한다. 인증과 무관한, 순수 라우팅 시작점 문제.
- **수정**: `start_url`을 실재 라우트 `/`로. `/`는 로그인 시 대시보드로 분기하니 "첫 화면 = 대시보드" 의도도 유지.
- **회귀 가드 — 시작점은 라우트라 정적 단언보다 라이브 검증**: `start_url` 문자열을 하드코딩 비교하는 테스트는
  "이 값이 실제로 응답하는가"를 못 본다(잘못된 값을 그대로 고정할 뿐 — 실제로 이 repo의 기존 테스트가
  `/dashboard`를 고정하고 있었다). manifest에서 `start_url`을 **읽어** 그 경로로 GET해 **404가 아님**을
  단언해야 시작점-라우트 정합을 진짜로 못 박는다.
...
## N-118. CSS 주석은 중첩 불가 — 주석 속 `*/`가 조기 종료해 다음 규칙을 침묵 드랍한다

**한 줄 요약**: CSS 주석 `/* … */`은 중첩이 안 돼서, 주석 *안*에 `*/` 토큰이 우연히 생기면(예: 와일드카드 셀렉터를 나열한 `.timer-*/.quick-*`) 거기서 주석이 **조기 종료**된다. 그 뒤 텍스트가 CSS로 파싱되며 바로 다음 규칙 블록을 invalid 셀렉터로 통째 삼켜 **조용히 드랍**시킨다 — 콘솔 에러도 없이 "그 규칙만 적용 안 됨".

### 증상

- 특정 클래스(예: `.dash-card`)의 스타일이 **전부** 안 먹는다(background·border·radius·shadow·padding·flex 동시에). 바로 다음 규칙(`.dash-pill`)부터는 정상 → "딱 한 블록만 사라짐".
- 콘솔에 CSS 에러 **없음**(브라우저는 invalid 규칙을 조용히 버린다).
- 파일은 문법상 멀쩡해 보이고(`Read`·`cat`로 봐도 정상), `fetch`로 받은 텍스트에도 그 규칙이 분명히 있다 → "파일엔 있는데 안 먹는다".

### 원인

```css
/* … 기존 .grass-cell/.timer-*/.quick-* 등 공유 클래스와 격리 … */
.dash-card { background: …; border: …; }   /* ← 이 규칙이 통째로 사라진다 */
.dash-pill { … }                            /* ← 여기부터 파서 복구 */
```

`.timer-*/`의 `*` 다음 `/` = **`*/`**. CSS 파서는 이를 주석 종료로 본다(중첩 불가). 그러면 `.quick-* 등 … */ .dash-card`가 하나의 **invalid 셀렉터**가 되고 뒤따르는 `{ … }`가 그 본문이 되어 규칙 전체가 버려진다. 다음 `{`(`.dash-pill`)에서 파서가 복구한다.

### 진단

브라우저 CSSOM에서 그 규칙이 *실제로 파싱됐는지* 확인한다(파일 내용 ≠ 파싱 결과):

```js
[...document.styleSheets[0].cssRules].some(r => r.selectorText === '.dash-card')  // false면 드랍됨
```

`fetch`로 받은 파일엔 규칙이 있는데 `cssRules`엔 없으면 = **파싱 단계 드랍**(캐시 아님). 캐시 의심은 링크에 cache-bust 쿼리(`?v=…`)를 붙여 분리한다(정적 서버가 쿼리를 무시하면 항상 최신 파일).

### 해결·예방

- 주석 안에서 `*/`를 만드는 토큰을 피한다: `.timer-*/.quick-*` → `.timer-*·.quick-*`(슬래시를 `·`로) 또는 `.timer-* / .quick-*`(공백으로 `* /` 분리).
- 와일드카드 셀렉터(`*`)를 주석에 나열할 때 특히 주의 — `-*/` 패턴이 흔한 함정.
- **하드픽스(3회차에 승격)**: 예방 규칙만으론 #522·#526·로그인 이식까지 3번 새서, 커밋 훅 `require-css-comment-safe.ps1`이 스테이징 `.css`에서 "주석 닫는 `*/`가 양옆 모두 셀렉터문자에 붙은" 패턴을 차단한다. "양옆 glued" 조건이라 정상 종료(` */`+개행)·인라인 주석(`/* c */color`, 앞이 공백)은 통과 → 과거 보류 사유였던 FP를 피한다. 우회: `SKIP_CSS_COMMENT_CHECK`.
- **잠복성이 높다**: 이 버그는 PR-2부터 있었지만 카드가 "배경색이 비슷해 어렴풋이 카드처럼 보여" 오래 안 들켰다. 시각이 "조금 이상"하면 추측 전에 CSSOM 파싱부터 의심.
...
## N-120. overflow-x:auto는 데스크톱에서 터치 스크롤만 — 휠·드래그는 JS로

**한 줄 요약**: `overflow-x:auto`로 만든 가로 스크롤 영역은 **모바일 터치에선 네이티브로 잘 되지만**, 데스크톱에선 ① 마우스 휠이 세로 `deltaY`라 가로 컨테이너를 안 움직이고 ② 클릭 드래그는 브라우저 기본 기능이 아니라 **둘 다 안 된다**. 휠→가로 변환과 드래그(grab)는 JS로 직접 붙여야 한다.

### 증상

- 가로 스크롤 갤러리가 **모바일에선 손가락으로 잘 넘어가는데 데스크톱(마우스)에선** 휠로도 클릭 드래그로도 안 움직임.
- 세로 스크롤바가 없는 가로 컨테이너 위에서 휠을 굴려도 가로로 안 가고, 페이지만 세로로 스크롤되거나 아무 반응 없음.

### 원인

- **마우스 휠**: 휠 이벤트의 주 신호는 `deltaY`(세로)다. 브라우저는 세로 스크롤 여지가 있는 컨테이너에만 휠을 자동 적용 — 가로 전용(`overflow-x`) 컨테이너는 `deltaY`로 안 움직인다.
- **클릭 드래그**: 데스크톱에서 잡아끌기는 **브라우저 기본 동작이 아니다**(터치만 네이티브 관성 스크롤을 제공). 마우스로 끌어 스크롤하려면 직접 구현해야 한다.

### 해결·예방

- **휠**: `wheel` 핸들러에서 `el.scrollLeft += e.deltaY` + `e.preventDefault()`. 단 좌/우 끝 경계를 더 넘는 방향이면 `preventDefault`를 **하지 않아** 페이지 세로 스크롤에 양보(컨테이너 위에서 페이지가 안 막히게).
- **드래그(grab)**: `pointerdown`에서 시작점·`scrollLeft` 저장 + `setPointerCapture`, `pointermove`에서 `scrollLeft = start - (clientX - startX)`, `pointerup`에서 해제. 커서 `grab`/`grabbing`.
- **터치는 건드리지 말 것**: `pointerType === 'touch'`면 드래그 핸들을 건너뛴다 — 모바일은 네이티브 가로 스크롤이 이미 잘 되니 가로채면 오히려 충돌·이중 처리.
- `scroll-behavior: smooth`는 프로그래매틱 `scrollLeft` 설정과 만나면 끈적이니 휠/드래그 영역에선 빼거나 드래그 중 `auto`로.
- 휠 경계 양보 로직은 순수함수로 빼 단위테스트한다(스크롤 불가·시작/끝 경계 양보·중간 전수) — DOM 없이 RED→GREEN 가능한 부분.
...
## N-123. overflow:hidden은 사용자 스크롤만 막는다 — 프로그래매틱 scrollTo는 그대로 동작

**한 줄 요약**: `overflow:hidden`인 컨테이너도 콘텐츠가 박스를 넘으면 여전히 **스크롤 컨테이너**다 — `scrollWidth>clientWidth`이고 JS `element.scrollLeft = x` / `scrollTo({behavior:'smooth'})`가 정상(애니메이션까지) 동작한다. `hidden`이 끄는 건 **사용자 발起 스크롤**(휠·드래그·터치·스크롤바)뿐. 그래서 "자동으로만 움직이고 사용자는 못 굴리는 캐러셀"은 `overflow:hidden` + JS 자동 `scrollTo`로 깔끔히 떨어진다.

### 배경 / 문제

- 자동 캐러셀(시간 경과 한 칸 스크롤)에서 **사용자 수동 스크롤만 제거**하고 싶다.
- 직관적 오답 ①: `overflow`를 `visible`로 → 콘텐츠가 안 잘리고 박스가 스크롤 컨테이너가 아니게 돼 **자동 `scrollTo`까지 죽는다**.
- 직관적 오답 ②: `auto`/`scroll` 유지 → 사용자가 휠·드래그·스크롤바로 굴릴 수 있어 목적 미달.

### 개념 (왜 되는가)

- CSSOM View 스펙상 스크롤은 **"scrolling box"** 에 작용하며, 이는 overflow가 `hidden`이든 `auto`/`scroll`이든 **동일하게 성립**한다(스크롤 컨테이너 여부의 경계는 `visible` 대 그 외).
- `overflow:hidden`은 **스크롤바와 사용자 입력 스크롤 메커니즘만** 억제할 뿐, 박스는 여전히 스크롤 가능 — `scrollWidth`/`scrollLeft`가 살아있고 `scrollTo({behavior:'smooth'})`가 부드럽게 애니메이션된다. 흔한 "JS 제어 캐러셀" 기법의 토대.

### 디테일 · 주의

- 자동 스크롤만 남길 땐 `overflow-x: hidden`으로 충분 — 휠·`pointer`(드래그)·터치 핸들러는 제거하고, 스크롤바 숨김(`scrollbar-width`·`::-webkit-scrollbar`)도 더는 불필요(`hidden`이면 스크롤바 자체가 없음).
- 프로그래매틱 smooth 스크롤도 **`scroll` 이벤트를 발사**한다 → 중앙 인덱스/라벨 갱신을 `@scroll`에 걸어두면 사용자 스크롤을 떼어내도 자동 스크롤에서 그대로 갱신된다.
- `scroll-snap-*`은 사용자 스크롤 안착용이라 **자동 전용이면 불필요**(자동 엔진이 정확히 `i·step`에 착지하므로 snap 없이도 정중앙).
- 역방향으로도 유용: 모바일 가로 스와이프를 막고 JS로만 넘기고 싶을 때 같은 원리.

### 관련

- **N-120**(overflow-x 데스크톱 휠·드래그 — 이 노트의 **반대편**: 사용자 스크롤을 *붙이는* 쪽. 함께 보면 overflow-x 스크롤의 ON/OFF 양면), **N-121**(중앙 포커스 캐러셀 — 같은 무대, 이번엔 사용자 입력 제거), **N-117**(섬 실 브라우저 검증). 대시보드 '내 정원' 무대 사용자 스크롤 제거.

---

## N-124. 반응형 분기를 JS·CSS로 이원화하면 스크롤바 폭만큼 경계가 어긋난다 — 단일 출처로 통일

**한 줄 요약**: 화면 폭으로 레이아웃을 가르는데 **분기 판단은 JS(`window.innerWidth`)로, 그에 따른 조상 요소 폭 변경은 CSS `@media (min-width:N)`로** 나눠 하면, 둘의 폭 기준이 **스크롤바 폭(~15px)만큼 달라** 경계에서 어긋난다 — 넓다고 2단(split)을 그렸는데 컨테이너는 아직 안 넓어져 찌그러지거나, 그 반대. 해결은 **분기와 폭 변경을 한 출처로**: JS가 `chooseLayout(innerWidth)`로 판정한 같은 결과로 `body`에 클래스를 토글하고, CSS는 그 클래스(`@media` 아님)로 컨테이너를 넓힌다.

### 배경 / 문제

- 독서 기록 기록 카드: 좁으면 pill 탭(stacked), 넓으면 좌/우 2단(split). split일 땐 모바일-우선 `.container{max-width:460}`를 넓혀야 2단이 안 찌그러진다.
- 순진한 구성: ① Vue가 `window.innerWidth`로 `layout`(split/stacked) 결정 → DOM 구조 분기, ② 컨테이너 확장은 `@media (min-width:880px){ .container{max-width:940px} }`.
- 경계(880px) 근처에서 **둘이 다른 값을 본다** → split DOM이 460 컨테이너에 끼거나(찌그러짐), 넓은 컨테이너에 한 단만(허전) 같은 *경계 깜빡임*이 폭 ~15px 구간에서 생긴다.

### 개념 (왜 어긋나나)

- `window.innerWidth`는 **세로 스크롤바 폭을 포함**한 뷰포트 내부 폭이다(MDN).
- CSS `@media (width)`의 스크롤바 포함 여부는 **역사적으로 브라우저마다 다르다**(스크롤바를 빼고 재는 구현이 흔함). 즉 같은 창에서 `innerWidth=880`인데 `@media`는 `865`로 보고 `min-width:880`이 **안 맞을** 수 있다.
- 기준이 다른 두 신호로 같은 경계를 판단하니, 정확히 그 둘이 갈리는 스크롤바 폭만큼의 창 크기 구간에서 한쪽만 트리거된다.

### 해결 — 단일 출처(single source of truth)

- 분기 **판단을 한 곳**(`chooseLayout(window.innerWidth)`, 순수함수)으로 두고, **컨테이너 폭도 같은 신호로**: split이면 `document.body.classList.toggle('history-wide', true)`, CSS는 `body.history-page.history-wide .container{ max-width:940px }`처럼 **클래스로만** 확장(미디어쿼리 사용 안 함). 이러면 DOM 분기와 컨테이너 확장이 항상 같은 순간에 함께 바뀐다.
- `resize`에 `chooseLayout` 재평가 + 클래스 토글, `onUnmounted`에서 클래스 제거(섬 페이지 밖 `body`를 건드리므로 정리 필수).
- 임계값은 순수함수로 빼 **경계 테스트**(임계 미만→stacked / 정확·초과→split)로 못박는다 — 시각이 아니라 로직이라 단위테스트가 값지다.
- 실측(static-preview, 스크롤바 있는 환경): 880px에서 `history-wide` 부착·컨테이너 833px로 정상 확장·panes 480/331(=1.45:1). `@media`였다면 880에서 미스했을 지점.

### 디테일 · 주의

- 반대로 **CSS만으로** 두 모드를 만들 수 있으면(마크업 동일·시각만 토글) 미디어쿼리 단독이 더 간단하다 — 이 노트는 **DOM 구조 자체가 모드마다 다를 때**(여기선 탭+한 패널 vs 헤딩 2개+2단) JS 분기가 필요해 생기는 함정이다.
- 섬(island)이 자기 **조상**(`.container`)의 폭을 바꿔야 할 때 `body` 클래스 토글이 흔한 수단 — 조상은 컴포넌트 밖이라 자기 스타일로는 못 넓힌다.
- overflow 스크롤바를 `scrollbar-gutter: stable`이나 overlay로 두면 불일치가 줄지만, 환경 의존을 없애려면 애초에 단일 신호가 안전하다.
...
## N-128. git trailer는 "맨 끝 문단" 하나만 파싱한다 — squash가 위치를 흔들면 %(trailers) 조회가 깨진다

> **한 줄 요약**: `git log --format='%(trailers:key=...)'`·`git interpret-trailers --parse`는 커밋 메시지의 **마지막 문단(블록) 하나**만 trailer로 본다. RFC822 헤더처럼 "어디 있든 `Key: value`면 잡는" 게 아니다. GitHub squash가 `Co-authored-by`를 `---------` 구분선과 함께 맨 끝 블록으로 덧붙이면, 그 앞에 둔 커스텀 trailer(`Session-Model`/`Session-Effort`)는 메시지 중간으로 밀려 **구조 조회로는 안 보인다**(텍스트는 남아 grep으론 잡힘).

### 자세한 설명

git이 trailer 블록을 찾는 규칙(porcelain `%(trailers)`와 plumbing `interpret-trailers`가 같은 알고리즘):

1. 메시지를 빈 줄로 문단으로 나누고, 제목을 뺀 **맨 끝 문단 하나**만 trailer 후보로 본다.
2. 그 문단이 인정되려면 줄들이 대체로 trailer 모양(`Key: value` 또는 들여쓴 continuation)이어야 한다 — 기본 휴리스틱은 **그 블록의 25% 초과가 trailer-like**일 것. 평문·구분선이 한 줄만 섞여도 블록 전체가 탈락한다.
3. 따라서 "마지막 문단 밖"의 `Key: value`는 전부 본문으로 간주된다.

**squash와의 충돌**: GitHub squash는 브랜치 커밋 메시지들을 이어붙인 뒤 끝에 `---------` + `Co-authored-by:`를 새 블록으로 붙인다 → 마지막 블록 = `Co-authored-by`뿐 → `Session-*`는 중간으로 밀려 조회 불가.

### 코드 위치 / 실증

- 규칙: BookTimer는 세션마다 `Session-Model`/`Session-Effort`를 커밋 trailer로 남긴다(CLAUDE.md 「계획 우선 → 세션 메타 기록」, #543).
- 실증(scratch repo, git 2.47.1): `Session-*`가 **마지막 블록**이면 `%(trailers:key=Session-Model,valueonly)` → `claude-opus-4-8`. **중간**이면 빈 값(대조 성공). `interpret-trailers --parse`도 동일. 평문 한 줄 섞인 블록은 진짜 trailer까지 빔.
- 조회 우회: `git log --grep='Session-Model'`(중간이어도 매치), 값은 `git log -1 --format=%B <c> | grep -oP '^Session-Model:\s*\K.*'`.

### Q&A 대비

- **Q. trailer면 메시지 어디 있든 `git log %(trailers)`로 꺼낼 수 있지 않나?** → 아니다. **맨 끝 문단 한정**이다. trailer는 "필드"가 아니라 "메시지 끝의 한 블록"이라는 위치 개념.
- **Q. 그럼 squash 후 세션 메타는 사라지나?** → 텍스트는 남는다(`--grep`·`%B`로 읽힘). 다만 `%(trailers)` 구조 조회로는 안 잡히니 조회 방법을 grep으로 바꾼다.
- **Q. 구조 조회를 살리려면?** → `Session-*`를 squash 메시지의 **마지막 trailer 블록**(`Co-authored-by`와 같은 블록)에 두거나, 위치 비의존이 필요하면 `git notes`.
...
## N-137. 같은 데이터라도 청중(audience)이 반대인 두 화면은 합치지 말고 가른다 — 가시성 게이트를 한 곳에 몰면 누수·IDOR

> **한 줄 요약**: 같은 도메인 데이터(예: 책 목록)를 보여주는 두 화면이라도, 한쪽이 **본인 전용(전체·관리)**이고 다른 쪽이 **공개(부분집합·읽기전용)**면 *청중(audience)이 정반대*다. "콘텐츠가 겹치니 합치자"는 충동이 들지만, 합치면 self/other·공개/비공개 분기와 IDOR·비공개 누수 불변식이 **한 컨트롤러에 집중**돼 보안 경계가 흐려진다. 화면은 **청중으로 가르는 게 안전**하고, 사용자가 느끼는 혼동은 대개 '기능 중복'이 아니라 '용어 겹침·동선 비대칭'이라는 **표면 문제**라 라벨·네비로 푼다.

### 무엇이 문제로 보였나

"내 책 목록을 보여주는 화면이 둘인데(개인 관리용 + 공개 프로필용) 합쳐야 하지 않나?" — 데이터·UI가 닮아 중복처럼 보인다. 하지만 합치기 전에 물어야 할 건 "콘텐츠가 같나"가 아니라 **"누가 보는가"**다.

### 왜 합치면 안 되나 — audience 경계 = 보안 경계

- **본인 전용 화면**: default-deny + 소유권 강제(`findByIdAndUser`)로 *타인 접근 경로 자체가 없음*. 전체(공개+비공개) 노출, 추가·삭제·상태변경 같은 쓰기.
- **공개 화면**: 누구나 열람하되 *항상 PUBLIC만* 거르고(미존재·차단·권한 3중 가드), 읽기전용 + 소셜 액션.
- 둘을 한 화면에 합치면 **가시성 모드 토글**이 상시 떠, self/other·공개/비공개 분기와 "비공개가 안 새는가"라는 불변식이 한 곳에 집중된다 → 노출 사고 표면이 커진다. 분리돼 있으면 두 보안 게이트(소유권 vs PUBLIC-only)가 다른 컨트롤러/패키지에 살아 경계가 구조적으로 지켜진다.

### 진짜 혼동의 원인은 표면이지 기능 중복이 아니다

사용자가 "헷갈린다"고 할 때 코드를 보면 중복이 아니라 보통 두 가지 표면 문제다:
- **용어 겹침** — 같은 단어가 두 맥락에 쓰임(예: '책장'이 *개인 관리 화면*과 *공개 화면 속 한 패널*에 동시에).
- **동선 비대칭** — A→B 링크는 있는데 B→A는 없거나 본문 힌트로만.

→ 해법은 화면 통합이 아니라 **용어를 갈라 명확히**하고 **네비를 대칭화**하는 것. 비용이 작고 보안·전략을 안 건드린다.

### 일반 원칙

- **통합 판단의 축은 "데이터 중복"이 아니라 "audience·가시성 게이트"다.** 같은 데이터라도 보는 주체가 다르면 다른 화면으로 둔다([[N-037]]: SNS 확장은 데이터를 새로 저장하지 않고 조회 주체만 바꾼다 — 그 조회 주체의 차이가 곧 화면 경계).
- 제품 전략(thesis)과도 맞물린다: 사적 도구(입문자의 작은 습관)와 공개 무대(소셜 연결)를 한 화면에 뭉개면 약한 사용자에게 소셜을 강요해 콜드스타트 면역을 깬다.
- "합치고 싶다"는 충동이 들면, 합치려는 두 화면의 **첫 줄에 'authenticated() + 본인'과 'public + PUBLIC만'이 동시에 필요한지** 본다. 그렇다면 합치지 말라는 신호다.
...
## N-139. 친구 추천의 두 엔진과 콜드스타트 사다리 — 그래프·관심사 하이브리드, 추천 이유의 효과

> **한 줄 요약**: 사람 추천은 "아는 사람"(그래프: 친구의 친구·상호친구)과 "좋아할 사람"(관심사: 같은 항목 소비·태그) 두 엔진의 하이브리드다. 데이터가 적을수록 인기→속성→그래프→협업필터링 순으로 켜고(콜드스타트 사다리), "왜 추천했는지"를 같이 보여주면 수락률이 오른다.

### 두 엔진

- **그래프 엔진 = "아는 사람"** — 친구의 친구(FoF)·상호 친구 수. 핵심 원리는 **삼각 닫힘(triadic closure)**: A-B, A-C가 이어져 있으면 B-C도 이어질 확률이 높다(공통 친구를 많이 공유할수록 같은 집단일 확률↑). 지표는 공통 이웃 수(Common Neighbors)·Jaccard·Adamic-Adar. Facebook PYMK·LinkedIn·Twitter WTF의 1순위 신호. **약점**: 작거나 희박한 네트워크에선 2-hop이 거의 비어 후보가 안 나온다(무작위 그래프에서 평균 연결수 ⟨k⟩가 1을 넘겨야 "거대 연결 덩어리"가 생겨 FoF가 작동).
- **관심사 엔진 = "좋아할 사람"** — 같은 항목을 소비한 사람(협업필터링)·공통 태그/속성. 그래프가 없어도 동작해 콜드스타트에 강하다. 독서 SNS **StoryGraph**가 대표적: *책 추천에 쓰는 취향 신호를 그대로 "나처럼 읽는 사람" 매칭에 재사용*한다(별도 사람 그래프를 새로 만들지 않고 가진 책 신호 하나로 책·사람 추천을 동시에 푼다). Goodreads는 책 신호가 있는데도 사람 추천엔 안 써 "취향 맞는 사람 자동 추천이 없다"는 미충족 수요를 남겼다.

### 콜드스타트 사다리

데이터가 적을수록 앞쪽부터 켠다: **① 인기/트렌딩**(0-데이터에서도 안전한 폴백) → **② 속성/콘텐츠**(온보딩 관심·태그·같은 책) → **③ 그래프(FoF)**(첫 연결 몇 개가 생긴 뒤) → **④ 협업필터링**(행동 로그가 쌓인 뒤). 순수 CF가 가장 늦다. 어떤 신호를 택하든 신규·저활동 유저용 **인기/랜덤 폴백**을 병행해 "빈 추천"을 막는다.
...
소개팅 주선과 같다. **그래프**는 "내 친구의 친구"를 소개하는 것(아는 사이라 신뢰), **관심사**는 "같은 취미를 가진 사람"을 소개하는 것(아직 모르지만 잘 맞을 사람). 주선소에 갓 들어와 둘 다 없으면 일단 "요즘 인기 많은 사람"부터 보여준다(콜드스타트 폴백). 그리고 "왜 이 사람인지"(공통 친구·같은 책)를 말해주면 만나볼 마음이 더 생긴다.
...
## N-142. 세로 모바일에서 부감형 게임 무대는 게임감 천장 — 세로 재미는 애착/수집, 반응형 2모드로 자세별 분기

> **한 줄 요약**: 가로로 넓은 부감(top-down·isometric) 게임 무대는 세로 화면에서 "도시 부감 와우"를 구조적으로 못 준다. 세로의 재미는 ①1:1 애착(돌봄) ②수집뿐 — 한 무대를 모든 자세에 욱여넣지 말고 **세로=경량 돌봄/수집 뷰, 가로·데스크탑=풀 무대**로 orientation 분기하는 게 매체에 맞다.

### 맥락

CoC식 아이소메트릭 마을(가로로 넓은 부감 무대)을 모바일 세로에 그대로 띄우려다 강제 회전(N-141)·반응형 fitCamera를 거쳤지만, "세로에서도 무대가 쾌적한가"는 기술 문제가 아니라 **매체 문제**였다. 세로 화면은 폭이 좁아 부감 전경(도시가 한눈에 펼쳐지는 와우)을 못 담는다 — 줌아웃하면 작아서 안 보이고, 줌인하면 부감감이 사라진다.

### 왜 — 세로는 부감 와우를 구조적으로 못 준다

부감/아이소 무대의 재미 = "내 도시·정원이 한눈에 펼쳐진 광경". 이건 **가로 종횡비**가 전제다. 세로(폭<높이)에선 같은 월드를 담으면 좌우가 잘리거나 너무 작아진다. 6개 컨셉(CoC유지/세로디오라마/다마고치/수집진열/사이드뷰/반응형2모드)을 심사해도 세로 게임감은 모든 안에서 천장이었다 — 세로 화면이 줄 수 있는 재미는 부감 와우가 **아니라** ①캐릭터 하나에 대한 1:1 애착(돌봄 루프) ②모으는 수집이다.

### 해법 — 반응형 2모드 (한 무대를 모든 자세에 욱여넣지 않기)

- **세로(모바일 portrait)** = 경량 돌봄/수집 뷰 — 히어로 1명 크게 + 돌봄 액션(먹이) + 성장 미터 + 수집 카드 스택. 공간 플레이를 포기하고 애착에 집중.
- **가로·데스크탑·앱** = 기존 부감 무대 그대로.
- `matchMedia('(max-width:…) and (orientation:portrait)')` 한 줄로 분기. 데이터·API는 공유(추가 비용 0), 뷰만 둘로 가른다.

핵심은 "세로에서 무대를 **살리려** 싸우는 대신, 세로에 **맞는 다른 재미**로 갈아탄다"는 발상 전환.

### 교훈

| 한 무대 강제 (모든 자세에 부감 무대) | 자세별 2모드 분기 |
|---|---|
| 세로에서 부감 와우 불가 → 줌·회전 우회하다 깨짐(N-141) | 세로는 애착/수집 뷰로 — 매체에 맞는 재미 |
| "반응형 = 한 레이아웃이 다 커버" 착각 | 반응형 = 자세별로 **다른 화면**일 수도 있다 |
...
## N-148. 열거완화·비동기 발송 설계에선 프론트로 실발송을 검증 못 한다 — 받은편지함·발송자 통계가 유일한 확증

> **한 줄 요약**: 계정 열거완화(존재/부재/소셜 모두 **동일 응답**)와 `@Async` 발송(실패를 catch+로그로 **삼킴**)이 겹치면, "메일 보냄" 페이지·HTTP 200 같은 프론트/요청 측 신호는 실제 발송을 증명하지 못한다 — 설계상 성공과 실패가 같은 화면이기 때문. 실발송의 확증은 요청 흐름 **밖**에만 있다: 수신자 받은편지함 도착, 또는 발송자 측 관측(SES 콘솔 Sends/Deliveries·CloudWatch 발송 실패 로그).

### 맥락

2026-07-09 "비밀번호 찾기·이메일 인증이 운영에서 활성화됐나?" 확인 세션. 토글(`BOOKTIMER_EMAIL_ENABLED=true`)·SMTP 자격증명(SSM)·코드 완비를 실측하고, 운영에서 비밀번호 재설정 폼을 제출해 **POST 200 + "메일을 확인해 주세요" 페이지**까지 봤지만 그게 발송을 증명하지 못했다. 두 설계가 겹친 탓:

- **열거완화**: `PasswordResetService.requestReset`은 계정 존재/부재/소셜 여부와 무관하게 동일한 안내 페이지를 반환한다(계정 열거 방지, [[N-052]]). 발송/무발송 차이가 화면에 안 드러난다.
- **비동기 발송**: `EmailDispatcher.dispatch`가 `@Async`라 SMTP 실패를 `catch`+로그로 격리한다 — 실패해도 요청 스레드엔 예외가 안 올라와 500조차 안 뜬다.

결국 시도한 계정이 구글 로그인(소셜)이라 발송 자체가 없었는데도 화면은 성공과 똑같았다. `testid`는 시드 이메일이 `@local.test`(라우팅 불가 예약 TLD)라 받은편지함이 없어 무용. 최종 확증은 **새 네이버 로컬 가입 → 인증 메일 실제 도착 → 인증 성공**을 사용자가 받은편지함에서 눈으로 확인해서 얻었다.

### 교훈 / 일반화

- **"메일 갔나?"는 요청 측이 아니라 수신 측·발송자 측으로 답하라** — 프론트 성공 화면·2xx·리다이렉트는 "앱이 발송을 *시도*했다"까지만 증명한다. 열거완화는 성공/실패 화면을 일부러 같게 만들고, 비동기 발송은 실패를 삼키므로, 실발송은 코드 밖 관측 지점(받은편지함 도착 / SES 콘솔 Sends·Bounces / CloudWatch 발송 예외 로그)에서만 확증된다.
- **검증엔 "수신함을 열 수 있는 계정"이 선결 자원이다** — 도착을 보려면 (a) 로컬(비번 보유) 계정이면서 (b) 그 이메일 받은편지함을 열 수 있어야 한다. 소셜 계정은 비번 리셋 발송이 아예 없고(리셋할 비번 없음), 플레이스홀더 이메일(`@local.test`·`@example.com`)은 수신함이 없다. 검증 설계 초입에 이 자원부터 확보하라 — 없으면 여러 경로(리셋→testid→새 가입)를 헛돈다.
- **활성화 검증은 계층으로 쪼개 각 계층의 증거를 따로 든다** — ①토글 ON(설정 실측) ②자격증명 주입(배선) ③앱 발송 경로(2xx·무500) ④실발송 도착(수신함/발송자 통계). ①~③이 다 초록이어도 ④는 별개다. 어디까지 증명됐고 어디부터가 추론인지 정직하게 표시하라.

### 관련

- [[N-091]](SES 프로덕션 액세스·토글 점등 ≠ 실발송 — 그 노트는 "발송이 *가능*한 조건", 이 노트는 "발송이 *됐는지 확인*하는 방법"의 짝). [[N-052]](계정 열거완화 — 동일 응답 흡수). 코드: `email/EmailDispatcher`(@Async 실패 격리)·`email/PasswordResetService`(열거완화 발송)·`web/SignupController`(가입 시 인증 메일 발송). SES 배관: `email/SmtpEmailSender`/`LoggingEmailSender`(토글로 갈림).

## N-149. 서점 출간일은 번역판 날짜다 — 원작 연도와 작가 국적은 Wikidata work로 얻고 ISBN 아닌 원제와 저자로 잇는다

> **한 줄 요약**: 국내 서점 도서 API(알라딘)의 `pubDate`는 원작 최초 출간일이 아니라 **한국어판(번역/재쇄) 출간일**이다 — "작품이 나온 시대"가 필요하면 이 값을 쓰면 틀린다(1866년 러시아 소설의 2020년 번역판 날짜). 원작 최초 출간연도·작가 국적을 **무료·무인증**으로 주는 사실상 유일한 소스는 **Wikidata 작품(work) 아이템의 `P577`(publication date)·저자 `P27`(country of citizenship)**이고, 링크 키는 한국어판 ISBN이 아니라(대부분 Wikidata에 에디션으로 없음) **원제(원서 제목)+저자**다 — 알라딘 `subInfo`의 원제 필드가 그 매칭 키이자 "번역서인가" 플래그가 된다. OpenLibrary `first_publish_year`·Google Books `publishedDate`는 번역판/에디션 연도라 원작연도로 신뢰하면 안 된다.

### 맥락

2026-07-14 "책 맞춤 선곡(책에 어울리는 플레이리스트)" 기능을 [plan.md](../plan.md) §기능 로드맵에 문서화하며(작가 국적·원작 시대·작품 분위기로 음악을 매칭) 웹 리서치 + 적대적 검증으로 확인했다. `Book`이 저장하는 `pubDate`(알라딘)가 원작연도가 아님은 엔티티 주석에 이미 있었고, 대안 소스를 실측했다:

- **Wikidata**: 작품 아이템 `Q165318`(『죄와 벌』) → `P577`=1866(무인증 SPARQL `query.wikidata.org/sparql` 실측), 저자 `P50`=Dostoevsky, 국적 `P27`=러시아제국, 생몰 `P569/P570`=1821/1881, 원어 `P407`=Russian 을 한 쿼리로 받았다.
- **OpenLibrary**: `search.json?title=Crime+and+Punishment&author=Dostoevsky`의 상위 결과 `first_publish_year`가 1941·2014(번역판)로 나온다 — 1866을 대표값으로 주지 않는다(전체 결과 어딘가엔 1866 work도 있으나 정렬 하위).
- **Google Books**: `volumeInfo.publishedDate`는 조회된 **에디션**의 날짜이며 원작 최초 출간연도 전용 필드가 없다(공식 Volume 레퍼런스).

### 교훈 / 일반화

- **"출간일" 필드는 "어느 판본의 날짜인가"를 먼저 물어라** — 서점·서지 API의 날짜는 대개 "이 상품(에디션)"의 출간일이지 "이 작품(work)"의 최초 출간연도가 아니다. 번역 고전에선 둘이 100년 넘게 벌어질 수 있다. 시대·역사 맥락이 필요한 기능(선곡·타임라인·"그때 그 시절")은 반드시 **work 단위 연도**를 따로 구한다. `pubDate`로 시대를 판정하는 순간 조용히 틀린다.
- **작품(work) vs 판본(edition)은 서지 데이터의 근본 분리(FRBR)** — Wikidata는 이 둘을 별개 아이템으로 나눠 원작연도는 work의 `P577`에 있다(에디션은 `P629`='edition or translation of'로 work에 연결). OpenLibrary도 work/edition을 나누지만 **번역마다 work를 쪼개** `first_publish_year`가 그 번역 work의 최초 에디션 연도가 돼버려 원작연도로는 신뢰 불가(크라우드 병합 품질에 의존).
- **동일성 키는 "그 데이터가 실제로 색인하는 키"로 골라라** — 한국어판 ISBN-13은 Wikidata/OpenLibrary에 에디션으로 대개 없어 ISBN 직접 조회로는 원작 work에 못 닿는다. 매칭 키는 **원제+저자**여야 하고, 국내 서점 API의 "원제(원서 제목)"는 **번역서에만** 채워지므로 그 존재 자체가 "번역서인가"를 알려주는 값싼 신호로 이중 활용된다(원제 없음 → 국내 원작 추정 → `pubDate`≈원작연도 근사 안전).
- **객관 사실은 무료·무인증 그라운딩 소스로 확정, LLM은 미스 폴백** — 원작연도·국적처럼 정답이 있는 축은 Wikidata SPARQL로 값싸게 확정하고, LLM은 롱테일 미스에만 쓰되 닫힌 어휘·enum 제약을 건다. 분위기처럼 객관 정답이 없는 축만 처음부터 LLM 분류로. (같은 정신: 사실집계=코드/서술=LLM 분리 — [[N-055]] 계열의 결정적 태거 접근.)

### 관련

- [plan.md](../plan.md) §🎵 책 맞춤 선곡(이 노트를 낳은 기능 — 보강 파이프라인 설계). [[N-055]](null-state 엔티티 누출 — 보강 실패 책이 목록에 새지 않게 경계 테스트). [[N-041]](외부 검색 API의 필드 옵션이 문서대로 안 도는 함정 — 결과를 신뢰 말고 후검증). 코드: `book/Book.java`(`pubDate`=알라딘 한국어판 출간일, `category`=국적 거친 프록시), `book/AladinBookSearchClient`(`subInfo`·원제 확장 지점), `book/BookSearchClient`(`lookupByIsbn` 백필 포트).
