...
| 2026-06-20 | N-104 (Thymeleaf `${}` 범위가 산술 의미를 바꾼다 — `${x} / n`(밖)은 Thymeleaf 산술로 소수, `${x / n}`(안)은 SpEL이 타입대로 → long/long=정수. 책 상세 누적 소수 버그 / T-074) |
| 2026-06-21 | N-105 (사파리/iOS 캐시 3계층 — ①HTTP 캐시(헤더) ②SW Cache Storage(시간만료 없음·코드 주도) ③SW 스크립트 갱신(iOS 게으름·상한 24h). iOS 7일 미사용 삭제+홈PWA 면제·스토리지 분리 → "Safari 탭 옛 버전 / 홈 PWA 최신" / N-101·T-071·T-075) |
| 2026-06-21 | N-106 (SSR 즉시 반영 vs SPA 캐시 갇힘 — 변경분이 어디 담기냐의 차이: SSR은 매 요청 새로 그리는 동적 HTML(비캐시·SW network-first)이라 배포 즉시, SPA는 고정명 정적 번들 garden.js(캐시 대상)라 stale / 정석 해법=콘텐츠 해시 파일명(cache busting): URL이 바뀌어 무조건 새로 받음·옛 파일 무해 / Vite 기본 해시를 끈 이유=Thymeleaf가 `@{/garden/garden.js}` 하드코딩 참조 → 해시 도입엔 manifest 주입·`?v=` 배선 필요 / 현재는 고정명+SW network-first(N-105 ②계층)로 우회 / N-098(섬 아키텍처)·N-105·T-075) |
