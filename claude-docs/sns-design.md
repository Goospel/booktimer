...
### 3.5 ⚠️ 잔디·총시간의 프라이버시 — 비공개 책 세션 처리 (결정 필요)

잔디(일자별 독서 시간)·총 독서 시간은 **세션을 합산**해 만든다. 그런데 일부 책이 PRIVATE면, **그 책의 세션이 타인이 보는 잔디·총시간에 합산되면 비공개 책 독서가 간접 누출**된다(무슨 책인진 몰라도 "이 날 N분 읽었다"가 샘).

**제안(일관성 우선)**: 타인에게 보이는 잔디·총시간은 **PUBLIC 책의 세션만**으로 계산(viewer-dependent). 본인이 보는 잔디는 전부 합산(지금과 동일).
- 책 미지정(book=null) 세션은 어떻게? → **제안: 타인 잔디에서 제외**(공개로 명시되지 않은 활동).
- 대안(노출 우선): 잔디는 "무슨 책인지"가 아니라 "읽었다"만 보여주니 전부 합산해도 된다는 관점 — 단 PRIVATE-only 사용자의 총 독서량이 새므로 프라이버시 약함.

→ **§11 열린 질문 ①.** 제안은 "PUBLIC 책 세션만". 구현 시 잔디 빌더에 viewer 가시성 필터를 끼운다.
...
> 요구사항으로 해결된 것: 관계=팔로우 / 공개단위=책별 / 핸들=유니크 닉네임 / 카운트=팔로우 스코프. 아래는 남은 것.

1. **잔디·총시간 프라이버시(§3.5)** — 타인이 보는 잔디·총시간에 PRIVATE 책 세션을 포함? (제안: **PUBLIC 책만**) 책 미지정 세션은? (제안: 타인 잔디 제외)
2. **닉네임 백필 규칙** — 기존 NULL/중복 닉네임을 무엇으로 채우나(`user{id}`? email local-part?). 유니크 제약 전 선결.
