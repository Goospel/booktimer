...
> **상태**: ✅ **구현 완료** — 이 설계대로 출하됨(`AdminUserController`·`AdminUserService`·`admin-users.html`·`admin-user-detail.html`, 이후 부채 드릴다운 `admin-user-debt.html` 추가). 관리자 대시보드 **④ 데이터 조회**(마지막 단계)로, ①접근경계 #144 · ②동선격리 #150/#151 · ③통계요약 #152 위에 얹었다. (이 문서는 설계 기록 — 구현 세부는 코드·changelog 참조.)
...
- **CSV 내보내기·필터(provider별·온보딩별)** — 수요 생기면.
  > **⚙️ QueryDSL/동적 쿼리 트리거 (2026-06-13 논의)**: 이 필터(provider·온보딩·가입일 범위 등 **임의 조합**)가 실제 구현되는 시점이 **동적 쿼리(QueryDSL)를 처음으로 검토할 지점**이다. admin 목록은 **전체 사용자 스코프**라 BookController 책장처럼 전건 로딩+`stream().filter()`로 못 푼다(이미 `Pageable` 페이징 중). 조건이 런타임 가변 조합이 되면 파생 쿼리는 메서드 폭발·정적 `@Query`는 분기 폭발 → 그때 QueryDSL `BooleanBuilder`가 비용을 정당화한다. 그전(정적·소량)엔 불필요. ※ Boot 4/Hibernate 7 호환은 도입 전 검증(io.github.openfeign fork 이력).
