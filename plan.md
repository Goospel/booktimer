...
>   ⚠️ **2026-08-29 갱신 — 그 「리마운트」는 이제 공짜가 아니다.** 작성이 바텀시트가 되면서(아래 절) 탭이
>   언마운트되지 않아, 서재 직행으로 글을 써도 이 박스가 **옛 「여백 N」 그대로 굳었다**(목 모드 실측).
>   `composeEpoch` 키로 명시 remount를 걸어 되돌렸다 — 캐시 보류 판단은 유효하나, **그 전제가 구조에서
>   명시 코드로 옮겨졌다**는 것이 이 줄의 요점이다.
...
>   - **후속 ✅ 2026-08-29 — 탭 라벨에서 개수를 걷었다**: 「내가 쓴 여백 3 / 모두의 여백 12」의 숫자가
>     **싸 보인다**는 사용자 판단. 담백하게 글자만 남겼다. 값을 잃지 않는다 — 「내가 쓴 여백」의 수는 바로
>     아래 게시판 머리글(`글 N`)이 이미 말하고, 「모두의 여백」은 그 탭을 눌러야 받는 값이라 탭 줄에서는
>     어차피 절반이 비어 있었다. 라벨이 상수가 되어 `marginTabLabel`과 `mineCount`/`allCount` 프롭이
>     **통째로 죽어** 함께 걷었다(순감 −28줄). ⚠️ 범위는 탭 줄뿐이다 — 서재 인라인 박스의 「여백 N」과
>     게시판 머리글은 그대로 둔다(개수를 말하는 자리는 거기다).
...
> ✅ **2026-08-29 — 「여백에 글 남기기」를 전체 화면에서 하단 슬라이드업 바텀시트로 (사용자 시안 승인).**
> 한 줄 끄적이러 들어가는데 화면이 통째로 갈아치워지는 값이 컸다 — 밑 화면이 언마운트됐다 다시
> 마운트되며 스크롤·탭 자리가 날아갔고, 쓰기가 읽던 자리와 이어지지 않는 별개의 여행처럼 느껴졌다.
>
> - **움직임은 `Sheet` 공용 껍데기에 붙였다** — 딤 0.25s 페이드 + 패널 `translateY(102%)→0` 0.28s.
>   태깅·서재 관리·좋아요 명단까지 **네 시트가 함께** 부드러워진다(일관성, 사용자 승인 사항).
>   ⚠️ **일회성 마운트 애니메이션 · `transform`·`opacity`만** — T-176(발광 `box-shadow` 무한 반복이
>   실기기를 무너뜨린 자리)의 규율을 그대로 따른다. `ui.test.tsx`가 keyframes의 **속성 집합**을
>   `{transform, opacity}`로 잠가, 다음 사람이 그림자를 얹으면 테스트가 운다.
>   인라인 style이 아니라 클래스인 이유는 `prefers-reduced-motion`이 인라인을 이길 수 없어서다.
> - **교체가 아니라 겹침** — 밑 화면 판정을 `underCompose`(= `composeBook`을 걷어낸 `marginScreen`)로
>   빼고, 깔릴 수 있는 화면 넷이 전부 `withCompose`를 지나 시트를 얹는다. 시트 zIndex 200/201이
>   탭바(100)를 딤으로 덮으므로 억제 코드는 없다.
> - ⚠️ **겹침이 「작성 후 반영」을 삼켰다 — 이 작업에서 가장 미묘한 자리다.** 전체 화면 시절엔
>   닫는 순간 밑 화면이 새로 마운트되고 **그 재조회가 곧 "방금 남긴 글이 보인다"**였다. 시트는 밑
>   화면을 안 죽이므로 그 반영이 **조용히** 사라진다 — 목 모드에서 **두 자리**가 굳는 것을 실측했다:
>   ① 여백 상세의 글 목록 ② **서재 인라인 여백 박스의 「여백 N」**(위 「응답 캐시 ⏸ 보류」 줄이
>   기대던 바로 그 리마운트다). 글을 남긴 뒤에만 `composeEpoch`를 올려 둘 다 remount한다(취소는
>   올리지 않는다 — 바뀐 것이 없다).
>   ⚠️ **remount가 공짜라는 뜻은 아니다**(2026-08-29 리뷰 정정): 탭·고른 책은 App이 들고 있어 살아남지만,
>   `MainTabs` remount는 축하 배너·태깅 시트 상태와 스크롤을, `BookMargin` remount는 열어 둔 탭·펼친 글을
>   버리고 로딩 화면을 한 번 보인다. **다만 회귀는 아니다** — 저장 경로는 전체 화면 시절과 **똑같이**
>   전부 새로 마운트되던 자리이고, 취소 경로는 오히려 개선됐다(예전엔 취소해도 날아갔다).
> - **출구는 셋** — 시트의 ✕ · 기존 「취소」 · 딤 탭. 안드로이드 뒤로가기는 셸의 `useBackClose` 그대로.
> - ✅ **T-183 재확인**: `localStorage.clear()` 후 첫 진입에서 `position:fixed · zIndex≥99` 요소는
>   상시 탭바(z=100) 하나뿐이고 시트·딤은 0개다 — 시트는 **사용자 탭으로만** 열린다.
> - ✅ **리뷰 반영 — 겹침이 새로 연 구멍 둘**(독립 리뷰, 같은 PR):
>   - ⚠️ **딤 탭이 확인 없이 원고를 버렸다.** 전체 화면 시절의 출구는 「취소」·뒤로가기뿐이라 둘 다
>     **의도적**이었는데, 시트 밖은 **스치기만 해도** 눌린다 — 인용 200 + 본문 500자가 저장 전에 날아간다.
>     확인 시트를 새로 세우는 대신 **딤만 막았다**(`dimClosable` — 빈 시트일 때만 딤이 닫는다). ✕·취소는
>     그대로라 갇히지 않는다. `Sheet`에 `onDimClose`(기본값 = `onClose`)를 두어 딤과 ✕를 갈랐다 —
>     **한 핸들러로 묶으면 딤을 막는 순간 ✕까지 잠긴다**(다른 세 시트는 기본값이라 무변화).
>   - ⚠️ **작성 중 밑 화면 배너가 딤 뒤에서 갱신됐다 (수익 경로).** main에선 화면 교체로 언마운트되며
>     `slot.destroy()`가 광고를 껐는데, 겹침 후엔 살아남아 autoLoad를 계속 받았다 — `toss.ts`가 접힌
>     슬롯을 되살리며 스스로 금지한 「사용자는 못 보는 노출만 집계」(무효 트래픽)와 같은 상황이다.
>     `adSuppressed`로 지면을 접어 **main과 같은 동작으로** 되돌렸다(조건부 렌더 = 언마운트라 destroy가
>     그대로 돌고, 닫으면 부착이 1회 돈다). 책축 지면은 무관 — 거기선 작성을 열 길이 없다.
>   - 도달 불가 `withCompose` 2곳(책축·남의 책방)을 걷었다 — 감싸 두면 「이 조합이 가능하다」는 거짓말이
>     코드에 남는다. 깔릴 수 있는 화면은 **둘**(사람축 상세·탭 화면)이고 그 근거를 테스트에 적었다.
>   - ⚠️ **`withCompose`의 무-remount는 React의 「key 없는 최상위 Fragment 언랩」에 기댄다** —
>     Fragment에 key를 달거나 `{composer}{under}` 순서를 뒤집으면 밑 화면이 **조용히** remount된다
>     (겹침으로 얻은 것이 통째로 사라진다). 그 자리에 경고 주석을 박았다.
...
- **배너 광고(IAA) — 「여백」 두 화면** ✅ 코드 · ✅ 점등 (2026-08-25) — 세 번째 광고 포맷. 지면은 **둘**이다:
  ...
  **96px을 예약하고 죽으면 0으로 접는다**: 인라인이면 광고가 뜨는 성공 경로마다 게시판이 밀리고(layout shift),
  ...
  📦 **컴포넌트(`MarginBannerAd`)는 `Story.tsx` 안에 둔다** — 지면이 둘이지만 삽입점 둘이 다 이 한 파일이라
  별도 `ads.tsx`로 빼면 읽을 파일만 는다. **분리 트리거는 「여백 밖 화면에 배너가 생길 때」**다(그때는
  파일을 넘나드는 재사용이라 분리가 값을 한다).
  ...
  ⚠️ **값 커밋만으로는 라이브가 안 바뀐다** — env는 **빌드 시점에 구워지므로**(T-148) 5차 배포(`01a037b6`)에는
  빈 값이 들어 있어 코드가 있어도 config-gate로 꺼져 있었다. 실제 점등은 **6차 배포 `01a037f3`부터**다.
  card 인셋 정합은 실기기 항목이었다가 **SDK 소스 실측으로 앞당겨 닫았다**(위 -10px).
  ✅ **원장 종료 — A6 충족 (2026-08-25 사용자 실기기 확인)**: 「내가 쓴 여백 ↔ 모두의 여백」
  탭 왕복에서 **유령 슬롯·중복 배너·이중 구멍 없음**. 배너가 실제로 그려지는 것을 사람 눈으로 본 첫 자리이기도
  하다(목 모드엔 광고가 원리상 없어 이 항목만 끝까지 남았다).
  ✅ **심사 승인·출시 완료 (2026-08-25)** — `01a037f3`이 현재 라이브다. **광고가 라이브 사용자에게 실제로
  송출되기 시작한 시점**이고, 여기부터 콘솔 리포트에 두 지면의 노출·수익이 지면별로 따로 쌓인다.
...
- **재참여 넛지 푸시 (7일 비활동 win-back)** ✅ 코드 완료 / ✅ **점등 완료** (2026-08-18 코드 · **08-25 사용자 확인**) — **§전략 「retention
  ...
  **Flyway 0건**이고, 이메일이 훗날 점등돼도 10:00(이메일) → 19:00(푸시) 순서라 같은 날은 이메일이 선점한다.
  ...
  📝 **관찰 — 실측 도달이 0이다 (2026-08-28, `push_stats`)**: 재참여 넛지 그룹(10852)이 **발송 시도 8건 /
  도달 0건 / 읽음 0**이다. 같은 계정의 v2 템플릿은 도달이 정상(2/2·1/1)이라 배선 문제는 아니고, 위 「미동의
  사용자에게 실제로 닿는지는 별개」가 실측으로 답을 받은 쪽에 가깝다 — 알림동의문은 1개뿐이고 고지 범위가
  「목표 달성·완독」 둘인데 넛지도 같은 `termsId=114526`에 매달려 있다. 🔜 **원인 규명은 별건**이나, 광고성
  재방문이 없어져 이게 **유일한** 재참여 채널이 된 만큼 우선순위가 올라갔다.
  📝 **관찰 — 콘솔에 「하루 목표 달성 푸시」가 2개 등록돼 있다**(캠페인명·소재 동일). 서버는
  `booktimer-daily-goal-met` 하나만 부르므로 나머지는 쓰이지 않는 잔재로 보인다. 당장 해는 없으나 정리 대상.

- **스마트 발송 현황 — 절반을 쓰고 있었고, 그 절반은 플랫폼이 접었다** ✅ 기능성 3종 운영 중 /
  ⛔ **광고성은 2026-10-01 플랫폼 종료**(08-25 등록 → 08-28 종료 공지) — 「스마트발송을 도입하자」에서
  출발했는데 **조사해 보니 절반은 2026-08부터 운영 중**이었다.
  ...
  | **광고성** — 신규 유입 유도 | 08-25 등록·발송 → ⛔ **10/1 플랫폼 종료** | **0줄**(콘솔 등록만) |
  | **광고성** — 재방문 유도 | 등록 후 꺼짐 → ⛔ **10/1 플랫폼 종료** | 켜 볼 기회 없이 끝났다 |

  ⛔ **광고성 스마트 발송은 플랫폼이 접는다 (공지·판정 2026-08-28)** — 「10월 1일부터 스마트 발송의 광고성 푸시,
  알림 운영을 종료해요」. 토스 설명은 소재는 계속 느는데 전체 발송량에 한계가 있어 **등록해도 발송되지 않는 경우가
  이어졌다**는 것이다. **일정**: 소재 등록·발송 ~9/30 / 성과 조회 ~10/31(지나면 다시 못 본다) / **기능성 푸시·알림·
  알림동의문은 변경 없음**. 대안으로 9월 초 「인증 프로그램」을 예고했다.
  ✅ **우리 코드 영향은 0줄**이다 — 광고성은 애초에 콘솔 등록뿐이었다. 콘솔 실측(`push_template_list`, 08-28)에서도
  발송 그룹 6개가 전부 `contentReachType=FUNCTIONAL` · `type=SERVER`고, 광고성으로 필터하면 0건이다(광고성은
  MCP에 안 잡히는 별도 체계라 성과 저장도 콘솔 웹에서 사람이 한다).
  🔜 **남은 조치는 둘뿐**: ① 9/30까지 신규 유입 캠페인 소진(새 소재 등록은 무의미) ② **10/31 전에 성과 데이터 저장**.

  ⚠️ **광고성의 값어치는 「우리가 코드로 원리상 못 하는 일」에 있었다** — 대상이 **미니앱을 안 써 본 토스 사용자**라,
  ...
  넛지가 그대로 유일한 재참여 채널이다). ⛔ **「9월 말에 판단한다」던 분기는 사라졌다 (2026-08-28)** — 광고성 재방문이
  10/1에 없어져 켤 선택지 자체가 없다. `BOOKTIMER_TOSS_RETENTION_ENABLED`는 계속 켠 채로 두고, 서버 넛지가
  앞으로도 유일한 재참여 채널이다.
  ...
  - **수수료: 9월 30일까지 면제(건당 0원)**, 이후 별도 고지 후 변동 — ⛔ **10/1 광고성 종료로 무효**(2026-08-28).
    걱정하던 유료 구간이 아예 오지 않는다.
...
- **딥링크 착지 탭 (`?tab=`)** ✅ **완결 — 코드·원장·콘솔 반영·운영 배포까지 (2026-08-25)** — 푸시를 누르고 들어온 사람이
  ...
  **모르는 값은 전부 홈**이다(콘솔에 손으로 적는 URL이라 오타·옛 링크가 온다 — 그때 필요한 건 오류가 아니라 홈).
  ...
  - 🔜 **남은 것은 코드가 아니라 콘솔이고, 순서가 있다**: 라이브 번들에 이 코드가 있어야 `?tab=`이 읽힌다.
    ...
    **홈 화면**으로 등록했다):
    ...
    ✅ **반영 완료 (2026-08-25, 출시 직후)** — 그런데 **콘솔 클릭 몇 번으로 끝나지 않았다.**
    ⚠️ **기능성 템플릿은 등록 후 수정이 불가능하다** — 착지 URL을 넣으려면 **복사본을 새로 만드는 수밖에 없고,
    복사본은 새 발송 코드를 받는다**(발송 코드 = 우리가 API로 부르는 templateSetCode. 입력란은 `booktimer-`
    접두가 고정이고 뒤만 적는다). 그래서 사슬이 이렇게 이어졌다: **콘솔 복사·URL 입력 → 심사 승인 →
    SSM 코드 교체 → 서버 재배포**. 새 코드는 `booktimer-finish-celebration-v2` ·
    `booktimer-daily-goal-met-v2`이고, SSM(v3) → EC2 `.env` 도달까지 `ssm send-command`로 실측했다
    (SSM만 바꾸고 재배포를 안 하면 앱은 옛 값을 계속 쓴다 — 이 확인이 그래서 필요하다).
    ⚠️ **승인 전에 SSM을 바꾸면 안 된다** — 검수 승인 전 발송은 5004로 거부되어 그 사이 푸시가 통째로 죽는다.
    등록 전 **「테스트 발송하기」로 착지를 먼저 확인**했다(심사·SSM·재배포 전에 방향을 확정하는 가장 싼 게이트).
    ⚠️ **수정만이 아니라 삭제도 불가능하다 (2026-08-25 확인 — 콘솔에 삭제 기능 자체가 없다).**
    그래서 옛 템플릿 3개(`booktimer-finish-celebration` · `booktimer-daily-goal-met` · 접두를 두 번 적은
    오타본 `booktimer-booktimer-daily-goal-met`)는 **영구히 목록에 남는다**. 정리하려 해도 방법이 없다 —
    이것이 「수정 불가」의 진짜 비용이고, 템플릿을 고칠 때마다 목록이 한 줄씩 늘어난다는 뜻이다.

    🧭 **그러므로 「어느 것이 현역인가」의 정본은 콘솔 목록이 아니라 SSM 값이다.** 옛것도 「승인됨」으로
    남고 이름까지 비슷해서 목록만 보면 못 가린다(실제로 「하루 목표 달성 푸시」 2줄 앞에서 헷갈렸고,
    그 정체는 자동 메모리에 적혀 있던 오타본이었다). 확인은
    `aws ssm get-parameters --names /booktimer/TOSS_*_TEMPLATE_CODE --profile booktimer`.

    **다음에 템플릿을 만들 때의 규칙** — ① 이름에 **버전과 용도**를 넣는다(`… v2 (서재 착지)`). 목록이
    무덤이라 이름이 유일한 구별 수단이다. ② 발송 코드는 `booktimer-` 접두를 빼고 뒤만 적는다.
    ③ **등록 전에 「테스트 발송하기」로 실제 푸시를 받아 본다** — 되돌릴 수 없으므로 이것이 유일하게
    싼 게이트다.
