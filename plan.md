...
- **페이지별 Claude Design 재스킨(롤링)** ✅ 진행 중: 입구 이후 인터랙티브 페이지를 시안대로 페이지 스코프(`body.*-page` + 전용 프리픽스 클래스)로 순차 재스킨 — `/history`·`/books`(2026-06-25)·로그인/가입(2026-06-26)·책방 헤더(2026-06-27)·`/personality`(책BTI, 2026-06-27)에 이어 **`/settings` 재스킨** ✅ (2026-07-01): 5개 폼 카드+헤더+미검증 배너+플래시를 종이톤+세이지 시안으로, `body.settings-page` 스코프 + `.set-*` 전용 클래스(공유 클래스 미접촉, auth-page와 동일 격리 규약). 이모지 라벨 제거 → 라인아이콘, **푸시 토글 2개를 스위치로 이관**(`.btn-push-toggle`+`.is-on`, JS `notification-settings.js`는 `textContent`→`classList.toggle('is-on')`+`aria-checked`로 상태만 표현·나머지 로직 보존). SSR Thymeleaf(Vue 섬 아님)·백엔드 무변경. 실브라우저 게이트에서 `[hidden]` 누수(display:flex가 UA `[hidden]` 덮음) 1건 발견·수정. 상세는 [changelog.md](claude-docs/changelog.md). 공통 함정: CSS 주석 내 `*/`(N-118)·전역 `button{width:100%}` 상쇄(#286)는 매 페이지 반복 등장.
...
- **알림 설정 통합 ✅ (2026-06-21)**: 마을 HUD 푸시 버튼 2개(L3a·L3b) 제거 → 설정 페이지 "알림" 섹션으로 통합. `notification-settings.js`(빌드 없는 ESM 섬, `pwa-install.js`와 동일 패턴, readyState 타이밍 보호·iOS 미설치 게이트·VAPID 미설정 비활성 포함) 신설. `SettingsController`에 vapidPublicKey·marketingPushConsent 모델 추가. `GardenController`·`garden.html`에서 vapid 주입 제거. VillageApp.vue 푸시 로직 전면 제거·garden.js 재빌드. CSS `.btn-push-toggle` 신설. 로직 중립 추출로 추후 설정 Vue 전환 시 재사용 가능.
  - ⚠️ **UI 마스킹 (2026-07-02)**: 운영 VAPID 키 미주입 등 점등 전이라 `/settings` "알림" 카드는 반투명 오버레이("알람 기능은 아직 개발 중 입니다")로 덮여 클릭·포커스 불가(`inert`) — 코드·라우트는 위와 동일하게 존재, 점등 시 오버레이만 걷어내면 됨(상세: changelog.md).
