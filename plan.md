...
- [x] **후속 — 그 87개 선택자 중 죽은 9군을 삭제 (2026-09-12)**: 위 ⏸는 「살아 있는 규칙을 안 건드린다」였고,
      그 전수 검사가 **어디에서도 렌더되지 않는 선택자 9군**을 덤으로 드러냈다(라이트·다크 × 1280·390, 26개 화면
      상태). `app.css`에서 규칙 **23개**를 통째로 지우고, 살아 있는 규칙 2개에서는 죽은 선택자만 뺐다
      ...
      별개다**) · `.shop-report-form`. **동작 변경 0**이 성공 조건이라, 근거는 게이트의 초록이 아니라 삭제 **전** grep
