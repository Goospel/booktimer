...
- **여백 좋아요 — 남의 문장에 하트를 단다** ✅ 코드 완료 / ⬜ 실기기 확인 대기 (2026-08-20) — 여백에 쌓인
  ...
  - **누를 수 있는 사람은 이미 정해져 있었다** — 여백은 `self`나 팔로워에게만 글을 내려주므로(비팔로워는 빈 배열)
    ...
    거기서 둘이 따라 나왔다: **비공개 책엔 자리 자체를 안 만들고**(남이 못 봐 개수가 영원히 0),
    **내 글엔 개수만 보인다**(자기 좋아요는 도메인이 금지 — 내 여백은 전부 내 글이라 페이지가 자기 버튼으로 덮인다).
  ...
  - **클라이언트 둘 다 했다**(미니앱 + 웹 Vue 아일랜드) — 미니앱에만 달면 웹에서 글을 쓴 사람은 자기가 받은
    좋아요를 영영 못 본다.
  - ⚠️ **명단은 없다** — 개수만 준다. 「누가 눌렀나」는 새 조회 화면과 새 노출 경계를 부르는데 그만한 요구가
    아직 없다. ⚠️ 손잡이 터치 크기 실측 34~46×27~31로 권장 44px보다 작다 — 옆의 지우기·신고와 같은 스타일이라
    회귀는 아니지만, 좋아요는 훨씬 자주 눌리므로 실기기에서 오탭이 잦으면 그때 카드 하단 전체를 키운다.
