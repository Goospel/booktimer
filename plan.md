...
> **→ 결과 (2026-08-24)**: ①#936 · ②#940 · ③-A#944 · B#946 · C#947 · D#948 + 글자색 #937을 묶어 배포했다. **업로드는 2회**다 — D 머지 뒤 독립 리뷰가 실제 결함 2건(측정 중 탭바 링이 초록으로 남음 · 서재 「여백에 글쓰기」 연필 프레임 누락)을 잡아 #950으로 고치고 다시 올렸다. **라이브 후보 deploymentId = `01a032db-8e68-7b26-a9be-72b532192878`**(1차 `01a0329b…`는 폐기). 남은 것은 콘솔 몫(심사 제출·[출시하기])과 **U-1 실기기 확인**뿐이다.
> 그때 지켜야 할 두 가지:
...
> - **`shoot.mjs`의 기본 포트는 5174인데 이 PC의 목 모드는 5300이다** — `MINIAPP_MOCK_URL=http://localhost:5300 node miniapp/screenshots/shoot.mjs`. README에 적혀 있지만 기본값에 걸려 헛도는 자리다(T-197 계열).
...
- [x] **③ 「또렷한 연필」 타이포·위계 시스템 (시안 2a~2e) — 완료 ✅ 2026-08-24 (4 PR: A 토대 #944 → B 홈·피드 #946 → C 서재·기록·목표 #947 → D 탭바 #948)**
      ...
      → **③ A~D 전부 완료. 남은 것은 배포 1회**(위 「배포는 ③-D까지 묶어 한 번에」 절의 조건을 그때 지킨다).
        원장 잔여는 **U-1(합성 볼드 실기기)** 하나이고, 배포 후 사용자 폰이 유일 경로다.
      - [x] **재배포 완료 ✅ 2026-08-24** — `deploymentId: 01a032db-8e68-7b26-a9be-72b532192878` **(라이브 후보)**
      ...
      - [x] ~~배포(업로드) 1차 ✅ 2026-08-24~~ — `deploymentId: 01a0329b-861a-746d-8c00-3f807ad6157c` **(폐기 — 위 결함 2건 포함)**
            ...
            ⚠️ **U-1(합성 볼드)은 이제 실기기로 볼 수 있다** — 이 배포분을 폰에서 열어 홈·서재·기록의
            700 지정 자리(섹션 제목·버튼·배지)가 뭉개지지 않는지 확인이 필요하다. 목·데스크톱은
            합성 볼드 렌더러가 달라 원리상 못 잡는다.
