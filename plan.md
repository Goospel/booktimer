...
> - **PR-2 미니앱 🔜**: 작성 화면 「함께 걸기」 체크 · 내 카드 상태 칩·토글 · 「이 책의 여백」 화면(비소유 진입) ·
>   내 책이면 「내 여백 / 모두」 탭 흡수 · 검색 행 「여백 N」 배지 · dev-mock. 웹(Thymeleaf)은 **비목표** —
>   웹엔 여백 UI 자체가 없어 이식이 확장이 아니라 신축이다.
...
- **여백 좋아요 — 남의 문장에 하트를 단다** ✅ 코드 완료 / ⬜ 실기기 확인 대기 (2026-08-20) — 여백에 쌓인
  ...
  - **누를 수 있는 사람은 이미 정해져 있었다** — 여백은 `self`나 팔로워에게만 글을 내려주므로(비팔로워는 빈 배열)
    좋아요의 관객은 자동으로 **공개 책을 팔로우하는 사람**뿐이다. 새로 정한 규칙이 아니라 기존 게이트의 결과다.
