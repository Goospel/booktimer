...
- [ ] **② 측정 시작 토스트 + 바꾸기 시트 (감사 3f → 시안 4b·4c) — 다음 🔜**
      다른 탭에서 가운데 ▶를 누르면 `timerStartBookId`(홈 캐러셀 선택)로 시작되는데 **그 화면 어디에도 대상
      책이 안 보인다.** 시작 토스트로 무슨 책인지 말하고 그 자리에서 바꿀 수 있게 한다.
      ⚠️ **핸드오프가 제시한 「서버 변경 0」안(b)은 실측 결과 불가능하다** — `tag-book`은 이미 책이 지정된
      세션에 **409**를 던지고(`ReadingSessionService.tagBook`), 애초에 `TimerState`에 **진행 중 세션 id가 없어**
      클라이언트가 대상을 지목할 수단 자체가 없다. 진행 중 세션의 책을 교체하는 엔드포인트가 필요하다
      (Flyway 없음 — `reading_session.book_id` 기존 컬럼).
