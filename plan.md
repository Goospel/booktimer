...
- **기본 이모지를 제품 UI에서 걷는다** ✅ (2026-08-18, 사용자 지정)
  ...
  - ⏸ **식물 이모지(`🌱🌿`)는 남겼다 — 여기는 「안 쓴다」가 아니라 「제대로 만든다」가 답이다.** 「정원·마을」이라는
    이 앱의 핵심 은유이고 반복 노출된다 — 자체 일러스트가 진짜 브랜드 자산이 되는 유일한 자리다.
    ~~서버가 내려주는 `growthStageEmoji`의 값이며~~ → **2026-08-29 그 사다리가 폐기돼 이 근거는 사라졌고,
    남은 것은 홈·목표 화면의 정적 사용이다.** 단계별 그림 세트가 필요해 별도 작업으로 남긴다.
