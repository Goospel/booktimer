...
- **「소셜」 탭 → 「책방」 탭 개편** ✅ 코드 완료 / 🔜 배포 대기 (2026-08-15) — 내 책방까지 **두 탭**이었다
  ...
  **곧장 내 책방**이 뜨게 했다 — 옛 소셜 중간 화면은 사라진다.
  ...
  - ⚠️ **뒤로가기 깊이 어긋남(T-166) — 이번 개편 이전부터 있던 `back.ts` 결함이 주 경로로 올라왔다.** 시트를
    닫으면서 다른 화면을 여는 교체 경로에서 `history.back()`(비동기)이 새 `pushState`를 되삼켜, 서브뷰가 열려
    있는데 히스토리 인덱스는 앱 루트다 → 그 화면에서 back·「돌아가기」가 **미니앱을 종료**시킨다. 스토리 뷰어 →
    「책방 보기」가 같은 경로(코드 무변경으로 실측 확인)라 회귀는 아니지만, 이제 남의 책방 진입이 전부 시트
    경유라 **배포 전 `back.ts` 수정이 선행돼야 한다**(별도 설계 필요 — 이 PR 범위 밖).
