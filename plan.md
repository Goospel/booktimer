...
> - 🔜 **맞팔 DM — 서로 팔로우한 두 사람만 여는 1:1 대화** (2026-09-18 착수, 미니앱 먼저·전부 킬스위치 `booktimer.chat.enabled=false` 뒤 다크 머지) — 처음엔 랜덤채팅을 검토했으나 앱인토스 「서비스별 주의사항」이 「랜덤 매칭·모르는 사람과의 연결」을 **만남·소개팅**으로 분류하고 그 트랙은 **법인 등록이 필수**라 접었다. 대신 「지인·ID 기반 특정 사용자와 1:1」인 **채팅 서비스** 트랙으로 간다 — 상대가 나를 팔로우해야 열리는 맞팔 게이트가 재분류 트리거(공개 프로필 기반 발송)를 피하는 근거다. 4단계: ✅ **PR-0 정책 초안**([claude-docs/chat-dm-policy.md](claude-docs/chat-dm-policy.md) — 약관 「대화」 조·신고 대응·제재 3단계·방지 조치·메시지 보호·수사 협조, 끝의 「구현 대응」 표가 전부 ✅여야 채널톡 제출. SSM `/booktimer/CHAT_MESSAGE_KEY`는 2026-09-19 생성 완료(SecureString, 32바이트 — PR-2에서 `render-env.sh`에 매핑). **푸시 템플릿·동의문 등록은 PR-3으로 옮겼다** — 등록 즉시 승인·수정 불가이고 테스트 발송은 등록본으로만 되므로, 대화함 착지 경로가 생긴 뒤 등록해 도착 화면까지 확인한다) → ✅ **PR-1 서버**(V94 제재 컬럼·V95 방·메시지, 자격은 발송·조회마다 재검증하고 「잠김」은 저장하지 않고 파생, 본문 AES-256-GCM 컬럼 암호화, 방·수신자당 30분 1통 푸시 자리, 차단 → 방 CLOSED, 탈퇴 purge, 프로필 `dmAvailable`) → ✅ **PR-2 서버 안전장치**(V96 — 신고에 방 id·처리 상태·조치·법적 보존, 방 쌍 정규화 CHECK. 방 안 신고 API, 서로 다른 신고자 2명이면 자동 7일 정지, 운영자 조치(무혐의·경고·7일·영구·연장·해제), 관리자 대본 열람·텍스트 내보내기·법적 보존, `/admin` 미처리 신고·제재 중 배너, 운영자 토스 푸시 자리(템플릿 없으면 no-op), 차단 종료 방 30일 보존 삭제(미처리·법적 보존 제외 — 탈퇴는 즉시), 약관 제10조 대화·제11조 문의, 처리방침 대화 항목. PR-1 리뷰 후속 3건: 웹 전용 발신자도 UNREACHABLE, 복호화 못 하는 방은 대화함에서만 빠짐, 닫힌 방은 열어도 첫 메시지 전까지 숨김) → 🔜 **PR-3 미니앱**(코드 ✅: 대화함·대화방(신고·차단·나가기·잠김 문구·flagged 수신자 배너)·방 3초/대화함 10초 폴링(hidden 정지)·남의 책방 종이비행기 버튼(대화가 켜져 있으면 늘 보이고 눌리는 건 맞팔 ∧ `dmAvailable`일 때만 — 2026-09-19 「메시지」 글자 버튼에서 교체)·맞팔 안내 한 줄·책방 헤더 대화함+미읽음 배지·홈 미읽음 카드(있을 때만)·`?chat=inbox` 딥링크·대화함 안 알림 동의 카드(`booktimer-dm-message`). `GET /api/chat/me`가 404·실패면 진입점 전부 없음 → 지금 배포돼도 다크. 점등(2026-09-19): 서버 스위치 ON — SSM `/booktimer/CHAT_ENABLED=true` + `render-env.sh` 매핑 → 번들 검수 요청. **채널톡 사전 제출은 건너뛰고 검수로 판정받는다**(사용자 결정 — 반려되면 그 사유로 대응, 정책 요지는 출시노트 「심사 참고」에 실었다). 남은 것: ⬜ 검수 결과 → 출시하기(사용자) → 콘솔 동의문·템플릿 등록 → 실기기 원장 U-5·U-6·U-10). **⏸ 2026-09-22 현재 번들 20260919-156(`01a0b937`)이 09-20 19:26 요청 이후 38시간째 검수중(`IN_PROGRESS`·반려 사유 없음)** — 이 앱 심사는 최근 8건 중앙값 15분·최장 6시간 38분(9/14)이라 확실히 다른 부류다. 화면 변경이 아니라 새 기능 분류(채팅)를 사람이 따지는 첫 건이고, 심사자도 대화방을 보려면 토스 계정 둘을 맞팔시켜야 한다(운영엔 양쪽 다 토스인 맞팔 쌍이 0). 공지 52775는 9/30 비게임 전수 점검을 예고하며 「9월 14일부터 검수 요청이 몰릴 수 있다」고 적었고, 같은 공지에 **「개별 미니앱이 특정 기준을 충족하는지 사전 판단은 채널톡에서 제공하지 않아요 — 콘솔에서 검수를 요청해 확인해 주세요」**가 박혀 있다(채널톡 사전 제출을 건너뛴 결정의 사후 근거이자, 지금 문의해도 기준 판정은 못 받는다는 뜻). ⚠️ **콘솔 앱 정보가 이 번들과 어긋나 있다** — 상세 설명(독서 타이머→공부 모드→PC 웹 연결)·검색 키워드 8개(전부 독서 계열)·스크린샷 8장 어디에도 대화가 없는데(카테고리 생활>교육>교육, 특수 카테고리 없음) 156의 핵심 추가분이 대화다. 사용자 방침은 **「반려도 승인도 안 나면 취소 후 재제출이 확실하다, 단 앱 정보와 검수 요청한 버전이 일치해야 한다」**. 재제출 수순(아직 실행 안 함): ① 상세 설명에 대화 한 문단·키워드 보강 후 `review_submit` — 앱정보 검토는 **전체 페이로드 재전송**이라 `miniapp_get`의 카테고리(`categoryPaths`→`categoryIds`/`subCategoryIds`)·`keywordList`·`images`를 그대로 이관하지 않으면 기존 값이 날아간다 → ② `review_cancel` — ⚠️ 취소하면 그 deploymentId가 `CANCELED`가 되어 **같은 번들로는 재요청이 불가능**하고(재요청 가능 상태는 `CREATED`·`REJECTED`뿐) 새로 빌드해 `bundle_upload`부터 다시 해야 한다. 검수 중에는 `ANOTHER_BUNDLE_IN_REVIEW`로 새 요청이 막히므로 취소가 선행 필수 → ③ `miniapp/deploy.sh`로 새 번들(코드는 무변경) → ④ `bundle_submit_review`. **대화 화면 스크린샷 추가 여부는 미결.** 참고: 비게임 체크리스트의 관련 항목은 「'앱 내 기능'으로 소개한 모든 서비스 기능을 미니앱에서도 동일하게 사용할 수 있어요」 한 줄뿐이라 방향이 반대(소개한 게 앱에 있어야 한다)여서 「앱에 있는 걸 다 적어라」는 명문 규정이 아니다 — 다만 채팅은 정책 트랙이 갈리는 기능이라 앱 정보에 드러내는 쪽이 안전하다. 주요기능은 미등록(`requiresFeatureRegistration=true`, 검수 요청의 `features=[]`). 설계 원본은 작업용 md(gitignore)라 PR 본문이 정본이다.
