...
- ✅ **후속 4건 마감 (2026-09-05)** — 리뷰가 「그대로 머지」로 판정하며 남긴 것들. ① **교체 404 자동 복구** —
  ...
  유일한 진입점이라(세션 id는 stop 응답에만 실린다) 닫으면 미태깅으로 굳는다. ② **`studyStateOf` 6자리 전부
...
- ⏸ **`/study/history` 월별 행의 책 제목** — 서버 `StudyHistoryService.Day`에 책이 없어 붙이려면 서버 변경이다.
  대신 공부 서재의 누적 시간 칩이 「어느 책에 시간을 썼나」를 책 축으로 답한다.
  트리거: 「어느 책을 공부했는지 기록에서 보고 싶다」 제보.
...
### 측정 대상 책 「고르기 · 바꾸기 · 붙이기」 개선 (R2) — 🔜 진행 중 (PR-1 서버 ✅ · PR-2 웹 홈 ✅)
