...
> **상태**: 비전은 캡처·일부 진행 중. **트랙 A 1단계(MVP)는 출하 완료 ✅** (PR #332) — 아래 「트랙 A」 참조. 트랙 B(숨은 조합 레시피)·드래그 커스텀은 여전히 미설계(별도 설계 세션 필요). "열린 질문"은 *아직 정하지 않은* 것들(트랙 A로 일부 해소됨 — 데이터 모델·식물 카탈로그·트랙 A 임계).
>
...
- **트랙 A — 타이머 할당량** ✅ **(1단계 출하 — PR #332, 위 상태 박스 참조)**: 타이머를 채워 *할당량*을 달성하면 식물을 얻는다(시간 기반 — 1차 발상의 연장). 1단계 확정: 할당량 = **그날 목표를 채운 날의 누적 수**, 보유는 저장 안 하고 유도, 읽기전용 도감/미니정원.
