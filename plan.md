...
> ⚠️ **1차 구현은 실기기에서 반려됐다(2026-08-18)** — 「어색하다, 그냥 선의 픽셀을 깨뜨린 것 같다」. 선을 미는 방식(`feDisplacementMap`)이 `border-image`의 `stretch` 압축을 만나 좁은 요소에서 가장자리가 깎인 탓이었다. 굴곡을 걷고 **농도 얼룩**(고주파 알파 노이즈)으로 바꿔 해결했다(T-190). 아래 두 항목은 그대로 유효하다.
...
- ⬜ **실기기 성능** — 책방 격자처럼 카드가 수십 개 깔리는 화면. 데스크톱·목 모드는 여유가 커서 이 클래스를
      **원리상 못 잡는다**(T-176 발광 box-shadow와 같은 자리). 애니메이션이 0이고 `mix-blend-mode`도 피했으며
      ...
      ✅ 2026-08-18 수정으로 `feDisplacementMap`(픽셀 재배치)이 빠져 **디코드 비용 자체는 더 싸졌다** — 남은 건 `feTurbulence` 한 번뿐이다. 그래도 「수십 개 카드」 확인은 여전히 실기기 몫이다.
