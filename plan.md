...
- [ ] **① 재참여 알림 (최우선)** — 떠난 사용자를 *다시 부르는* 장치. 습관 앱 retention의 핵심 레버인데 현재 전무(목표 달성 알람은 *탭 열림 전제*라 이미 온 사람만 대상 — 게다가 모바일에서 책 읽는 동안 화면 꺼지면 백그라운드 throttle로 전이 감지조차 못 함. N-### 후보: 웹 인앱 알림의 한계).
  - **✅ 결정 (사용자 합의 2026-06-06) — 채널 = 이메일, 용도 = 재참여 넛지.**
    ...
    - **역할 분담 (중요)**: 이메일은 *실시간이 아님* → "목표 달성 **순간**" 알람엔 부적합(도착 지연·받은함 안 봄). 그건 **인앱 알람(현행 유지)**의 몫. 이메일은 **"오늘 안 읽었어요" 재참여**에만 쓴다(이게 retention 가치도 더 큼).
...
- **thesis에 이미 잘 맞는 기존 장치**(유지·강화): 상한(cap, 과몰입 방지), 목표 달성 축하(알람·배지, PR #181), 잔디(습관 시각 증거).
...
### 목표 달성 알람 — 잔여가 0이 되는 순간 알림 (완료 ✅ 2026-06-06)

> **구현 완료** — 측정 중 잔여가 0으로 떨어지는 **전이(미달→달성)**에 1회 알람. ①소리(WebAudio 짧은 비프)+
> 화면 플래시(`.timer-card.goal-flash`, reduce-motion 무효화) + ②Web Notifications(권한 허용 시, 측정 진입 시
> 1회 요청). 서버·DB·마이그레이션 0(`dashboard.js`+CSS+HTML 바인딩만). 전이 로직은 순수 `createGoalAlarm`
> (prime/update)으로 분리해 **Node 내장 러너**(`src/test/js/goal-alarm.test.js`, 무의존성)로 Red→Green 검증:
> 미달→달성 1회·로드시 이미달성 무발화·계속미달 무발화·재무장 재발화. 아래는 설계 근거(참고용).
