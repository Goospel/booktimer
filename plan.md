...
> 설계는 `claude-docs/plans/2026-09-01-study-goal-schedule.md`(PR-A 절). 달력·탭바(PR-B)는 아래 백로그에 남아 있다.
...
- 🔜 **공부 모드 탭바 개편 — 「여백」 대신 달력·공부 일정 (PR-B)**: 공부 모드에서 여백 탭을 빼고 달력/공부 일정 탭을 세워
  일일 지킴·미지킴을 체크하는 상호작용. **가능성은 실측 완료** — 탭바는 `App.tsx`의 자체 `TABS` 배열을 렌더하므로
  모드별 분기가 구조적으로 가능하다(TDS 컴포넌트에 갇혀 있지 않다). ⚠️ 다만 탭 구성이 갈리면 `tabSlot`·코치마크
  좌표(`slotCenter`)·딥링크(`initialTab`)가 함께 흔들린다 — 설계부터 다시 뜬다.
