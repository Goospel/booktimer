...
- **밀린 부채 합산 표시 토글(PR #257)**: 헤드라인에 윈도우 미상환 부채(어제까지 밀린 빚)를 합산해 "갚을 시간"으로 쌓아 보여줄지를 설정에서 on/off(`ReadingTimer.debtCarryover`, V30, 기본 ON). **표시 전용** — 부채 모델은 여전히 per-day 독립이라 어제 빚은 측정으로 안 줄고 '빠뜨린 날 채우기'로만 갚는다. 헤드라인=`WeeklyDebt.totalDebtSeconds()`(오늘+빠뜨린 날 합), floor(`carriedDebtSeconds`/`data-floor`)=빠뜨린 날 합 — 측정 카운트다운이 이 floor에서 멈춰 측정 종료 후 서버 재계산과 어긋나지 않는다(`goalMet`=잔여가 floor 도달). 끄면 floor 0이라 현행(오늘 부채만)과 동일. 배경: '50분 남기고 자정 지났는데 타이머가 70분으로 리셋'된 실사용 피드백 — #217의 이월 없음이 의도대로였으나 사용자가 누적 부채를 원해 합산 표시를 더했다([[N-058]]).
...
### 독서 정원 — 잔디를 게임화한 "수집형 정원"으로 (아이디어 📝, 2026-06-10)

> **상태**: 아이디어/비전 캡처 — **미확정·미설계**. 일부 방향 결정(명칭=정원 · 획득 2트랙 · 도입 순서 · 리치 UI 선후=프론트 전환에 묶음[A])했으나 데이터 모델·식물 카탈로그·구체 해금 레시피는 아직 미정. 착수하려면 별도 설계 세션 필요(계획 우선). "열린 질문"은 *지금 정하지 않은* 것들.
...
- **트랙 A — 타이머 할당량**: 타이머를 채워 *할당량*을 달성하면 식물을 얻는다(시간 기반 — 1차 발상의 연장).
- **트랙 B — 책 조합 (숨겨진 레시피)**: **완독(`BookStatus.FINISHED`)** 과 **읽고 싶음(`BookStatus.WANT_TO_READ`)** 책의 *조합*으로 식물을 얻는다(매핑은 위 3축 메타로 판정). **핵심: 어떤 책을 완독하고 어떤 책을 읽고 싶음으로 두면 어떤 식물이 나오는지 사용자에게 안내하지 않는다** — *발견(discovery)* 자체가 재미인 숨은 레시피(요리·연금술 게임의 숨은 조합처럼).

**도입 순서 (결정 2026-06-10)**: **초반엔 기존 잔디(53×7 히트맵)를 유지**하며 정원과 **공존**시키고, **정원의 완성도가 올라가면 잔디를 제거**(정원이 흡수)한다 → 앞서 "대체 vs 공존" 열린 질문을 *단계적 대체*로 해소.

**리치 UI 기능 · 프론트 선후 (결정 2026-06-10)**: 정원에 두 인터랙티브 기능을 둔다 — **① 식물 도감**(보유/미보유 식물 그리드 + 작가·장르 필터 + 클릭 상세)과 **② 드래그 커스텀 꾸미기**(식물을 끌어 배치해 정원 레이아웃을 꾸미고 저장).
- **도감**은 현 스택(Thymeleaf SSR + htmx 부분 swap + Alpine)으로 충분 — 책장 필터 칩과 같은 패턴.
- **드래그 커스텀**은 실시간 위치 상태 + **레이아웃 영속화(서버 저장)**가 필요 → HTML만 내려주는 현 SSR엔 없는 **JSON API + 본격 프론트(SPA)** 를 요구. 이는 모바일 앱의 선행조건(§프론트엔드 전환·N-017)과 같은 갈림길이다.
- **결정 = (A) 리치 버전을 프론트 전환 스코프에 묶는다** — 드래그 커스텀을 지금 SSR로 짰다가 SPA로 갈아엎는 "두 번 짜기"를 피해, **전환 + JSON API를 깐 뒤 한 번** 제대로 만든다. 도감처럼 현 스택으로 가능한 부분(읽기전용 미니 정원 등)은 먼저 출하 가능하고, *드래그 커스텀*만 전환 이후로 미룬다. → 정원이 §프론트엔드 전환을 정당화하는 **첫 구체 동인**(그동안 plan은 SNS만 동인으로 봄).
...
- **정원/도감 화면 디테일** — 드래그 배치의 격자 스냅 vs 자유 배치·좌표 저장 모델, 도감 필터 축(작가/출판사/장르)·미해금 표시(실루엣/잠금). (큰 틀은 「리치 UI 기능」에서 결정 — 디테일·영속화 스키마만 미정.)
- **비주얼·기술** — 이모지 → SVG/일러스트 전환, 반응형·접근성(스크린리더 대체텍스트).
...
### 📚🧬 책BTI — 독서 성향 분석 ("책장 기반 MBTI") (v1 ✅ 2026-06-07 · 공개 책 기반·항상 책방 공개로 단일화 ✅ 2026-06-08 · 분석 히스토리 3개+대표 고르기 ✅ 2026-06-08)
...
> - **인터랙션 동인은 이제 SNS만이 아니다 — 「독서 정원」의 드래그 커스텀 꾸미기가 구체 동인으로 합류**(§기능 로드맵 「독서 정원」, 결정 2026-06-10). 드래그 배치는 레이아웃 영속화=JSON API를 요구해 *앱 프론트와 같은 선행조건*을 건드린다 → 정원 리치 버전(드래그)을 **이 전환 스코프에 묶어** 한 번만 짠다(도감 등 현 스택 가능분은 먼저 출하 가능).
...
- [ ] **수신동의 토글**(opt-in, **기본 OFF**) — 온보딩/설정. 수신거부 시 영구 제외.
- [ ] **발송 규약** — 제목 `(광고)` 표시 + 발신자 명칭·연락처 + **서명 토큰 수신거부 링크**(추측 가능 id면 IDOR) + **20시 등 야간(21~08시) 회피**(야간은 별도 동의) + **인증된 주소에만**(반송·pre-hijacking 차단).
- [ ] **넛지 로직** — 매 저녁 스케줄 잡이 유저 TZ 기준 "오늘 세션 0" 사용자 조회 → 1통(하루 1통 상한). 자정·"오늘 읽었나" 판정은 Clock+유저 TZ(N-010) 재사용. (§전략 레버 ① 정본.)
...
| 2026-06-09 | **force-push-to-main 하드 차단 + push 감지 false-positive 수정** — `block-main-push.ps1` 확장. ① main/master force-push(`--force`/`-f`/`--force-with-lease`…)를 전용 차단 — 일반 `ALLOW_MAIN_PUSH`로도 안 뚫리고 전용 토큰 `ALLOW_FORCE_PUSH_MAIN`만 통과(히스토리 덮어쓰기 방지, 사용자 요청). force 판정을 override 앞에 둠. ② 발견·수정: 기존 `\bpush\b` 게이트가 브랜치명/경로 속 'push'까지 오판(실제 `…-push-…` 브랜치 생성 오차단) → `git` 서브커맨드 push만 잡게 정밀화. targetsMain 단일 불리언 리팩터. 합성 stdin 16케이스 검증. 훅만, 앱·DB 무관. (PR #275) |
| 2026-06-09 | **검색결과 '추가' 버튼 콤팩트화·우측 정렬 + 계획-우선 규칙 보강** — `/books` "추가" 버튼이 전체폭(전용 규칙 없이 베이스 `button{width:100%; padding:12px 18px}` 그대로)으로 `flex-wrap` 폼에서 셀렉트 아래 좌측에 떨어지던 걸 교정. `app.css`에 `.book-add-form{margin-left:auto}`+`.book-add-form .btn-primary{width:auto; padding:6px 14px; font-size:.85rem}` 6줄(책장 행 `.book-actions`와 동톤). CSS만, 마크업·서버·DB·JUnit 0. `.preview` 하니스+실 app.css로 데스크탑(410px)·모바일(375px) DOM 지오메트리 검증(버튼 width auto 55px·묶음 우측 끝). 부수: **CLAUDE.md 「계획 우선」에 '계획 요청 시 구현 승인은 별개' 경고**(설계 방향 옵션 선택=구현 승인 오해 재발 방지). (PR #276) |
| 2026-06-10 | **이메일 발송 인프라 — 법적 부담 기준 2단계 분리(plan.md)** — 재참여 넛지 착수 전 컴플라이언스 정리. transactional(가입 인증·비번 재설정·열거 통지)은 *서비스 이행 안내*라 정보통신망법 광고성 규제 무관 → **1단계**로 먼저(보안 '높음' 갭 3개 닫힘·법적 리스크~0). 넛지는 영리목적 광고성이라 **제50조**(옵트인 기본OFF·(광고)표시·무료 수신거부·야간21~08시 별도동의·2년 재확인)+개인정보보호법(방침 고지·선택동의 분리) 갖춰 **2단계**. 넛지 먼저 쏘면 미검증 반송→평판 하락으로 transactional까지 스팸함 동반 사망(연쇄). §전략 레버 ①·§이메일 인프라 동기화 + learning-notes **N-067**. 문서만. (PR #277) |
| 2026-06-10 | **독서 정원 — 잔디 게임화 비전 plan.md 정리(아이디어)** — 잔디를 수집형 '정원'으로 재정의: 명칭 전환·식물 획득 2트랙(타이머 할당량 / 완독×읽고싶음 숨은 레시피)·매핑 3축(출판사·작가·장르=`Book` 메타)·도입 순서(잔디 공존→흡수)·리치 UI(도감=현스택 / 드래그 커스텀=JSON API 필요)+프론트 선후[A: 드래그를 프론트 전환 스코프에 묶어 한 번만]. §기능 로드맵 「독서 정원」 신설 + §프론트엔드 전환 동인 연결. 문서만. (PR #278) |
| 2026-06-10 | **책장 검색 결과 페이지네이션 → 고정높이 스크롤 박스** — `/books` "책 추가" 검색 결과를 "← 이전 / 다음 →" 페이저(전체 페이지 리로드·스크롤 리셋)에서 **한 번에 ~40개 받아 ~3행 고정높이 박스 안에서만 스크롤**되게 교체. `BookSearchClient.PAGE_SIZE` 10→40(알라딘 MaxResults≤100), `books.html` 결과 `<ul>`에 `.search-scroll` + 커버 `loading="lazy"`(40장 중 ~3장만 즉시) + `<nav class="pager">` 통째 제거, `app.css`에 `.search-scroll{max-height:430px; overflow-y:auto; scrollbar-gutter:stable; overscroll-behavior:contain; …}` 신설(`.shelf-scroll` 작은 높이판) + 사용처 사라진 전역 `.pager` 규칙 제거(admin-users.html은 자체 scoped). 트레이드오프: 40 초과 결과는 잘림(더보기 없음)—책 추가 용도엔 상위 40에 거의 다 있고 못 찾으면 검색어 정제가 빠름. 기존 테스트 green(알라딘 URL 테스트 `MaxResults` 무단언). 설계 핸드오프 `claude-docs/plans/2026-06-10-book-search-scroll.md`. (PR #279) |
| 2026-06-10 | **계획 우선 규칙 보강 — 계획 md TDD 절에 RED → GREEN 명시 의무화** — 구현 세션이 핸드오프 계획을 읽고 TDD를 건너뛰지 않게 CLAUDE.md 「계획에 담을 것」 TDD 항목 강화: "평소의 RED→GREEN(실패 테스트 먼저→Red→최소 구현→Green)을 명시, 테스트 가능 로직 있으면 생략·약화 금지, 순수 시각만 preview 게이트로 분리". 세션 분리 핸드오프 md에도 전파. 배경: #279 계획 md가 'view/CSS라 테스트 안 만듦'으로 약화한 사례. 위치는 사용자 선택=CLAUDE.md(소프트). 문서만. (PR #280) |
| 2026-06-10 | **책BTI 태그 — 규칙 기반 8종족 닫힌 어휘 (Phase 6b 1단계)** — `ReadingTribe`(8종족 enum + 알라딘 대분류 매핑), `ReadingTagger`(3축 결정적 태그: 장르 정체성·폭·저자 충성도), `PersonalityView.from` 배선(LLM 태그→결정적), 태그 칩 노출. Flyway 무변경(온더플라이). TDD Red→Green 3 Increment. (PR #281) |
...
| 2026-06-10 | **책BTI 태그 클릭 → 근거 책 드릴다운 (Phase 6b 3단계)** — 책방 헤더의 책BTI 태그 칩을 **클릭하면 공개+완독 근거 책**을 칩 아래에 htmx로 인라인 펼침. 장르 종족·한우물형만 클릭(폭 태그 비클릭). 신규 `PersonalityTagAttribution.booksForTag`(순수)·`ProfileTag` record·`GET /u/{loginId}/personality-tag` 핸들러·`profile :: tagBooksPanel` 프래그먼트. `ReadingTribe.ofLabel` 역매핑 + `AUTHOR_LOYAL_TAG` 상수 + `ReadingProfileAggregator.primaryGenre` public. `ProfileService.resolveVisibleTarget`로 가드 체인 추출(운영자·차단·없는 아이디 → 404 공유). `ProfileView.personalityTags`가 `List<String>`→`List<ProfileTag>`로 변경(#282 후속). Flyway·태거 규칙·LLM 무관. TDD Red→Green: `PersonalityTagAttributionTest`(장르 합산·완독 필터·동률 결정성·null/blank·알 수 없는 태그) + `ProfileControllerTest`(공개만·PRIVATE 누출 차단 N-055·운영자/차단/없는 아이디 404·비로그인 리다이렉트·칩 clickable 분류). 설계 핸드오프 `claude-docs/plans/2026-06-10-tag-evidence-drilldown.md`. (PR #283) |
