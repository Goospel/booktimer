...
- **홈 책 고르기 = 표지 캐러셀** ✅ (2026-08-13 코드 · 2026-08-14 실기기 확인) — 위 「홈 UX 정정」(2026-08-11)의
  ...
  **하단 풀폭 「목표 바꾸기」 → 타이머 카드 우상단 손잡이로 흡수** ✅ 코드 완료 / 🔜 실기기 확인 대기 (2026-08-14) —
  ...
  **홈 화면 제목 「○○님의 오늘」 제거** ✅ 코드 완료 / 🔜 실기기 확인 대기 (2026-08-14) —
  ...
  **게이지 최대치 = 목표 + 밀린 시간 · 「남은시간」 설명 상자** ✅ 코드 완료 / 🔜 실기기 확인 대기 (2026-08-14) —
...
- **기록 탭 — 잔디 아래 날짜별 독서 기록** ✅ 코드 완료 / 🔜 실기기 확인 대기 (2026-08-14) — 기록 화면이
...
- **서재 탭 — 3섹션 세로 나열 → 상태 탭 + 표지 캐러셀 + 시트** ✅ 코드 완료 / 🔜 실기기 확인 대기 (2026-08-14) —
...
- **책방(프로필)도 같은 캐러셀로** ✅ 코드 완료 / 🔜 실기기 확인 대기 (2026-08-15, 사용자 지적) — 서재만 고치고
...
- **책방 상단을 인스타 프로필 문법으로 접는다** ✅ 코드 완료 / 🔜 실기기 확인 대기 (2026-08-16, 사용자 시안) —
...
- **독서 성향(책BTI) 추출 — 리워드 광고 관문** ✅ 코드 완료 / ✅ 출시 (2026-08-15 코드 · 2026-08-17 출시) / 🔜 실기기 확인 대기 — 내 책방은 성향을
  **보기만** 했다(`profile.personality`는 저장된 대표의 읽기 전용 복사). 생성 경로가 미니앱에 없어, 웹에 들른 적
  ...
  맥락 없는 광고 노출이고, 최근 홈 다이어트 흐름과도 역행한다. 광고 그룹은 부채 청산과 **같은 리워드 그룹**을 쓴다.
...
- **탭바 가운데 측정 액션 — 4칸 → 5칸** ✅ 코드 완료 / 🔜 실기기 확인 대기 (2026-08-17, 사용자 결정) —
...
- **첫 진입 코치마크 — 측정 버튼을 1회 가리킨다** ✅ (2026-08-17, 사용자 결정 · 위 5칸 개편의 후속) —
  ...
  - 🔜 실기기 확인 대기: 덮개가 홈 인디케이터 영역까지 덮는지, 말풍선이 원을 정확히 가리키는지.
...
- **코치마크를 5개로 — 탭바 투어 3걸음 + 그 자리 안내 2개** ✅ (2026-08-17, 사용자 결정 · 위 코치마크의 확장) —
  ...
  - 🔜 실기기 확인 대기: 투어 3걸음의 꼬리가 각 칸을 정확히 가리키는지(목 모드에선 꼬리 중앙 = 칸 중앙 실측
...
### ⚠️ 홈 소식 피드 재부상 — 완독↔읽는중 토글로 남의 피드 맨 위를 반복 점유 (발견 ⏳ 2026-08-18, 우선순위: 중)
> 완독 축하 푸시 반복 발송(2026-08-18 수정 — 책당 1회 마커)과 **같은 뿌리**에서 발견됐다: "전이했는가"만 보고
> "이미 알렸는가"를 안 보는 구조. 푸시만 고치고 이쪽은 선결 결정이 있어 분리했다.
- **증상**: 완독↔읽는중을 토글하면 `book.finished_at`이 매번 오늘로 재스탬프되고(`Book#changeStatus`), 읽는중 재진입 시
  `started_reading_at`도 재스탬프된다(재독 재스탬프는 **의도** — `Book.java` 주석에 명시). 홈 소식 피드는 이벤트를 따로
  적재하지 않고 **이 두 시각에서 파생**하므로(`BookRepository#feedFinished`·`#feedStarted`, 창 14일·상한 30건),
  **3주 전 완독한 책이 팔로워 피드 맨 위에 "방금 완독한 것처럼" 반복 재부상**한다. 시각이 거짓말을 하는 게 본질.
- **심각도**: 푸시(자기 자신)와 달리 **타인에게 노출**된다. 단 ① 피드 이벤트로는 **푸시가 나가지 않고**(화면 오염이지 알림
  폭탄이 아니다) ② book 행 파생이라 토글을 몇 번 하든 그 책이 차지하는 자리는 **완독 1줄 + 시작 1줄, 최대 2줄**이다
  (행이 불어나는 폭주가 아니라 타임스탬프 갱신에 의한 재부상). 상한이 있어 푸시보다 덜 급하다고 판단했다.
- ⏸ **왜 지금 안 하는가 — 제품 결정이 선결**: 「진짜 재독한 책은 팔로워 피드에 떠야 하는가?」에 먼저 답해야 한다. 떠야 하면
  지금 동작이 맞고 어뷰징만 걸러야 하고, 안 떠야 하면 `finished_at` 재스탬프를 막아야 하는데 그러면 책방 「완독 시간 순」
  정렬이 옛 시각으로 고정된다. `started_reading_at`의 "재독은 재스탬프" 의도를 뒤집을지도 함께 결정해야 한다.
  명확한 버그 픽스(푸시)를 이 논의에 볼모로 잡히지 않게 분리했다.
- **후보 해법**(택일 아님, 조합 가능): ⓐ 재완독 시 `finished_at` 보존(첫 완독 시각 고정) ⓑ 피드용 시각을 정렬용과 분리
  ⓒ 쿼리 측에서 재부상 억제(첫 완독 시각 기준 창).
