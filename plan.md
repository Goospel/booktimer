...
- **전면 광고(IAA) — 「목표 바꾸기」 진입** ✅ 코드 (2026-08-14) — 리워드에 이은 두 번째 광고 지면. 홈 우상단
  ...
  구현은 `showInterstitialAd()` 한 줄을 `App.onGoGoal`에 — **await하지 않는다**(로드 1~2초를 기다리면 탭이
  먹통으로 느껴지고, 노 필이면 진입 자체가 막힌다). 실패는 전부 삼켜 광고가 안 뜨면 그냥 목표 화면이다.
...
- **홈 책 고르기 = 표지 캐러셀** ✅ (2026-08-13 코드 · 2026-08-14 실기기 확인) — 위 「홈 UX 정정」(2026-08-11)의
  ...
  **「책 없이 시작」 보조 버튼 → 캐러셀 0번 「책 없이」 카드로 흡수** ✅ 코드 완료 / 🔜 배포 대기 (2026-08-14) —
...
- **홈 잔디 카드 → 「소식」·「책 뉴스」 2탭 피드 박스** 🔜 진행 중 (2026-08-14, 설계 승인 완료) — 홈에서 잔디
  미리보기를 걷어내고 그 자리에 탭 두 개짜리 피드 박스를 놓는다. 잔디 전체는 기록 탭에 이미 있어 손실이 없고,
  매일 여는 동기인 「연속 N일」만 한 줄로 남긴다. ① 「소식」 = 팔로우한 사람의 **공개(PUBLIC)** 책 활동
  (완독했어요 / 읽기 시작했어요 + 상대 시간) ② 「책 뉴스」 = 내가 **완독한** 책 관련 뉴스(네이버 뉴스 API를
  ...
  - **PR-3 뉴스 수집** ✅ (2026-08-14, 점등 대기) — `book_news`(V65) + `NaverNewsClient` + 새벽 04:30(KST) 배치.
    ...
    ⚠️ **네이버 API 키가 아직 없어 꿄 있는 상태로 머지됐다** — 점등하려면 SSM `/booktimer/NAVER_CLIENT_ID`·
    `/booktimer/NAVER_CLIENT_SECRET`에 실값을 넣고 재배포하면 된다(코드·미니앱 변경 0). 점등 확인은 다음 새벽 배치 로그.
    ⚠️ `parse`의 필드명·`pubDate` 포맷은 **공개 문서 기준 픽스처**라, 실키 발급 후 첫 응답과 1회 대조할 것.
  - ⚠️ **네이버 API 키는 아직 없다**(발급은 사용자 외부 절차). 그래서 게이트를 **서버 응답의 `newsEnabled`**에
    뒀다 — 키가 없으면 탭 자체를 안 그리고, SSM에 키가 들어오면 **미니앱 재배포 없이** 점등된다(빌드타임에 굽는
    `VITE_*` 방식보다 우월. 리워드 광고 게이트와 이 점만 다르다).
