...
- **홈 잔디 카드 → 「소식」·「책 뉴스」 2탭 피드 박스** 🔜 진행 중 (2026-08-14, 설계 승인 완료) — 홈에서 잔디
  ...
  - **PR-2 미니앱** 🔜 — 잔디 카드 제거·연속일 한 줄·`HomeFeed.tsx` 신설(탭·3줄 미리보기·더 보기·빈 상태).
  - **PR-3 뉴스 수집** 🔜 — `book_news`(V65) + `NaverNewsClient` + 새벽 배치. 매칭은 기사에 **제목 AND 저자**가
    둘 다 있어야 채택(『총, 균, 쇠』류 오탐 차단 — 대신 저자 표기가 흔들리는 번역서를 좀 놓치는 트레이드오프).
