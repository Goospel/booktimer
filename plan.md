...
**1단계 완료 ✅** — 웹의 「독서/공부」가 히어로·잔디·타일·정원·페이지·네비 전부에서 갈렸다. 2단계(공부 서재·
공부 목표 게이지·공부 세션에 책 걸기)는 아래 ⏸.

#### 보류 (⏸)
- ⏸ **공부 서재 · 공부 목표 게이지 · 공부 세션에 책 걸기** — 분리 2단계. 서버 문(`/api/study/books`·
  `goalSeconds`·`start {bookId}`)은 이미 있고 웹 화면만 없다.
