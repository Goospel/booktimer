...
- ✅ **PR-5 AI 일정 생성 → 미리보기 → 달력 적용 (2026-09-03)** — `POST /api/study/plan/generate`
  ...
  ⚠️ 처리방침 5-1은 **미갱신** — 일정 생성이 보내는 시험일·하루 공부 시간·주 공부일수가 5-1의 열거
  항목에 없다(과목·범위는 이미 덮여 있다). 문안은 사용자 확인 대상이라 이 PR에서 손대지 않았다.
...
- ✅ **개인정보처리방침 법정 필수항목 보강 + 만 14세 미만 가입 게이트 (2026-09-05)** — 5-1절 문안을 사용자 확인받다
  **문서 전체가 제30조 제1항을 여럿 빼먹은 것**을 발견해 한 번에 메웠다. 5-1은 고지 5항목을 의식해 쓴 티가 나는데
  ...
  서버만 재는 계측기다). `step="5"`가 버본 자리를 이번엔 양쪽으로 메웠다.
  ⏸ 남김: OAuth·토스 경로는 하드 게이트가 아니다(각각 로그인 화면 고지 · 토스 19세 제한에 의존). 확인 시점별 증적이 필요해지면 컴럼 + Flyway로 승격.
