...
>
> ⚠️ **세기 전에 읽을 것 — 이 머리 블록(첫 21줄)이 범례·설명 속에 다음-할-일 3 · 흰 네모 4를 품는다. 집계에서 그만큼 빼라.**
> 범례는 마커를 **실물로 보여 줘야** 쓸모가 있어서 뺄 수 없다. 그러니 지우는 대신 **빼야 할 개수를 여기 적어 둔다**
> — 계측 대상 문서 안에 범례를 두면 범례가 계측에 잡히고, 그건 결함이 아니라 **명시해야 할 상수**다.
> 2026-09-26 기준 실제 미완 = **`🔜` 18 · `⬜` 27** — 파일 전체 집계에서 이 머리 블록(첫 21줄)의 범례·설명 속 마커(다음-할-일 3 · 흰 네모 4)를 뺀 값이다(08-25 값 8·15는 그 뒤 항목이 늘어 낡았다). 같은 이유로 **닫은 항목의 옛 마커를 산문에서
...
3. **채팅은 맨 마지막** — 가장 무겁고(모더레이션·법적) 위험. 가벼운 소셜 재미(성향 배지·비교·"같은 책 읽는 사람")부터, 채팅은 그 후.
   - 🔜 **2026-09-18 착수 — 형태는 「맞팔 DM」이다.** 랜덤채팅은 앱인토스가 만남·소개팅(법인 필수)으로 분류해 접었고, 서로 팔로우한 사람끼리만 여는 1:1 대화(채팅 서비스 트랙)로 바꿨다. 밀도가 낮아(DAU 1~3) 대화함이 오래 비어 있는 것이 기본 상태라는 위험은 인지하고 넘겼다 — 빈 기능이 화면을 차지하지 않게 홈 카드는 미읽음이 있을 때만 뜬다. 진행은 §SNS 「맞팔 DM」 항목.
...
- [ ] 🔜 **① 재참여 알림** — **재설계 중(N3 — 사용자가 켜는 독서 알림, §미니앱 「독서 알림 (N3)」)** / ⏸ **웹 이메일 갈래는 의도적 보류** — ⚠️ **2026-09-26 확정 — 토스 넛지는 6주 동안 도달 0이었다.** 대상(이탈자)이 알림동의문에 동의한 적이 없어 토스가 SUCCESS를 주고 조용히 버렸다(양성 대조). 자동 win-back을 버린다 — 옛 넛지는 같은 날 SSM으로 끄고 코드째 지웠다(T-257). 그 전까지 이 줄은 「토스 채널로 충족(2026-08-18 점등 · 08-25 사용자 확인)」으로 체크돼 있었는데, 근거였던 서버 「발송 성공」이 도달의 증거가 아니었다. (그보다 앞선 08-25 마커 감사 기록: 하위 불릿엔 미니앱 푸시 점등이 이미 체크돼 있었는데 머리글만 미체크라 최우선 레버가 통째로 비어 보였다.) 웹 갈래는 **사용자 지시(2026-08-25)로 보류**한다 — SES 프로덕션 액세스는 2026-07-06에 승인됐으니(§회원 인증/계정 보안 하드닝 「외부 관문 해제」) 막혀서가 아니라 쫓지 않는 것이다. 이하 원문은 이메일 채널을 전제로 쓰인 2026-06 합의라 **역사 기록으로 남긴다**. 떠난 사용자를 *다시 부르는* 장치. 습관 앱 retention의 핵심 레버인데 현재 전무(목표 달성 알람은 *탭 열림 전제*라 이미 온 사람만 대상 — 게다가 모바일에서 책 읽는 동안 화면 꺼지면 백그라운드 throttle로 전이 감지조차 못 함. N-### 후보: 웹 인앱 알림의 한계).
...
- ~~**연속일(streak) → 성장 잔디로 완료 ✅ 2026-06-08 (PR #255)**: 연속 독서 일수에 따라 잔디 카드 우측 상단 식물이 🟫땅→🌱새싹(1~3)→🌷꽃(4~13)→🌳나무(14+)로 자란다(신규 enum `GrowthStage` + 빌더 `currentStreak()`, 오늘 유예·끊기면 리셋). 대시보드·히스토리·프로필 잔디에 노출.~~
  → **폐기 (사용자 결정 2026-08-29)**. 사다리가 실제로 준 것은 「다음 단계까지 N일」이라는 재촉뿐이었고,
...
### ❌ 폐기(2026-09-09) — 독서 마을/서재 캐릭터 (아래는 이력 보존)

> **❌ 전면 폐기 (2026-09-09)** — 사용자 결정으로 서재(작가 도감·먹이주기·`/village`)와 그 파생인 **프로필 아바타(도감 작가 얼굴)**까지 통째로 걷었다. 아바타는 닉네임 이니셜 폴백으로 돌아간다.
> 아래 기록은 **지우지 않고 이력으로 남긴다** — 무엇을 왜 만들었다 걷었는지가 다음 판단의 재료다.
> 코드 폐기는 **PR-1(스키마 무변경) ✅**, `author_character`·`author_affection` 테이블과 `users.profile_character_code` drop은 **PR-2(V88) ✅** 로 따로 나갔다 — 한 배포에 섞으면 blue-green 전환 중 이전 컨테이너가 없는 컬럼을 SELECT해 인증 요청이 500이 된다. PR-2로 `com.booktimer.garden` 패키지까지 소멸했다.
...
- ~~`GrowthStage` enum이 이미 식물 단계(🟫→🌱→🌷→🌳)와 "SVG 교체 여지"를 열어둠(`com.booktimer.session`) → 식물 비주얼·단계의 토대.~~ → **무효 (2026-08-29 사다리 폐기)**: `GrowthStage`는 삭제됐다. 애초에 이 줄이 스스로 말했듯 **수집형엔 단일 enum이 부족**했고(아래), 실제로 정원은 `garden/`의 식물 카탈로그로 따로 갔다 — 잃은 토대는 없다. 필요한 것은 여전히 **식물 카탈로그 + 사용자 보유/해금 상태**를 담는 데이터 모델이다(아래 열린 질문).
...
**연관**: 위 「독서 잔디」(→ 정원으로 흡수·개명), §프론트엔드 전환(리치 UI=드래그 커스텀이 전환 동인), ~~`GrowthStage`·~~(2026-08-29 폐기·삭제)`ContributionGraph`·`ContributionGraphBuilder`(`com.booktimer.session`), `Book`의 `author`/`publisher`/`category` 메타, §전략 retention 레버.
...
> - ✅ **맞팔 DM — 서로 팔로우한 두 사람만 여는 1:1 대화** (2026-09-18 착수 → **2026-09-22 출시**, 미니앱 먼저·전부 킬스위치 `booktimer.chat.enabled=false` 뒤 다크 머지) — 처음엔 랜덤채팅을 검토했으나 앱인토스 「서비스별 주의사항」이 「랜덤 매칭·모르는 사람과의 연결」을 **만남·소개팅**으로 분류하고 그 트랙은 **법인 등록이 필수**라 접었다. 대신 「지인·ID 기반 특정 사용자와 1:1」인 **채팅 서비스** 트랙으로 간다 — 상대가 나를 팔로우해야 열리는 맞팔 게이트가 재분류 트리거(공개 프로필 기반 발송)를 피하는 근거다. 4단계: ✅ **PR-0 정책 초안**([claude-docs/chat-dm-policy.md](claude-docs/chat-dm-policy.md) — 약관 「대화」 조·신고 대응·제재 3단계·방지 조치·메시지 보호·수사 협조, 끝의 「구현 대응」 표가 전부 ✅여야 채널톡 제출. SSM `/booktimer/CHAT_MESSAGE_KEY`는 2026-09-19 생성 완료(SecureString, 32바이트 — PR-2에서 `render-env.sh`에 매핑). **푸시 템플릿·동의문 등록은 PR-3으로 옮겼다** — 등록 즉시 승인·수정 불가이고 테스트 발송은 등록본으로만 되므로, 대화함 착지 경로가 생긴 뒤 등록해 도착 화면까지 확인한다) → ✅ **PR-1 서버**(V94 제재 컬럼·V95 방·메시지, 자격은 발송·조회마다 재검증하고 「잠김」은 저장하지 않고 파생, 본문 AES-256-GCM 컬럼 암호화, 방·수신자당 30분 1통 푸시 자리, 차단 → 방 CLOSED, 탈퇴 purge, 프로필 `dmAvailable`) → ✅ **PR-2 서버 안전장치**(V96 — 신고에 방 id·처리 상태·조치·법적 보존, 방 쌍 정규화 CHECK. 방 안 신고 API, 서로 다른 신고자 2명이면 자동 7일 정지, 운영자 조치(무혐의·경고·7일·영구·연장·해제), 관리자 대본 열람·텍스트 내보내기·법적 보존, `/admin` 미처리 신고·제재 중 배너, 운영자 토스 푸시 자리(템플릿 없으면 no-op), 차단 종료 방 30일 보존 삭제(미처리·법적 보존 제외 — 탈퇴는 즉시), 약관 제10조 대화·제11조 문의, 처리방침 대화 항목. PR-1 리뷰 후속 3건: 웹 전용 발신자도 UNREACHABLE, 복호화 못 하는 방은 대화함에서만 빠짐, 닫힌 방은 열어도 첫 메시지 전까지 숨김) → ✅ **PR-3 미니앱**(코드 ✅: 대화함·대화방(신고·차단·나가기·잠김 문구·flagged 수신자 배너)·방 3초/대화함 10초 폴링(hidden 정지)·남의 책방 종이비행기 버튼(대화가 켜져 있으면 늘 보이고 눌리는 건 맞팔 ∧ `dmAvailable`일 때만 — 2026-09-19 「메시지」 글자 버튼에서 교체)·맞팔 안내 한 줄·책방 헤더 대화함+미읽음 배지·홈 미읽음 카드(있을 때만)·`?chat=inbox` 딥링크·대화함 안 알림 동의 카드(`booktimer-dm-message`). `GET /api/chat/me`가 404·실패면 진입점 전부 없음 → 지금 배포돼도 다크. 점등(2026-09-19): 서버 스위치 ON — SSM `/booktimer/CHAT_ENABLED=true` + `render-env.sh` 매핑 → 번들 검수 요청. **채널톡 사전 제출은 건너뛰고 검수로 판정받는다**(사용자 결정 — 반려되면 그 사유로 대응, 정책 요지는 출시노트 「심사 참고」에 실었다). **✅ 출시 (2026-09-22)** — 번들 `20260919-156`(`01a0b937`, SDK 3.1.1)이 09-20 19:26 요청 → **09-22 13:08 승인 → 13:44 라이브**(`bundle_get_live_version` 실측). ⏱ **41시간 42분** — 이 앱의 최근 8건 중앙값 15분·최장 6시간 38분과 자릿수가 다르다. 화면 변경이 아니라 **새 기능 분류(채팅)를 사람이 따지는 첫 건**이었고, 심사자도 대화방을 보려면 토스 계정 둘을 맞팔시켜야 한다(운영엔 양쪽 다 토스인 맞팔 쌍이 0). 공지 52775의 9/30 비게임 전수 점검 예고(「9월 14일부터 검수 요청이 몰릴 수 있다」)도 겹쳤다. 📌 **다음에 정책 트랙이 갈리는 기능을 올릴 땐 이 기간을 기본값으로 잡는다 — 하루를 넘겨도 정체가 아니다.** 검토했던 취소·재제출은 실행하지 않았고(취소하면 그 deploymentId가 `CANCELED`가 되어 같은 번들로 재요청이 불가능해 새로 빌드해야 했다) 채널톡 사전 제출을 건너뛴 결정도 결과로 정당화됐다 — 같은 공지가 **「개별 미니앱이 특정 기준을 충족하는지 사전 판단은 채널톡에서 제공하지 않아요」**라고 못 박는다. **점등 완료 (2026-09-22)**: ✅ **콘솔 동의문·템플릿 등록** — 동의문 termsId `125007` · 템플릿 `12740`/`booktimer-dm-message`, AI 검수 4항목 전부 pass로 즉시 APPROVED(`consent_match`까지 — 9/13 공부 푸시가 막힌 자리다). ✅ **SSM 2개 생성**(`TOSS_DM_MESSAGE_ENABLED=true`·`TOSS_DM_MESSAGE_TEMPLATE_CODE=booktimer-dm-message`) + `render-env.sh` 매핑 → **배포 success 후 운영 컨테이너 `booktimer-app-blue-1`에 세 값(`BOOKTIMER_CHAT_ENABLED`·`BOOKTIMER_TOSS_DM_MESSAGE_ENABLED`·`BOOKTIMER_TOSS_DM_MESSAGE_TEMPLATE_CODE`)이 실린 것을 실측**(`aws ssm send-command`로 `docker exec printenv`, 인스턴스 `i-07a649585c25707a3`). ⚠️ **그래도 당분간 실제 발송은 0건이다** — 운영에 양쪽 다 토스인 맞팔 쌍이 없어 **보낼 메시지 자체가 생기지 않는다**. 배선 문제가 아니므로 「알림이 안 온다」를 코드에서 찾지 말 것, U-10(2인 왕복)이 곧 첫 실사용 검증이다. ✅ **원장 U-6 충족** — 테스트 푸시 도달 후 누르니 대화함이 열렸다. 📌 **MCP도 `booktimer-` 접두를 자동으로 붙인다**(`dm-message` → `booktimer-dm-message` 실측) — 전체를 넣었으면 미니앱 `DM_TEMPLATE_CODE`와 어긋나 동의를 영영 못 받았다. ⚠️ **동의자 0명이면 테스트 발송도 0건이다** — 1차는 `{"success":true}`인데 `sentCount 0`으로 안 갔고 동의 후 2차가 도달했다(**접수 성공 ≠ 도달**). **남은 것**: ⬜ **실기기 원장 U-5**(hidden에서 폴링 정지·재개)·**U-10**(2인 왕복 — 운영에 양쪽 다 토스인 맞팔 쌍이 0이라 토스 계정이 하나 더 필요하다). ⏸ **앱 정보에 대화가 없는 건 그대로다** — 상세 설명(독서 타이머→공부 모드→PC 웹 연결)·검색 키워드 8개·스크린샷 8장 어디에도 대화가 없다(카테고리 생활>교육>교육). 심사가 통과시켰으니 규정 문제는 아니고 남은 건 **발견성**이다(대화가 있는 줄 모르면 안 쓴다). 고칠 땐 `review_submit`이 **전체 페이로드 재전송**이라 `miniapp_get`의 카테고리(`categoryPaths`→`categoryIds`/`subCategoryIds`)·`keywordList`·`images`를 그대로 이관하지 않으면 기존 값이 날아간다. 주요기능은 미등록(`requiresFeatureRegistration=true`). 설계 원본은 작업용 md(gitignore)라 PR 본문이 정본이다.
...
>   ⚠️ **2026-08-29 갱신 — 글이 0장인 책에서는 헤더가 말만 남는다**: 「여백 **0**」과 「전체 보기 ›」를
>   접고(문 너머가 같은 빈 화면이라 출구가 아니다) 아랫선까지 걷었으며, 빈 문구가 다음 한 걸음
>   (「위의 『여백에 글쓰기』로 첫 장을 남겨보세요」)을 말한다. **로딩·실패에서는 「전체 보기」가 그대로**다 —
>   그때는 전체 화면의 자체 재시도가 유일한 출구라 접으면 막다른 길이 된다(위 「헤더는 세 상태 모두 선다」와
>   충돌하지 않는다: 접는 것은 네 번째 경우인 *내용 0장*뿐이다).
> - **후속 ✅ 2026-08-29 — 서재 탭에서 권수 숫자를 걷는다**: 상태 탭 3개가 늘 「읽는 중 **3**」처럼 숫자를
>   달고 있어 **고르는 자리가 세는 자리로** 읽혔다(여백 탭 개수 제거와 같은 판단의 연속). 권수는 「펼쳐보기」
>   시트 제목(`읽는 중 N권`)이 계속 말하므로 값을 잃지 않는다. ⚠️ **책방(`Profile.tsx`)의 공개 책 상태
>   필터는 어휘(`SECTIONS`)만 공유하고 애초에 숫자를 안 그린다** — 그쪽 카운트는 소제목(`shelfTitle`)의
>   몫이라 이번 변경의 사거리 밖이다(실측 확인).
>   ⚠️ **2026-08-29 갱신 — 그 소제목도 뒤이어 손봤다**: 「사거리 밖」은 *칩*에 대해선 지금도 참이지만,
>   소제목 쪽은 같은 판단(세는 자리 ↔ 고르는 자리)이 하루 뒤 그대로 적용됐다 — 전체 상태의 숫자만 접었다
>   (좁힌 갈래는 유지). 상세는 위 「전체 상태에서만 소제목의 수를 접는다」.
...
> - **후속 ✅ 2026-09-10 — 웹 책장(`/books`)에 「여백 N」 손잡이**: 웹에서 여백에 닿는 길이 책방 칩(공개 책만)과
>   홈 카드(「지금 그 책」 한 권)뿐이라 **비공개 책의 여백은 웹 어디서도 못 열었다** — 2026-08-16 결정 2가
>   서버에선 이미 허용하는데(`marginOf`의 소유자 예외) 문이 없었던 것. 책 행 `.book-actions` 맨 앞에 책방과
>   **같은 부품**(`.shop-margin-btn`)을 세워 같은 공용 `MarginPanel`을 연다. 라벨 「여백 N」은 `/api/books`가
>   이미 싣던 `storyCount`로 만들어 **서버 변경 0**이고, 글이 0건이어도 손잡이는 선다(그 자리가 작성 진입이다 —
>   책장은 언제나 본인이라 발광(`lastStoryAt`) 신호는 옮기지 않았다). ⚠️ **작성·삭제 뒤 재조회를 `load()`로
>   배선하면 안 된다** — `loading=true`가 스켈레톤 분기를 켜 **열려 있는 패널이 통째로 언마운트**된다.
>   스켈레톤을 안 켜는 `refreshShelf()`로 갈랐고, 그 배선은 「emit 직후 한 틱」을 보는 테스트가 못 박는다
>   (왕복이 끝난 뒤에만 보면 패널이 되살아나 있어 갈아끼움이 안 보인다 — 사후 단언은 공허했다).
>   딥링크(`?margin=`)·「내 여백 전체 목록」은 범위 밖 — `margin=` 링크를 만드는 곳이 아직 0건이라
>   소비자만 먼저 만들면 아무도 안 쓰는 코드가 된다(책 상세 SSR에 여백 링크를 달 때 함께 켠다).
...
> ✅ **2026-09-22 — 책방 탭 「책 | 여백」 (사용자 결정, 설계
> [claude-docs/plans/2026-09-22-bookshop-margin-tab.md](claude-docs/plans/2026-09-22-bookshop-margin-tab.md)).**
> 책방은 지금껏 **책만** 보여 줬고 여백은 책을 거쳐야만 닿았다. 책 격자가 서던 자리에 밑줄 2분할 탭을
> 세워, 「여백」을 누르면 **같은 자리에서** 그 사람의 글 전체(책 구분 없이 최신순)가 선다 — 인스타 프로필의
> 격자↔탭과 같은 관계다.
>
> - **서버 ✅**: `GET /api/stories/of/{loginId}/all` → `ProfileMarginEntry[]`(`MarginEntry` + 책 라벨).
>   노출은 `resolveVisibleTarget`(차단·ADMIN·미존재 → 404) + 책 가시성(남 `{PUBLIC}` / 본인
>   `{PUBLIC, PRIVATE}`)이고, 쿼리 `recentByUser`는 **`marginOf`에서 책 좌표만 뺀 것**이다(`sharedByIsbn`류의
>   미러가 아니다 — 거기 있는 `shared`·차단·ADMIN은 대상이 한 사람이라 게이트가 대신 진다). 상한 100 ·
>   `ix_story_user_created` 인덱스 · book fetch join · 좋아요 배치 2쿼리 = **마이그레이션 0, 3쿼리 고정**.
> - **미니앱 ✅**: `BookshopTabs`(여백 화면의 `tabStyle` 재사용 — 같은 몸짓은 같은 모양) · 탭 진입 시
>   lazy fetch · `MarginCard`에 책 라벨 슬롯(`book`/`onOpenBook`, `author`/`onOpenAuthor` 관례) ·
>   빈 문구 두 갈래(내 책방에만 「서재에서 책을 고르고」).
>   - ⚠️ **탭은 읽는 자리, 만지는 자리는 책 여백** — 하트·좋아요 명단·⋯ 관리·글쓰기는 탭에 넣지 않았다
>     (사용자가 비용을 듣고 내린 결정). 전부 책 라벨 한 번으로 닿는 `BookMargin`에 이미 있고, 옮기려면
>     `useMarginLikes`·시트 2종의 뒤로가기 스택까지 컨테이너에 복제해야 한다. 좋아요 **수**는 카드에 남는다.
>   - ⚠️ **404를 「0건」으로 접지 않는다** — 서버가 먼저 나가므로 번들이 앞서는 구간은 원리상 없지만, 그때
>     목록도 빈 문구도 그리지 않고 에러 줄 + 「다시 시도」만 준다(목 라우트를 끈 채 실측 — 「글 N」 머리와
>     빈 문구가 0개인 것을 DOM에서 확인).
>   - ⚠️ **기본 상태에선 「공개한 책」 소제목을 접는다** — 바로 위 「책」 탭이 같은 말을 한다. 좁혔을 때만
>     「다 읽음 2」·「한우물형 근거 책 1」로 필터 정보를 전한다(2026-08-29 「세는 자리 ↔ 고르는 자리」의 연속).
>   - ⚠️ **드릴다운은 탭을 오가도 살아 있다** — `activeTag`·`statusFilter`는 책 축 도구라 「여백」에선 숨기기만
>     하고 값은 건드리지 않는다. 초기화하면 「여백 잠깐 보고 왔더니 필터가 풀렸다」가 된다(목 모드 실측 확인).
>   - ⚠️ **`load()`의 탭 초기화 3줄은 「싼 방어」로 남긴다 — 일하는지는 확증하지 못했다.** 설계는 「다른 JSX
>     분기라 loginId 변경 = 재마운트」를 근거로 이 줄을 *방어*로만 뒀다. 구현 중 목 모드에서 **남의 책방을
>     닫고 내 책방으로 돌아올 때 `statusFilter`가 남는 것**을 봤고 그것을 「재마운트가 아니다」로 읽었으나,
>     리뷰의 돌연변이 실측에서 **이 3줄을 전부 지워도 미니앱 1688건이 그대로 통과**했고, 코드 판독은 반대
>     결론이다 — `Bookshop.tsx:162`·`App.tsx:1337`은 early return이라 루트 엘리먼트 타입이 바뀌어 React가
>     재마운트하고, 살아 있는 `Profile`에서 loginId만 바뀌는 경로는 찾지 못했다. 즉 **그 관측의 원인은 아직
>     모른다**(관측 실수이거나 못 본 경로). 3줄은 싸므로 그대로 두되, **「실측으로 확인됐다」고 적지 않는다.**
>     ⏸ 그 `statusFilter` 잔존은 이 변경 이전부터 있던 별개 함정이라 범위 밖으로 둔다.
> - ⏸ **비목표**: 탭 상태 유지(화면을 떠났다 오면 「책」) · 목록 캐시 · 탭 라벨의 개수(2026-08-29 규칙) ·
>   웹 책방.
...
- **온보딩은 login_id 미강제** — 미니앱 신규 계정은 핸들 없이 시작(불변 핸들 작명 마찰 제거). `onboarded ⟹ login_id`
  ...
  그 전까지 토스-시작 계정은 웹 로그인이 불가해 핸들을 만들 경로 자체가 없었다. **2026-09-04 보정: 그 「웹 로그인
  불가」 자체가 풀렸다** — 아래 「토스 → 웹 코드 로그인」 참조. 다만 핸들을 미니앱 안에서 만드는 길은 그대로 유지한다.)
...
- **v2 확장** ✅ (2026-08-10 결정 → **2026-08-11 심사 통과·출시**) — 실기기 확인 후 "UI 빈약·웹 기능 미반영" 판정으로 **출시 심사 전 확장**으로 방향 변경.
  ...
  **PR-6 소셜(책방·팔로우·차단/신고) ✅** · **PR-7 스토리 ✅** · **UI 폴리시 ✅**(실기기 "이질감" 피드백 —
  ...
  - 후속 fix(2026-08-16): **책방 공개 책 격자에 상태 필터 칩**(전체·읽는 중·다 읽음·읽고 싶어요, 기본 전체).
    ...
    - **후속 ✅ 2026-08-29 — 전체 상태에서만 소제목의 수를 접는다**: 위 「상단 「공개 책 N」과 격자 권수가
      어긋나는 문제」의 해결이 **같은 수를 두 번 말하는** 부작용을 남겼다 — 스탯 줄이 「공개 책 3」을 말한
      바로 아래에서 소제목이 또 「공개한 책 3」이었다(서재 탭 권수 제거와 같은 판단의 연속). `shelfTitle`의
      전체 갈래만 `공개한 책`으로 접는다. ⚠️ **좁힌 갈래는 그대로다** — 「읽는 중 1」·「소설 근거 책 2」의
      수는 상단이 말하지 않는 **새 정보**라(전체 3권 중 지금 1권) 접으면 값을 잃는다. 시그니처도 그대로:
      `count` 인자는 두 갈래가 계속 쓴다.
    - **후속 fix ✅ 2026-09-22 — 필터가 사람을 따라 넘어오던 것을 `key={loginId}`로 끊는다**: 남의 책방에서
      「다 읽음」을 걸고 뒤로 나오면 **내 책방**이 그 칩이 눌린 채 섰다(목 모드 실측). 칩만 남은 게 아니라
      소제목 「다 읽음 3」 아래 격자엔 **전체 3권**이 그대로 — 라벨이 내용과 모순됐다. 태그 드릴다운도 같다.
      ⚠️ **「분기가 다르니 재마운트된다」는 틀린 전제였다**: 책방 셸은 남의 책방을 이른 return(`<Profile/>`
      단독)으로, 내 책방을 프래그먼트(`<>…</>`) 안에서 그리는데 React는 **키 없는 최상위 프래그먼트를 배열로
      풀어** 맞춰 둘이 「index 0 · key=null · type=Profile」로 **같은 fiber에 이어진다**. 처방은 필터 둘을
      `load()`에서 비우는 것이 아니라 **호출부마다 `key={loginId}`** — `Profile`은 마운트 때만 도는 초기화에
      기대어 살고(`useState(() => cacheGet(cacheKeyProfile(loginId)))`), 그것이 안 돌면 재조회 1 RTT 동안
      **앞사람의 헤더·격자**가 새 사람 화면에 남는다. 마운트 스코프 상태가 이미 열 개 넘어 비우기 목록은
      늘 한 발 늦는다. 가드는 `profile-identity-guard.test.ts`(소스 스캔 — 호출부 3곳 전부 key=loginId), 함정은 **T-256**.
      ⚠️ 이 하니스(`renderToStaticMarkup`)는 재마운트를 원리상 계측 못 한다 — **재마운트 판정은 목 모드 실측**이
      진다(`aria-pressed` 카운트).
...
- **미니앱 주소 이전 — 토스가 도메인을 옮긴다** ✅ 선반영 (2026-08-21) — 콘솔 공지로 알았다: **8/25부터
  ...
  **403 Forbidden**, 옛 오리진 200. **코드 변경은 0**: SSM `MINIAPP_ALLOWED_ORIGINS`를 **신·구 4개 병기**(v3)로
  ...
  ✅ **옛 2개(`web`·`private-web`) 정리 (2026-09-15)** — 날짜가 아니라 **라이브 번들의 SDK 버전**이 조건이다(SDK 3.0.0~3.1.0 번들이
  옛 주소를 쓴다, 콘솔 공지 53009). 라이브 `20260915-151`(SDK 3.1.1)을 재확인한 뒤 SSM을 v4(`apps`·`private-apps` 2개)로 줄이고 재배포했다.
  배포 후 프리플라이트 apps·private-apps 200 · web·private-web·통제군 403, 실기기에서 미니앱 전 화면 정상(사용자 확인).
  ⚠️ **SDK 3.0.0~3.1.0 번들로 롤백하면 옛 주소가 다시 필요하다** — 롤백 전에 대상 번들 `sdkVersion`을 보고, 해당하면 SSM을 v3(4개)로 되돌려 재배포한다.
...
- **핸들(@아이디) 만들기** ✅ (2026-08-14) — 토스로 가입한 계정(`login_id=null`)이 **미니앱 안에서 스스로 공개 핸들을**
  ...
  없어 웹 로그인 자체가 불가능**한 죽은 안내였다(운영 6명 해당). ⚠️ 이 「웹 로그인 불가」 전제는 2026-09-04에
  깨졌다(아래 「토스 → 웹 코드 로그인」) — 이 항목의 결정은 그대로 유효하고, 여기 서술은 당시 사실의 기록이다.
...
- **토스 → 웹 코드 로그인** ✅ 서버·웹 화면 (2026-09-04, PR-1) · ✅ 미니앱 발급 화면 (2026-09-04, PR-2 — 배포 대기) — 위 항목이 「웹 로그인
  자체가 불가능」이라 적은 그 막다른 길을 연다. 토스로 시작한 계정은 `password_hash=null`이라 폼 로그인이 원리상
  불가하고 `login_id`도 null일 수 있어, **PC 웹(booktimer.app)에 들어갈 방법이 아예 없었다**. 미니앱이 발급한
  일회용 8자 코드를 웹 로그인 화면에 옮겨 적으면 세션이 열린다 — 「웹에서 발급 → 미니앱에 입력」 연결 코드의 **거울상**.
  저장소는 기존 `toss_link_code`를 그대로 쓰고 **`purpose` 컬럼 하나**(V86, `LINK_TOSS`/`WEB_LOGIN`)로 두 방향을
  가른다. 그 컬럼이 이 변경의 안전성 전부다 — 없으면 웹 설정에서 발급한 **연결 코드가 그대로 웹 로그인 토큰으로
  승격**돼, 어깨너머·스크린샷으로 코드를 본 사람이 얻는 힘이 「내 토스 신원을 붙임」에서 「즉시 로그인」으로 넓어진다.
  「지금은 `user.toss_user_key`로 판별된다」는 추론에 기대지 않은 이유는 그 추론이 발급 조건을 완화하는 날 조용히
  **통과 쪽으로** 고장 나기 때문이다(선례: `EmailTokenService.consume(raw, type)`). 목적 불일치는 「없는 코드」와
  동일 처리하고 `used_at`을 찍지 않는다 — 엉뚱한 소비 지점에 들이민다고 남의 코드가 소모되지 않는다.
  서버는 `POST /api/miniapp/web-login-code`(Bearer, 발급 조건 = 토스 신원 있음 — `issue()`의 거울) +
  `POST /login/toss-code`(소비). 세션은 **커스텀 필터가 아니라 컨트롤러에서 수동으로** 만든다 — 폼 로그인 필터가 하는
  일 중 필요한 것이 「세션 고정 방어 · 컨텍스트 저장 · 성공 이벤트」 셋뿐이라, `AuthenticationProvider`+필터 3파일 대신
  Spring Security 레퍼런스의 수동 저장 예제를 따랐다(`SecurityConfig`는 permitAll 한 줄). principal 이름은
  `login_id != null ? login_id : email`로 **온보딩 전 OAuth 첫 세션과 같은 규칙**이고 `CurrentUserService`가 브리지한다.
  **CSRF는 유지**한다 — 예외로 두면 다른 사이트가 사용자의 브라우저로 코드를 대신 제출할 수 있다(공격자 계정으로
  로그인시키는 벡터). 브루트포스 상한은 **소비 지점**에 IP 키로(`TOSS_CODE_LOGIN` 10회/10분, 코드 검증 **전에** 센다 —
  검증 후에 세면 틀린 추측이 카운트되지 않아 상한이 무력해진다). 발급 쪽엔 상한을 안 건다(Bearer 인증 + 재발급이
  직전 코드를 무효화해 남용 이득이 0 — 웹 `/settings/toss-link-code`와 같은 잣대).
  웹 화면은 Google 버튼 아래 네이티브 `<details>` 한 겹(JS 0) — 기본은 접혀 있고 실패하면 펼쳐진다.
  **미니앱 발급 화면(PR-2)**은 설정 화면의 「PC 웹에서 이어 보기」 섹션이다 — 상태 셋(발급 전 / 발급 후 = 4자씩
  끊은 코드 + 5분 안내 + `booktimer.app/login 열기`(`openExternal`) + 새 코드 받기 / 에러 = 서버 평문)이고
  **시트·모달이 아니다**(T-183 — 진입 직후 화면을 덮지 않는다. 사용자가 누른 뒤 그 자리에서 펼친다).
  ⚠️ **아직 라이브가 아니다** — 미니앱 번들 업로드·검수·[출시하기]가 남아 있고 그건 사용자 지시가 있을 때 한다.
  **비목표(의도)**: 토스 계정에 비밀번호 부여(복구 경로가 합성 이메일이라 성립하지 않는다) · 딥링크 자동 로그인
  (`?code=`는 히스토리·리퍼러·프록시 로그로 샌다 — 주소만 열어 주고 코드는 사람이 옮겨 적는다) ·
  이미 로그인된 브라우저에서의 계정 전환(이전 계정의 세션 속성이 새 계정으로 새는 경로).
  ⚠️ 배포 순서 = **서버 먼저**(엔드포인트 없이 미니앱이 나가면 404). 미니앱 설정 화면은 스토어 스크린샷 4장에 없다.
...
- **프로필·설정 화면 + 닉네임 변경** ✅ (2026-08-14) — 토스로 가입한 계정(운영 6명)은 닉네임이 전원 기본값
  ...
  **남는 백로그(이 작업 아님)**:
  ...
  - ❌ ~~프로필 사진(도감 작가) 선택~~ — **2026-09-09 폐기.** 웹에서 서재 캐릭터 기능을 전면 폐기하면서
    도감·스프라이트·`owned-characters` GET·select POST가 전부 사라졌다(컬럼 `profile_character_code`는 PR-2 V88으로 drop 완료).
    미니앱에 남은 `profileCharacterCode` 타입 선언 2줄은 다음 미니앱 작업 때 픽스처와 함께 정리한다.
...
- **미니앱 회원 탈퇴 + 개인정보 처리방침** ✅ (2026-08-14) — 위 백로그의 ⚠️ 항목을 닫았다. 미니앱 전용(토스로 시작한)
  ...
  **비목표**: 웹 탈퇴 UI/로직 변경 · 소프트 삭제·유예 기간·데이터 내보내기(웹도 즉시 하드 삭제 — 채널 간 일관성) ·
  ...
  - ⬜ **탈퇴 성공 경로 실기기 확인** (2026-08-21 이후 · 출시 후) — **아직 아무도 실행해 본 적이 없는 유일한 구간**이다.
    ...
    **방법(잃을 것이 0인 경로)**: 미니앱 진입 → 게스트 홈 헤더 아이콘으로 **신규 토스 계정 생성**(목표 화면이 뜬다) → 아무것도 쌓지 않은 그 계정으로
    바로 탈퇴 → **게스트 홈 복귀 후 헤더 아이콘을 다시 누르면 목표 화면(`created:true`)** 확인(2026-09-17 선택 화면 제거 반영 — 옛 절차는 「새로 시작」 화면을 봤다). 탈퇴해도 재가입이 막히지 않으므로 같은 토스
...
- **완독 축하 푸시** ✅ 코드 완료 / ✅ **점등 완료** (2026-08-11 코드 · 2026-08-12 점등) — 사용자가 책을 **완독으로 전환하는 순간** 토스 앱의
  ...
  **범위 밖(의도)**: 데일리 독서 리마인더 — 동의 UI·크론이 필요해 미뤘다(하게 되면 발송 시각은 "사용자 설정 시각" 방식).
  ...
  「재참여 넛지 푸시」(7일 비활동)가 나왔다(2026-09-26 폐기). 한때 **데일리 리마인더는 범위 밖**이라 적었고 그 이유는
  **아래 목표 달성 푸시와 합치면 사실상 전원 매일 1푸시**가 되기 때문이었다(스팸 체감). → **2026-09-26: 사용자가 켜는
  방식으로 N3에서 다룬다** — 켠 사람에게만, 그 사람이 고른 시각에, 안 읽은 날만 가니 「전원 매일 1푸시」가 아니다
  (아래 「독서 알림 (N3)」).
...
- **재참여 넛지 푸시 (7일 비활동 win-back)** ⛔ **폐기(2026-09-26, 도달 0)** — 2026-09-26 SSM `TOSS_RETENTION_ENABLED=false`로 끄고(S-0)
  코드·설정·배포 매핑을 지웠다(N3 PR-1). 6주 동안 도달 0/29였고, 원인은 대상이 알림동의문에 동의한 적이 없어 토스가
  SUCCESS를 주고 조용히 버린 것이다(114526 동의 계정으로 10852를 테스트 발송하자 실기기에 도착 — 양성 대조, T-257).
  대체는 아래 「독서 알림 (N3)」. 이하 원문은 이력으로 남긴다 — (옛 상태: 2026-08-18 코드·점등 · 08-25 사용자 확인) — **§전략 「retention
  ...
  **Flyway 0건**이고, 이메일이 훗날 점등돼도 10:00(이메일) → 19:00(푸시) 순서라 같은 날은 이메일이 선점한다.
  ...
  ⚠️ **이 전제는 틀렸다(2026-09-26 확정)** — 토스는 미동의 수신자에게도 `resultType=SUCCESS`를 주고 조용히 버린다.
  그래서 서버는 매번 「성공」으로 마킹했다(`last_nudge_sent_at` 22명이 전부 UTC 10:00). 서버 「발송 성공」은 도달의 증거가 아니다(T-257).
  ...
  ⚠️ **콘솔의 제약 3가지**: ① **푸시 제목 7자 제한**(콘솔 실제 제목은 「북타이머」 — 옛 기록의 「책을 읽읍시다」는 틀렸다, 2026-09-26 `push_template_list`) ② **발송 코드에 `booktimer-`
  ...
  ⚠️ **(2026-09-26 정정) 「열린 리스크 ①②는 해소됐다」는 틀렸다** — 같은 절 아래 08-28 관찰(도달 0)이 붙은 뒤로 모순이었다.
  검수 승인은 문구가 기능성으로 통과했다는 뜻일 뿐, 미동의자에게 닿는다는 뜻이 아니었다. 해소 아님. 옛 기록: 콘솔이 **저장 시 AI로 내용을 검토해 알림동의문 필요 여부를 자동 판정**하는데,
  이 문구는 동의문 요구 없이 **즉시 「승인됨」**이 됐다(검수 대기조차 없었다 — 리마인더성이라 반려될까 걱정했으나
  기능성으로 통과). 다만 **미동의 사용자에게 실제로 닿는지는 별개**라, 첫 발송이 나가야 최종 확인된다.
  (당시 남은 검증 — 폐기로 닫힘) **남은 검증**: 7일 비활동 계정을 인위로 만들 수 없어 경계는 H2 테스트가 맡고, 실발송은 **첫 19시 배치 로그**
  ...
  「목표 달성·완독」 둘인데 넛지도 같은 `termsId=114526`에 매달려 있다. (당시 미완 — 2026-09-26 원인 확정으로 닫힘) **원인 규명은 별건**이나, 광고성
  재방문이 없어져 이게 **유일한** 재참여 채널이 된 만큼 우선순위가 올라갔다.
  📝 **관찰 — 콘솔에 「하루 목표 달성 푸시」가 2개 등록돼 있다**(캠페인명·소재 동일). 서버는
  `booktimer-daily-goal-met` 하나만 부르므로 나머지는 쓰이지 않는 잔재로 보인다. 당장 해는 없다 — 콘솔 기능성 템플릿은
  수정·삭제가 안 되므로 정리할 수 없다(2026-08-25 실측). 넛지 그룹 10852도 같은 이유로 남는다 — 서버가 안 부르면 끝이다.

- **독서 알림 (N3 — 사용자가 켜는 리마인더)** 🔜 (설계 2026-09-26 `claude-docs/plans/2026-09-26-n3-reading-reminder.md`) — 위 재참여 넛지의 대체.
  ① **동의한 사람에게만 보낸다** — 새 CONDITION_BASED 알림동의문 1장에 SERVER 템플릿 2개(매일 / 3일 쉬면), 동의 성공 뒤에만 서버에 켬이 저장된다.
  ② **사용자가 방식·시각을 고른다** — 「매일 정한 시각까지 그날 독서 기록이 없으면」 또는 「3일째 독서 기록이 없으면」, 8~22시 매시 정각 배치, 하루 최대 1통.
  ③ **켜는 자리는 화면 안** — 독서 측정 종료 직후 홈 카드(「3일」 하나만) + 설정 화면. 진입 직후 덮는 요소 없음(T-183).
  진행: PR-1 옛 넛지 폐기 + 발송 결과 관측(`TossMessengerClient`가 성공 본문의 push·inbox 수를 로그로 남긴다) → 콘솔 동의문·템플릿 → PR-2 서버(V97, 다크런치) → PR-3 미니앱.
...
- **스마트 발송 현황 — 절반을 쓰고 있었고, 그 절반은 플랫폼이 접었다** ✅ 기능성 운영 중(재참여 넛지는 2026-09-26 폐기) /
  ...
  | 탭 | 우리 상태 | 코드 |
  |---|---|---|
  | **기능성** — 직접 API로 발송하기 | ✅ 캠페인 운영 중(완독 축하 · 하루 목표 달성 · 공부 회당 시간 · 대화 새 메시지). 재참여 넛지는 ⛔ 2026-09-26 폐기(도달 0 — T-257) | `TossMessengerClient` + 각 서비스 |
  ...
  앞으로도 유일한 재참여 채널이다. → ⛔ **2026-09-26 정정**: 그 서버 넛지는 6주 동안 도달 0이었다. 같은 날 S-0로
  껐고(SSM `TOSS_RETENTION_ENABLED=false`) N3 PR-1에서 코드째 지웠다 — 대체는 N3(사용자가 켜는 독서 알림).
...
- **심사 반려 대응 — 자체 「돌아가기」 제거 + 토스 「<」를 앱이 받아 처리(A′)** ✅ (2026-09-02 코드 · 재심사 대기) — 번들
  `01a06122`(20260902-137)가 「내비게이션 바의 뒤로가기 버튼과 미니앱 자체 헤더 및 뒤로가기 버튼이 함께 노출돼요」로 반려됐다. **그 번들의 diff엔 뒤로가기가 한 줄도 없다** — 「‹ 돌아가기」 알약은 08-16(#830·#831)부터였고 그 뒤 열 번쯤 통과했다. 그래도 T-185(무변경 재제출)를 쓰지 않은 것은 이 사유가 [비게임 출시 체크리스트](https://developers-apps-in-toss.toss.im/checklist/app-nongame)의
  **필수 항목**이기 때문이다(편차가 아니라 규칙 위반). **① 자체 뒤로가기 제거**: `ui.tsx` `Screen`의 `onBack`·`backDisabled` prop과 알약 블록 삭제(호출부 12곳), `Screen`에만 쓰이던 `onBack`은 prop째 삭제, `Goal.tsx` 하단 버튼은 `firstRun`의 「나중에 정할래요」만 남긴다.
  ⚠️ **② 실기기가 전제를 뒤집었다**(원장 U-1 실패) — 1차 번들(`01a061c2`, 20260902-138)을 아이폰에서 보니 **토스 「<」는 히스토리 뒤로가기가 아니라 어느 화면에서든 미니앱을 통째로 닫는다**(iOS 스와이프만 이전 화면으로). 사용자 재결정 = **A′안**: `graniteEvent`의 `backEvent`를 구독해(구독하면 기본 동작이 차단된다) 앱이 정한다 — `back.ts`의 `handleNativeBack`이 「서브뷰가 열려 있으면 `history.back()`으로 하나만, 첫 화면이면
  `Screen.close()`」를 판정하고, `toss.ts`의 `subscribeNativeBack`(try/catch만 — 앱 밖에선 no-op)을 `App.tsx`가 마운트 1회 건다. **`useBackClose` 배선·SDK 3.0.4·`apps-in-toss.config.ts` 전부 무변경.** ⚠️ **수용하는 회귀**: `backDisabled`의 「요청 중 못 나감」 잠금은 네이티브 버튼에 못 건다. 재발 가드 = 소스 가드 `miniapp/src/native-back-guard.test.ts`(제품 `.tsx`에 「돌아가기」 리터럴 0건). 승격: CLAUDE.md 「미니앱 개발 루프」에 **검수 요청 직전 체크리스트 대조** 한 줄(T-220).
...
- **화면 진입 계측(퍼널) — `screen_*` 16종 + `login_started`** ✅ 코드 완료 / 🔜 배포·검증 대기 (2026-09-03 코드) —
  위 전환 이벤트가 「독서를 시작했나」를 세는 사이, **진입 341건과 독서 시작 81건 사이가 통째로 깜깜했다**(콘솔 카탈로그
  실측 `logType: SCREEN` **0건**). 어디서 멈추는지를 화면 단위로 읽으려 `toss.ts`에 `trackEvent`의 형제 `trackScreen`을 두고
  (`log_type: 'screen'` + `screen_` 접두), `App.tsx`에 파생 화면 이름 순수 함수 `currentScreen()` + `useEffect([screenName])`
  **한 개**로 배선했다. 화면 컴포넌트 15개는 무변경 — 붙이는 자리를 App 루트로 고른 이유가 `MainTabs`·여백 상세의
  `key={composeEpoch}` **remount**라, 각 화면에 달면 작성 시트를 닫을 때마다 중복 발화한다. `Analytics.screen`이 아니라
  `Analytics.log`를 쓴 것도 같은 계열의 판단이다(전자는 https가 아니면 조용히 no-op — 죽어도 티가 안 나는 계측기).
  로그인 브릿지의 첫 탭만 이벤트 1건(`login_started`, `await` 앞이라 인가 실패도 「눌렀다」로 센다) — 「안 눌렀다」와
  「눌렀는데 안 왔다」는 처방이 완전히 다르다(소개문 문제 vs 토스 인가·약관 단계 문제).
  🔜 **배포 후 확인**(사용자 폰 필요): `log_type: 'screen'`이 콘솔에서 SCREEN으로 분류되는가(문서에 규칙이 없어 방증뿐이다 —
  EVENT로 잡히면 기능은 살아 있고 타입만 어긋나므로 `search: "screen_"`로 읽는다) · 화면당 1회만 찍히는가(시트 개폐로
  안 는다) · `login_started`가 카탈로그에 뜨는가.
...
- **목표 설정 화면 휠 피커(시/분)** ✅ 코드 완료 / ✅ 출시 (2026-08-13 코드 · 2026-08-17 출시) — 목표를 프리셋 6개(10분·20분·30분·
  ...
  **순수 UI 작업**(서버·api.ts 무변경). 초(`selected`)가 여전히 단일 소스고 시/분은 풀었다 다시 합치는 파생값이라,
  ...
  - **후속 ✅ 2026-08-29 — 목표 화면 다듬기 3건(배치 · 환산 줄 · 문구)**: 서재 #986 · 책방 #987과 같은 판단의
    연속으로, 골격은 그대로 두고 잔티를 걷었다. ① **배치** — 콘텐츠가 전부 위에 몰려 「돌아가기」 하단이
    390×844에서 y=477이었다(아래 43%가 빈 채로 버튼이 화면 중턱에 떴다). Screen 자식을 세로 flex로 감싸
    휠은 `margin: auto 0`으로 세로 가운데, 버튼 두 개는 바닥에 붙인다. ⚠️ **`minHeight`는 `calc(100dvh - 120px)`
    이고 120은 상단 패딩 24 + 제목 줄 ≈56 + 하단 패딩 40이다** — 제목 줄까지 빼야 한다(덜 빼면 그만큼 화면
    밖으로 밀려 버튼이 스크롤해야 보인다. 넘게 빼면 버튼이 조금 위에 설 뿐 무해하므로 안전한 방향은 과하게
    빼는 쪽). dvh 미지원 브라우저는 calc가 무효라 옛 상단 몰림으로 자연 강등된다. ② **일주일 환산 줄** —
    하루치 숫자 하나로는 크기가 안 잡혀(「10분」은 하찮게 읽힌다) 휠 아래 `weeklyLine(selected)`로
    「일주일이면 1시간 10분씩 쌓여요」를 가운데 한 줄. 0 이하면 문장이 통째로 없다(목표를 지운 사람에게
    「0초씩 쌓여요」는 조롱이다). ③ **문구** — 바꾸기 모드 안내에서 「부담 없는 값으로 시작해 보세요」를
    걷어 사실 두 문장만 남기고(처음 정하는 사람에게만 맞는 말이다), firstRun 안내의 이모지 `🌱`를 뗐다.
    ⚠️ **①은 단위테스트 대상이 아니다** — 정적 렌더(jsdom 없음)로 레이아웃 결과를 못 본다. 게이트는
    목 모드 브라우저 실측이고, ②③만 테스트가 잠근다.
  - **후속 ✅ 2026-08-29 — 같은 함정 나머지 5곳 정리(TDS `Text`가 삼킨 가운데 정렬)**: 위 ②를 만들다 확인한
    「`Text`에 준 `textAlign`은 DOM에 안 실린다」가 **서재 빈 탭 안내 2곳 · 「모두의 여백」 · 개인 여백**에도
    그대로 있었다(목 모드 computed 전부 `start`). 같은 처방(바깥 div가 정렬을 갖는다)으로 닫고 회귀 가드 4건을
    걸었다. ⚠️ **서재 격자 제목은 셀이 이미 가운데라 죽은 키만 지웠다** — 실측 없이 고쳤으면 쓸모없는 래퍼가
    하나 더 생길 자리였다. ⚠️ 이건 **T-198의 2회차**다(1회차가 자기 한 곳만 고치고 같은 패턴을 안 훑었다) —
    재발 트래커에 군을 신설했고, T-198이 「단위 테스트가 원리상 못 잡는다」고 적은 것은 이 회차가 뒤집었다
    (걸러진 키는 못 세도 **해결 형태인 래퍼 div는 정적 마크업에 남는다**).
  - **후속의 후속 ✅ 2026-08-30 — 위 스윕이 「죽은 키」로 넘긴 여섯째 자리가 사실 진짜 결함이었다**: 스윕은
    `Home.tsx:809`의 `textAlign: 'left'`를 「기본값과 같아 동작이 없다」고 적고 남겼는데, **재 보니 아니었다** —
    그 자리는 `<button>` 안이고 **UA 기본이 `button { text-align: center }`**라, 태깅 시트의 책 제목이 행 폭
    가운데에 떠 있었다(목 모드 computed `center`). 처방은 지우기가 아니라 **`textAlign`을 부모 버튼으로 이동**
    (리포의 다른 버튼 5곳이 이미 그 관례다 — 래퍼 신설 없음). ⚠️ **바로 위 항목과 정확히 반대 경우다** — 격자
    제목은 부모가 *원하는* 값을 이미 가져서 지우는 게 답이었고, 여기는 부모가 *원치 않는* 값을 가져서 지우면
    결함이 굳는다. 둘 다 소스만 보면 「죽은 키」로 똑같이 보인다 → **지우기 전에 부모의 computed를 잰다**
    (T-216 후일담에 등재).
...
- **홈 잔디 카드 → 「소식」·「책 뉴스」 2탭 피드 박스** ✅ (2026-08-14, 뉴스만 점등 대기) — 홈에서 잔디
  ...
  - **같은 기사 중복 제거** ✅ (2026-09-08) — `book_news`의 유니크가 `(isbn13, link)`라 한 기사가 내 완독 책
    여러 권에 걸리면 그만큼 행이 내려왔다. 화면엔 같은 글이 겹쳐 보이고, 미니앱이 link를 목록 key로 써서
    **중복 key → 탭을 옮겨도 남는 고아 DOM**까지 났다(T-233). `newsFor`가 link 기준으로 걷는다 —
    스캔이 최신순이라 가장 최신 행이 남는다.
...
- **홈 피드에 「여백」 탭 신설 + 책 뉴스 절대 날짜** ✅ 출시 (2026-09-09, 번들 `20260908-144` — 테스트 푸시 실기기 확인 → 검수 승인 → 라이브) —
  사용자 요구 두 건. ① 피드 박스에 **나와의 관계를 따지지 않고 공개된 여백을 랜덤으로** 보여 달라
  ② 책 뉴스의 「N일 전」 대신 **날짜를 박아** 달라 — 수집이 갱신되는지 알 수가 없다.
  - **「여백」 탭 = 「소식」의 반대편이다** — 소식은 팔로우한 사람만 보고, 이 탭은 팔로우를 아예 안 본다.
    자리는 「소식」 바로 뒤(팔로우한 사람 → 모두 순으로 넓어진다), 라벨은 「모두의 여백」이 아니라 **「여백」**
    (네 탭이 한 줄에 서므로 라벨 하나가 길어지면 줄이 넘친다). `DEFAULT_TAB`은 **「소식」 그대로** —
    기존 홈 경험을 바꾸지 않는다(사람 탭을 붙일 때 세운 규칙과 같다).
  - ⚠️ **차단 필터가 쿼리에 필수다** — 소식(`feedRecent`)은 "팔로우 존재 → 차단 없음" write-시점
    불변식 덕에 생략했는데, **팔로우 무관 목록엔 그 불변식이 없다**. 술어는 「모두의 여백」
    (`sharedByIsbn`)을 그대로 진다: 「올린 글만」(`shared = true`) · 책 PUBLIC · ADMIN·핸들 없는 작성자
    제외(N-055) · 차단 양방향 제외 · **내 글 제외**. 신설 `StoryRepository.sharedRecent`.
  - **섞는 자리는 인메모리다** — `order by rand()`는 H2(테스트)·MySQL(운영)로 방언이 갈려 이식성이 없다.
    최신 100건(`DISCOVER_POOL`)만 받아 `Collections.shuffle` 후 30건. **날짜 창(cutoff)은 안 뒀다** —
    공개 여백 풀이 작아 14일 창이면 탭이 늘 빈다. 소식과 달리 **묶지 않는다**(글 1장 = 1행, `count`는
    언제나 1) — 묶는 이유였던 「완독·시작 소식을 밀어내지 않기」가 이 탭엔 없다.
  - **미니앱은 줄 렌더를 공유한다** — 소식 탭의 행 렌더를 `socialRow`로 뽑아 두 탭이 같은 함수를 쓴다
    (복붙하면 두 탭이 따로 늙는다). `feed.discover ?? []`로 읽어 구버전 서버와도 붙는다(`readers` 선례).
  - **책 뉴스만 절대 날짜** `formatDate` → `YYYY.MM.DD` — **연도까지 적는 것이 요점**이다. 「N일 전」이
    답하지 못한 질문이 「수집(새벽 배치)이 지금도 도는가」인데, 1년 전 기사가 「9월 8일」로 새것처럼
    보이면 같은 질문에 다시 답할 수 없다. 소식·사람 탭의 상대 시각은 **그대로**(거기선 「방금」이 값이다).
  - ✅ 스토어 스크린샷 갱신 완료(2026-09-09) — 탭 줄이 실리는 컷은 `01-home`이 아니라 **`02-feed`**였다.
    `02-feed`(+합성 `10-landscape`)만 재촬영해 레포·콘솔 양쪽 반영(콘솔 앱정보 검토 승인). 실운영 공개 여백은
    출시 시점 **5건**(작성자 1명·책 2권) — 「여백」 탭이 본인 글을 제외하므로 그 작성자 본인 화면에선 빈 탭이다.
  - ⬜ **인덱스 `(shared, created_at)` — 공개 여백 규모가 커지면**. `story`의 인덱스는 `(user_id, created_at)`뿐이라
    `sharedRecent`의 `order by created_at desc`가 전역 filesort로 떨어진다(반환 30건은 고정이지만 **정렬 비용은
    공개 여백 전체 수에 비례**). 지금 규모에선 무해해 이 PR엔 안 넣었다 — 수만 건대가 되면 마이그레이션으로 추가.
...
- **여백 좋아요 후속 — 누가 눌렀는지 보이고, 내 글에도 누른다** ✅ 코드 완료 / ⬜ 실기기 확인 대기
  ...
  - ⚠️ **2026-08-29 갱신 — 밑줄이 갈라진 손잡이를 못 따라갔다.** 위에서 손잡이를 둘로 가를 때 개수 줄의
    스타일(`likesLine`)이 **두 갈래에 공유**된 채 `textDecoration: underline`을 늘 깔고 있었다 → 서재
    인라인 미리보기의 `<p>`가 **밑줄인데 눌러도 아무 일 없는 죽은 글자**였다. 밑줄을 버튼 갈래로 옮겼다
    (밑줄은 「눌러진다」는 약속이다). 전체 화면 쪽 밑줄은 그대로.
...
- **화면 밀도 손질 3건 — 텍스트만 있던 자리에 형태를 준다** ✅ 코드 완료 / ⬜ 실기기 확인 대기 (2026-08-20) —
  ...
  - **② 기록의 성장 단계를 카드로** — 이 앱이 주는 유일한 보상이 스탯과 잔디 사이 13px 한 줄로 끼어
    ...
    → ⛔ **이 카드는 2026-08-29에 폐기됐다**(「독서 잔디」 절 참조) — 자리를 잘 잡아 준 것과 별개로,
    사다리 자체가 재촉만 남긴다는 판단. 그 자리는 연속·읽은 날·총 시간 세 수의 스탯 줄로 돌아갔다.
...
- **기록 탭 — 잔디 아래 날짜별 독서 기록** ✅ 코드 완료 / ✅ 실기기 확인(2026-08-19) (2026-08-14) — 기록 화면이
  ...
  - **후속 fix — 하루 막대의 기준을 「그 달 최대」에서 「그날 목표」로** ✅ (2026-09-09, 사용자 제보) — 목표를
    채운 날도 같은 달에 더 오래 읽은 날이 하나 있으면 막대가 짧게 그려졌다. 막대가 「내가 오늘 할 일을 했나」가
    아니라 「그 달 누구보다 길었나」를 답하고 있었던 것. 서버가 `/api/history` 각 날에 **그날 유효했던 하루 목표**
    (`goalSeconds`)를 실어 주고(`DailyReadingRecord` 컴포넌트 추가 · 잔디가 쓰던 `GoalSchedule`을
    `ReadingContributionService.goalSchedule`로 공개 추출해 재사용 — 막대 100%와 잔디 lv4가 같은 날에 같은 답을
    한다), 미니앱은 `barPercent(totalSeconds, goalSeconds)`로 그린다(100% 클램프, 목표 0이면 읽은 날은 가득).
    펼침 영역 맨 아래에 「그날 목표 1시간」/「그날 목표 없음」 한 줄을 둬 무엇에 견줘 쟀는지 밝힌다.
    공부 기록 탭은 범위 밖(공부 목표는 이력이 없어 「그날 목표」가 정의되지 않는다 — V79 설계 의도).
    - ⚠️ **배포 순서: 서버 먼저**(main 머지 → `deploy.yml` 자동) → 그 뒤 미니앱 번들. 옛 앱 + 새 서버는 필드를
      무시해 무해하고, 새 앱 + 옛 서버는 전부 「목표 없음」(막대 가득)이 되므로 순서를 뒤집지 않는다.
  ...
  - **후속 fix — 이미 서재에 있는 책을 형태로 알린다** ✅ 코드 완료 / ⏳ 실기기 확인 대기 (2026-08-19, 사용자 제보) —
    ...
    **눌러도 아무 일이 없는 칸**이 되어 앱 고장으로 읽힌다. 그래서 신호를 두 겹으로 나눴다: **읽어야 아는 것**
    ...
    - **웹 제품 UI의 기본 이모지 전량 정리 + 가드** ✅ (2026-08-19) — 위 배지를 고치다 드러난 것: 미니앱은
      ...
      `🌱🌿`는 미니앱과 같은 예외(핵심 은유 + ~~서버 `GrowthStage` 값~~ → **2026-08-29 사다리 폐기 후엔
      화면 4곳의 실사용**이 근거: `BooksApp`·`TimerCard`·`VillageApp`·`DexDetailSheet`) — **자체 일러스트로
      제대로 만드는 자리라 백로그로 남긴다**. 420/420 그린, 11섬 재빌드, `bootRun`으로 5개 화면 실측.
...
- **책방 상단을 인스타 프로필 문법으로 접는다** ✅ 코드 완료 / ✅ 실기기 확인(2026-08-19) (2026-08-16, 사용자 시안) —
  ...
  **아바타 + 카운트 3개(공개 책·팔로워·팔로잉) 한 줄 → 닉네임 → @아이디 → 성향 bio(3줄 접기·「더보기」) → 태그**.
  ...
  - **곁들여 잡은 것 — 「더보기」가 한 화면에 둘** : bio 접기 손잡이를 달자 남의 책방의 차단·신고 진입 버튼과
    ...
    - **후속 ✅ 2026-08-29 — 그 「신고·차단」을 ⋯ 로 강등**: 이름은 맞았는데 **크기가 틀렸다** — 드문
      안전장치가 주 동작(팔로우)과 대등한 글자 버튼으로 나란히 서서 남의 책방 첫인상이 방어적이었다.
      팔로우가 전폭을 갖고, 신고·차단은 여백 카드와 같은 **정사각 ⋯ 버튼**(연필 프레임 + `#FCFAF5`)이
      된다. 이름은 사라지지 않고 `aria-label="신고·차단"`으로 옮겨 간다 — ⋯ 는 스크린리더에 아무 말도
      안 하므로 라벨이 유일한 이름이다. 여는 것은 그대로 `SafetyPanel`(무변경).
      ⚠️ **높이를 숫자로 안 맞춘다** — flex stretch가 옆 TDS Button에 맞춘다(목 모드 실측 57=57).
      폭만 상수 `TDS_BUTTON_HEIGHT=57`인 이유는 **`aspect-ratio`가 flex 주축(폭)을 못 정하기** 때문이다:
      먼저 `aspectRatio: 1`로 시도했다가 폭이 글자 폭(17px)에 머무는 것을 실측했다. 이 배치라면 TDS가
      높이를 바꿔도 어긋나는 것은 「정사각이 아님」이지 「높이 불일치」가 아니다.
...
- **기본 이모지를 제품 UI에서 걷는다** ✅ (2026-08-18, 사용자 지정)
  ...
  - **가드는 소스 스캔 테스트**(`miniapp/src/no-emoji.test.ts`) — 자리마다 고쳐 봐야 다음 기능에서 무심코 다시
    ...
    주석은 대상이 아니다(개발자용). 허용 목록은 `✕`(기호지 이모지가 아니다) 하나뿐이다 — 옛 예외 `🌱🌿`는
    아래 후속으로 해소됐다.
  - ✅ **식물 이모지(`🌱🌿`) → 자체 새싹 SVG로 이행 (2026-08-30)** — 「안 쓴다」가 아니라 「제대로 만든다」가
    답이라 백로그로 남겨 뒀던 자리다. 미니앱 홈에 남은 세 자리를 걷었다: 달성 머리말·첫 기록 배너는 획 기반
    인라인 `SproutMark`(24 격자·2px 획·둥근 끝 — 카드의 ⓘ와 같은 문법, 색은 `--adaptiveBlue700` 토큰)로,
    측정 중 안심 문구의 `🌿`는 **문장 꾸밈일 뿐이라 그냥 걷고 마침표로** 닫았다. 단계별 그림 세트는 애초에
    필요 없어졌다 — 성장 사다리가 2026-08-29에 폐기돼 남은 건 정적 표식 하나였다. 가드 허용 목록도 `✕`만
    남겨 **제품 UI의 이모지는 이제 0**이다.
...
~~서재 마을(Phaser 성능·TDS 이질감 검증 필요)~~ → **2026-09-09 폐기** — 웹에서 기능 자체를 걷어 이식할 대상이 없다.
~~미니앱 소셜 기능~~ → v2 범위로 승격(2026-08-10).
기능 동등성 원칙은 유지 — 웹이 본진.
...
### 📚✏️ 독서/공부 타이머 모드 분리 — 1차 (완료 ✅ 2026-09-01, 미배포)

> 같은 타이머로 **공부 시간도 재고 싶다**는 요구. 설계는 `claude-docs/plans/2026-09-01-timer-mode-toggle.md`
> (구현 완료 후 폐기 대상 — repo 로드맵인 이 문서와 다르다).

- **격리는 구조가 지킨다** — `reading_session`에 mode 컬럼을 다는 대신 **별도 테이블 `study_session`**(V78)을 팠다.
  독서 집계(잔디·부채·기록·홈피드·책 통계)가 그 테이블을 **아예 모르므로** 공부 시간이 샐 경로가 없다.
  컬럼 방식이었다면 기존 집계 전부에 `mode='READING'`을 더해야 하고 하나만 빠져도 조용히 오염된다.
- **한 일**: V78 + `StudySession`/Repository/Service + `POST /api/study/start|stop` + `/api/dashboard`에 `study` 블록 /
  미니앱 홈 히어로 우측 상단 「독서 | 공부」 토글 · `body.study-mode` 토큰 스왑(세이지 → 흐린 파랑 #5F7E96·#47657C) ·
  공부 히어로(스톱워치 + 당일 누적) · 탭 잠금·시작 토스트·탭바 원 라벨의 모드 분기.
- **공부 모드에 없는 것**(부재가 곧 규칙이다 — 억제 코드가 0줄): 목표·게이지·부채·「변경 ›」·책 캐러셀·「읽는 중」
  카드·여백 문·태깅 시트·첫 완료 축하. 그 부재로 **광고 두 자리**(부채 지우개 리워드·목표 변경 전면)도 자연 소멸한다.
- **독서등은 공부 모드에서 켜지 않는다**(1차 결정) — `body.reading-lamp`와 `body.study-mode`가 동시에 붙을 수
  없어 밤 세이지 vs 파랑 명시도 싸움이 통째로 사라진다. 공부용 밤 팔레트는 ⏸.
- **곁다리로 닫은 결함**: `study_session`이 users를 FK 참조하는 순간 `AccountService.purge()`에 그 삭제가 없으면
  **공부 기록을 가진 사용자의 탈퇴가 통째로 실패**한다(T-168이 운영에서 실제로 낸 사고와 같은 자리).
  `FlywayMigrationTest`의 양방향 FK 대조가 구현 도중 이걸 잡아 함께 고쳤다 — 그 가드가 값을 한 번 더 했다.
- ✅ **토글 세로 정렬 결함**(2026-09-01 실기기 제보로 발견, 미배포분에 포함) — 강조 알약이 테두리 안에서 위 4.5px /
  아래 −0.5px로 치우쳐 아래를 삐져나갔다. 원인은 세그먼트 `<button>`이 inline 흐름이라 알약이 baseline에 앉고
  상속 폰트(16px)의 **line box strut**이 아래로 민 것. 버튼을 flex 컨테이너로 바꿔 strut을 없애고, 그 strut이
  벌어주던 7px을 잃는 대신 히트영역 44는 `minHeight`가 명시로 든다(시각 21px·17px·11px는 불변).
- ⬜ **실기기 확인 · 배포** — 목 모드 실브라우저로 색 전환·시작/종료·독서 모드 불변까지 확인했다.
  실기기와 `deploy.sh`는 사용자 지시 대기.

### 📚✏️ 공부 모드 2차 — 공부 목표 분리 + 색 체감 (PR-A · 완료 ✅ 2026-09-01, 미배포)

> 설계는 `claude-docs/plans/2026-09-01-study-goal-schedule.md`(PR-A 절). 달력·탭바(PR-B)는 바로 아래 항목이다.

- **목표는 `users` 컬럼 하나**(V79 `study_daily_goal_seconds`, 0=목표 없음) — 독서의 `ReadingGoalChange` 같은
  **변경 이력 테이블을 만들지 않았다**. 그 이력이 존재하는 유일한 이유는 「그날 목표로 과거를 판정」(부채 계산)인데
  공부엔 이월·부채가 없어 판정할 것이 없다. 이력이 필요해지는 날 컬럼을 현재값 캐시로 남긴 채 승격할 수 있다.
- **한 일**: V79 + `User.updateStudyDailyGoal` + `POST /api/study/goal`(+ `StudyState.goalSeconds`가 대시보드·
  start/stop 응답에 공짜로 실린다 — 맨 뒤 필드라 하위호환) / 미니앱은 공부 히어로에 게이지·「남은 시간 | 하루 목표 ·
  변경 ›」 복원(파랑), `Goal.tsx`에 `variant` prop을 얹어 **휠 화면을 재사용**(갈리는 건 문구와 저장 함수뿐), App에
  `view='studyGoal'` 추가.
- **격리는 목표 축에도 있다** — 공부 목표 저장은 `ReadingGoalService.record`를 **부르지 않는다**(부르면 독서 부채
  판정 원장이 오염된다). 회귀 테스트가 「공부 목표를 저장해도 독서 목표·이력·대시보드가 불변」을 못 박는다.
- **전면광고는 기존 그룹 재사용** — 「목표 바꾸기 = 광고 1회」 규칙이 모드와 무관해야 사용자가 규칙을 하나만 배운다.
  콘솔 그룹 신설 0(env·심사 표면이 안 는다). 대신 콘솔 보고에서 두 진입이 합산된다(수용 — 전면 지면이 원래 하나다).
- **색 체감 강화 3종**(실기기 피드백 「토글해도 색이 안 보인다」의 처방): ① 게이지·통계가 파랑으로 돌아오고
  ② 히어로 카드에 **전용 토큰 `--heroCardBg`**를 신설해 공부 모드만 푸른 종이(#EFF3F6)로 틴트하고
  ③ 큰 시계 잉크가 `blue700`이 된다. grey100을 스왑하지 않은 이유는 그게 전 화면 공용이라서다.
  **독서 렌더는 픽셀 불변**이 계약이라 낮 기본값은 #FCFAF5 그대로 두고, 「변경 ›」 알약의 리터럴 배경도 안 건드렸다.
- ✅ **목표 없이 지내기(0 복귀)** (2026-09-02) — 서버는 0을 「목표 없음」으로 이미 받는데 그리로 가는 문이 UI에
  없었다(`Goal.tsx`의 휠 0 가드 `disabled={selected === 0}`가 variant를 안 봐 공부까지 막았다). 가드는 그대로 두고
  **공부이고 `current > 0`일 때만** 서는 약한 버튼을 별도로 냈다(`showClearGoal`). 독서엔 안 선다 — 독서의 0은
  「목표 없음」이 아니라 이월·부채 원장이 깨지는 값이다. 설계는 `claude-docs/plans/2026-09-02-study-goal-clear.md`.
- ⬜ **실기기 확인 · 배포** — 목 모드 실브라우저로 틴트·잉크·게이지·저장 왕복·독서 불변까지 수치로 확인했다.
  전면광고 실기기 노출(U-A4)은 목 모드로 원리상 불가라 미검증으로 남았다. 실기기와 `deploy.sh`는 사용자 지시 대기.

### 📚✏️ 공부 모드 2차 — 일정 달력 + 탭바 개편 (PR-B · 완료 ✅ 2026-09-01, 미배포)

> 설계는 같은 md의 PR-B 절. **교체되는 탭은 「책방」**이다(사용자 확정) — 요구에 나온 「여백」은 탭이 아니라
> 탭 밖 전역 뷰였고, 책방이 여백 글이 전시되는 사회면이라 「공부 중엔 안 쓰는 화면」에 가장 가깝다.

- **원장은 수동 체크, 자동은 정보** — `study_daily_check`(V80)의 행 1개 = 하루 1판정이고, 서버는 어떤 경로로도
  이 행을 만들거나 고치지 않는다. `kept boolean` + **행 부재 = 무기록**으로 3상태를 컬럼 하나에 담고,
  `UNIQUE(user_id, check_date)`가 「하루 한 판정」을 DB에서 지킨다. 달력이 스스로 말하는 것은
  **「그날 측정이 있었나」(점)까지** — 목표 대비 달성 배지는 안 그린다(공부 목표엔 변경 이력이 없어 과거를
  현재 목표로 판정하면, 목표를 올린 순간 과거 달성일이 소급 취소되는 거짓이 생긴다).
- **한 일**: 서버 = V80 + `StudyDailyCheck`/Repository/`StudyCalendarService` + `GET /api/study/calendar?month=` ·
  `POST /api/study/check`(미래 날짜는 유저 tz 기준 400) / 미니앱 = `screens/StudyCalendar.tsx`(월 달력 · 셀 탭이
  무기록→지킴→못 지킴→무기록으로 순환 · 측정 있음 점 · 범례) + 공부 탭바.
- **탭바는 목록 하나만 갈아 끼운다** — `STUDY_TABS = TABS`에서 책방 자리를 `CALENDAR_TAB`으로 바꾼 것이고,
  **두 목록 길이가 같아**(4) `TIMER_ACTION_SLOT`·`slotCenter`가 값 그대로 남는다(설계가 걱정한 좌표 흔들림이
  구조적으로 사라졌다). index↔화면 계약은 `tabChangeHandler(onChange, tabs)` 한 줄로 일반화 —
  **그리는 목록과 index를 푸는 목록이 같은 배열**이어야 하는데, 어긋나도 마크업이 똑같아 정적 렌더로는 못 잡는다
  (돌연변이가 전 스위트를 초록으로 통과했다) → 소스 단언으로 잠갔다. 딥링크(`initialTab`)는 TABS만 검증 유지.
- **모드 전환으로 현재 탭이 사라지면 홈으로** — `reconcileTab`은 **setState 없는 파생값**이라 동기화 코드가 0줄이다.
  사용자 손으로는 이 조합이 안 생기고(토글은 홈에만 있다) 원격 플립에서만 생긴다.
- **코치마크는 공부 모드에서 잠근다** — 투어가 TABS 좌표로 책방 칸을 가리키는데 공부 탭바엔 그 칸이 없다.
  시작을 막는 게이트(안내 카드) + 걷는 도중 플립되면 접는 effect 둘 다 둔다.
- ⚠️ **탈퇴 경로도 함께 늘렸다** — `study_daily_check`가 users를 FK 참조하는 순간 `purge()`에 한 줄이 없으면
  「달력을 한 번이라도 눌러 본 사용자만」 탈퇴가 실패한다(1차 `study_session`과 같은 자리, T-168 계열).
- ⬜ **실기기 확인 · 배포** — 목 모드 실브라우저로 3상태 순환·저장 왕복·미래 no-op·월 경계·히트영역(50×44)·
  진입 덮개 0·잔디와의 시각 대비까지 수치와 스크린샷으로 확인했다. 실기기와 `deploy.sh`는 사용자 지시 대기.
- **후속 fix(2026-09-02)**: 요일 머리글·월 제목이 칸 왼쪽에 붙어 날짜 열과 어긋나던 것(실기기 스크린샷)을 TDS `Text`의
  `textAlign` **prop**으로 고쳤다 — `style.textAlign`은 DOM에 안 실린다(T-219, T-216의 3회차). 재발은 소스 가드
  `miniapp/src/text-style-guard.test.ts`(모든 `<Text>`의 `style` 리터럴에서 `textAlign` 금지)가 막는다.

### 📚✏️ 공부 서재 — 회독 기반 별도 서재 (PR-1 서버 · PR-2 미니앱 · 완료 ✅ 2026-09-01, 미배포)

> 설계는 `claude-docs/plans/2026-09-01-study-library.md`.

- **요구**: 공부 모드의 서재 탭이 **공부 책만** 보여준다(독서 서재에 공부 책이 섞이지 않고 그 반대도).
  공부 책의 분류는 상태(읽는중/완독)가 아니라 **회독 수(0독/1독/…)**다.
- **또 테이블을 가른다** — `study_book`(V81). 같은 `book` 테이블에 구분 컬럼을 두는 안은 기각: 격리를
  `BookRepository`의 기존 쿼리 **전부**(책방 공개 책·홈 피드·추천·책BTI·뉴스 매칭·popularity·완독 축하 배치)가
  필터로 지켜야 하고 하나만 빠지면 조용히 샌다. V78~V80과 같은 원칙이고, 이번 요구가 바로 그 격리다.
- **스키마는 8컬럼뿐** — status·visibility·피드 스탬프 3종·제휴 클릭 카운터 4종·category/pubDate를 전부 뺐다
  (공부엔 소비처가 없다). `purchase_link`만 남긴다: 검색 응답에 공짜로 실려 오고 수험서는 구매 전환이
  실제로 기대되는 자리다(제휴 수익 경로 — 사용자 확정).
- **한 일**: V81 + `StudyBook`(register + `changeReadCount` 음수 IAE) / `StudyBookRepository` /
  `StudyBookService`(isbn 멱등 — 같은 user+isbn13이면 기존 행 반환, **회독 수 보존**) /
  `GET·POST /api/study/books` · `POST /api/study/books/{id}/read-count` · `/delete`.
- **회독은 절대값 설정 API**(`{readCount:n}`)다 — 델타(+1/-1)와 달리 멱등이라 연타·재시도에 안전하고,
  미래의 직접 편집(숫자 입력)도 같은 문을 쓴다. 음수는 400, IDOR·미존재는 404(독서 서재와 같은 계약).
  음수 검사가 소유권 조회보다 **앞**이라 남의 책 id로 400/404를 갈라 존재를 캐낼 창이 없다.
- **검색은 서버 무변경 재사용** — `GET /api/books/search`는 알라딘 프록시라 도메인 중립이다. 다만 그 응답의
  `owned`는 독서 책장 기준이라 공부 화면이 자기 isbn 집합으로 다시 계산한다(PR-2 몫).
- ⚠️ **탈퇴 경로도 함께 늘렸다** — `study_book`이 users를 FK 참조하는 순간 `purge()`에 한 줄이 없으면
  「공부 책을 한 권이라도 담은 사용자만」 탈퇴가 실패한다(1차 `study_session`·2차 `study_daily_check`와 같은
  자리, T-168 계열). **purge 줄을 넣기 전에 실 H2로 RED를 먼저 확인**했다.
- ⏸ **타이머-책 연결은 이번에 안 한다** — 공부 타이머는 지금처럼 책 없이 잰다. 투기 컬럼을 미리 깔지 않았고,
  필요해지면 `study_session.book_id` nullable 하나면 된다(서재가 먼저 있어야 연결할 대상이 생긴다).

**PR-2 (미니앱 화면) ✅** — `StudyLibrary.tsx` 신규 + `App.tsx` 분기 한 줄(+ import).

- **독서 서재 바이트 불변이 계약**이고 별도 파일이라 `git diff origin/main -- Library.tsx Home.tsx`의 **0줄**이
  곧 그 증명이다(실측 충족). `Library`에 `mode` prop을 넣어 내부 분기하는 안은 1,500줄 파일 전체에 조건이
  스며 그 증명을 잃으므로 기각했다.
- **화면** — 상태 탭 없는 단일 캐러셀(등록 최신순) + 표지 아래 **「N독」 칩**(0독 포함 — 「아직 안 돌았다」는
  빈칸이 아니라 정보다) + 주 손잡이 **「회독 +1」** / 보조 「관리」. 관리 시트는 「회독 -1」(**0독이면 행 자체가
  없다** — 눌러도 안 되는 죽은 행 금지) · 구매 행 + 제휴 고지 · 「서재에서 삭제」(확인 1단계).
- **담기가 독서보다 한 단계 짧다** — 회독은 언제나 0독 시작이라 「어디에 담을지」 시트가 없다(행 탭 → 즉시 추가
  → 목록 복귀). 검색 행의 담김 판정은 서버 `owned`(**독서 책장 기준**)를 버리고 공부 서재 isbn 집합으로 다시
  센다 — 안 그러면 독서 서재에 있는 책을 공부용으로 못 담는다(목 모드에서 실제로 그 경로를 밟아 확인).
- **주 손잡이는 채움**이다(독립 리뷰 W2) — 독서 서재의 구조적 대응물은 「검색해서 담기」(0번 칸 전용 틴트)가
  아니라 같은 2:1 줄의 「여백에 글쓰기」이고, 그쪽 레시피(채움 `--adaptiveBlue700` + 연필 프레임 + 크림 잉크
  `#F7F2E8`)를 그대로 쓴다. 「채움 주 버튼은 화면당 1개」 대장(`typography.test`의 파일 목록)에 한 줄을
  더했다 — **그 목록은 갱신이 설계된 계측기**다(채움을 새로 세우는 건 축을 늘리는 결정이라 눈에 띄어야 한다).
- ⚠️ **채움 스타일은 헬퍼 삼항이 아니라 호출부 리터럴로 쓴다** — D5 계측기는 소스를 정규식으로 훑으므로
  헬퍼 안에 숨은 채움을 **못 본다**(리뷰어 실측 W1). 같은 색을 칠하고도 계측기 밖에 서는 사각이라,
  중복처럼 보이는 인라인이 여기서는 계약이다(그 사각을 돌연변이로 재현해 대장이 붉어지는 것까지 확인).

**스토어 스크린샷 — 공부 컷 3장 추가 ✅ (2026-09-01)** — 공부 모드가 처음 라이브되는 출시인데 콘솔 그림이
전부 독서 모드라 새 기능이 한 장도 없었다. `shoot.mjs`를 늘려 `06-study-home`(파랑 히어로 + 모드 토글) ·
`07-study-library`(「N독」 칩 + 「회독 +1」) · `08-study-calendar`(월 달력의 지킴·못지킴·측정 점)을 추가했다.
모드는 클릭이 아니라 `localStorage`(`booktimer.timerMode`)를 심고 다시 여는 방식이다 — 앱이 그 값을 마운트
1회만 읽으므로 이게 짧고 흔들리지 않는다. 기존 6장은 **바이트째 동일**. ⚠️ 달력은 목 픽스처가 표식을 최근
4일에 놓아 **달 초에 찍으면 이번 달이 백지**라, 표식이 0개면 지난달로 물러서는 줄을 촬영기에 뒀다.
⚠️ 콘솔 업로드는 「앱정보 검토」를 새로 여는 별개 절차라 사용자 판단으로 남긴다(레포 자산만 갱신).

**공부 기록 탭 분리 (공부 잔디) — 완료 ✅ (2026-09-02)** — 공부 모드에서 하단 「기록」을 열면 **독서 타이머의
잔디·월별 기록**이 그대로 떴다(`App.tsx`에 모드 분기가 없었다). 서재와 같은 방식으로 화면을 가른다 —
새 `GET /api/study/history`(`StudyHistoryService`) + 새 `screens/StudyHistory.tsx`, **독서 화면 렌더는 바이트 불변**
(`history.test.tsx` 무수정 green이 그 증거다).
- **잔디는 복제하지 않는다** — `ContributionGraphBuilder.build`가 순수 static이라 일자별 공부 초 맵만 넣으면
  그대로 돈다. 「`weeks[0]`이 최신 주」 같은 규약을 두 번 밟을 자리가 없다.
- **농도 분모 = 고정 절대 눈금 4h**(상수 하나). 현재 공부 목표를 분모로 쓰면 목표를 올린 순간 **과거 칸이
  소급 재채색**된다(N-059의 잔디 판) — 공부 목표엔 변경 이력이 없어(V79) 그 함정을 피할 길이 눈금뿐이다.
- **두 화면의 경계**: 「일정」(달력) = 사용자가 손으로 남긴 판정이 원장 · 「기록」 = 타이머가 잰 측정 사실만.
  그래서 기록엔 원·체크가 한 픽셀도 없고, 책이 없으니 표지 열·펼침도 없다.
- 캐시 키도 갈랐다(`CACHE_STUDY_HISTORY`) — 공유하면 모드 전환 직후 **첫 프레임**에 상대 모드 기록이 선다.
- **색은 뒤늦게 따라왔다 (2026-09-02)** — 화면은 갈렸는데 잔디·범례·하루 막대가 공부 모드에서도 **초록(세이지)**
  이었다. `ui.tsx`의 `LEVEL_COLORS`가 hex 리터럴이라 `body.study-mode`의 토큰 스왑이 닿지 않았기 때문이다.
  팔레트를 `var(--grass0..4, <옛 리터럴>)` 경유로 바꿔, 공부 블록이 1~4단을 파랑 사다리(200·400·600·800)로
  갈아 끼운다(0단은 「기록 없음」의 종이색이라 두 모드가 같다). 리터럴은 fallback으로 남아 **독서 렌더는
  바이트 불변**이고, 소비처(`GrassGrid`·`Legend`·두 화면의 하루 막대)는 상수를 보므로 코드 0줄로 따라온다.

#### 공부 세션 자정 분할 (PR-2) — 완료 ✅ (2026-09-02)

22:00~01:00 공부가 전날 3h로 서던 어긋남을 저장 시점에 없앴다(6h 상한이 이를 허용해 실제로 생긴다).
독서(#1008)와 **같은 규칙·같은 함수**다 — `ReadingSessionService.splitByMidnight`가 같은 패키지의
package-private static이라 호출이 공짜였고, 복제하면 0초 조각 금지·DST 규칙을 두 번 밟게 된다.
- **배선은 종료 2경로**(`stop` · `closeStaleSessions`) — 공부엔 수동 입력·책 태깅이 없어 독서의 3경로 중
  `recordManual`에 해당하는 자리가 없다. 클램프(6h) → 분할 순서도 독서와 같다.
- **하위 집계는 한 줄도 안 고쳤다** — 기록(`StudyHistoryService`)·달력(`StudyCalendarService`)·
  「오늘 공부한 시간」이 전부 세션 행에서 유도되므로 저장을 바르게 하면 공짜로 맞는다. H2 통합 테스트가
  그 주장 자체를 잰다(23:50→00:40 공부가 기록·달력에서 두 날짜로 갈린다).
- **엔티티·스키마·Flyway·프론트 변경 0**, 과거 저장분 소급 마이그레이션도 없다(독서와 같은 판단).

### 📚✏️ 공부 타이머–책 연결 — 독서와 같은 3단 (설계 `claude-docs/plans/2026-09-02-study-timer-book.md`)

> 바로 위 「공부 서재」가 ⏸로 미뤄 둔 그 자리다 — 서재가 생겨 연결할 대상이 이제 있다. 시작 시 선택 +
> 종료 후 태깅 + 측정 중 교체까지 **독서와 같은 문법**(사용자 결정 2026-09-02).

- ✅ **PR-1 서버 (2026-09-02)** — `V82 study_session.book_id`(nullable, FK → `study_book`) ·
  엔티티 `tagBook`/`changeBook`(독서 가드에 **진행 중 태깅 거부**를 더했다 — 재는 도중은 교체 문의 몫) ·
  API 3문(`start` 바디 `{bookId?}`는 **`required=false`**로 12차 라이브 번들의 `{}`·바디 없음을 그대로 받는다 /
  `POST /api/study/sessions/{id}/tag-book` / `POST /api/study/active/book`) · `StudyState` 맨 뒤 4필드
  (`activeBook`·`recentBookId`·`books`·`untaggedSessionId` — 태깅 좌표는 **stop 응답에서만** 산다) ·
  `StudyBookRow.totalSeconds`(책별 누적) · 책 삭제 시 `unlinkBook`(시간은 남고 라벨만 풀린다).
  독서 경로 diff **0줄**.
- ⚠️ **바로 위 「자정 분할」과 합류했다** — 설계 전제 「공부엔 분할이 없다」가 #1012로 깨져, 독서 `tagBook`의
  **조각 체인 워크를 결국 복제**했다(리뷰 C-2). 분할 조각은 **시작 책을 상속**하고(C-1), 책 없이 잰 세션을
  종료 후 태깅하면 `untaggedSessionId`(마지막 조각)에서 **뒤→앞으로 걸어 올라가** 앞 조각까지 함께 붙는다 —
  안 그러면 자정 전 몫이 미태깅으로 남아 책 칩이 절반만 센다.
- ✅ **PR-2 미니앱 (2026-09-02)** — 공부 홈 대기 화면에 **「무엇을 공부할까요?」 캐러셀**(목록·기본 선택은 서버
  `StudyState.books`·`recentBookId`), 시작이 `POST /api/study/start {bookId}`로 그 책을 실어 보내고, 측정 중 히어로는
  「측정 중 N분 · 『제목』」, 공부 서재 카드엔 둘째 칩 **「3시간 20분 공부」**(0초면 칩 없음 — 0독과 달리 부재다).
  선택 슬롯은 독서 `homeBookId`와 **별개**이고(두 서재의 id 공간이 다르다), 독서 경로는 `Library.tsx` **0줄** +
  `Home.tsx`·`App.tsx` hunk가 전부 study 갈래 안이다.
- ✅ **PR-3 미니앱 (2026-09-02)** — 독서와 같은 3단의 나머지 둘: 책 없이 끝낸 공부 측정에 **「무슨 책을 공부하셨나요?」
  시트**(stop 응답 `untaggedSessionId`가 유일한 좌표, 붙일 책이 0권이면 안 연다)로 나중에 붙이고, 시작 토스트의
  **[바꾸기]**로 재는 도중 대상을 갈아 끼운다(`null` = 「책 없이」 — 측정은 안 멈추고 시간은 통째로 새 책에 붙는다).
  시트 두 장은 **독서 것을 그대로 재사용**, 모드가 바꾸는 것은 재료(`study.books`)와 제목뿐. 공부 모드에도 [바꾸기]가
  서면서 그것을 가리던 게이트(`toastHasBookControls`)는 항상 참이 되어 삭제했고, 세이지 리터럴 2곳은
  `var(--accentPill, …)` 토큰으로(독서 알파 .14/.16→.18 — 「독서 diff 0」의 의도된 예외 2건). 태깅 직후엔 공부 서재를
  `key`로 다시 세워 그 자리의 카드 칩이 바로 바뀐다(리뷰 반영).
...
- ~~⏸ **달력의 자동 달성 판정 배지**~~ — 2026-09-13 하루 목표 폐기로 전제 소멸 — 폐기(컨셉 전환 절).
- ⏸ **공부 책의 회독수 정렬·그룹 탭·목표 회독수** — 단일 목록 + 「N독」 칩으로 시작한다. 필요가 실측되면 그때.

---

### 📚✏️ 웹 「공부」 화면 — 백지복습 + AI 일정 (설계 `claude-docs/plans/2026-09-02-web-study-recall-and-plan.md`)

> 지금까지의 공부 기능은 전부 미니앱에만 있었다. 이번엔 **웹**에 `/study`를 세워 ① 달력 + 일정 원장,
> ② 백지복습(타이핑·사진 전사 → Claude 분석 → 다음날 복습문제), ③ AI 일정 생성을 얹는다.
> AI 호출은 **관리자가 승인한 사용자만**(PR-2가 PR-3보다 앞이라 게이트 없이 AI가 열리는 창이 없다).

- ✅ **PR-1 웹 달력 + 일정 원장 (2026-09-02, AI 없음)** — `V83 study_plan_item`(user·plan_date·book_id nullable·
  subject 스냅샷·task) · `StudyPlanService`(수동 추가·삭제·달 조회 + 「오늘 이후 전부 교체」 `applyReplacingFuture`) ·
  `GET /api/study/agenda` + 일정 추가·삭제 2문(전부 **웹 세션** 인증 — Bearer 없는 `/api/**`는 세션 체인으로 흐른다) ·
...
- ✅ **PR-5 AI 일정 생성 → 미리보기 → 달력 적용 (2026-09-03)** — `POST /api/study/plan/generate`
  (**첫 줄이 `requireApproved`** → 검증 → 키 → 상한 선점 → 어댑터 → 실패면 환불) + `/apply`
  (AI를 안 쓰므로 게이트 없이 세션 인증만 — 수동으로 짠 일정을 한 번에 넣는 폴백이기도 하다) ·
  어댑터 `generatePlan`+`PlanInput`/`PlanDay`/`PlanDraft`+`planUserPrompt`(**후보 날짜를 서버가 계산해
  넣는다** — 날짜 산술은 모델이 조용히 틀리는 자리고, 시험 당일을 후보에서 빼면 규칙이 지시가 아니라
  입력의 형태가 된다) · **`sanitizePlan`이 이 판의 방어선**(범위 밖·중복·빈 task·파싱 실패 제거,
  task 500자 절단, **ISO 주(월~일)당 주 N일 상한**, 0개면 `UNAVAILABLE`) · 섬 `PlanForm`(폼 → 주차별
  미리보기 → 「달력에 적용」). 하루 상한 `PLAN 3`. **`replaceCount`는 생성 시점에 센 값**이라 화면이
  「지금 기준」이라 말하고, 적용 뒤에는 서버가 준 실제 `removed`를 보여 준다(거짓 약속 금지).
  ✅ **처리방침 반영 완료 (2026-09-06, #1045)** — 시험일·하루 공부 시간·주 공부일수·후보 날짜가
  6-1절(구 5-1) 열거 항목에 들어갔다. 그 확인 과정에서 문서 전체를 감사해 법정 필수항목을 함께 메웠다.

- ✅ **입력칸 종이·연필 테마 적용 (2026-09-04)** — 기능이 아니라 미감. `/study`는 5단을 다 올리는 동안
  **폼 스타일 배선만 빠져 있었다** — `.study-recall input`류가 `width:100%`만 줘서 나머지가 UA 기본값으로 떨어졌다.
  다른 폼 화면이 쓰는 `--pencil-frame-soft`·`--focus-ring`을 그대로 당겨 썼다(**새 색·새 값 0** — 재료를 만들지 않고 빌린다).
  더해 책 select의 OS 화살표 → 직접 그린 chevron(`.study-select-wrap`), 백지복습 **본문칸만** 줄노트.
  실브라우저 실측이 잡은 것 둘: ⓐ `.study-recall-body`(0,1,0) < `.study-day textarea`(0,1,1)이라 `background` 단축이
  줄을 `none`으로 덮었다 ⓑ `font: inherit`가 `.study-plan-row label`의 12px 캡션을 값 입력칸까지 끌어내렸다(캡션은 `> span`으로).
  `input[type=file]`은 대상에서 제외 — 브라우저가 그리는 「파일 선택」 버튼이 테두리 안에 갇혀 두 겹이 된다.

- ✅ **개인정보처리방침 법정 필수항목 보강 + 만 14세 미만 가입 게이트 (2026-09-05)** — 5-1절 문안을 사용자 확인받다
  **문서 전체가 제30조 제1항을 여럿 빼먹은 것**을 발견해 한 번에 메웠다. 5-1은 고지 5항목을 의식해 쓴 티가 나는데
  정작 본체가 비어 있었다 — **한 문서 안에서 같은 사안을 다른 기준으로 쓴 것**이 가장 큰 결함이었다(GA4도 같은 국외이전인데 5항목이 없었다).
  신설: 위탁 · 정보주체 권리·행사방법 · 안전성 확보조치 · 보호책임자 · 권익침해 구제 · 변경 공지. 1..13 재번호.
  Anthropic 보유기간·연락처는 **그쪽 정책 페이지에서 직접 받아** 넣었다(30일 · 플래그 시 최대 2년).
  연령은 **생년월일을 받지 않는다** — 나이를 수집하면 개인정보가 하나 느니, 「14세 미만이 아니다」만 확인한다(수집 최소화).
  게이트는 **두 겹을 각각 증명**했다 — 브라우저 `checkValidity()`=false(실측) + 서버 `@AssertTrue`(MockMvc는 제약검증을 건너뛰어
  서버만 재는 계측기다). `step="5"`가 버린 자리를 이번엔 양쪽으로 메웠다.
  ⏸ 남김: OAuth·토스 경로는 하드 게이트가 아니다(각각 로그인 화면 고지 · 토스 19세 제한에 의존). 확인 시점별 증적이 필요해지면 컬럼 + Flyway로 승격.

- ✅ **백지복습 「구멍」이 틀렸다는 사실만 말하던 것 — 실측으로 프롬프트 두 줄 (2026-09-09)** — 「분석도
  Gemini Flash에 맡기면 어떠냐」는 물음에서 시작해 **같은 글을 두 모델에 넣어 실측**했다(세 케이스: 범위의 주제
  하나가 통째로 빠진 글 · 좁은 범위를 충실히 덮은 글 · 틀린 서술 4개를 심은 글). **찾아내는 능력은 대등했다** —
  셋 다 핵심을 놓친 쪽이 없다. 갈린 곳은 **서술**이었다: Gemini는 「~에 대한 설명이 사실과 다릅니다」로 *틀렸다는
  사실만* 말하고(내일 뭘 고칠지 알려면 결국 교과서를 다시 펴야 한다), 범위 밖 주제까지 구멍으로 집었다.
  그 둘이 **프롬프트로 고쳐지는지**를 대조군과 함께 다시 재서, 효과가 확인된 두 줄만 `ANALYZE_SYSTEM`에 넣었다 —
  「「범위」에 없는 주제는 올리지 않는다」 · 「한 줄 라벨로 끝내지 말고 …틀린 것이면 올바른 내용을 함께 적는다」.
  이 두 줄은 **모델과 무관한 이득**이라 Claude 경로에 그대로 넣는다. 모델 이전은 아래 ⏸로 보류.
  📌 프롬프트 문구는 단위테스트로 못 박지 않는다 — 문자열 검사는 「무엇을 배제하는가」에 답을 못 하고, 이 변경의
  계측기는 실측(같은 입력·대조군)이다.

- ✅ **용어를 날마다 다르게 부르던 자리 — 사용자 표기를 정본으로, 단 따르기 전에 검수 (2026-09-09)** — 사용자 지적에서
  나왔다: 문제는 한 답 안의 표현이 다양한 게 아니라 **어제 답과 오늘 답이 같은 것을 다르게 부르는 것**이다. 어제
  「능동수송」이던 것이 오늘 「활성 수송」이면 사용자는 내용을 의심하기 전에 「그게 그건가」부터 묻고, **자격증처럼
  채점이 단어 단위인 공부에서는 그 혼동이 그대로 손해**가 된다. 그래서 `ANALYZE_SYSTEM`에 용어 규칙을 넣었다 —
  ① 사용자가 쓴 표기를 그대로 쓴다(동의어·줄임말·띄어쓰기 치환 금지) ② **다만 표준 용어가 아니면 따르지 않고**
  holes에 「무엇을 무엇으로」로 올린 뒤 그 답의 나머지에서 올바른 표기를 쓴다.
  📍 실측(before=origin/main · after=이 변경, Claude 6회 165원): **틀린 표기 검수에서 갈렸다** — before는 「반투명막」은
  잡았으나 **「원형질 붕괴」는 지적 없이 자기 문장에서 조용히 「원형질 분리」로 바꿔 썼다**(사용자는 자기가 틀린 줄
  모른 채 못 보던 단어를 보게 된다 — 바로 그 혼동이다). after는 둘 다 명시적으로 짚었다. 표기 유지 쪽은 **before도
  이미 지켜서 차이가 안 났다**(동의어 혼입 양쪽 0건 — 첫 측정의 1건은 「원형질분리」 안의 「형질분리」를 센 부분
  문자열 오검출이었다). 「효과 없음」이 아니라 「미측정」이라 그 줄은 남긴다 — 지금 잘하는 것을 우연이 아니라
  규칙으로 굳히는 값이다.
  ⚠️ **temperature로는 이 문제가 안 풀린다** — 그것은 *같은 입력*의 흔들림을 줄일 뿐인데 백지복습은 날마다 입력이
  다르고, 어제 무슨 단어를 썼는지가 애초에 요청에 없다.
  ⏸ **「최초 1회만 지적」은 상태로 만들지 않았다** — 매 분석에서 검수하되 표기가 맞으면 침묵하므로 사용자 눈에는
  결과가 같다(1일차에 지적받아 고치면 2일차부터 조용하다). 갈리는 것은 사용자가 지적을 무시하고 같은 표기를 계속
  쓸 때뿐이고, 그때는 계속 알려주는 쪽이 맞다. 과거 용어를 저장·주입하는 구조(용어 사전)는 **실사용이 붙어 같은
  표기가 여러 날 반복되는 것이 실제로 보일 때** 다시 연다.

#### 보류 (⏸)
- ⏸ **백지복습 분석의 Gemini Flash 이전** (2026-09-09 검토 — 실측 근거) — 원가는 **1회 27.7원 → 3.4원**이 된다
  (실측 토큰: Claude in 2,081 / out 1,564 · Gemini in 1,139 / out 838 — 단가만이 아니라 **한국어 토큰이 절반**이라
  8배가 된다). 그런데도 지금 옮기지 않는 이유가 셋이다. ① **아낄 돈이 아직 0원** — `study_recall` 0행 · 운영 로그의
  `claude analyze` 0건이라 실사용이 없고, 사용자당 하루 1회 상한이라 매일 써도 월 830원이다. ② **처리방침이 따라온다**
  — 6-1(Anthropic: AI 분석·사진 전사) / 6-2(Gemini) / 8절 표 세 곳을 고쳐야 하고, 6-1이 약속한 「받은 때로부터
  **30일 이내 자동 삭제**」가 Gemini의 「제한된 기간」으로 **후퇴**한다. ③ **가장 위험한 실패가 안 잡혔다** — 구멍이
  없는 글에 「정확하지만 ~라는 관점도 있다」는 *첨언*을 구멍으로 올리는 버릇이 프롬프트로 안 고쳐졌다(그 문형을
  명시적으로 금지했는데 그대로 나왔다). 더해 temperature 0.4의 편차가 커서 케이스당 1~2회로는 확정이 어렵다
  (같은 입력에 구멍 0개 ↔ 2개). **재검토 트리거**: 실사용이 붙어 월 비용이 실제로 보일 때 — 이사 자체는 싸다
  (일정 생성이 같은 경로로 옮겨간 선례). ⚠️ **사진 전사는 Claude에 남으므로 공급자를 줄이는 이득은 없다**.
- ⏸ **복습문제 1→3→7일 간격 자동 재출제** — 이번 산출물은 「다음날 1회 표시」까지.
- ⏸ **미니앱 이식** · **문서 파일(PDF/DOCX) 입력** · **밖에서 받은 일정 붙여넣기** · **일정 항목 편집**(추가·삭제만) ·
  **복습 정답 채점** · **공부 시간과 일정의 자동 대조** · **사진 원본 보관**.
- ⏸ **승인 신청 알림·자동 승인 기준·승인 이력 테이블** — 1인 운영 규모라 `/admin`을 여는 것으로 충분하다.
- ⏸ **실패 환불의 남용 여지** — AI 호출이 실패하면 상한 몫을 전액 환불하므로, 「항상 실패하는 입력」을 아는
  사용자는 상한 없이 반복할 수 있다. 설계 §2.3이 정한 규칙 그대로이고(장애로 오늘 몫을 잃지 않게 하는 것이
  환불의 목적이다), 항목 수 상한(2026-09-04)으로 「항상 실패하는 입력」의 대표 사례가 사라져 실효 위험이
  줄었다. 실제 남용이 관측되면 「환불은 하루 N회까지」로 좁힌다.
- 🔜 **손글씨 사진 전사 품질 — 사용자 폰이 유일 경로다 (미확인)**. 전사 기능은 배선·상한·무저장까지
  끝났지만 **「실제 손글씨를 얼마나 옳게 읽는가」는 한 번도 재지 못했다.** 목 이미지·인쇄체로는
  이 축이 원리상 안 잡힌다(T-176 계열 — 검증 환경이 실환경보다 관대하다). 필요한 것은
  **사용자가 자기 손글씨 메모를 찍어 넣고 결과를 눈으로 보는 것** 한 번이다.
  못 재는 동안은 「전사 확인 단계」(사용자가 고쳐서 저장)가 유일한 방어선이라는 점을 기억한다.

- ✅ **AI 호출 원가가 설계 가정의 5.5배다 (2026-09-03 실측) → 선택지 ②로 해소 (2026-09-08)**.
  일정 생성 실호출에서 **회당 약 153원**이 나왔다 — 설계가 잡은 **30원**의 5.5배이고, 최악 관측은
  **455원**이었다. 출력 토큰이 큰 것이 원인이다(79일치 일정 = out 10,574토큰). 하루 상한(PLAN 3)이
  1인당 상한은 걸지만 **사용자 수에 비례해 원가가 늘어서**, 승인제를 푸는 날 그대로 터진다.
  선택지는 ① 출력 축소 ② 더 싼 모델 ③ 상한 하향 ④ 감수였고 — **②를 골랐다**(아래 「일정 생성을
  Gemini Flash로」 절). ①·③은 사용자가 받는 것을 줄이는 쪽이라 뒤로 미뤘다.

- 🔜 **구글 애널리틱스 데이터 보관 설정값 확인 (처리방침 7절)**. 처리방침은 **「최대 14개월」**로
  적었다 — GA4 표준 등급이 2개월/14개월 중 하나라 **어느 쪽이든 참인 표현**이다. 콘솔에서 실제
  설정값을 한 번 보면 숫자로 못 박을 수 있다(관리 → 데이터 설정 → 데이터 보관). #1045 PR body에
  `ABANDON: GA4-RETENTION`으로 남겨 둔 항목이다.

- ✅ **스트리밍/비동기 호출 — 트리거 소멸로 닫음 (2026-09-13, 코드 0)**. 2026-09-03 Sonnet 실호출
  **75.7초**(79일)·28.9초(15일)가 60s 기준을 넘어 승격 후보였으나, 09-08 Gemini Flash 이전 뒤 같은 조건
  실측이 **16~26초**로 내려가 근거가 사라졌다(아래 「일정 생성을 Gemini Flash로」 절의 📍 「타임아웃 걱정은
  사라졌다」가 먼저 적었고, 이 항목만 남아 두 기록이 어긋나 있었다). 「클라이언트 타임아웃 90s」도 현재
  코드엔 없다 — `api.ts` fetch·Caddy 모두 타임아웃 없음, 실효 상한은 `GeminiStudyPlanner` readTimeout 90s 하나.
  09-13에 설계까지 마친 뒤(`claude-docs/plans/2026-09-13-study-plan-async-generation.md`, gitignore —
  인메모리 작업 + 3초 폴링 추천, SSE·월 분할 기각) 사용자 결정으로 **옵션 0(하지 않음)**. 다시 필요해지는
  조건: 실측이 60s를 다시 넘거나, 꼬리 지연·탭 닫힘 복구·`MAX_PLAN_ITEMS` 상향 같은 내구성 값을 사야 할 때 —
  그때 그 설계를 꺼낸다. ⚠️ 타임아웃 상향은 여전히 답이 아니다(원장 U-6 — 기다리는 요청 스레드가 늘 뿐이다).

### 📚✏️ 웹 대시보드 「공부 타이머」 (설계 `claude-docs/plans/2026-09-04-web-study-timer.md`)

> 대시보드 히어로에 독서/공부 모드 토글과 공부 스톱워치를 붙인다. **서버 변경 0** —
> `/api/dashboard`가 이미 `study` 블록을 보내고 `/api/study/start|stop`은 웹 세션 체인으로 닿는다. 2단으로 나눈다.

- ✅ **PR-1 복귀 재조회 + 409 재조회 (2026-09-04)** — 공부 없이도 독서의 낡음을 고치는 독립 분이라 먼저 떼었다.
...
- ✅ **PR-2 독서/공부 타이머 토글 (2026-09-04)** — `types.ts`의 `study` 블록(+`IDLE_STUDY` 폴백)·`ModeToggle`·
  ...
  ✅ **스위치를 화면 아래 가운데로 옮김 (2026-09-17)** — 위 「모드가 바꾸는 것은 히어로 카드 하나」는 낡았다: 지금 모드는
  타이머 카드 + 여백↔필기 카드 + 양옆 바 흐림까지 바꾸는 **페이지 단위** 스위치라, 카드 머리줄 자리가 타이머 부속처럼 읽혔다.
  `ModeToggle`을 카드 슬롯에서 꺼내 `DashboardApp`에 한 번(`position: fixed` 종이 알약, z 94)만 두고 카드의 slot·wrapper는 지웠다.
  공부 측정 중(독서등)엔 그리지 않고, 독서 측정 중엔 잠긴 채 힌트를 알약 위에 띄운다. 공부 잉크는 wrap의 `.is-study`(`.rail-study`와 같은 관용구).
  ✅ **스위치를 바 있는 모든 화면으로 (2026-09-17)** — 비홈 화면은 바 fragment가 같은 모양의 SSR 알약을 그리고, 누르면 저장 후 홈으로 간다.
  `.rail-study`는 바를 하나로 합치며 사라졌다(「웹 페이지 이동 → 양옆 세로 바」 절).
- ✅ **PR-3 상호배타를 서버에서 양방향으로 (2026-09-04)** — PR-2까지 상호배타는 **한 방향뿐**이었다:
  공부 start는 독서·공부 둘 다 보는데 독서 start는 자기 것만 봐서, 공부가 도는 중에 독서를 시작하면 둘 다 돌았다.
  이건 버그가 아니라 2026-09-01의 의도된 결정이었고(그땐 웹이 공부를 몰라 막으면 화면에 보이지도 않는 세션에
  막혀 영문 모를 거절이 났다), **PR-2의 토글로 그 전제가 사라져** 이제 막아도 안전하다.
  `ReadingSessionService.start`에 공부 진행 중 검사 한 겹(공부 쪽 코드의 대칭) — **클라이언트 변경 0**이다
  (두 진입점이 이미 이 예외를 409·flash로 옮기고, 웹은 PR-1의 `conflict()`가 강제 재조회까지 한다).
  가드는 **`start`에만** 건다 — `recordManual`은 이미 끝난 과거를 적는 경로라 진행 중 세션과 충돌하지 않는다.

### 🪞 웹 「독서 / 공부」 분리 — 1단계 (설계 `claude-docs/plans/2026-09-04-web-study-separation.md`)

> 사용자 요청: 「웹도 앱처럼 공부탭과 독서탭은 분리시키자. 웹은 타이머만 분리되어 있는 거 같아.」
> 위 타이머 판이 모드를 **히어로 카드 한 장**에만 태웠으니, 그 아래 잔디·타일·정원이 아직 독서 고정이다.
> 웹엔 탭바가 없다 — **페이지가 탭이고 URL이 모드다.** 그래서 분리의 손잡이는 둘뿐이다:
> 대시보드 QuickNav 타일 세트(모드를 아는 유일한 표면)와 공부 페이지끼리의 정적 하단 네비.

- ✅ **PR-1 공부 기록 화면 `/study/history` + 공부 네비 (2026-09-04)** — 같은 셸(`study.html`)·같은 번들에
  루트 컴포넌트만 갈아끼운다(`studyView(location.pathname)`) — 서버는 `@GetMapping({"/study", "/study/history"})`
  **한 줄**만 넓혔고 API·Flyway는 0이다. 잔디·월별 목록은 `/history`의 조각(`ContributionGraph`·`MonthlyRecords`)을
  **import 재사용**한다 — 잔디 방향 규약(`weeks[0]` = 최신 주 = 왼쪽)을 두 번 밟지 않으려는 선택이다.
  두 조각엔 **기본값 있는 prop만** 들어가 독서 렌더는 불변이다(`mode='reading'` · `emptyText` 기본값 ·
  「독서」라는 낱말이 `<strong>` 밖에 그대로 남아 마크업 경계도 안 옮겼다).
  하단 네비는 공부 세계 안에서만 돈다(`/study` → 홈·공부 기록 / `/study/history` → 홈·일정) — 「내 책장」을
  뺀 것은 의도다(미니앱 공부 모드에도 독서 서재로 가는 문이 없다). 공부 잉크는 `.is-study` **카드 스코프**
  토큰 스왑을 일반화해 대시보드 히어로와 이 화면이 규칙 하나를 공유한다(잔디 사다리 파랑 4단 포함).
  아이콘 사전 `calendar`를 Vue·SSR **두 벌에 함께** 넣고, 여태 주석뿐이던 두 벌 동기화를 테스트 계측기로 만들었다.
- ✅ **PR-2 대시보드가 모드를 따른다 (2026-09-04)** — 모드가 히어로 한 장에서 **잔디·빠른이동·정원**까지
  내려갔다. 공부 모드에서 잔디는 공부 잔디(`→ /study/history`, 「적게…많이」)가 되고, 빠른 이동은 공부
  세트(일정·공부 기록)로 갈리고, 정원은 숨는다(`.dash-grid-2col`이 auto-fit이라 CSS 0줄로 타일이 전체폭).
  **독서 QuickNav의 「공부」 타일은 삭제했다** — 공부로 가는 문은 히어로 토글 하나만 남는다(미니앱과 같다).
  공부 잔디 데이터는 `/api/study/history` **별도 GET**이다(`/api/dashboard`의 `StudyState`는 미니앱과
  공유하는 record라 53주 집계를 얹지 않는다 — **서버·API·Flyway 0**). 페이지 수명 캐시 + 공부 stop 뒤 재조회.
  ⚠️ 여기서 **대시보드 잔디에도 「독서 기본값 양성 대조군」을 함께 세웠다** — PR-1에서 같은 꼴의 `mode`
  기본값을 뒤집어도 546건이 전부 초록이던 사각이 최다 사용 화면에 생기는 자리였다.

**1단계 완료 ✅** — 웹의 「독서/공부」가 히어로·잔디·타일·정원·페이지·네비 전부에서 갈렸다.

### 🪞 웹 「독서 / 공부」 분리 — 2단계 (설계 `claude-docs/plans/2026-09-05-web-study-stage2.md`)

> 2단계는 셋이다: ① 공부 서재 ② 공부 목표 게이지 ③ 공부 세션에 책 걸기. 서버 문은 전부 이미 있어
> **API·Flyway 0**이고, 서버 변경은 1단계와 같은 꼴로 **셸 라우트 한 줄**뿐이다. 순서 고정 A → B → C
> (③은 ①이 있어야 의미가 있고, ②·③이 `StudyTimerCard`를 함께 건드려 판을 나눠야 리뷰가 한 결정씩 본다).

- ✅ **PR-A 공부 서재 `/study/books` + 공부 네비 3자리 (2026-09-04)** — 분류 축이 **회독 수 하나**인 서재
  (담기·회독 ±1·삭제). 독서 서재 `BooksApp.vue`(408줄)를 **재사용하지 않는다** — 상태 3종·공개여부·인기도·
  책방 링크가 공부엔 없어 재사용이 곧 조건문 비용이다(미니앱이 `StudyLibrary.tsx`를 따로 둔 것과 같은 판단).
  빌려 쓰는 것은 `books/pure.ts` 순수 헬퍼(`initialOf`·`coverColor`·`byline`)와 CSS 클래스뿐이라
  **독서 전용 소스 diff 0줄**이다. 서버는 `@GetMapping({"/study", "/study/history", "/study/books"})` 한 줄.
  네비가 셋이 되면서 `studyNavLinks(current)`가 **지금 화면을 뺀 나머지 둘**을 준다(자기 자신으로 가는 링크는
  문이 아니다). QuickNav 공부 세트는 3타일(공부 서재·일정·공부 기록)이 되고 마지막이 odd 규칙으로 전체폭
  — 독서 세트의 「책BTI」와 같은 모습이다. **구매 링크는 일부러 안 붙였다** — 공부 책엔 `/books/{id}/buy`
  같은 클릭 추적 라우트가 없어 직링크면 수익 경로가 무추적으로 열린다(⏸).
- ✅ **PR-B 공부 목표 게이지 + 히어로 인라인 변경 (2026-09-05)** — 히어로에 게이지(`studyProgress` — 미니앱
  `Home.tsx` 포팅)와 **분 단위 숫자 입력**(`POST /api/study/goal`)을 얹었다. 설정 페이지(독서 관례)로 보내지
  않은 이유는 ⏸ 항목에 그대로 있다 — 서버 ≈30줄이 붙고 설정의 「빠뜨린 날은 나중에 채워」 힌트가 공부엔
  거짓이다. **목표 0은 「목표 없음」이라는 정당한 상태**라 게이지 대신 「하루 목표 정하기」가 서고, 편집 폼의
  「목표 없이 지내기」는 **지울 목표가 있을 때만** 뜬다(미니앱 `showClearGoal`). 독서 `computeProgress`에
  분기를 더하지 않고 `studyProgress.ts`를 새로 둔 것은 그 기본값이 최다 사용 화면에 닿기 때문이다 —
  **독서 전용 소스·독서 테스트 diff 0줄**, 서버·API·Flyway **0**. ⚠️ **실브라우저가 잡은 결함 1건**:
  설계의 `<input step="5">`는 스피너 간격이 아니라 **유효성 제약**이라 7·23분이 `stepMismatch`가 되어
  네이티브 검증이 submit을 막았다 — 조용한 건 **앱 쪽이고**(요청 0건) 사용자에겐 크롬이 검증 버블
  「가장 근접한 유효 값 2개는 7 및 8입니다」를 띄운다(jsdom의 `trigger('submit')`은 제약검증을 건너뛰어
  테스트는 초록이었다) → `step="1"` + `validity`를 직접 재는 회귀 가드. `step="1"`은 **정수 분만 받는다는
  의도**다(7.5를 조용히 7로 내리지 않는다) + `max="1440"`(하루) 상한.
  → **2026-09-13 컨셉 전환으로 폐기 — 아래 「공부 타이머 컨셉 전환 — 하루 목표 → 책별 회당 시간」 절 참조.**
- ✅ **PR-C 공부 세션에 책 걸기 (2026-09-05)** — 시작 시 선택 · 종료 후 태깅 · 측정 중 교체. 시트는 **공부 전용
  새 컴포넌트**(`StudyBookSheet.vue`)다 — 독서 `BookPickSheet`에 `world` prop을 더하면 그 기본값이 최다 사용
  경로에 닿고, 픽스처가 새 prop을 모르니 **기본값이 뒤집혀도 초록인 사각**이 이 판에서 두 번 재현된 그대로
  다시 열린다. 목록만 그리고 **fetch가 0**인 것은 `/api/dashboard`가 `study.books`를 이미 싣고 오기 때문이다
  (미니앱 `BookSheet`/`ChangeBookSheet`와 같은 계약 — 시트 안 검색·담기는 ⏸). `StudyState`를 서버 8필드로
  넓히면서 **정규화 `studyStateOf` 하나**를 두어 대입 6곳을 전부 지나게 했다 — 필드가 빠진 옛 응답에서
  `undefined !== null`이 참이 되어 시트를 띄우려다 죽는 자리를 그 한 줄이 막고, 옛 픽스처가 무수정 green으로
  남는 것이 「독서 경로는 새 필드를 모른다」의 증명이다. **서버·API·Flyway 0 · 독서 전용 소스 diff 0줄.**

- ✅ **후속 4건 마감 (2026-09-05)** — 리뷰가 「그대로 머지」로 판정하며 남긴 것들. ① **교체 404 자동 복구** —
  `/api/study/active/book`가 404(다른 곳에서 지운 책)면 `start`와 같은 규칙으로 시트를 닫고 재조회한다. 열어
  두면 지워진 그 행이 목록에 남아 눌러도 계속 실패했다. **태깅(`tag`)은 일부러 그대로** — 그 시트가 세션 태깅의
  유일한 진입점이라(세션 id는 stop 응답에만 실린다) 닫으면 미태깅으로 굳는다. *(2026-09-25 R2 PR-3(웹)·PR-5(미니앱)로 해소 — 이제
  「공부 기록」 날짜를 펼쳐 측정 한 건씩 다시 붙일 수 있다. 태깅 시트를 열어 두는 규칙은 그대로다.)* ② **`studyStateOf` 6자리 전부
  계측기로 잠금**(테스트 636 → 642) — 선언은 6곳이었는데 잠긴 건 `stop` 하나뿐이었다. 지렛대는 게이지
  하나뿐이다(`StudyTimerCard`의 `withDefaults`가 나머지 필드를 스스로 메워 관측 수단이 없다 — 그 사실도
  `types.ts` 주석에 같이 적었다). ③ **T-229** 등재 — 돌연변이 「생존」을 단일 파일 실행으로 판정하면 조용히
  과대·과소 보고된다(이번 6자리는 전량 전체 스위트로 쟀다). ④ 끝난 설계 md 3장에 완료 배너(`EVIDENCE: pending`
  오독 방지).

**2단계 완료 ✅** — 공부 서재 · 공부 목표 게이지 · 공부 세션에 책 걸기가 모두 웹에 섰다. 웹 공부는 이제
미니앱과 같은 세 축(서재 · 목표 · 책 걸기)을 갖췄고, 남은 것은 아래 ⏸ 목록뿐이다.

#### 보류 (⏸)
- ⏸ **공부 서재의 구매 링크·제휴 고지** — `StudyBookRow.purchaseLink`는 오지만 공부 책엔 `/books/{id}/buy`
  (클릭 추적 리다이렉트)에 해당하는 라우트가 없다. 직링크로 열면 추적이 빠진 수익 경로가 열린다.
  트리거: 공부 서재 사용자가 생기면 `/study/books/{id}/buy` 서버 PR로.
- ✅ ~~⏸ **`/study/history` 월별 행의 책 제목**~~ — 사용자 결정(2026-09-24)으로 재개해 R2에서 풀었다: 서버가
  `Day.sessions[]`를 싣고(PR-1), 기록 날짜를 펼치면 측정 한 건씩 책 제목과 [책 붙이기]/[바꾸기]가 선다(웹 PR-3 · 미니앱 PR-5).
  접힌 줄에 책 제목 목록은 여전히 없다 — 책 축의 답은 공부 서재 누적 시간 칩이 계속 맡는다.
- ⏸ **정원이 비운 자리의 카드**(공부 서재 요약 등) — 서재로 가는 문은 타일 하나면 충분하다.
  트리거: 「공부 대시보드 아래가 허전하다」 제보.
- ~~⏸ **설정 페이지의 공부 목표 항목**(독서 관례)~~ — **폐기(2026-09-13)**: 공부 하루 목표 자체가 컨셉 전환으로 사라졌다
  (회당 시간 절 참조). (옛 사유: PR-B가 히어로 인라인을 택했고, 서버 SSR 폼·「빠뜨린 날」 힌트가 공부엔 거짓이었다.)
- ⏸ **페이지가 모드를 쓰는 규칙**(`/study*` 방문 시 저장 모드를 study로) — 딥링크로만 어긋나는 드문 경우다.
  트리거: 「공부 페이지에서 홈을 눌렀는데 독서 대시보드가 떴다」 제보.
- ⏸ **`body.study-mode` 전역 잉크** — 웹 셸은 JS 이전에 그려져 첫 페인트가 세이지로 뜬 뒤 바뀐다.
  미니앱이 body에 얹는 이유(탭바 가운데 원이 측정 손잡이)가 웹엔 없다.
- ⏸ **`/study/history` 닉네임 인사말** — `/api/study/history`에 닉네임이 없고 미니앱도 「공부 기록」 제목뿐이다.
- 공부엔 **「빠뜨린 날」이 없다** — 부채 개념 자체가 없어 억제 코드 0줄(안 그리면 끝).

### 📚✏️ 공부 항목에 「인강」 — 링크 한 칸 (완료 ✅ 2026-09-16)

> 인강(인터넷 강의)은 ISBN도 표지도 없어 검색으로 담을 수 없는데, 공부 시간의 상당 부분이 거기서 난다.
> 설계 `claude-docs/plans/2026-09-16-study-timer-online-lecture.md`.

- ✅ **인강 직접 등록 (2026-09-16)** — `study_book`에 nullable `link_url`(V93) **한 컬럼**. 웹 `/study/books`의
  기존 「직접 추가」에 강의 링크 칸을 얹고, 목록 행에 「강의 열기」(새 탭)·「링크」(교체·해제)를 뒀다.
  서버는 등록에 `linkUrl`을 받고 링크 전용 문 `POST /api/study/books/{id}/link`를 연다(빈 값 = 해제 —
  강의 URL은 개편·만료로 죽는데 「지우고 다시 담기」는 회독 수를 잃고 필기가 붙은 책은 409라 아예 불가능하다).
  링크 검증(http/https·host·1000자)은 **서버가 유일한 벨트**다 — Vue `:href`도 `<input type="url">`도
  `javascript:`를 걸러 주지 않는다.
  - **종류(책/인강) 컬럼을 만들지 않은 것이 이 설계의 핵심 판단이다** — 잔디·통계·회당 시간·필기·회독이 전부
    StudyBook 단위라 구분 없이 돌고, 화면의 유일한 차이(「강의 열기」)는 `linkUrl != null`로 충분하다.
  - `purchaseLink` 재사용도 기각했다 — 거긴 제휴 구매 링크라 미니앱이 「알라딘에서 구매」 + 수수료 고지를
    그린다. 인강 URL을 실으면 **법적 고지가 거짓**이 된다.
- ⏸ **미니앱 인강 추가 화면** — 이번엔 타입 한 줄 + 깨짐 방지 테스트 1건만 했다(서버가 싣는 `linkUrl`을
  미니앱 화면은 아직 안 읽는다). 트리거: 미니앱에서 인강을 담고 싶다는 제보, 또는 웹 등록분이 쌓여
  「미니앱에서 강의를 못 연다」가 불편해질 때.
- ⏸ **종류별 필터·통계(책/인강)** — 소비처가 0이라 안 만들었다. 트리거: 「인강만 모아 보고 싶다」 요청.
  그때 `kind` 컬럼 + 백필(`isbn13 is null and link_url is not null → LECTURE`)인데, **링크 없는 인강은
  못 가른다** — 그게 지금 미루는 비용이다.
- ⏸ **링크 생존 검사·미리보기(og:image)·favicon** — 강의 페이지는 대부분 로그인 벽이라 서버가 열어 볼 수 없다.
...
- ✅ **PR-2 웹 (2026-09-13)** — 공부 히어로의 하루 목표 게이지·「하루 목표 정하기」·인라인 편집·「목표 없이 지내기」를 걷고
  (`studyProgress.ts` 삭제, 「오늘 공부한 시간」 숫자는 유지) 칩 아래 「회당 50분 · 변경」/「회당 시간 정하기」 → 분 단위 인라인 폼
  ...
  같이 폼 닫기 + 재조회, 저장 왕복 중엔 복귀 재조회를 건너뛰고, 폼을 열면 입력칸에 포커스. 폼은 좌열이 아니라 우측 패널의
  손잡이 바로 아래에 열린다(좌열이면 넓은 화면에서 ≈650px 떨어지고 좁은 화면에선 손잡이를 밀어냈다 — 실브라우저 실측).
  독서 경로 diff 0. 실브라우저 탭 제목 확인(U-4) 충족 — PR body 원장(폼 이동 후 재확인은 구조 테스트로 잠금).
- ✅ **PR-3 미니앱 (2026-09-13)** — 공부 홈의 하루 목표 게이지·「목표 정하기」·「변경 ›」·공부 새싹과 공부 목표 화면(`'studyGoal'`)을 걷었다 —
  공부 「목표 바꾸기 = 전면광고 1회」 진입점이 사라졌고 회당 시간 설정엔 광고 없음(Q1). 캐러셀 아래 「회당 50분 · 바꾸기」/「회당 시간 정하기」 →
  자체 `Sheet` 안 시/분 휠(0~6시간·없는 책은 30분 시작, 0분·6시간 초과는 저장 잠금) → `setStudySessionGoal`. 측정 중 「회당 N분 · M분 남음」→
  「회당 N분 달성 · M분 더 공부했어요」 + 닿는 순간 햅틱 1회. 알림 동의 카드 문구에 「공부 시간」 추가.
  **배포(2026-09-13)**: 번들 `20260913-148`(`01a09a8f`) 업로드 → 실기기 확인(햅틱 1회·휠 0분/6시간 초과 잠금·안드로이드 뒤로가기는 시트만, U-3) →
  검수 요청 + 스토어 스크린샷 06 교체(앱 정보 검토). ⚠️ `-148`(SDK 3.0.4)은 16시간 PENDING이라 2026-09-14 검수 취소 —
  내용은 아래 `-149`에 전부 들어 있다.
- ✅ **PR-4 서버 — 회당 시간 도달 토스 푸시(다크런치) (2026-09-13)** — `V91` `study_session.goal_notified_at`(세션당 1회 멱등) + `idx_study_session_ended_at`,
  ...
  + `StudyGoalPushScheduler`(`study-goal-enabled` 게이트). 마킹은 **컬럼 단독 UPDATE**라 `stop`과 겹쳐도 `endedAt`을 되살리지 않고,
  배치 트랜잭션 없이 **세션마다 즉시 커밋**한다(한 세션 실패가 이미 보낸 세션의 마킹을 롤백하지 않게).
  ...
  **다크런치 — 점등은 콘솔 템플릿 승인 후**(사용자 지시 → 템플릿 등록·테스트 발송 → 승인 → SSM 실값 → 재배포 → 실기기).
  ⚠️ SSM엔 빈 값을 못 넣어 운영의 `TEMPLATE_CODE`는 `placeholder`다 — 실제 다크런치 게이트는 `ENABLED=false` 하나뿐이니
  **점등 시 `ENABLED`와 `TEMPLATE_CODE`를 함께 바꾼다**(`ENABLED`만 켜면 잘못된 코드로 매 틱 발송 실패 warn).
- ✅ **공부 알림 동의문 분리 + 모드별 동의 카드 (2026-09-13, 설계 E20 분기)** — 콘솔 AI 검수가 공부 푸시를 독서 동의문(114526
  「독서 목표 달성과 완독 소식」)에 붙이는 걸 `consent_match: fail`로 거부했다. 사용자 결정으로 「공부 알림 동의문」(termsId 122175,
  「정한 공부 시간을 채우면 알려드릴까요?」)을 새로 만들고 템플릿 `booktimer-study-goal-met` 등록·**승인**(제목 「공부시간 달성」 —
  띄어쓰기 포함 7자 제한, 본문 「정한 공부 시간을 채웠어요.」, 홈 착지) → 테스트 발송 성공. 미니앱 홈 동의 카드를 모드별로 갈랐다 —
  독서는 원래 문구 「목표 달성과 완독 소식을 토스 알림으로 받아보세요」·기존 캐시 키, 공부는 「정한 공부 시간을 채우면 토스 알림으로
  알려드려요」·`booktimer.notificationAgreement.studyGoal`(독서에 답한 사람에게도 공부 알림을 물을 수 있게). 목 모드 브라우저엔
  ...
  **실기기 수신(2026-09-15, U-10·U-11 충족)**: 회당 1분으로 닿자마자(4~12초 뒤) 정지한 3회는 발송 0건 — 60초 폴링이 **측정 중인**
  세션만 찾아 틱이 한 번도 안 걸렸다(설계 조건대로). 닿은 뒤 정지하지 않은 4회차에 닿고 11초 뒤 발송·수신, 전체 발송 1건(중복 0).
  발송 여부의 정본은 DB `study_session.goal_notified_at`이다(콘솔 `push_stats`는 서버 발송을 안 보여 줬다).
- ✅ **회당 시간 최소 10분 (2026-09-15)** — 서버 `StudyBook.MIN_SESSION_GOAL_SECONDS` 60→600(400 문구 「10분에서 6시간 사이」) · 웹 입력칸
  `min="10"` · 미니앱 휠 10분 미만 저장 잠금(「10분 이상으로 골라 주세요」)·목 서버 같은 범위. 폴링 주기는 60초 유지(사용자 결정 —
  1분 단위로 공부하지 않는다). 이미 저장된 10분 미만 값(운영 2권, 사용자 본인 시험용 60초)은 그대로 읽힌다(검사는 쓰기에만).
  미니앱 휠은 **10분 미만으로 저장된 책도 10분 칸에서 연다**(`sessionGoalWheelStart` — 잠긴 저장 버튼으로 시작하지 않게, 0~9분 칸은
  남기고 판정이 잠근다. 칸을 없애는 안은 시간 휠에 따라 분 휠 옵션이 바뀌어 재마운트가 필요해 기각 — 사용자 선택).
  서버·웹은 #1134로 배포 완료. **미니앱 출시(2026-09-15)**: 번들 `20260915-151`(`01a0a344`, SDK 3.1.1) 실기기 확인 → 검수 승인 →
  17:39 KST 라이브. 번들 `-150`은 휠 시작 칸 변경 전이라 검수에 올리지 않았다(업로드만 된 채 미사용).
...
> 회색 글 3줄과 버튼 하나뿐인 첫 화면에서 난다. 처방 = 첫 화면을 실제 타이머로 바꿔 **로그인 없이 1회 측정**하게
> 하고, 「기록을 남기려면」 시점에 로그인을 청한다. 체험 결과는 localStorage에 두었다가 로그인 직후 서버로 흘려보낸다.

- ✅ **PR-1 서버 — 체험 세션 받는 문 (2026-09-10)** — `ReadingSessionService.recordCompleted` + 멱등 조회
  `findFirstByUserAndStartedAt` + `POST /api/sessions/import`. **Flyway 0**이고 기존 정책을 그대로 재사용한다
  (6h 클램프 → 유저 TZ 자정 분할 — `stop`과 같은 순서). `recordManual`을 쓰지 않은 이유는 그쪽이 **책 필수 +
  `manualEntry=true`**(잔디 테두리 = 「손으로 채운 날」)라 **실측인 체험에 의미가 틀리기** 때문이다. 저장된 행은
  완료 세션 한 건일 뿐이라 오늘 읽은 양·부채·잔디가 전부 기존 유도 규칙을 탄다. ⚠️ **멱등 키는 저장 정밀도를
...
- ✅ **PR-4 카드 — 책 카드와 잠긴 피드 (2026-09-11)** — 게스트 홈이 시안대로 찼다. 「무엇으로 측정할까요?」는
  로그인 홈과 **같은 자리·같은 카드**이고 고를 수 있는 칸만 하나다(`NoBookCard` export + 자물쇠 점선 칸 둘 +
  「책 없이」·캡션·점 3개 + 연필선 버튼 「토스로 시작하고 책 고르기」 → `login_started{source:'book_card'}`).
  피드는 `FeedBox`에 `locked` 프롭 하나를 더해 **머리만 남기고** 본문을 잠금 안내로 바꾼다 — 「소식·여백·책 뉴스」가
  있다는 사실 자체가 「로그인하면 무엇이 열리는가」의 답이라 박스를 다른 카드로 갈지 않는다. **서버 호출 0**
  (`feed={null}` + no-op 핸들러) · 로그인 홈은 프롭 미지정이라 무변경. 목 모드 실측: 첫 진입 `position:fixed &&
  z≥99` = **1개**(탭바 `nav`) · `/api/` **0건** · 콘솔 에러 0 · 책 카드 버튼 → 로그인 홈 · 합류 1회(25:00 → 25:25)
  · 로그아웃 → 게스트 홈 복귀. **심사(U-1·U-2)·실기기(U-5)·이벤트 카탈로그(U-6)는 배포 후**.
- ✅ **출시 (2026-09-11)** — 번들 `01a08feb`(20260911-147)가 검수 승인 후 19:53 라이브(`bundle_get_live_version` 실측).
  PR-1~PR-4가 한 번들로 나갔다. **판정은 2026-10-02 이후**: `login_started(전 source)/guest_entered` 19% → 35% 이상,
  n≥40 — 그 전엔 수치가 나와도 판정하지 않는다. 남은 원장: U-5(신규 계정 목표 화면, 실기기) · U-6(새 이벤트 5종 카탈로그
  등재 — 콘솔에서 며칠 뒤 확인). ⚠️ 이 작업의 서버 커밋이 **T-235 2회차**(스위트 8분 19초 > 게이트 8분)에 걸렸다.
- ❌ **PR-0 콘솔 — 실행 불가 (2026-09-11 확정)** — `USER_NAME`은 토스 콘솔의 **고정 필수 스코프**라 선택·해제가
  불가하고, `toss_login_update_terms`도 `requiredScopes`에 `USER_NAME`이 없으면 거부한다(문서 명시). 현재 설정
  (필수 이름 / 선택 이메일 / 약관 1건)이 이미 하한이라 호출하지 않았다. 서버가 이름을 안 쓰는 사실은 그대로지만
  동의창은 못 줄인다 — 원장 U-2·U-3 종결.
- 🔜 **2단계 — 첫 진입 붙잡기: 선택 화면 제거 + 게스트 「책 뉴스」 (설계 `claude-docs/plans/2026-09-17-first-entry-login-and-guest-news.md`)**
  — 앱인토스 「구경하고 포인트 받기」 프로모션(9/21~)이 책 없는 사람을 데려오기 전에 (A) 로그인 뒤 「새로 시작 /
  기존 계정 연결」 화면을 없애고 (B) 게스트에게 유일하게 살아 있는 탭으로 책 뉴스를 연다.
  - ✅ **PR-S 서버 (A) (2026-09-17)** — `/api/toss/register`가 find-or-create 결과에 `created`를 싣는다(login·link는 늘
    false — 미니앱이 인가 1회로 신규/기존을 가른다). ⚠️ 한계: 같은 신규 userKey의 동시 register 두 건은 둘 다 조회에서
    빈손이라 경합한다(이번엔 안 고침).
  - ✅ **게스트 뉴스 서버 — 일반 책 뉴스 (2026-09-17, 설계 `claude-docs/plans/2026-09-17-guest-general-book-news.md`)** —
    처음 안(공개 완독 책 isbn의 캐시 기사)은 리뷰 차단으로 뺐다: 아무 계정이나 제목·ISBN을 넣은 가짜 공개 완독 책으로
    게스트 뉴스의 책 제목·기사 주제를 조종할 수 있었다. 사용자 결정대로 **운영자 고정 주제 3종**(`신간`·`출판계`·
    `베스트셀러 도서`, 전부 `when:7d`)을 `GeneralBookNewsFeed`가 기동 즉시 모으고 채운 뒤로는 6시간마다(못 채웠으면 5분마다 재시도)
    **메모리 스냅샷**에 둔다(구글 뉴스 링크만·14일·제외어 8종 필터, link·제목 키 중복 제거, 주제별 10/전체 30, 0건이면 옛 목록 유지, 킬스위치 OFF면 비움).
    `GET /api/public/news`는 그 스냅샷만 내보내고 리포지토리에 의존하지 않는다 — `bookTitle` = 주제 라벨, 빈 응답은 `no-store`. `/api/public/`은
    미니앱 체인의 익명 공개 계약. 구글 RSS 클라이언트에 연결 3초·읽기 5초 타임아웃(단일 스케줄러 스레드 보호).
    배포 후 원장 U-2(운영 CORS·비지 않음)·U-3(14일·제외어)·U-4(새로고침 소요)·U-5(타임아웃 설정)를 채운다.
    ⚠️ 로그인 홈 뉴스의 `min(title)` 오염(N-5)은 별도 과제.
  - ✅ **PR-M1 미니앱 (A) — 로그인 직후 선택 화면 제거 (2026-09-17)** — 손잡이 → 토스 동의창 → 「새로 시작 / 기존 계정 연결」의
    **세 번째 칸을 없앴다**. 손잡이 여섯은 처음부터 멱등 find-or-create인 `register`를 불러 **인가 1회**로 계정까지 가고(옛 흐름은
    선택 화면 뒤 `register`가 appLogin을 한 번 더 받았다), 위 PR-S의 `created`로 신규면 목표 화면·`account_created{source}`.
    웹 계정 보유자의 문은 **인가 전** 게스트 홈 맨 아래 「이미 booktimer.app 계정이 있나요? [기존 계정 연결]」(`web_link`) —
    조회(`login`)만 하고 미등록이면 연결 화면으로 직행한다(다른 손잡이로 계정이 생기면 토스 신원이 once-set으로 박혀 웹 계정에
    다시 못 붙기 때문). 서버 의존(`created`)은 PR-S 머지로 해소 — 번들 배포는 서버 배포 뒤. 남은 원장(번들 배포 후·검수 제출 전):
    U-A1(실기기 동의창 1회) · U-A2(연결 경로 동의창 횟수 기록) · U-A3(`account_created` 등재).
  - ✅ **PR-M2 미니앱 (B) — 게스트에게 「책 뉴스」를 연다 (2026-09-17)** — `GuestShell`이 마운트 때 익명 공개 뉴스
    (`GET /api/public/news` — 위 재설계의 **일반 책 뉴스(운영자 고정 주제 `신간`·`출판계`·`베스트셀러`)**, 사용자 입력 무관)를
    받아 피드 박스의 **「책 뉴스」 탭만 열고 기본 탭으로** 세운다. 소식·여백·사람은 **알약별로** 흐린 채 눌리고, 누르면
    「소식·여백은 계정이 있어야 보여요」가 선다. 게스트 배지는 `bookTitle`에 실려 오는 **주제 라벨을 겹낫표 없이** 그대로 쓴다
    (게스트에게 「내 책」도 『』도 거짓). 실패·꺼짐·빈 목록은 옛 전체 잠금 그대로. 공개 요청은 `anonymous`라 토큰을 안 싣고
    401에도 토큰을 안 지운다. 로그인 홈 피드는 `locked` 미지정 경로라 무변경. 이벤트: `guest_news_opened`,
    `guest_entered{variant:'news'}`. 목 모드 실측(390×844): 첫 진입·체험·완주·거절·새로고침 모두 `position:fixed && z≥99` = **1**.
    서버(일반 책 뉴스)가 없으면 옛 잠금으로 조용히 남는다(안전) — 서버는 먼저 머지·배포됐고 운영 응답 29건·구글 뉴스 링크만·
    14일 초과 0·CORS(apps·private-apps 200, 그 외 403)를 확인했다(2026-09-17). 남은 원장(번들 배포 후·
    검수 제출 전): U-B2(실기기 뉴스·외부 열기) · U-B4(이벤트 등재).
  - ✅ **심사 반려 대응 (2026-09-17)** — 번들 `20260917-152`가 「미니앱 최초 접속 20초 초과」로 반려(이 앱 첫 사례). 무게(+0.5KB)·서버는
    배제, 첫 화면에서 바뀐 동작은 게스트 뉴스 요청 하나 → 공개 뉴스 요청에 5초 상한(`timeoutMs`) + `load` 뒤로 미룸(`whenPageLoaded`).
    원인 확정은 아니다(심사 환경 재현 불가). 새 번들 `20260917-153` 재제출 → **승인·출시 2026-09-17 21:04**. 함정 기록 T-252.
  - ✅ **게스트 뉴스 품질 ① 스팸 출처 차단 (2026-09-21, 설계 `claude-docs/plans/2026-09-21-guest-news-quality.md`)** — 운영
    `/api/public/news` 25건 실측에서 SEO 도박 스팸 2건이 **로그인 전 첫 화면(= 심사자가 보는 자리)**에 올라왔다. 주 방어는
    이름 차단목록이 아니라 **매체명 표기 규칙**(`trustedSource`) — 구글 RSS `<source>`가 등록 표시명이 없으면 호스트명을 주는
    성질을 써서, 한글 표시명·약어(`YTN`)·`.kr`/일반도메인 호스트는 통과시키고 설명형 라틴 표시명(「Histoire pour tous」)과
    외국 국가도메인(`.fr`·`.vn`)은 뺀다(스팸 이름은 한 주에 셋 갈려 차단목록이 늘 한 발 늦는다). 보조로 도박어 4종
    (카지노·슬롯·바카라·먹튀 — 「도박」·「배당」·「토토」는 책 제목에 걸려 **안** 넣는다)과 블로그 플랫폼 `브런치` 차단.
    실측 110건에 스팸·잡음 17/17 탈락 · 라틴 표기 국내 매체(`v.daum.net`·`chosun.com`·`nc.press` 등) 0 탈락.
    ⏸ 알려진 오탐 계급 = 한글 없는 다단어 국내 매체 표시명(「SBS Biz」류, 실측 0회) — 배포 후 로그 감시.
  - ✅ **게스트 뉴스 품질 ②③ 같은 사건 묶기 + 제목 기반 배지 (2026-09-21, 설계 `claude-docs/plans/2026-09-21-guest-news-quality.md`)** —
    운영 25건 중 5건이 한 사건(전산망 예산)이었고 그중 4건이 「베스트셀러」 배지를 달고 나갔다(옛 배지 = 물어온 질의).
    ② 제목 문자 2-gram 자카드 **0.20** 또는 **3자리+ 숫자·단위 토큰**(`645억`) 공유면 같은 사건으로 보고 **최신 1건만**
    남긴다(2-gram 6개 미만인 짧은 제목은 자카드로 안 묶는다). ③ `labelOf(title)` — 대괄호 태그 → 신간 /
    `[베스트셀러]`·「베스트셀러」+순위어 → 베스트셀러 / 나머지 → 출판계. **라벨 값 집합 3종은 유지**라 미니앱·번들·심사 0.
    파이프라인 순서를 「중복 제거 → 질의별 상한」으로 뒤집고(옛 순서는 한 질의 몫이 통째로 1건이 됐다) `TOPICS` → `QUERIES`.
    10건 미만이면 WARN만 남기고 교체는 한다. 운영 스냅샷 재생: 23 → **20건**, 배지 신간 8 · 출판계 11 · 베스트셀러 1.
    자카드 경로에는 **절대 교집합 하한 4개**(`MIN_SHARED_BIGRAMS`)를 AND로 얹는다 — 조사·어미만 겹친 다른 책 소개
    (「[신간] 과학적으로 옳다는 착각」 ↔ 「[신간] 아무렇지도 않다는 착각」, 교집합 3개로 0.200)가 묶이던 실제 반례를 막는다.
    ⚠️ 임계값 0.20·숫자키·교집합 하한은 **2026-09-21 하루치 튜닝값** — 병합 INFO 로그의 묶임 쌍을 1주 보고 조정한다(배포 후 U-5).
  - ✅ **배포 후 운영 실측 (2026-09-22)** — `000b7347` 배포 뒤 운영 `GET /api/public/news`(Origin `apps`): **30건**,
    배지 신간 9 · 출판계 20 · 베스트셀러 1, `Cache-Control: max-age=600`. 도박어 **0건**(09-21 2건) · 라틴 설명형 매체명 0
    (남은 라틴은 `v.daum.net`·`m-i.kr`·`dailian.co.kr` = 호스트형 국내 매체) · 구글 뉴스 링크 외 0 · 14일 초과 0.
    전산망 예산 사건 **5건 → 2건**(남은 둘은 `134억`·`133억`으로 숫자키가 다르고 제목 겹침이 얕다 — 설계가 예고한 한계선),
    「645억」 1건. 그 기사들의 배지도 「베스트셀러」 → 「출판계」로 바뀌었다. 30건이라 얇아짐도 없다.
    남은 관찰(며칠): **U-5** 탈락 매체명 로그에 국내 매체가 없는가 · 묶인 쌍이 정말 같은 사건인가(09-22 응답의 성시경
    일본 아마존 1위 2건은 **안 묶였다**) · **U-6** 한 사건에 3건 이상 남지 않는가 · **U-7** 첫 새로고침 WARN 0.
    ⏸ 「베스트셀러」 배지는 30건 중 1건 — 태그·순위어가 있어야 붙는 규칙이라 배지 3종 중 하나가 화면에서 거의 안 보인다
    (규칙을 넓힐지는 제품 판단, 지금은 안 건드린다).
  - 🔜 **프로모션 「미니앱 구경하고 포인트 받기」 등록 — 보류 중(비즈월렛 충전 대기)** — 상설 기능이라 **기한이 없다**
    (9/21 오픈, 마감 공지 없음). 등록은 콘솔에서만 되고 예산은 비즈월렛에서 나가는데 **최소 충전이 5만 원**이라 잔액 0인
    지금은 걸 수 없다. 충전하면 참여 횟수·기간·예산을 정해 등록한다. ⚠️ **그 전까지 2단계 배선은 판정 불가다** —
    9/21 점검에서 `newAu`가 한 주 내내 **0**이라 새 이벤트(`account_created`·`guest_news_opened`·`guest_entered{variant:'news'}`)가
    카탈로그에 오를 유입 자체가 없었다. 즉 U-A3·U-B4는 「배선 실패」가 아니라 **「유입 없음」으로 대기**다 — 유입을 만드는
    행동이 프로모션 등록이고, 그것이 5만 원에 걸려 있다.
  - ⚠️ 판정식은 프로모션 코호트(`guest_entered{variant:'news'}`) 기준으로 재정의 — 35%는 참고선(설계 §7).
...
| 자정을 넘겨 읽으면 **그 시간이 통째로 시작한 날에 잡힘** — 23:50~00:40에 읽은 50분이 전부 전날 몫이 돼, 다음 날 잔디가 안 켜지고 부채도 안 줄어든다 | 날짜 귀속 규칙이 `startedAt`의 유저 TZ 날짜 하나뿐(`ReadingHistoryService`·`sumCompletedSeconds`·잔디·부채가 전부 이 규칙 유도). 한 행이 자정을 걸쳐도 쪼갤 곳이 없었다 | **저장 시점 분할** — 종료 시각이 확정되는 3경로(`stop`·`closeStaleSessions`·`recordManual`)가 순수 함수 `splitByMidnight`으로 구간을 유저 TZ 자정마다 잘라 조각당 한 행씩 저장. 소비처는 **한 줄도 안 바뀐다**(전부 세션 행에서 유도되므로). 규칙: 클램프(6h) → 분할 순서 · 0초 조각 금지 · 진행 중 세션 불분할 · DST 안전(`atStartOfDay`). 조각 링크는 스키마 없이 **시각 인접성**(앞 endedAt == 뒤 startedAt)이라 종료 후 태깅이 체인을 걸어 앞 조각까지 붙인다. `firstCompletedSession` 판정은 stop **전** `count==0`으로 이동(안 하면 첫 자정 넘김 독서가 축하를 영영 못 받음), 수동 과거 기록 앵커는 정오 역산 → 그날 00:00 순방향(12h 초과가 전날로 새던 버그). **엔티티·스키마·Flyway·프론트 변경 0**, 과거 저장분 소급 마이그레이션 없음(잔디·부채가 소급 변동하면 체감 급변이 이득보다 크다) | ✅ |
...
### ⏸ 깨진 한글 후속 복원 — 원래 글자 확정 뒤 별도 PR (2026-09-25)
- ⏸ **대상**: `app.css:189` 들쑓날롭하게 · `plan.md:3780` 콠 · `changelog.md` 777·779행의 옇·멀잡아·섬였다·읽힐다·안 읽힐을·꿨짐·쟰 · `home.test.tsx:1881` 주석(튀다) · 미확정 반쥴이·쟰. **왜 지금 안 하는가**: 깨진 글자라는 것까지만 보이고 원래 글자는 문맥·같은 PR 본문으로 하나씩 확정해야 한다(반쥴이·쟰은 근거가 아직 없다). 확정된 5곳만 고친 「다른 음절로 깨진 한글 5곳 복원」과 섞지 않고 별도 PR로 한다.

### 웹 다크모드 — 사용자 토글 + 브라우저 강제 다크 차단 (1단계 ✅ · 2단계 ✅)
...
- ✅ **2단계 — raw 색 규칙 토큰화 + 토글 점등 (2026-09-10)**
      `app.css`의 raw 색 **158선언**을 의미 토큰(`--on-accent`·`--surface`·`--paper-1`·`--line-1/2`·
      `--warn-*`·`--danger-*`·`--ok-*`·`--info-*`·`--scrim*` 등)으로 걷어 올리고, `theme.js` 끝에 자동 마운트
      한 줄을 더해 헤더 토글을 켰다. 1단계가 남긴 크림 목록(`.dash-state-panel`·`.dash-nav-tile`·`.set-input`류·
      `.shelf-empty`)은 전부 닫혔고, 실측이 **목록 밖에서 한 자리를 더 찾았다** — PWA 설치 칩(`pwa-install.js`)이
      인라인 스타일로 라이트 세이지를 칠하고 있었다(인라인은 어떤 테마 규칙보다 세다. 코드만 읽어선 안 보인다).
      회귀 가드는 **래칫**이다 — 허용목록(box-shadow · `.is-study` 라이트 원본 · 구글 브랜드 파랑 ·
      `.story-bg-*` 콘텐츠 프리셋) 밖 색 리터럴 **0개**를 CSS와 정적 JS 양쪽에서 못 박고, `--on-accent`는
      존재가 아니라 **대비비를 계산해** AA를 지킨다(다크 accent 위 흰 글자가 2.45였던 그 자리).
      ✅ **「라이트 렌더 보존」 — 사용자 결정에 따라 채웠다 (2026-09-10)**. 처음엔 근접값 통합(설계 §4.5가
      테라코타 두 벌을 합치라고 한 것의 확장)으로 리터럴 46종의 라이트 값이 바뀌었는데, **요청은 다크모드
      도입이지 라이트 재색칠이 아니라서** 전부 되돌렸다 — Δ20~97짜리 12종뿐 아니라 **Δ1~6짜리 34종도**
      되돌린다(「미세하니 괜찮다」를 한 번 허용하면 라이트가 조용히 다른 화면이 된다). 토큰화는 유지하고
      **값이 다른 리터럴마다 제 토큰**을 준다 → 라이트 색 토큰 **38 → 119개**(계열+숫자 접미사. 다크에선
      계열 대표값 하나로 모인다), 공부 모드 토큰 **10 → 15개**. **두 수 모두 `origin/main` → HEAD 기준이다**
      (첫 구현 시점의 60개와 섞어 적지 않는다 — 한 문장에 기준선이 둘이면 어느 쪽도 검증되지 않는다).
      늘어난 토큰이 중복처럼 보여 나중에 합쳐지는
      것이 이 변경의 유일한 회귀 경로라, `:root` 주석에 「근접값 통합 금지」와 근거를 박아 뒀다.
      ⚠️ **`.is-study` 안에서만 갈리던 자리도 같이 되돌렸다** — 공부 카드는 `--accent`·`--ok`·`--sage-soft`를
      파랑으로 갈아끼우는데, 원본은 그 자리들을 세이지 **리터럴**로 적어 둬서 공부 카드 안에서도 초록이었다.
      `.is-study`가 갈아끼우지 않는 **채움 전용 토큰**(`--accent-fill`·`--ok-fill`·`--sage-soft-2`·
      `--accent-hover-2`·`--nav-hover-border-2`)을 따로 둬 라이트는 어디서나 세이지, 다크만 테마를 따른다.
      계측: `origin/main`과 현재 `app.css`를 같은 문서에 각각 실어 **규칙 1,206개(선언 4,686개)**를 원문에서
      뽑아 프로브에 꽂고 `getComputedStyle`로 색 longhand 18종을 대조(라이트 + `.is-study` 두 문맥) →
      **해석된 색 차이 0건**. 양성 대조군 2종(`--paper-1`·`--sage-line`을 각각 1/255만 흔들면 10건·4건이 잡힌다).
      ⚠️ **다만 「픽셀 diff 0」은 엄밀히 참이 아니다 — 독립 리뷰가 합성 픽셀로 다시 쟀다** (합성 픽셀의 **0.12%·2,068px**가 1/255 다르다). 원인은 `color-mix(in srgb, C P%, transparent)`가 크롬에서 부동소수 `color(srgb …)`로 계산돼 8비트 `rgba()`와 **합성 반올림이 갈리는** 것이다(최소 재현으로 확정: `rgba(220,38,38,.12)` → (248,225,220) vs `color-mix 12%` → (247,225,220)). ΔE00 ≈ 0.1이라 지각 불가지만 문구는 사실대로 둔다 — 여기에 「0」이라고 적어 두면 나중에 **다른 원인의 1/255 표류를 이 문장이 덮어 준다**.
- ⏸ **남은 자리 2건(의도적 보류)**
      ① **랜딩(`/`)엔 토글이 없다** — 마운트 지점이 `header.brand, header.auth-brand`인데 랜딩은 `<header>`가
      아니라 `nav.lp-nav`다(설계가 「36/36」이라 적었지만 실측은 랜딩 미포함). 다크 렌더 자체는 정상이고,
      켤 거면 셀렉터에 `.lp-nav-inner` 한 항목을 더하는 일이다. ② **로그인이 필요한 화면은 실 브라우저
      확인을 못 했다**(비밀번호 입력 금지 규칙) — `/books`·`/history`·`/settings` 등은 그 화면의 CSS 클래스를
      실제 `app.css`가 로드된 페이지에 심어 계측했다(크림 0 · 대비 미달 0). 사용자가 로그인해 두면 그 세션에서
      확장 브라우저로 한 번 훑는 것이 남은 확인이다.
...
### 연필 프레임 안쪽 띠 가림 — 3곳 국소 수정 (완료 ✅ 2026-09-11)
> 위 「종이·연필 테마」의 후속. `border-image ... 8 / 8px` + `border-width: 1px`은 남는 7px을 **패딩 박스 안쪽**에
> 그려서, 그 자리에 놓인 불투명 자식·`<img>` 콘텐츠가 연필선을 덮는다(자식은 부모 테두리 **위**에 그려진다).
> SVG 실측상 스트로크는 바깥 가장자리 **0.65~3.1px**에만 살아서, 처방은 「불투명한 것을 4px 띄우거나 프레임을
> 자식 위에 얹거나」 둘뿐이고 세 자리 모두 `app.css` 국소 수정으로 끝난다.
- [x] ① **편집기**(`.study-editor`) — 툴바(`--card-bg` 배경)가 띠에 붙어 있었다 → 부모 `padding-top: 4px` +
      툴바 `margin: 0 4px`. `margin-top`이 아니라 부모 패딩인 이유는 sticky `top: 0` 계산에 확실히 안 끼기 때문.
- [x] ② **세그먼트 칩**(`.shelf-filter-chips`) — active·hover 배경과 셀 구분선이 띠를 가로질렀다. 셀이 가장자리에
      붙는 것이 이 컴포넌트의 모양이라 띄울 수 없어 **`::after` 오버레이**(`inset: -1px`, `pointer-events: none`)로
      프레임을 자식 위에 얹었다. 컨테이너의 `border-image`는 제거(남기면 농도 2배).
- [x] ③ **책 상세 표지**(`.book-detail-head .book-cover`) — `<img>`가 자기 콘텐츠를 자기 테두리 위에 그린다
      → `padding: 4px` + `background-clip: content-box`. 전역 `box-sizing: border-box`라 외곽 84×120 불변.
- [x] **게이트는 정적 테스트가 아니라 렌더 트리 프로브** — `frontend/e2e/pencil-frame.spec.ts`(로컬 수동 e2e).
      `elementsFromPoint`로 「누가 누구 위에 그려지는가」를 재고, **양성·음성 대조군을 스위트에 내장**해
      계측기가 죽으면 스위트가 먼저 실패한다. app.css를 문자열로 파싱하는 vitest는 「선언이 있는가」밖에 못 봐
      구현을 베낀 테스트가 된다(그래서 안 만들었다).
      ⚠️ **1차 프로브는 ② 자리에서 공허했다(독립 리뷰가 돌연변이로 확증, 2026-09-11)** — 프레임 선언을 지우면
      검사 대상이 0개가 되어 초록이고(`borderImageSource !== 'none'`이 첫 관문), 오버레이는 `pointer-events: none`이라
      `elementsFromPoint`에 안 나와 **자식이 그 위로 쌓여 선이 실제로 사라져도** 프로브가 못 본다. 그래서 자리마다
      ⓐ 프레임 존재(①③ `borderImageSource` / ② `::after`의 소스·`inset -1px`·`absolute`·`pointer-events:none`)
      ⓑ ②는 자식 스택 조건(칩 `z-index` auto 또는 ≤ 0)과 **화면 픽셀의 선 대비**(왼쪽 변 1~3px vs 8px 안쪽, 최솟값 > 25)
      ⓒ 스킵 카운터(16px 미만·뷰포트 밖을 세어 「0건」과 「안 재봄」을 가른다)를 단언한다.
- ⏸ **전역(`border-image`를 쓰는 67개 규칙·87개 선택자)은 안 건드린다** — 결함은 3곳인데 전역 오버레이·`border-width` 확대는 회귀가
      81곳이다. 프로브 전수 실행(`/`·`/books`·`/study`·`/u/testid`, 라이트·다크)에서 **다른 자리 0건**을 확인했다.
- [x] **후속 — 그 87개 선택자 중 죽은 9군 + 고아 1군을 삭제 (2026-09-12)**: 위 ⏸는 「살아 있는 규칙을 안 건드린다」였고,
      그 전수 검사가 **어디에서도 렌더되지 않는 선택자 9군**을 덤으로 드러냈다(라이트·다크 × 1280·390, 26개 화면
      상태). `app.css`에서 규칙 **26개**를 통째로 지우고, 살아 있는 규칙 2개에서는 죽은 선택자만 뺐다
...
### 웹 본문 타이포 — 미니앱 「또렷한 연필」의 웹판 (완료 ✅ 2026-09-03)
> 사용자 요청 「웹 보니깐 폰트가 너무 눈아프다. 앱에 있는 폰트 그대로 레퍼런스해서 웹에 적용해.」
> 새 디자인을 발명한 게 아니라 **아래 ③-A(2026-08-24)가 미니앱에서 내린 결론을 웹에 옮긴 것**이다.
- [x] **기능 글자를 개구 → 고운돋움 400으로** — `app.css` `body`의 서체·굵기가 #857(2026-08-18) 이전으로 돌아갔다.
      Gaegu는 `@import`에서도 폴백 스택에서도 **완전히 걷었다**(폴백에 남기면 고운돋움이 못 그리는 글리프만
      손글씨로 떨어져 한 단어 안에서 서체가 갈린다 — 미니앱과 같은 판단). woff2 3웨이트 요청도 함께 사라졌다.
- [x] **`html { font-size: 112.5% }`는 유지** — 사용자가 실측 후 확정. 이제 개구 보정이 아니라 **독립된 배율 결정**이고,
      주석도 그렇게 고쳤다(근거가 사라진 주석은 다음 사람을 속인다 — T-211).
- [x] **강조 축 복구** — body가 700이던 동안 명시 700(55)·600(55)·`<strong>`이 전부 본문과 같은 굵기로 그려져
      강조가 죽어 있었다(미니앱과 같은 병). 400으로 내리자 살아났다 — 실측에서 `<strong>`이 700 대 본문 400.
- [x] **세리프 15 규칙에 `font-weight: 700` 명시** — 고운바탕 규칙 24개 중 굵기 미명시분은 body 700을 물려받아
      우연히 굵던 것이라, 값·제목·이니셜만 700으로 못 박고 **긴 글 9개는 400으로 뒀다**(세리프 문단은 400이 읽기 맞다).
      `.onb-preview-avatar`는 웹에서 **유일하게 상속에 기대던 장식**이라 다른 아바타처럼 세리프를 명시했다.
- [x] **폼 요소까지 마저** — 서체를 옮기고 보니 **사용자가 직접 글자를 치는 칸만 Arial로 남아 있었다**
      (`/search` 검색칸 실측 `Arial 17.1px`). 상속 누락이 아니라 **UA 기본 스타일이 `input/textarea/select`에
      자기 폰트를 직접 걸어, author 규칙이 없으면 그 값이 이기는** 구조다. 전역 한 줄
      `input, textarea, select { font-family: inherit; }`로 되돌렸다 — 미니앱엔 TDS가 같은 규칙을 줘 이 구멍이 없었다.
      ⚠️ **타입 선택자(0-0-1)로 둔 것이 요점**이다: `.auth-input`·`.set-input`, 세리프를 의도적으로 쓰는
      `.story-composer-input`을 덮지 않는다. `font-size`는 일부러 안 옮겼다(칸 높이가 함께 바뀐다).
- [x] **회귀 가드** `AppStylesheetTypographyTest`(4건) — Gaegu 복귀 · 폴백 밀림 · 700 복귀 · 폼 요소 상속 구멍을
      각각 잡는다. css 주석을 걷고 단언한다(주석이 값을 인용하면 소스 단언이 공허하게 통과한다 — T-205).
- ⚠️ **웹의 장식 축은 손글씨가 아니라 고운바탕이다** — 전수 조사 결과 웹은 손글씨 시절에도 표지·아바타 이니셜,
      여백 글·인용, 명언 각주를 전부 세리프로 **명시**해 뒀다. 그래서 미니앱의 `HANDWRITING` opt-in에 대응하는
      장치를 웹엔 만들지 않았다(남길 자리가 0이었다). 손글씨 웹폰트는 이제 미니앱 전용이다.

### 웹 데스크톱 폭 확장 — 홈 760 → 1160px + 긴 책 제목 (완료 ✅ 2026-09-07)
> 사용자 요청 「데스크톱에서 UI의 모양을 더 넓게 사용하자. 긴 책의 이름이 짤려.」
> 전제로 방침이 하나 정해졌다 — **웹의 주 사용처를 데스크톱·태블릿으로 두고 모바일은 미니앱으로 유도한다.**
> 그래서 모바일-우선 460px 기준을 데스크톱 쪽으로 과감히 풀 수 있었다.
- [x] **홈 컨테이너 760 → 1160px** — 1440px 창에서 화면의 40%만 쓰던 자리. `.container`는 `width:100%`라
      좁은 화면은 그대로 뷰포트에 맞으므로 기존 `min-width:640px` 경계는 손대지 않았다.
- [x] **잔디 1년치가 가로 스크롤 없이 들어온다** — 53주 × 13px + gap = 845px인데 760px 컨테이너의 안쪽은
      708px이라 137px이 잘려 있었다(실측: 스크롤바 노출 → 지금은 `scrollWidth == clientWidth`).
- [x] **긴 책 제목** — `.dash-book-chip-title` 한 줄 ellipsis → **2줄 클램프 + `:title`(호버 툴팁)**.
      실측 최장 제목 57자가 **9자**(「2026 이기적 …」)에서 **약 45자**로. 2줄로도 넘치는 초장문은 툴팁이 받는다.
- [x] **넓어진 카드에 안의 요소 크기도 맞춤(`@media min-width:1100px`)** — 타이머 숫자 76→96px, 잔디 칸 13→16px,
      진행바 좌열 전폭, 빠른 이동 3열. 경계 1100px의 근거는 「16px 칸 잔디가 스크롤 없이 들어가는 최소 폭」이다
      (그보다 좁으면 칸만 커지고 스크롤이 되레 길어진다).
- [x] **`/study` 셸에 `wide-page`(1160px)** — 달력 한 칸이 60px이고 긴 책 제목이 3줄로 접히던 자리.
      **폼은 620px로 묶었다** — 입력 한 칸이 1100px이면 되레 쓰기 나쁘다.
- ❌ **`/search`(사용자 검색)는 넓히지 않았다** — 폭만 올려 실측해 보니 **손해**였다. 폼과 한 줄짜리 결과가
      전부라 넓히면 「팔로우」 버튼만 저 멀리 떨어진다. 리스트를 그리드로 재배치하지 않는 한 넓힐 이유가 없다.
      **넓혀서 손해인 화면은 넓히지 않는다**가 이번 판단 기준이다(폭은 목적이 아니라 수단).
- [x] **「모바일은 미니앱으로」 유도 UI** ✅ 2026-09-07 — 아래 절에서 이어 했다(새 배너를 만들지 않고
      기존 PWA 설치 칩을 플랫폼으로 갈랐다).

### 모바일 → 미니앱 유도 — PWA 설치 칩을 플랫폼으로 가른다 (완료 ✅ 2026-09-07)
> 사용자 질문 「PWA 홈 화면에 추가를 토스 미니앱으로 연동할 수 있어?」에서 출발했다.
> **직접 연동은 불가능하다** — manifest의 `start_url`은 같은 오리진의 https여야 해서 `intoss://`를 넣을 수
> 없고(넣으면 manifest가 통째로 무효), 토스 SDK에도 홈 화면 추가 API가 없다(26개 전수 확인, 0건).
> 그래서 「무엇을 설치하느냐」가 아니라 **「어느 플랫폼에 무엇을 띄우느냐」**로 풀었다.
- [x] **새 배너를 만들지 않았다** — 모바일에서 PWA 칩과 미니앱 배너는 같은 말을 두 번 하는 것이라
      어차피 겹쳤다. `pwa-install.js`의 기존 분기·7일 침묵·닫기 기억을 그대로 재사용한다.
- [x] **데스크톱은 PWA 설치 칩 그대로** — 바탕화면 바로가기는 「웹 = 데스크톱」 방침에서 버릴 것이 아니라
      **남는 자산**이다. 사용자가 「지워지는 건 감안하겠다」고 했지만 감안할 손실이 아니었다.
- [x] **모바일(iOS·Android)은 「토스 앱에서 이어보기」** — `detectPlatform`의 판별 기준을 「PWA를 설치할 수
      있는가」에서 「모바일인가」로 바꿨다. 딥링크는 beforeinstallprompt와 무관하므로 iOS Chrome·Android
      Firefox도 이제 유도 대상이다(옛 판별은 그 둘을 `unsupported`로 버렸다).
- [x] **⚠️ 계정이 갈리는 사고를 막는 게 이 기능의 핵심이다** — 로그인 사용자를 딥링크로 바로 보내면
      미니앱에서 토스로 로그인해 **별개 계정이 생기고**, `toss_user_key`는 once-set 불변이라 되돌릴 수 없다.
      그래서 로그인 상태에서는 연결 코드를 먼저 받는 **설정 카드(`/settings#toss-link`)로** 보낸다.
      비로그인은 계정 자체가 없어 갈릴 것이 없으므로 딥링크로 바로 간다.
- [x] **fail-safe 방향을 뒤집었다** — 로그인 여부는 `<meta name="bt-auth">` 한 줄로 읽는데(전역 모델 속성도
      추가 DB 조회도 없다), **「anon」이라고 명시됐을 때만** 비로그인으로 본다. 두 오판의 대가가 대칭이
      아니기 때문이다 — 로그인을 비로그인으로 읽으면 계정이 갈리고(되돌릴 수 없다), 그 반대는 로그인
      화면을 한 번 더 볼 뿐이다(되돌릴 수 있다).
- [x] **설정 카드에 「휴대폰에서 토스 앱 열기」 추가** — 지금까지 코드만 주고 「토스 앱에 입력하세요」로
      끝나 **정작 토스 앱으로 갈 길이 없었다**. 유도와 무관하게 빠져 있던 조각이다.
- [x] **iOS 수동 설치 안내 오버레이(`_showIosOverlay`)를 걷었다** — 「공유 → 홈 화면에 추가 → 추가」 3단
      안내가 설 자리가 없어졌다. 덤으로 화면 하단을 덮던 시트가 사라졌다(웹에도 T-183의 「덮지 않는다」 적용).
- 📍 **검증**: 순수 함수 TDD(RED 6건 → 구현 → 새 fail-safe RED 4건 → GREEN, 프론트 651건) + 로컬 실측
      5경로(비로그인 meta=anon / 로그인 meta=user / 모바일 칩 문구 / 로그인 클릭이 딥링크가 **아닌**
      설정으로 / 코드 발급 후 딥링크 노출 — 마지막이 양성 대조군이다).

### 웹 페이지 이동 → 양옆 세로 바 (완료 ✅ 2026-09-15)
> 사용자 요청 — 데스크톱·태블릿 웹에서 페이지 이동 버튼을 화면 양옆으로. 시안 `claude-docs/plans/2026-09-15-web-desktop-layout/`,
> 설계 `claude-docs/plans/2026-09-15-web-side-rails.md`(작업용 임시 산출물).
- [x] **왼쪽 = 독서(내 책장·내 책방·책BTI·독서 기록·사용자 검색), 오른쪽 = 공부(일정·백지노트·공부 서재·공부 기록)** —
      로그인 사용자 화면 14개. 평소 92px(아이콘 위·짧은 이름 아래), 펼치면 232px로 본문 위에 덮는다.
      ⤷ **낡음 — 아래 「바 하나로 합침」(2026-09-17)이 대체했다.** 좌우 두 바는 없어졌고 메뉴 목록·폭·펼침은 그대로다.
- [x] **바 하나로 합침 + 「독서 \| 공부」 스위치를 바 있는 모든 화면에** ✅ 2026-09-17 — 설계 `claude-docs/plans/2026-09-17-single-rail-mode-switch.md`.
      왼쪽 바 하나 안에 독서·공부 메뉴를 두고 `#side-rails[data-mode]`가 가리키는 쪽만 보인다(속성 부재 = 독서). 경로가 정하는
      페이지는 서버가 모드를 쓰고, **홈·중립 화면(설정·피드백·차단·수동 기록·남의 책방)은 서버가 비우고 인라인 부트가 저장값으로
      채운다** — 바가 하나라 중립 화면이 경로대로 reading이면 공부 모드 사용자의 메뉴가 설정에서 뒤집힌다. 독서등은 홈 전용.
      스위치는 홈이 Vue `ModeToggle`, 비홈이 fragment의 SSR 알약(같은 클래스·aria — 리터럴 동기 테스트)이다. 비홈 알약은 누르면
      그 모드를 저장하고 홈으로 가며 지금 모드 버튼은 무동작이다. 비홈은 측정 잠금을 모르므로 홈이 서버 진실로 열고, 저장값과
      다르게 열렸으면 로드 때 「측정을 끝내면 바꿀 수 있어요」를 한 번 띄운다. 좌우 여백 124는 대칭 유지(1100 경계·중앙 정렬),
      PWA 칩의 오른쪽 바 우회는 삭제, 알약 밑 하단 여백은 홈 전용에서 바 있는 화면 전부로 넓혔다.
      ⏸ 스크롤 중 알약이 콘텐츠 위에 겹치는 것(고정 요소의 본성)·홈과 비홈의 Tab 위치 한 칸 차이(Teleport 안 함)는 범위 밖.
- [x] **홈은 바에 없다 — 로고(`a.brand-home`)가 홈이다** ✅ 2026-09-16 — 모드 공용 목적지를 모드별 바 한쪽에만
      두던 비대칭을 없앴다(홈에서는 로고가 `aria-current="page"`). 덤으로 태블릿에서 홈이 2탭 → 1탭이 된다.
- [x] **공부 쪽을 들렀으면 홈이 공부 타이머로 열린다** ✅ 2026-09-16 — 활성 키가 있는 **공부 바** 페이지가
      `#side-rails[data-remember="study"]`를 내보내고 `rail.js`가 `booktimer.timerMode`에 적는다. 중립 화면
      (설정·피드백·차단·수동 기록·남의 책방)은 활성 키가 없어 **안 건드린다** — 무조건 쓰면 경로상 reading이라
      「공부 모드 → 설정 → 로고 → 독서 타이머」로 튄다. 진행 중 세션이 있으면 서버 진실(`effectiveMode`)이 이긴다.
      ⤷ **독서 쪽은 기억하지 않는다**(사용자 결정 2026-09-16 — 초판의 대칭 규칙에서 개정). 대칭이면 「공부 측정 중
      → 내 책장 → 로고」가 독서 타이머로 열려 측정하던 자리로 못 돌아왔다. 독서로 돌리는 주인은 홈의 토글 하나다.
- [x] **판별은 입력 장치** — 마우스는 0.15초 머물면 펼치고 0.1초 뒤 접는다(스침은 안 펼친다), 태블릿은 접힌 바 첫 탭은
      펼치기만·펼친 바 탭은 이동·바깥 탭은 접힘. 지연은 CSS가 아니라 `rail.js` 타이머다 — CSS hover로 펼치면 안의 가로 배치가
      즉시 뒤집혀 스치기만 해도 글자가 깨져 보인다.
- [x] **지금 모드 반대쪽 바는 흐리게**(펼치면 진하게). 경로가 곧 모드이고, 홈만 저장 모드를 인라인 부트로 첫 페인트 전에 반영한 뒤
      Vue가 토글마다 같은 속성(`#side-rails[data-mode]`)을 쓴다.
      ⤷ **낡음(2026-09-17)** — 흐림은 바가 하나가 되며 걷었다. 반대쪽 메뉴는 숨기고, 부트는 홈에 더해 중립 화면에서도 돈다(위 「바 하나로 합침」).
- [x] **SSR fragment 하나 + 정적 JS 하나**(`fragments/side-rails`·`static/js/rail.js`) — 활성·내 책방 링크는 `RailModelAdvice`가
      싣는다(사용자 해석은 지연 — REST에 DB 조회를 붙이지 않는다). 온보딩 전(login_id 없음)·ADMIN은 「내 책방」을 그리지 않는다.
- [x] **걷은 것** — 홈 「빠른 이동」 카드(QuickNav), Vue 섬 11곳과 SSR 4곳의 하단 이동 타일. 위 「웹 데스크톱 폭 확장」의
      「빠른 이동 3열」과 공부 웹 절의 「QuickNav 공부 세트·하단 네비」는 이 작업으로 사라진 옛 기록이다.
      바 없는 화면(에러·약관·비밀번호 찾기·관리자)의 하단 타일은 그대로다.
- [x] **웹 본문 폭 재조정** ✅ 2026-09-15 — 바 있는 화면은 기본 1160(`body.has-rails .container`), 목록·폼·글 한 열 화면 8개만
      `.narrow-page` 720, 기록 split은 기존 940. 페이지별 폭 규칙 5곳을 걷고 `ContainerWidthTest`가 허용 목록으로 막는다.
      2열 경계(책장 880·책방 860·기록 880)는 바 여백 248을 뺀 **1100 하나**로 — 1024 태블릿 가로는 전부 1열. 바 없는 화면은 460 그대로.
- [x] **「본문으로 건너뛰기」 스킵 링크** ✅ 2026-09-16 — 바가 본문보다 DOM 앞이라 로고(= 유일한 홈 링크)에 닿기까지
      키보드로 Tab 10번이었다(바 링크 9개를 지나서). fragment 맨 앞(바보다 먼저)에 링크 하나를 두고 바 쓰는 페이지 14개의
      본문 컨테이너에 `id="main-content" tabindex="-1"`을 달아 **Tab 1번 + Enter + Tab 1번**으로 줄였다.
      평소엔 화면 밖(`.skip-link`)이고 포커스를 받을 때만 내려온다 — `.sr-only`는 포커스해도 안 보여 쓸 수 없다.
      바 순서를 바꾸는 안은 배치 코드 회귀 위험이 커서 기각했다.
- ⬜ (조건부) **640px 미만 폰 웹의 이동 수단** — 지금은 바를 숨기고 브랜드 홈 링크만 남는다(웹 = 데스크톱·태블릿 방침).
      폰 웹을 지원하기로 바뀌면 하단 탭바를 단다.

### 홈 잔디 철거 — 그 자리를 「오늘 쓰는 것」이 잇는다 (완료 ✅ 2026-09-07)
> 사용자 요청 「메인 페이지에서 잔디가 안 보였으면 좋겠어. 가로 넓이를 늘렸더니 잔디가 같이 늘어나고
> 좀 지저분해 보여. 잔디가 있던 부분에는 여백쓰기, 공부 페이지면 백지복습 작성부분으로.」
> 위 폭 확장의 부작용을 **되돌리는** 것이 아니라 그 자리의 **용도를 바꾸는** 작업이다 — 기록은 이미
> `/history`·`/study/history`에 그대로 있고, 홈은 「오늘 쓰는 자리」가 된다. 시안 4장을 먼저 보이고
> **독서 B(내가 쓴 여백 미리보기) + 공부 A2(백지복습 2열)**로 골랐다.
- [x] **잔디를 걷을 때 컴포넌트·테스트·CSS까지 함께 지웠다** — 대시보드 전용 `ContributionGraph.vue`,
      그 테스트 2파일(10건), 잔디 CSS 22줄. 렌더만 빼고 남기면 **아무도 안 그리는 화면을 지키는 테스트**가
      남는다. `/history`·`/study/history`의 잔디(`history/ContributionGraph.vue`)는 별개 파일이라 무관하다.
- [x] **독서 = 「지금 그 책」의 여백 카드** — 최근 2건 미리보기(넓으면 2열) + 「《책》에 여백 남기기」가
      기존 작성 모달을 그대로 연다. 어느 책인지는 타이머 칩과 **같은 함수**(`defaultBookOf`)가 고른다.
- ⚠️ **시안의 「내가 쓴 글 전부」는 못 만들었다** — 여백 조회가 `loginId + bookId` **두 축 필수**라
      사람축으로 내 글을 모으는 경로가 없다(새 엔드포인트가 필요하다). 지금 읽는 책 하나로 좁혔고,
      **보이는 것은 시안과 같지만 범위가 다르다.** 사람축 미리보기를 원하면 그때 API부터 만든다.
- [x] **공부 = 백지복습 카드** — `/study`의 패널을 그대로 싣고 오늘치(오늘 날짜·오늘 일정·남은 몫·어제
      문제 여부)만 먹인다. 새로 그리는 UI가 0이고, 공부 서재는 부모가 이미 들고 있어 다시 안 부른다.
- [x] **넓은 화면(≥1100px) 2열** — 본문 758px 왼쪽 · 책·과목·범위 330px 오른쪽. 1열로 두면 한 줄짜리
      「과목」 입력이 1100px로 늘어나 **잔디를 걷어낸 이유를 그대로 되부른다.** 패널을 두 묶음으로
      가르되 `/study`에선 `display:contents`로 래퍼가 사라져 **옛 세로 배치가 그대로다**(실측: 간격 8px
      동일, 과목·범위가 본문 앞). CSS만으로는 못 한다 — grid는 DOM 순서를 따라가므로 묶음이 필요했다.
- [x] **왕복 — 공부는 1↔1, 독서는 +1** (리뷰 실측으로 문안 정정) — 공부 잔디가 쓰던 `/api/study/history`
      1건이 사라지고 `/api/study/agenda` 1건이 들어왔다. **독서 모드는 여백 조회 1건이 새로 는다** — 카드가
      글을 보여 주니 당연한데, 처음 「왕복 수는 그대로」라고 뭉뚱그려 적은 것이 틀렸다. 덤으로 **모드 토글
      캐시가 사라졌다**(옛 공부 잔디는 페이지 수명 캐시였는데 `v-if`/`v-else`가 카드를 언마운트한다) —
      홈에서 모드를 오가는 것이 잦은 동작이 아니라 복원하지 않았다.
- [x] **연속일 칩을 여백 카드 머리로 옮겼다** — 처음엔 「타이머 카드가 이미 들고 있으니 안 사라진다」고
      판단했는데, 구현 중 실측하니 그 「연속 N일째」는 **목표 달성 패널 전용**이라 평소엔 안 보인다.
      그대로 갔으면 홈에서 연속일이 통째로 사라졌다. 공부 모드는 agenda에 streak이 없어 안 띄운다.
- [x] **2026-08-16 §D5-1을 뒤집는다** — 「대시보드에 여백 대체 진입을 만들지 않는다」. 그때는 타임라인
      스트립을 없애는 맥락이었고 지금은 잔디가 비운 자리를 **책 한 권 단위로** 채우는 것이라 맥락이
      다르다(진입은 여전히 책 한 권이다). 코드 주석도 함께 갱신해 뒤집은 기록을 남겼다.
- 📍 **검증**: TDD RED → GREEN — 신설 3파일 **28건**(여백 카드 13 · 백지복습 카드 9 · `defaultBookOf` 6)
      + RecallPanel 두 묶음 2 + `dayTitle` 2, **프론트 전체 673건**. 잔디 제거는 「없다」와 「그 자리에
      여백 카드가 있다」를 **한 테스트에 묶어** 쟀다(부정 단언만 두면 컴포넌트를 통째로 안 그려도 통과).
      로컬 실측: 홈 1160px 여백 2열(547×2, 3건 중 2건만) · 백지복습 2열(758/330) · 420px에선 둘 다 세로 ·
      가로 스크롤 0 · `/study` 무회귀.
- 🔁 **머지 전 독립 리뷰(밝은선 ④ ±150줄)가 결함 2건을 잡았다 — 둘 다 돌연변이가 살아남은 자리였다.**
  - **`MarginCard`가 fetch 자체의 거부를 안 잡았다**(오프라인·DNS) — `fetchMargin`은 HTTP not-ok만 null로
    수렴시켜서, 예외가 나면 `failed`가 false인 채 **「불러오는 중」에 영구 정지**했다. 실패 상태를 만들어
    두고 **가장 흔한 실패 경로가 거기 안 닿았다.**
  - **책 A→B 전환에서 A의 늦은 응답이 B의 화면을 덮었다** — 「바꿀 때 비운다」로는 못 막는다(늦게 온 A가
    그 빈 칸을 다시 채운다). 세대 카운터로 버린다.
  - 곁다리: 홈 백지복습이 **기기 달과 서버 오늘이 갈리는 날**(시차 월말·월초) 오늘 일정을 못 찾아 프리필이
    조용히 비던 것을 1회 재조회로 막았다. 픽스처 날짜가 **실제 오늘과 같아** 「서버 today를 쓰는가」 단언의
    판별력이 0이던 것도 못 박았다(기기 시계 구현으로 바꿔도 초록이었다).
  - 소비자가 사라진 `cellTone`과 그 테스트 7건도 함께 걷었다 — **이 PR이 스스로 세운 원칙이 정작 여기엔
    안 적용돼 있었다.** 긴 제목·긴 글 상한도 뒀다(제목 1줄 · 미리보기 4줄 · 부가입력 460px).
  - **최종 670건**(cellTone 7건 삭제 + 리뷰 대응 4건 신설).
- ⏸ **안 한 것** — ⑴ 홈 번들이 +17KB다(`RecallPanel`이 독서 모드에도 실린다). vite 설정이
  `inlineDynamicImports: true`라 `defineAsyncComponent`로도 **번들이 안 쪼개진다** — 지연 로드는 다운로드가
  아니라 평가만 미룬다. ⑵ `/api/dashboard`가 여전히 1년치 graph를 통째로 보내는데 이제 쓰는 것은
  `currentStreak` 하나다(서버 변경이라 이 작업 범위 밖 — 후속 후보).

### 공부 「과목」 → 「주제」 — 자격증 밖으로 (완료 ✅ 2026-09-07)
> 사용자 고민 「과목으로 두면 이 공부 타이머는 자격증에만 쓸 수 있는 거잖아」.
- ❌ **「책 제목」으로 바꾸는 안은 기각** — ⑴ 바로 위에 책 고르는 select가 이미 있어 진짜 중복이 되고,
      ⑵ 책 없이 하는 공부(인강·학교 수업·필기노트)를 적을 칸이 사라진다.
- 📌 **시스템은 이미 「과목 또는 책 제목」으로 다루고 있었다** — `StudyPlanItem.subject` 주석이 「제목
      스냅샷 … 자유 제목으로 짜는 일정도 정당한 사용」이라 못 박고, 상한 주석도 「과목·책 제목 상한」이며,
      일정 화면·AI 일정 폼은 책을 고르면 제목으로 자동 채운다. **혼자 「과목」이라 말하던 건 UI 문구뿐.**
- [x] **라벨 「주제」** — 자격증·학교·취미가 다 들어가고 책 제목을 넣어도 어색하지 않다.
- [x] **예시를 넓혔다** — 「예: 정보처리기사 실기」 → 「예: 미적분 · 정보처리기사 실기」.
      「자격증 앱」이라는 인상의 대부분은 라벨이 아니라 **괄호 안 예시**에서 온다.
- [x] **백지복습에 빠져 있던 「책 고르면 주제 채우기」를 메웠다** — 다른 두 폼엔 있는데 여기만 없어서,
      책을 골라 놓고도 같은 값을 다시 쳐야 했고 **그 순간 「주제인가 책 제목인가」 고민이 생겼다.**
- ⚠️ **경계**: 저장된 글을 불러오면 `bookId`도 함께 세팅된다 — 그때 watch가 그대로 돌면 **적어 둔 주제가
      책 제목으로 덮인다**(글은 그대로라 눈치채기 어렵다). `hydrating` 빗장 + `flush: 'sync'`로 막았다.
      기본 flush는 다음 틱이라 빗장이 풀린 뒤에 돌아 무용지물이다.
- [x] **AI 프롬프트의 `[과목]` 라벨도 함께** — 모델이 읽는 말이라 넓게 이해되는 편이 낫다.
- ⏸ **일정 생성기의 「수험 일정 보조」 페르소나는 그대로** — 그 폼은 시험일이 필수라 실제로 수험 도구다.
- 📌 **DB 컬럼명 `subject`는 그대로**(마이그레이션 0) — 바뀐 것은 사람이 읽는 말뿐이다.
- 📍 RED 4건 → GREEN, 프론트 679건 + 서버 전체 통과.

### 홈 백지복습 카드의 중복 링크 제거 (완료 ✅ 2026-09-07)
> 사용자 지적 「우 상단 공부 기록 버튼은 없애. 바로 밑에 있는데 저기에 또 있을 이유가 없어.」
- [x] **카드 머리의 「공부 기록 →」를 걷었다** — 바로 아래 「빠른 이동」에 같은 곳으로 가는 타일이 있다.
      잔디를 걷을 때 그 카드의 기록 진입점을 **자리째 물려받은 것**이 원인이다. 잔디 시절엔 둘이 화면상
      멀찍이 떨어져 있었는데, 카드가 짧아지면서 붙어 버려 중복이 드러났다.
- ⚠️ **독서 카드의 「전체 기록 →」는 남긴다** — 독서 쪽 빠른 이동엔 기록 타일이 없어 그 링크가 홈에서
      `/history`로 가는 **유일한 길**이다. 두 카드를 대칭으로 맞추려다 길을 끊는 것이 더 나쁘다.
- 📍 계측기는 「카드엔 없다」와 **「타일에는 있다」**를 한 쌍으로 뒀다 — 부정 단언만 두면 두 자리 모두에서
      사라져도 초록이다. 프론트 677건.

### 홈 책 칩에 실제 표지 (완료 ✅ 2026-09-07)
> 사용자 지적 「앱은 이 책 부분이 실제 책으로 렌더링되는데 웹도 가능해?」.
> **새로 가져올 데이터가 없었다** — 서버는 `BookOption.coverUrl`을 처음부터 보내고 있었고,
> 웹 타입이 그 필드를 빼 놔서 화면이 버리고 있었다.
- [x] **`types.ts`의 `BookOption`에 `coverUrl` 추가** — 서버가 보내던 것을 타입에 적기만 했다.
      옛 픽스처엔 없으므로 optional이다.
- [x] **두 칩을 「표지 있으면 `<img>`, 없으면 색 박스」로** — `BookPickForm`(독서)·`StudyTimerCard`(공부).
      공부 쪽은 `StudyBookRow.coverUrl`이 **타입에도 이미 있었는데** 화면이 안 쓰고 있었다.
- [x] **폴백을 반드시 남긴다** — 표지 없는 책(직접 추가·알라딘 이미지 없음)이 실제로 있다. 계측기도
      「img가 뜬다」와 「없으면 색 박스」를 **한 쌍**으로 뒀다(앞엣것만 재면 폴백을 지워도 초록이다).
- [x] **`hasCover` 구현은 한 곳** — `profile/format.ts`를 `books/pure`에서 재수출한다. 두 벌이 되면
      「공백만 있는 문자열은 표지인가」가 화면마다 갈린다.
- [x] **책 시트의 실패 폴백도 표지를 잃지 않는다** — `/api/books` 조회가 실패했을 때 쓰는 경로가
      `coverUrl: null`로 떨어뜨리고 있었다(정상 경로는 원래도 실표지였다).
- 📍 **위험이 낮았던 이유**: 웹은 이미 내 책장·내 책방·책BTI에서 실표지를 띄우고 있고 CSP도
      `img-src 'self' data: https:`로 열려 있다(그 줄 주석이 「표지가 알라딘 등 외부에서 온다」고 명시).
      **새로 뚫을 구멍이 0이라 코드 34줄로 끝났다.**
- 📍 **검증**: RED 2건 → GREEN, 프론트 676건. 로컬 실측 — 두 모드 모두 `img` 렌더 + 로드 성공
      (`naturalWidth 512`) + 박스 30×42 + `object-fit: cover`, 표지 없는 책은 `span` 폴백 유지.

### 일정 생성을 Gemini Flash로 — 제일 비싼 것 하나만 (2026-09-08)
> 사용자 고민 「AI 연결을 하자니 경제적으로 힘들고 수익화 방안도 마땅치 않다 — 사용자가 자기 AI에
> 넘길 프롬프트를 받아 가는 방식으로 바꾸면 어떤가」에서 출발해, 조사 끝에 **범위를 좁혀 착지**했다.
- 📌 **먼저 잰 것이 방향을 정했다**: 미니앱 광고 수익 **최근 한 달 277원**(노출 59회·MAU 40).
      「수익화 방안이 없다」가 아니라 **볼 사람이 40명**이라는 뜻이다 — AI를 걷어도 277원은 그대로다.
      비용 문제와 수익 문제는 같이 왔을 뿐 한 번에 안 풀린다.
- 📌 운영 Claude 키는 **2026-09-03 생성**(SSM 버전 1, 이후 변경 없음) — 나흘짜리다. 승인제 + 하루 상한
      + MAU 40이라 지금 총액은 작다. 그런데 **회당 153원**(위 원가 항목)이라 승인제를 푸는 날 터진다.
- ❌ **「프롬프트를 사용자에게 넘기기」는 기각** — 출력이 앱으로 **돌아와야** 하는 기능(분석·일정)은
      붙여넣기 파싱이라는 새 실패 지점을 만든다. 전사만 성질이 맞는데, 전사는 제일 싼 축이 아니다.
- ❌ **BYO API 키도 기각** — 40 MAU의 연령 분포(40대 이상 45%)에 키 발급을 시킬 수는 없다.
- ⏸ **A안(같은 Anthropic 안에서 Haiku로)은 보류** — 코드 3줄·처리방침 0줄로 낙차의 대부분을 가져가지만,
      사용자가 B안을 골랐다. 되돌릴 일이 생기면 여기서 시작한다.
- [x] **`GeminiStudyPlanner` 신설** — 일정 능력을 통째로 이사(프롬프트·`planUserPrompt`·`sanitizePlan`·
      형 3종) + Gemini 배관(`buildRequestBody`·`parsePlan`·`buildEndpoint`). `ClaudeStudyAssistant`는
      분석·전사만 남는다.
- [x] **`StudyAi` 신설** — `AiResult`·`Failure`를 공급자 중립 자리로. Gemini 어댑터가
      `ClaudeStudyAssistant.AiResult`를 돌려주는 모양은 읽는 사람을 속인다. import 형태는 그대로다.
- 📌 **방어선이 공짜였다** — `sanitizePlan`이 애초에 「모델을 믿지 않는다」로 짜여 공급자와 무관했다.
      규칙째 이사했고 **그 테스트 15건이 초록인 것이 이사 회귀 게이트**다.
- ⚠️ **Claude와 갈리는 한 곳이 이 작업의 핵심 위험** — 거기선 SDK 구조화 출력이 스키마를 보장하고
      `stopReason`으로 잘린 응답까지 걸러 줬다. Gemini는 봉투를 우리가 연다 →
      **`finishReason != "STOP"`을 안 보면 반쪽 일정이 달력에 그대로 들어가고 화면은 멀쩡하다.**
      돌연변이로 그 검사를 죽여 **정확히 그 2건만** 빨간불이 되는 것을 확인했다.
- [x] **로그 위생** — 키가 URL 쿼리(`?key=`)에 실리므로 catch에서 `e.toString()`을 안 찍는다(예외 클래스명
      + HTTP 상태만). 성향 서술(narrator)보다 좁게 갔다.
- [x] **RestClient를 narrator와 공유하지 않는다** — 거기 읽기 타임아웃 20초는 stale 캐시 폴백을 빠르게
      하려는 의도적 설계인데, 90항목 일정은 그 안에 안 끝난다(90초로 별도).
- [x] **처리방침 4개 지점** — 국외이전 **수령자가 Anthropic→Google로 바뀐다**. 6-1에서 일정을 걷고,
      6-2에 이전 항목(사용자가 친 **주제·범위 자유 텍스트**)·목적·보유기간·거부방법을 다시 적었다.
      6-2는 원래 「집계 수치」만 보낸다고 명시하던 절이라 **카테고리가 바뀐 것**이다.
- 📌 **SSM 신설 0** — `LLM_API_KEY`가 2026-06-07부터 이미 있고 `render-env.sh`가 이미 매핑한다.
- ⚠️ **머지 전 사용자 판단이 필요한 것**: 처리방침 13절이 「이용자에게 불리하게 바뀌는 중요한 변경은
      **시행일 30일 전부터** 공지」라고 스스로 약속한다. 국외이전 수령자 변경이 여기 걸리는지는
      제품 결정이라 보고만 한다.
- 📍 **리뷰 반영 (`reviewer` 독립 리뷰, 2026-09-08)** — Critical 0. 리뷰어가 돌연변이 **31종**을 4배치로
      주입해 「예상 FAIL = 실제 FAIL」을 전수 확인했다(**공허한 단언 0건**). 고친 것:
  - ⚠️ **토큰 눈금이 사라져 있었다(I-1)** — Claude 어댑터는 `in=`/`out=`를 찍었고 **plan.md의 「회당
        153원 · 출력 10,574토큰」이 바로 그 로그에서 나온 숫자**다. 비용 때문에 옮겨 놓고 눈금을 뗀
        꼴이라 `usageOf()`로 복원했다(Gemini 봉투의 `usageMetadata`는 이미 오고 있었다). 테스트 2건 추가.
  - **Gemini의 400을 `BAD_INPUT`으로 접던 것을 `UNAVAILABLE`로(M-3)** — Gemini에서 400은 사용자 입력이
        아니라 **우리 쪽 오설정**(plan-model 교체 등)에서 난다. 그대로 두면 화면이 「이 범위로는 일정을
        만들 수 없어요」라며 **사용자에게 범위를 고치라 하고**, 사용자는 헛되이 재시도한다.
  - **런북이 어긋나 있었다(I-2)** — `deploy-aws.md` §12-3·§12-4가 「일정은 Claude 키」라고 적고 있어,
        일정 장애 때 운영자가 엉뚱한 키를 본다. 두 절을 고치고 **모델 프로퍼티가 둘로 갈린 것**
        (`BOOKTIMER_LLM_MODEL`로는 일정이 안 바뀐다)과 **쿼터 공유**를 명시했다.
  - 주석 정정 3건 — `finishReason` 화이트리스트 근거(M-1) · 로그 위생 전제(M-2) · `MAX_PLAN_ITEMS`
        상한 근거가 Sonnet 시절 값이라 무효임(M-5) · 클래스 javadoc의 「공급자 교체 계획 없음」(M-6).
  - **내 보고 숫자가 틀렸다(M-4)** — 「이사분 13건」은 실제 **15건**(planUserPrompt 4 + sanitizePlan 10 +
        generatePlan_whenDisabled 1)이었다. 「보고 감사」 규칙대로 재측정해 세 문서에서 고쳤다.
- ✅ **닫힘 (2026-09-09) — 운영 Gemini 키의 요금 티어 확인 (I-3)**. 실측 결과 **무료 티어였고**, 같은 날
      **유료로 전환**했다(25,000원 선충전 · 자동 충전 off). 유료 티어는 전송 내용을 학습에 쓰지 않고 정책
      위반 탐지 목적으로만 제한 기간 보관하므로, 처리방침 6-2의 보유·이용 기간을 그 사실로 고쳐 적고
      **무료 티어였던 기간에 대한 안내 한 줄**을 함께 넣었다(개정 2026-09-09). `deploy-aws.md` §12-3의 🔜도 닫았다.
  - **소급 고지를 넣은 이유는 「대상이 적어서」가 아니다** — 6-2에 「유료라 학습에 안 쓰인다」를 쓰는 순간
        그 문장이 *과거에도 그랬다*는 인상을 만든다. 안 적으면 새 부정확을 심는 것이라 함께 적었다.
        (법적으로는 유출이 아니라 처리라 개인정보보호법 §34 통지 의무 대상은 아니다.)
  - **노출 규모 실측(운영 DB 집계, 개인정보 미조회)**: 성향 분석 **4행 / 2명** — 본인 3건 + **타인 1명 1건
        (2026-06-08)**. 전체 계정 42. 성향 분석은 도입(#209, 2026-06-07)부터 줄곧 Gemini였으므로 이 4건이
        전부 무료 티어로 갔다. ⚠️ **둘 다 하한이다** — 히스토리는 사용자당 3행까지만 남고(같은 사람 안에서만
        잘리므로 distinct 2명은 영향 없음) 탈퇴 계정은 흔적이 없다.
  - **처리방침에 Gemini가 처음 등장한 건 2026-09-06(#1045)** — 타인 1건은 국외이전 고지가 아예 없던 때
        전송됐다. 무료/유료 이전에 그렇다는 점이 소급 한 줄의 실제 근거다.
- ✅ **닫힘 (2026-09-09) — 잔액 소진 시 429 안내가 지킬 수 없는 말이 되던 문제**. 자동 충전을 껐으므로
      Gemini 잔액이 마르면 429가 오는데, 문구가 「잠시 후 다시 시도해 주세요」라 충전 전까지 **기다려도
      안 되는** 약속이었다(리뷰어가 FALLBACK에서 잡았던 것과 같은 부류의 거짓말).
  - **문구** — `StudyPlanService`의 RATE_LIMITED를 사실 진술 「AI 이용량이 한도에 닿았어요 — 조금 뒤
        다시 시도해 보세요」로. 원인이 분당 한도든 잔액이든 참이다.
  - **로그** — `GeminiStudyPlanner.errorReasonOf`가 429/403 봉투에서 `error.status`와 `quotaId`를 뽑는다.
        상태 코드만으로는 「분당 한도」와 「돈이 떨어졌다」가 같아 보였다. **quotaId에 `FreeTier`가 박혀
        있으면 유료 전환이 안 먹었다는 뜻**이라 그것까지 이 한 조각이 가른다. `error.message` 원문은
        일부러 안 싣는다(키가 URL에 실리는 구조).
  - **403 분리** — 결제 중단·API 비활성은 403으로 오고 **기다려도 안 풀린다**. 429(「이용량 한도」)로
        접으면 사용자가 헛되이 기다리므로 `UNAVAILABLE`로 옮겼다. 전엔 포괄 `Exception` catch로 떨어져
        클래스명만 로그에 남았다.
  - 📌 **화면이 읽는 것은 `status().reason()`이 아니라 응답 본문이다** — 이 앱에서 reason은 null이고
        (Spring이 `sendError`를 타지 않는다) `pure.ts`의 `errorMessage`가 본문 평문을 그대로 띄운다.
        문구 회귀 테스트는 `content().string(...)`으로 잡아야 한다.
- ✅ **닫힘 (2026-09-09) — GCP 예산 알림**. `Gemini 선불 크레딧 소진 경보` · 맞춤 기간 2026-09-08~2027-09-08 ·
      ₩25,000 · 50%·90%·100% · 알림만. 설정 전문과 결제 구조는 `deploy-aws.md` §12-3 ①-2.
  - **기본값 둘이 「경보가 안 울리는 쪽」으로 고장 나 있었다** — ① 기간이 「월별」이라 매달 리셋되어
        400일치인 ₩25,000은 영영 50%에 안 닿는다 ② 크레딧 차감 체크 10개가 켜져 있어 선불 크레딧으로
        지불된 사용액이 예산에서 빠질 수 있었다(₩0으로 보임). 둘 다 바꿨다.
  - ✅ **후불로 넘어가지도, 무료 등급으로 강등되지도 않는다** (공식 문서 확인 — 처음엔 「카드로 넘어갈
        가능성」을 걱정했으나 근거가 없었다). 선불 크레딧은 Gemini API에만 묶여 있어(「strictly locked to
        Gemini API usage」) 소진되면 그 계정의 Gemini API가 즉시 멈춘다. 화면의 「후불 — Google Cloud
        서비스」는 다른 서비스용이다. **후불을 끌 필요가 없다**(끄는 방법도 계정 폐쇄뿐이라 껐다면 선불
        크레딧까지 죽었다). 더 중요한 쪽도 함께 닫혔다 — 「Your projects aren't automatically downgraded
        to the Free Tier」라 **잔액이 말라도 처리방침 6-2의 「학습 미사용」이 거짓이 되지 않는다.**
        소진 시 응답은 `429 RESOURCE_EXHAUSTED`가 유력하다(공식 문서 미명시 · 포럼 실사용 보고 일관).
        출처: <https://ai.google.dev/gemini-api/docs/billing>
  - ⚠️ **선불 크레딧은 2027-09-08에 만료**된다 — 현실 사용량(≈400일치)이 만료일과 거의 겹쳐, 다 못 쓰고
        소멸할 수 있다.
- 🔜 **미해결로 머지했다 — 처리방침 13절 30일 공지 조항**. 「이용자에게 불리하게 바뀌는 중요한 변경은
      시행일 30일 전부터 공지합니다」에 **국외이전 수령자 변경**이 걸리는지는 제품·법무 판단이다.
      개정일은 2026-09-08로 적었고 사전 공지는 하지 않았다.
- 📌 리뷰어가 **내 주장 하나를 반증했다** — spring-web 7.0.8 바이트코드에서
      `createResourceAccessException`이 URI의 `?` 뒤를 잘라내는 것을 확인했다. **키는 예외 message에 안
      실린다.** 따라서 `GeminiReadingPersonalityNarrator`의 `e.toString()`도 누출이 아니다(별건 PR 불필요).
      방어 로깅은 공짜라 유지하되 주석의 전제를 고쳤다.
- 📍 **검증**: RED(컴파일 실패) → GREEN. 신설 `GeminiStudyPlannerTest` **33건**(게이트 4 · 요청 조립 5 ·
      토큰 눈금 2 · 파싱 성공 3 · 파싱 실패 4 · 이사분 15) 포함 **서버 전체 통과**. 요청 본문 실물 확인 —
      temperature 0.2 · `responseMimeType` json · `maxOutputTokens` 32,000 · `thinkingBudget` 0 · 후보 91일.
- 📍 **실키 실측 3회 (2026-09-08) — 원장 5건 전부 충족**. 같은 입력(정보보안기사 5단원 · 시험일 3개월 뒤 ·
      주 6일 = 후보 91일)을 **실제 `buildRequestBody` 출력 그대로** 보냈다.

      | | 1회차 | 2회차 | 3회차 |
      |---|---|---|---|
      | `finishReason` | STOP | STOP | STOP |
      | 소요 | 17.1초 | 16.1초 | **25.8초** |
      | 출력 토큰 | 2,850 | 3,208 | 3,208 |
      | 원본 → 정제 | 91 → **79** | 77 → **77** | 77 → **77** |

- 📍 **1회차에서 방어선이 실제로 일했다** — 모델이 주 6일 규칙을 무시하고 후보 91일을 **전부** 채웠고,
      `sanitizePlan`이 주당 초과 12건을 잘라 79건으로 맞췄다. 2·3회차는 모델이 스스로 지켜 버려진 것이 0.
      **이 변동성이 방어선을 공급자와 무관하게 둔 이유다.**
- 📍 **타임아웃 걱정은 사라졌다** — 최악 25.8초로 90초 상한에 여유가 크다. Sonnet 실측 **75.7초**의
      3분의 1이라, 위 「스트리밍/비동기 승격」 항목의 근거(긴 쪽이 60s를 넘는다)도 **일정에 한해 해소**됐다.
- ⚠️ **품질 차이는 있다 — 정직하게 적는다**: task가 **21~25자**로 짧다(Sonnet은 항목당 토큰이 3배였다).
      「1장 시스템 보안 - 계정관리」 수준이라 뜻은 통하지만 **덜 자세하다.** 시험 전날 총복습 규칙은
      세 번 다 지켰고, 못 읽는 날짜는 0건이었다.
- 📌 **temperature 0.2가 먹었다** — 2·3회차 응답이 바이트까지 동일했다(「다시 만들기」가 도박이 아니다).

### AI 비용 방어 — 전역 일일 상한 + 승인 정원 (완료 ✅ 2026-09-08)
> 사용자 질문 「어드민 계정을 해킹해서 다른 계정을 모두 풀어버리면 AI 사용료 폭탄을 맞을 수 있잖아」에
> 대한 보안 리뷰(S-1~S-5)와 그중 S-1·S-5 구현.
- 📌 **전제를 먼저 교정했다** — 「전부 승인」 자체로는 돈이 안 나간다. 승인은 문을 여는 것이고 **돈은
      호출이 쓴다**. 실사용자 100명을 승인해도 그들이 갑자기 상한을 다 쓰지 않는다. 진짜 공격은
      「계정 대량 생성 → 승인 → 각 계정 상한 소진」이라 **폭발 반경이 계정 수에 비례**한다.
- ✅ **잘 되어 있던 것**: 가입으로 ADMIN이 될 수 없다(env 시드가 유일 경로) · `/admin/**`이 세션+CSRF
      체인(`csrf disable`은 미니앱 Bearer 전용 `@Order(0)`이라 안 닿는다) · 로그인 5회/15분 잠금 ·
      일괄 승인 없음 · 어드민도 AI 특혜 없음 · `tryConsume`이 원자적.
- [x] **S-1 전역 일일 상한** — `study_ai_daily_total`(V87). `StudyAiUsage`에서 user만 뺀 모양이라
      조건부 UPDATE 패턴 재사용. 상한 50 = 최악 약 6,000원/일. **0으로 내리면 배포 없이 전면 차단**.
- ⚠️ **날짜 키는 UTC다** — 사용자당 상한은 유저 타임존인데 그 값은 사용자가 제한 없이 바꿀 수 있다
      (S-3). 같은 키를 썼으면 자정 근처에 타임존만 바꿔 몫이 두 배가 됐다 — **막으려는 것에 같은
      구멍이 뚫릴 뻔했다.** 회귀 테스트로 못 박았다.
      → **2026-09-13 사용자당 키도 UTC로 통일(S-3 해소)** — 전역만 막아 둔 그 구멍이 사용자당 상한엔
      그대로 남아 있었다. 대가는 「오늘 남은 몫」이 UTC 자정(KST 09:00)에 리셋되는 것이다.
- [x] **S-5 감사 로그 + 승인 정원 1** — 세 전이에 로그 신설(그전엔 한 줄도 없었다). 정원은 상수다 —
      전역 상한은 사고 대응 차단기라 프로퍼티지만, 정원 확대는 **제품 결정**이라 배포를 거치는 게 맞다.
      소유자가 이미 승인돼 있어 **정원이 이미 차 있는** 상태다.
- 📌 **설계에서 바꾼 것**: 호출부마다 환불 지점이 둘씩이라 3곳×2=6군데를 손으로 맞추는 구조였다 →
      `tryConsumeBoth`/`refundBoth`로 묶어 누락을 구조적으로 불가능하게 했다. 반환은 `Grant` enum —
      「내 몫 소진」(429)과 「전체 몫 소진」(503)은 서로 다른 안내다.
- 📍 **돌연변이가 내 누락을 잡았다** — `refundBoth`에서 전역 환불을 지워도 전부 초록이었다. 설계에
      U-2로 적어 놓고 **호출부 계측기를 안 쓴** 것이다. 세 호출부 실패 테스트에 「행이 있고(=전역을
      썼다) + used==0(=되돌렸다)」 **쌍 단언**을 넣어 사살을 확인했다(한쪽만 보면 전역을 아예 안
      건드려도 초록이라 계측기가 아니다).
- ⚠️ **테스트가 다른 테스트를 깨뜨렸다** — 비트랜잭션 클래스에서 사용자를 커밋했더니 그 행이 다른
      컨텍스트의 집계에 새어 `AdminStatsServiceTest`의 「사용자당 평균」이 1000 → 666(=2000/3)이 됐다.
      그 테스트만 트랜잭션으로 감쌌다(경합을 재지 않으므로 무해).
- 📍 **리뷰 반영 (`reviewer` 독립 리뷰, 2026-09-08)** — 우회 경로 0건·환불 6자리·원가 수치까지 재현됐다.
      다만 **Critical 하나를 실측으로 재현했다**:
  - 🔴 **정원 검사가 TOCTOU였다** — `SELECT COUNT` 후 별도 UPDATE라, 8스레드가 정원 1을 **전원 통과**했다
        (`approvedAfterRace=8`). **위협모델이 「어드민 탈취」인데 어드민을 쥔 공격자에게 동시 요청은 공짜다** —
        2차 방어선이 자기가 지목한 적 앞에서 무너진 것. 아이러니하게 **같은 PR의
        `StudyAiDailyTotalRepository` javadoc이 정확히 그 패턴을 경고**하는데, 카운터에만 적용하고
        정원엔 안 했다. 먼저 경합을 테스트로 재현(RED)한 뒤 고쳤다.
  - ⚠️ **`SERIALIZABLE`은 H2에서 안 막혔다** — 같은 경합이 그대로 8/1로 뚫렸다. DB마다 구현이 갈리는
        것에 보안 통제를 걸 수 없어 **명시적 행 잠금**(`lockApprovalCandidates`)으로 바꿨다.
        **PENDING까지 잠그는 것이 요점** — APPROVED만 잠그면 정원이 빈 순간 잠글 행이 없어 직렬화가
        사라지는데, 하필 그때가 경합이 나는 시점이다. 승인 대상은 반드시 PENDING이라 집합이 안 빈다.
  - **`Grant` switch 문 → 식** — switch 문은 enum 망라성을 강제하지 않아, `Grant`에 값이 추가되면
        **경고 없이 그대로 유료 호출로 진행**한다. 상한을 우회하는 문이 미래의 한 줄 추가로 열리는 자리다.
  - **전역 상한 발동 경로에 계측기가 0건이었다** — `case GLOBAL_EXHAUSTED`를 지워도 전 스위트가 초록이었다.
        `StudyAiKillSwitchTest` 신설: 503과 **어댑터 미호출**을 쌍으로 잰다(전자만 재면 「503 주면서
        호출은 했다」가 초록이고, 그건 상한이 아니다).
  - **정원 초과를 전용 예외로 갈랐다** — 관리자 화면이 「이미 처리된 신청이에요」로 뭉개고 있었는데,
        **운영은 정원이 이미 찬 상태로 배포**되므로 앞으로의 **모든** 승인 시도가 그 틀린 문구를 봤을 것이다.
  - 공허하던 킬 스위치 테스트에 **양성 대조군** 추가(행 없이 재면 「상한 0」과 「행 없음」이 구분 안 된다) ·
        V87 타임스탬프 not null.
- ⚠️ **곁다리로 빌드가 터졌다** — 컨텍스트가 둘 늘자(`daily-cap=0` + 목) `OutOfMemoryError`. 테스트 수가
      아니라 **@SpringBootTest 컨텍스트 수**가 원인이고(properties·목이 다르면 별개 컨텍스트가 캐시에 동시
      상주), 실패 지점이 무작위라 원인이 코드에 있는 것처럼 보인다. `build.gradle`에 테스트 힙 2g 명시.
- 📍 **검증**: RED(컴파일 실패 + 경합 재현) → GREEN. 신설 15건(전역 8 · 정원 3 · 경합 1 · 킬스위치 3) +
      호출부 3곳 단언 보강. 돌연변이 4종 전부 KILLED. 서버 전체 **2,275건** 통과.
- ✅ **남은 리뷰 항목 전부 해소**: ~~S-2 가입 레이트리밋 없음~~ ✅ 2026-09-11(아래 「API 부정접근 감사 후속」) ·
      ~~S-3 타임존으로 사용자당 상한 우회(전역은 막았다)~~ ✅ 2026-09-13 해소 — 사용자당 상한의 날짜 키를
      UTC로 통일(`tryConsumeBoth`·`refundBoth`·`remaining`). 우회 재현 테스트로 RED를 먼저 봤다(같은
      시각에 존만 UTC+14로 옮기면 `OK`가 났다) · ~~S-4 이메일 인증이 기능을 안 막음~~ ✅ 2026-09-13 해소 —
      신청은 403(평문 한글 사유), 승인은 미검증 대상을 거절(배포 시점의 미검증 PENDING 방어).
- ⚠️ **S-4의 수용한 트레이드오프**: 합성 주소(`@noreply`) 토스 계정은 **검증 경로가 아예 없어** 이 게이트를
      영구히 못 넘는다. 웹 공부 AI는 현재 소유자 전용(정원 1)이라 실피해 0이고, 프론트 신청 버튼은 그대로
      보이며 누르면 403 문구가 뜬다. 토스 계정에 AI를 열 날이 오면 검증 경로부터 만들어야 한다.
- ✅ **프론트 문구 후속** (2026-09-13): 화면 문구 3곳(`PlanForm.vue` 163행 · `RecallPanel.vue` 사진 전사·분석
      두 자리)을 서버 429와 같은 「오늘 몫을 다 썼어요 — 한국 시간 오전 9시에 다시 채워져요」로 통일하고
...
> 확정된 Medium 3건 + 구매링크 가드 1건만 한 PR로 닫았다.
- [x] **A. `POST /signup` IP 레이트리밋** (`RateLimitAction.SIGNUP` 시간당 10) — 가입 폼은 임의 주소로
      메일이 나가는 문이다(인증 메일 / 「이미 계정 있음」 통지). 상한은 **바인딩 검증을 통과한 요청만** 센다 —
      폼 오타 재제출이 상한을 먹으면 정직한 사용자가 잠긴다. CGNAT를 고려해 시간당 10.
      `signup.html`에 전역 에러 블록이 아예 없어서 함께 신설했다(그전엔 `reject()`가 화면에 안 떴다).
- [x] **B. `POST /password/forgot` IP + 이메일 이중 레이트리밋** (IP 시간당 10 · 정규화 이메일 시간당 3) —
      이메일 키는 **한 주소로 나가는 발송량 상한**이다(IP만 세면 출처를 분산해 한 사람의 수신함을 채운다).
      키는 `trim().toLowerCase(Locale.ROOT)`로 정규화하고, 없는 계정도 똑같이 센다(상한 반응이 갈리면
      그게 열거 채널이다).
      - ⚠️ **이메일 키가 「링크 봉쇄를 끊는다」는 최초 주장은 사실과 반대였다 (리뷰가 잡음)**:
        `PASSWORD_FORGOT_EMAIL`은 **누가 요청했든 그 이메일 앞으로** 세므로, `EmailTokenService.issue`의
        직전 토큰 무효화로 인한 봉쇄는 그대로 가능하고(공격자는 시간당 3회면 충분하다), 오히려 그 3회를
        소진하면 **피해자 본인의 요청이 `?limited`로 거부**돼 한 시간 재설정이 잠긴다 — 상한이 없던 옛
        코드에선 저빈도 공격자가 거의 무해했다. 즉 봉쇄를 **없앤 게 아니라 옮겼다**.
      - ✅ **근본 해소 완료 (2026-09-12, 후속 PR)** — `issue`가 직전 미사용 토큰을 죽이는 것을 그만뒀다
        (유효 토큰 복수 허용 + TTL 만료). 「한 번 성공하면 형제는 끝」이라는 위생은 `consume` **성공 시**로
        옮겼고, 실패한 소비는 형제를 건드리지 않는다(쓰레기 토큰으로 남의 링크를 끊지 못하게). 이제
        **재설정 요청 반복만으로는 피해자의 링크를 무효화할 수 없다**.
      - ⏸ **남은 트레이드오프 — 쿼터 선점**: 공격자가 시간당 3회를 먼저 소진하면 피해자 본인의 요청이
        그 시간 동안 `?limited`로 거부된다(한 시간 뒤 또는 이미 받은 링크로는 가능). 발송량 상한의
        대가로 **수용**한다.
- [x] **C. SNS 웹훅 TopicArn fail-closed** — 빈 값이 「체크 건너뜀」이었다. 운영에서
      `BOOKTIMER_SES_SNS_TOPIC_ARN` 주입이 빠지면 공개 엔드포인트의 1차 방어가 조용히 꺼진다.
      이제 미설정이면 전부 403이고 기동 시 `log.error` 1회. dev는 SNS가 localhost에 못 닿아 손실 0.
- [x] **D. `/u/{loginId}/books/{bookId}/buy*` 4개에 소유자·차단 검사** — 공개 여부만 보고 `findById`로
      끌어와, 차단한 사람의 책방에서도·경로의 loginId와 무관한 남의 공개책으로도 집계·리다이렉트가 됐다.
      컨트롤러는 `ProfileService.resolveVisibleTarget`(프로필 조회와 **같은 가드** — 운영자·차단·없는 아이디),
      서비스는 `findByIdAndUser(bookId, owner)`로 스코프. 근본 수정이라 4개 경로가 한 번에 닫힌다.
- 📍 **검증**: 항목마다 RED → GREEN. 신설 **13건**(A 2 · B 4 · C 1 · D 6). 돌연변이 **8종 전부 KILLED**.
      (수치는 `git diff $(git merge-base HEAD origin/main) -- src/test`로 재측정한 값이다.)
- ⚠️ **리뷰가 「새 가드가 옛 테스트를 공허하게 만든 것」을 잡았다** — 기존 「비공개 책 게이트」 4건이 경로
      loginId를 없는 핸들(`"somehandle"`)로 써서, `resolveVisibleTarget`을 앞단에 끼우자 **공개 검사에 닿지도
      못하게** 됐다(교보의 `!isPublic()`을 떼도 59건 전부 초록 — 교보는 서비스 단위 테스트가 없어 커버리지 0).
      또 최초 소유자-스코프 돌연변이를 **알라딘에만** 넣어서 쿠팡·Yes24·교보는 잠기지 않았다(3곳을 `findById`로
      되돌려도 7클래스 87건 초록). 핸들 4건을 주인 실제 loginId로 고치고 스코프 테스트 3건을 더해 닫았다.
      **교훈: 가드를 앞단에 새로 끼우면 그 뒤의 기존 테스트가 조용히 공허해진다** — 새 테스트만 보지 말고
...
- [x] **2. `followScopePopularity`에 `role <> ADMIN`** — 형제 명단 쿼리(위 I)와 같은 불변식으로 맞춰 숫자와
      ...
      (리뷰 지적) — 1명이면 조건 역전(`<>`→`=`) 돌연변이에서도 값이 1이라 조용히 통과한다.

### 웹 홈 「바꾸기」가 타이머를 시작하던 자리 — 고르기와 시작을 가른다 (완료 ✅ 2026-09-12)
> 사용자 지적 「책 바꾸기로 책을 바꾸면 자동으로 타이머가 시작돼. 책을 바꾸기를 했으면 책만 바껴야 되는 게 맞아.
> 공부용 타이머에서만 확인했고 교양용은 확인 안 했어.」
- [x] **교양(독서)도 같았다** — 확인 결과 두 타이머가 같은 골격이다. 칩의 「바꾸기」 → 시트 → 책을 고르는 순간
      `onSheetPick`/`onStudySheetPick`이 곧장 `handleStart`를 불렀다. 기존 테스트가 그 동작을 못박아 두고 있었다.
- [x] **선택 상태를 `DashboardApp`으로 올렸다**(`pickedBook`·`pickedStudyBook`) — 시트에서 고르면 칩과 여백 카드의
      책만 바뀌고, 측정은 시작 버튼이 누를 때만 시작된다. 없으면 지금까지처럼 기본 규칙(최근 읽은 책 → 첫 책).
- [x] **시트가 id가 아니라 고른 책을 통째로 낸다** — 독서 시트 목록은 `/api/books`, 칩은 `/api/dashboard` 목록이라
      **출처가 갈린다**. id만 넘기면 부모가 못 찾는 날(다른 탭에서 담은 책) 칩이 조용히 다른 책을 가리킨다.
- [x] **`conflict()`에서 선택을 함께 버린다** — 「그 책이 서재에 없어요」의 그 책이 칩에 남으면 눌러도 계속 같은 404다.
- 📌 **곁다리로 같은 뿌리 둘이 닫혔다** — 여백 카드의 「책 고르기」와 서재 0권일 때의 「책 고르기」도 같은 시트를 여는데,
      **책을 담아 고르기만 해도 측정이 켜지고 있었다**.
- 📍 **미니앱은 해당 없음** — 홈이 시트가 아니라 표지 캐러셀이라(「가운데 온 것이 곧 측정 대상」) 고르기와 시작이
      이미 갈려 있다. 측정 중 교체(`change` 모드)도 원래부터 책만 바꾼다.
- 📍 **검증**: RED 2건(「고르기가 start를 부른다」가 요청 0건 단언에서 실패) → GREEN, 프론트 **762건** 초록.
      계측기는 **요청 0건 + 양성 대조군**을 한 쌍으로 뒀다 — 앞엣것만 두면 「아무 데서도 시작 안 됨」이 통과한다.
      실브라우저 게이트는 **머지 시점엔 못 돌렸다**(로컬 확인이 로그인 폼 입력을 요구) — 로드순서·타이밍이 아니라
      상태 전달 변경이라 jsdom 마운트 테스트가 같은 컴포넌트 트리를 실제로 클릭해 재는 것으로 갈음했다.
- [x] **✅ 배포 후 사용자가 운영에서 실확인(2026-09-12)** — 「실제로 보니 잘 고쳐졌다」. 위 갈음이 남긴 구멍
      (동작은 확정, **픽셀·레이아웃 미확인**)이 이걸로 닫혔다. 미검증 항목을 원장에 남겨 뒀기 때문에
      **무엇이 아직 안 봤는지**가 배포 뒤에도 살아 있었다 — 「검증됨」과 「통과됨」을 붙여 적지 않은 값이다.

### 웹 책BTI 생성 차단 — 읽기는 유지 (완료 ✅ 2026-09-08)
> 사용자 지적 「토스 미니앱은 광고를 봐야 성향을 돌릴 수 있는데 웹은 그렇지 않다. 수익 구조를 못 넣고
> 광고도 이미 심사에서 떨어져 또 하기엔 번거롭다.」
- 📌 **조사가 작업을 크게 줄였다** — 두 LLM 경로가 이미 **웹 전용**이라 채널 판별이 필요 없었다.
      `GET /api/personality`(부트스트랩 생성) · `POST /refresh`. 미니앱은 `/status` + `/ad-refresh`를 쓴다.
- ⚠️ **눈에 안 띄는 쪽이 진짜였다** — GET이 히스토리가 비면 첫 분석을 자동 생성해서, 웹 사용자가
      /personality를 **열기만 해도** Gemini가 호출됐다. `refresh`만 막았으면 이 문이 남는다.
- 📌 **미니앱은 이 함정을 이미 알고 피해 놨다** — `miniapp/src/api.ts`가 GET 대신 부작용 없는 `/status`를
      쓰는 이유가 「히스토리가 비면 첫 분석을 공짜로 만들어 **광고 관문을 무력화한다**」라고 적혀 있다.
      웹만 그 문이 열려 있었다.
- [x] **`currentPersonality`를 읽기 전용으로** — 대표가 있으면 그대로, 없으면 사실만. LLM 호출 0.
- [x] **`POST /refresh` 제거** — 웹 전용이라 그냥 걷었다. `/ad-refresh`(미니앱)는 무변경.
- [x] **프론트** — 「다시 분석」 버튼·`refresh()`·`btnState`·죽은 ref 제거, 「토스 앱의 북타이머에서 만들 수
      있어요」 안내로 교체. FALLBACK 문구 「잠시 후 다시 분석해 주세요」도 거짓말이 돼서 고쳤다.
- 📌 **이미 만들어 둔 서술은 그대로 보인다**(사용자 선택 「생성만 막고 읽기는 유지」) — 돈 주고 만든 것을
      화면에서 거두지 않는다. 서술이 없는 사용자는 통계(장르·저자 분포)를 본다.
- 📌 **곁다리 효과** — 리뷰가 짚었던 「책BTI가 전역 상한 밖」이 실질적으로 닫힌다. 남는 LLM 경로가
      광고 관문을 지난 `/ad-refresh` 하나뿐이다.
- 📌 **리뷰가 덤으로 찾은 것 — 광고 관문의 두 번째 문도 닫혔다**. 옛 `POST /select/{남의id}`는 `select()`가
      조용히 무시된 뒤 `currentPersonality`가 **무료 분석을 생성·대표 지정**했다(카운터 소비도 광고도 없이).
      `/status` javadoc이 자기를 「부작용 없는 유일한 성향 GET」이라 부른 근거가 그건데 `/select`는 못 봤던 것.
- ⚠️ **정확히 말하면 「UI를 걷었다」**다 — `SecurityConfig`의 채널 스위치가 경로가 아니라 `Authorization: Bearer`
      헤더라, 세션+CSRF를 든 웹 사용자가 `/ad-refresh`를 직접 때리면 여전히 200이 난다(이 PR의 초록 테스트가
      그걸 증명한다 — `.with(user(...)).with(csrf())`로 200을 단언). 이 PR 전에도 같았고 하루 10회 상한이
      걸려 있어 **악화는 아니지만**, 「생성 자체를 없앴다」로 읽히면 안 된다.
- 📍 **리뷰 반영**: COLD_START 문구가 「분석해 드릴게요」로 **지킬 수 없는 약속**을 하던 것(FALLBACK에서 고친
      것과 같은 종류의 거짓말이 한 상태 옆에 남아 있었다) · 제 주석이 **같은 커밋 안에서 반증**되던 것
      (「화면이 잔여를 쓴다」 — 같은 PR이 그 필드를 프론트에서 뺐다) · 이 PR이 **근거로 인용한** `/status`
      javadoc과 미니앱 `api.ts` 주석이 정작 stale이던 것(순환 인용인데 한쪽만 갱신) · FALLBACK 정의 ·
      호출자 0인데 무조건 LLM을 부르던 `analyze(User)` 제거 · 죽은 프론트 코드 · 단언 범위 좁힘.
- 📍 **검증**: RED 2건 → GREEN. 돌연변이(부트스트랩 부활) **3건 사살** — 리뷰어가 5종을 독립 주입해 전부
      KILL 재현했고, 픽스처가 콜드스타트로 조기 리턴해 초록인 가짜가 **아님**도 확인했다.
      옛 규칙을 인코딩하던 **서버 삭제 7·수정 9 + 프론트 삭제 3·수정 2**를 새 규칙으로 옮겼다.
      서버 전체 **2,273건** · 프론트 **673건**.
- ⚠️ **보고 숫자가 틀렸다** — 「서버 테스트 12건 이관」은 어떤 셈법으로도 안 나온다(리뷰 실측). 재측정해 고쳤다.
- ⚠️ **작업 중 자기무력화를 한 번 했다** — `/refresh` → `/ad-refresh` blind 치환이 「`/refresh`가 사라졌다」를
      증명하는 **바로 그 테스트의 요청까지** 바꿔 200을 만들었다. 테스트가 빨간불이라 잡혔지만, 초록이었으면
      「엔드포인트가 사라졌다」는 거짓 확인이 남았을 자리다.

### 백지복습 본문 = 노션식 편집기 (완료 ✅ 2026-09-09)
> 사용자 결정 「WYSIWYG으로. 툴바 **와** 슬래시 메뉴 둘 다. Tab 들여쓰기는 필수 — 이걸 많이 쓴다.
> 마크다운 문법을 사용자가 배우지 않아도 되게.」 계획 md는 `claude-docs/plans/2026-09-09-recall-wysiwyg-editor.md`(gitignore).
- [x] **Tiptap 3(`@tiptap/vue-3` + `@tiptap/markdown`) 도입** — 저장 형식은 **마크다운 문자열**이라
      `StudyRecall.body`(VARCHAR 8000)·API·미니앱·AI 프롬프트가 **전부 무변경**이다. 되돌리기는 컴포넌트를
      textarea로 되돌리는 것으로 끝나고, 그때 옛 글은 문법이 보일 뿐 깨지지 않는다.
- [x] **같은 명령에 입구 셋** — 툴바 13개(눈으로 찾는 사람) · `/` 메뉴 7개(손이 키보드에 있는 사람) ·
      Tab/Shift+Tab(목록을 이미 쓰는 사람). 세 경로가 도달 방식이 달라 **따로 잰다**(하나가 끊겨도 나머지는 초록).
- ⚠️ **옛 평문 글이 이 작업의 진짜 위험이었다** — 파서에 `breaks: true`가 없으면 줄바꿈 하나로 갈라 둔
      옛 글이 열 때 **통째로 한 문단**이 된다. 마이그레이션·변환 스크립트 대신 **파서 옵션 한 줄**로 끝났고,
      로컬에 평문을 심어 두고 열어 실측했다(3줄 보존 + 저장 안 누르면 서버 값 불변).
- ⚠️ **직렬화가 저장값을 오염시켰다 — 둘 다 `cleanMarkdown` 한 함수가 경계에서 턴다** — ⓐ `trailingNode`가
      목록·구분선 뒤에 붙이는 빈 문단이 `&nbsp;` 한 줄로 나와 `trimEnd`로는 하나도 안 털렸다(그 줄을 지우고
      연속 빈 줄을 하나로 접는다) ⓑ 글자로 친 `>`·`&`가 `&gt;`·`&amp;`로 이스케이프됐다(실측 `a > b` → `a &gt; b`) —
      **뜻이 안 바뀌는 것만** 되돌린다: 줄 시작 `&gt;`는 남기고(인용 블록으로 승격된다), `&lt;`는 손대지 않으며
      (인라인 HTML로 먹힌다), `&amp;`는 마지막에 푼다. 8000자 예산과 AI 프롬프트를 함께 갉던 자리다.
      **밖으로 내보내는 길과 「같은 값인가」를 비교하는 길이 같은 함수를 쓴다** — 한쪽만 다듬으면 매 입력마다
      문서를 갈아치워 커서가 튄다.
- [x] **상한 두 겹** — `maxlength`가 사라지므로 화면(`bodyBudget` → 저장 버튼 잠금 + 안내)과 서버 400이 함께 막는다.
- [x] **줄노트 배경 제거** — 제목·목록·인용이 들어오면서 블록마다 줄 높이가 달라져 28px 격자와 어긋난다.
- 📍 **슬래시 팝업 좌표는 하니스가 원리상 못 잡는 자리였다** — jsdom엔 레이아웃이 없어 팝업 테스트는
      늘 초록인데, 실 브라우저에서 캐럿을 화면 아래쪽에 두고 열자 팝업이 **화면 밖으로 82px** 나갔다
      (좌표를 잰 상자와 그 좌표가 먹는 상자가 달라 툴바 높이만큼 밀리는 버그도 함께 나왔다). 뷰포트 기준으로
      계산해 아래가 모자라면 위로 뒤집는 `slashPosition`(순수 함수 + CSS `position: fixed`)으로 고쳤다 —
      **좌표 규칙을 순수 함수로 꺼내니 그제야 계측기가 생겼다**(뒤집기·좌우 clamp 5건).
- 📍 **검증**: RED → GREEN, 새 테스트 49건(pure 27 · 편집기 19 · 패널 2 · 프롬프트 1) · 프론트 전체 646건 ·
      돌연변이 7종(`breaks` 끄기 · 상한 경계 넓히기 · 목록 밖 Tab 무력화 · 프롬프트에서 마크다운 벗기기 ·
      줄 시작 `&gt;` 보호 제거 · `&amp;`를 먼저 풀기 · `&nbsp;` 줄 제거 삭제) 사살. ⚠️ 그중 `&amp;` 순서 돌연변이는
      **처음에 살아남았다** — 테스트 입력이 줄 시작이라 인용 보호가 대신 막고 있었다(같은 초록이 두 이유로
      나던 자리). 글 가운데로 옮겨 다시 죽였다.
      로컬 실측(폰 375×812 · 데스크톱 1280×800) — 툴바 sticky(편집기 top −260일 때 툴바 top 0) ·
      들여쓰기 버튼이 저장까지 도달(서버 저장값 「- 하나 / 두 칸 들여쓴 - 둘」) · 슬래시 팝업이 양쪽 뷰포트의
      화면 하단에서도 화면 안 · 옛 평문이 3줄로 열림.
- 🔁 **리뷰 반영 4건**(2026-09-09) — 위 저장값 정리 둘 · 슬래시 팝업이 떠 있을 때 **Tab이 선택을 확정**한다
      (전엔 Tab 키맵이 먼저 먹어 고르려던 손이 `- /제` 목록을 만들었다) · 팝업 접근성(`role="listbox"`/`option` ·
      `aria-selected` · 글 상자 `aria-expanded` · 고른 항목이 `max-height` 밖이면 `scrollIntoView`) ·
      **죽은 계측기 2건** — 패널 테스트의 `.study-recall-body`는 편집기 도입 뒤 대역(mock)에만 남은 클래스였다
      (스텁·실물 양쪽에 있는 `data-testid`로 교체).
- ⚠️ **번들이 커진다 — 머지 전 사용자 판단이 필요한 자리다**: study.js 110,727 → **591,857 bytes**
      (gzip 40,326 → 194,074), **홈 대시보드도 같은 패널을 실어** dashboard.js 139,818 → **620,706**
      (gzip 48,785 → 202,361). 설계의 「500KB raw 초과면 보고」 선을 넘었고 대시보드 섬은 설계가 아예
      세지 않았다. 지연 로딩은 `inlineDynamicImports: true`라 지금 구조에선 안 된다 — 문제가 실측되면
      그때 설정을 푼다.
- ⏸ **한글 IME 조합 중 입력은 미검증**(ABANDON: U-1) — 어느 브라우저 도구도 OS IME를 구동하지 못한다.
      사용자가 30초만 쳐 보면 닫히는 항목이다(`- 가나다`를 조합해 입력 → 자모 잔여·중복 없이 들어가는지).

### 공부 필기(실시간 노트) — 책별 자유 다장 · 자동저장 (완료 ✅ 2026-09-10 · PR-1~PR-3, 홈 탭 PR-4 2026-09-11)
...
- 📍 **검증**: RED → GREEN. `FlywayMigrationTest`는 마이그레이션만 올린 상태에서 **먼저 빨간불을 봤다**
      ...
      책 삭제 2) + `FlywayMigrationTest` 갱신 1, 돌연변이 8종(revision 대조 제거 · 목록 정렬 뒤집기 ·
      소유자 조건 제거 · 책 필수 400 가드 제거 · 책 삭제 409 가드 제거 · 평문 핸들러 제거 · purge 한 줄 제거 ·
      purge 순서를 책 뒤로)이 전부 겨눈 테스트에 사살됐다.
- 📍 **리뷰 반영 (독립 리뷰 Needs fixes)**: ① **갱신의 입력 검증이 400이 아니라 404로 샜다** — 컨트롤러가
      모든 IAE를 404로 옮기는 `mutate`로 갱신을 감싸 「없는 필기」와 「빈 본문·길이 초과」가 뭉개졌다
      (생성은 `mutate` 밖이라 400을 주던 터라 **같은 규칙인데 문마다 답이 달랐다**, 설계 §3.4 위반).
      갈래를 호출부가 아니라 던지는 자리에서 가른다 — `StudyNoteService.owned`가 404를 직접 던지고
      `mutate`는 걷어냈다(삭제 문도 같은 함수를 지나므로 한 곳 수정이 두 문을 고친다). **소유권 404가
      검증 400보다 먼저**인 순서는 계약이다 — 뒤집으면 400/404 차이로 남의 필기의 존재를 캐낼 수 있다.
      ② 목록 정렬에 `id DESC` **동률 깨기**를 넣었다(정답지가 이 순서로 장을 고르므로 동률이 미정의면
      채점 기준이 요청마다 흔들린다 — PR-3 전에 닫는 게 싸다). ③ 위 돌연변이 수치를 8종으로 정정
      (changelog는 이미 8, plan만 7로 남아 있었다). RED → GREEN, 새 테스트 5건(갱신 400 3 · IDOR 순서 1 ·
      정렬 동률 1), 돌연변이 3종(404로 뭉개기 복원 · 검증을 소유권보다 먼저 · `IdDesc` 제거)이 각각 겨눈
      테스트에만 사살됐다.
- 🔒 **생성에만 레이트리밋 — 시간당 30장** (`RateLimitAction.STUDY_NOTE_CREATE`, 리뷰 2라운드): `study_note`는
      이 레포에서 **자연 상한도 레이트리밋도 없는 첫 사용자 작성 테이블**이었다(백지복습은 `UNIQUE(user, date)`로
      하루 한 행, 여백 글은 `STORY_CREATE` 시간당 10). 겨누는 것은 악의보다 **PR-2의 클라이언트 버그**다 —
      자동저장이 「첫 비공백 본문에서 생성, 이후 `/{id}`로 갱신」이라는 순수 클라 상태기계라, id 배선이
      어긋나면 1.5초 디바운스마다 새 행이 생긴다(분당 약 40행). 목록은 위 비목표대로 페이징이 없어 그
      사용자의 목록 왕복이 계속 커진다. **갱신·삭제·조회에는 걸지 않는다** — 자동저장이 1.5초마다 두드리는
      문이라 걸면 정상 사용이 막힌다. 여백 글(10)보다 넉넉한 것은 필기가 한 세션에 1~3장·하루 5~10장이
      정상이어서고, 그래도 위 폭주는 1분 안에 걸린다. 초과 응답은 **429 평문**(위 📍 평문 핸들러와 같은 이유 —
      `error.html`이 본문이 되면 안내가 안 닿는다).
- [x] **PR-2 웹 필기 탭 (2026-09-10)** — `DayPanel`에 [필기]/[백지노트] 탭(`v-if`로 갈라 편집기는 언제나 하나) ·
      `NotesPanel.vue`(자동저장 상태기계) · `notes.ts`(순수: `noteLabel`·`savedAtLabel`·`nextSaveState`) ·
      `RecallEditor` 재사용(`ariaLabel` prop 1줄 — Tiptap 복사본 0, `study.js` +8,401B) · 번들 재빌드.
- ~~⚠️ **탭은 `RecallPanel` 안이 아니라 `DayPanel`에 둔다** — `RecallPanel`은 홈 대시보드 `RecallCard`도
      import하므로 안에 넣으면 **홈에 필기 탭이 샌다**(홈은 백지노트만 보인다 — 설계 비목표).~~
      **→ 2026-09-11 번복(아래 PR-4).** 「홈에 샌다」가 이제 의도다.
- 📍 **실패가 셋으로 갈린다 — 이 PR의 설계 결정 전부다.** 필기 화면엔 저장 버튼이 없어(자동저장) 백지복습의
      `canSave` 같은 빗장이 없고, `RecallEditor`는 8000자를 **표시만 하고 막지 않는다**. 그래서 상한을 넘으면
      자동저장이 400을 받는다. ① **400 → `invalid`: 다시 보내지 않는다**(사용자가 줄이기 전엔 영원히 400이라
      재시도에 넣으면 키 입력마다 무한 왕복이고, 서버가 준 안내가 그 소음에 묻힌다). 서버 평문을 그대로 띄우고,
      글이 상한 아래로 내려오면 그때 되살아난다. ② **409 → `conflict`: 자동저장 정지 + 편집기 잠금 + [새로고침]**
      (내 미저장분은 버린다 — 남의 판을 덮는 것보다 정직하다). ③ **5xx·네트워크 → `error`: 다음 변경·버튼에 재시도**.
- 📍 **`revision`은 요청에 항상 숫자로 싣는다** — 빠지거나 `null`이면 서버 `int`가 못 받아 **500 + HTML 본문**이
      온다(레포 전역 성질이라 PR-1이 서버에서 안 고치고 클라가 막는 것을 전제로 뒀다). 값의 출처는 화면 상태가
      아니라 **서버 응답**이다(손으로 세지 않는다). `typeof === 'number'`를 테스트가 못 박는다.
- 📍 **검증**: RED → GREEN. 새 테스트 31건(순수 16 · 패널 11 · 탭 4), 프론트 전체 695 → **726건** 초록.
      돌연변이 14종(400 갈래 삭제 · 409 갈래 삭제 · invalid에서 재시도 · conflict 빗장 제거 · ok가 dirty 큐 삭제 ·
      마크다운 벗기기 제거 · onEdit invalid 가드 제거 · 디바운스 0 · revision 미탑재 · 빈 초안 가드 제거 ·
      필기 전환 플러시 제거 · 언마운트 플러시 제거 · 두 패널 동시 마운트 · 409 잠금 해제)이 전부 겨눈 테스트에
      사살됐다. ⚠️ 그중 **3종은 처음에 「주입 실패」였다** — 여러 줄 패턴을 `\n`으로 잡았는데 파일이 CRLF라
      0건 매치였다. 주입기가 그 자리에서 크래시하지 않았으면 「테스트가 못 죽임」과 구분되지 않는다(hookify
      `warn-inplace-replace-crlf`가 경고하는 바로 그 자리).
- ⛔ **실 브라우저 게이트 미충족 — 머지 전에 채워야 한다(U-3·U-5·U-6, U-4 절반).** 자동저장은 타이밍·반응성이
      걸린 변경이라 단위테스트로 끝낼 수 없는데(T-053 계열), 이 세션에선 **로그인된 브라우저 표면을 얻지 못했다**:
      `claude-in-chrome` 확장 미연결 + `chrome-devtools` 프로필 배타 점유(T-201) + 내부 패널은 `/study`가
      `/login`으로 튕긴다. **비밀번호를 대신 입력하는 것은 금지**라 시드 계정 로그인을 스스로 할 수 없다.
      확인된 것은 공개 자원뿐이다 — `/css/app.css`에 `.study-notes-item`·`.study-notes-status`가 실려 있다.
      ⚠️ **`/study/study.js` 마커 검사는 「전부 false」로 나왔는데 이건 회귀가 아니라 인증 리다이렉트다**
      (받은 본문이 5,734바이트짜리 로그인 HTML이었다 — 「0건」을 결과로 읽었으면 오진할 자리).
      남은 절차: 사용자가 `http://localhost:8080/login`에 `testid`로 로그인한 뒤 ⓐ 필기 작성 → 「저장됨 · 시각」
      ⓑ 8000자 초과 → 서버 평문 안내 + **네트워크 탭 요청이 늘지 않음** ⓒ 탭 전환 플러시 ⓓ 콘솔 에러 0
      ⓔ `document.querySelectorAll('.ProseMirror').length === 1`.
- ⚠️ **설계와 어긋난 자리 1건 — 목록 라벨**: 설계 §3.7이 목록 라벨을 `noteLabel(title, body)`로 뒀는데
      §3.4의 목록 응답(`NoteRow`)엔 **body가 없다**(§3.6의 「본문 없는 목록」과 충돌). 그래서 제목 없는 장은
      지금 열어 둔 장만 첫 줄에서 이름을 얻고 나머지는 「제목 없음」이다. 서버를 PR-2 범위 밖에서 고치지 않고
      그대로 뒀다 — 고치려면 `NoteRow`에 미리보기 필드를 더해야 한다(PR-3에서 판단).
      **리뷰 반영: 제목 칸 placeholder가 「없으면 첫 줄이 이름이 돼요」라고 약속하던 것을 걷었다** —
      제목을 안 적는 게 이 기능의 기본 사용법이라 그 약속은 대부분 거짓이었다. 화면이 거짓말하지 않게
      문구만 고쳤고(「목록에 이름이 필요하면 적어 주세요」) 근본 처방(`NoteRow.preview`)은 여전히 PR-3 몫이다.
- 🔁 **리뷰 반영 라운드 (2026-09-10) — 머지 차단 1건(C-1)이 실측으로 재현됐다.** 세 증상의 뿌리가 하나였다:
      ⓐ `save()`가 왕복 중이면 `flush()`가 **아무 일도 안 하고 즉시 반환**하는데 `openNote`/`newNote`는 그 반환을
      「플러시 완료」로 믿고 초안을 갈아 끼웠다 ⓑ 응답에서 **지금 어느 장인지 안 보고** `id`/`revision`을 되썼다
      ⓒ 초안의 책을 **전송 시점에** 읽었다.
      - ⛔ **C-1(Critical)** — 필기 A 저장 왕복 중 「＋ 새 필기」를 누르면 빈 초안에 A의 id가 붙어, 이후 입력이
        **생성이 아니라 A에 대한 갱신**으로 나갔다(`POST /api/study/notes/5` body `{"body":"완전히 새 글"}`).
        **A의 본문이 사라지고 새 필기는 만들어지지 않는다.** 목록에서 B를 여는 경로도 같다 — 화면은 B인데
        `draft.id`는 A라 B의 글이 A에 기록됐다. 자동저장이라 사용자가 인지할 계기가 없고 휴지통·버전도 없다.
      - ⚠️ **I-1** — 왕복 중에 친 글자가 전환 때 사라졌다(설계 §3.4 「즉시 플러시」 미이행). 실측: `A1`→`A2` 왕복
        중에 `A2A3`까지 쳤는데 서버로 나간 본문은 `["A1","A2"]`뿐. 기존 돌연변이 `m11_noSwitchFlush`는
        **왕복이 없는 상태의 플러시만** 재고 있었다 — 무계측 구간이 정확히 거기였다.
      - ⚠️ **I-2** — 책을 바꾸는 순간 앞 책의 초안이 **새 책에** 생겼다(`{"bookId":9}`). 필기 이동이 비목표라
        복구가 없고, PR-3에선 다른 책의 채점 기준으로 들어간다.
      - **처방 3조각**: `seq`(편집 대상의 세대 — 갈아탈 때 오르고, 저장은 보낼 때의 값을 들고 갔다가 응답에서
        대조해 다르면 **id·revision·state 전부 손대지 않는다**) · `flush()`가 날아가 있는 왕복을 **기다렸다가
        다시 보낸다** · **초안이 자기 `bookId`를 들고 다닌다**.
      - 함께: **404·429를 재시도 갈래에서 뺐다**(`invalid` + `permanent` — 400과 달리 글을 고쳐도 안 풀린다.
        404는 편집할 때마다 없는 행을 두드리고, 429는 생성 레이트리밋에 걸린 초안이 **최대 한 시간 동안
        1.5초마다 스스로 레이트리밋을 때린다**). 404 문구는 화면 몫으로 옮겼다 — 공용 `errorMessage`의 404는
        「책을 찾을 수 없어요」라 필기 상황에 어긋난다. **생성에도 `keepalive`**(아직 생성 안 된 초안은 잃는 양이
        1.5초가 아니라 **장 전체**다). 탭에 `aria-controls` + `role="tabpanel"`(`role=tab`만 붙고 여는 곳이
        없던 상태) · 규칙 0건이던 죽은 클래스 `.study-day-pages` 제거.
      - **검증**: RED(새 테스트 10건 전부 빨간불) → GREEN. 새 테스트 **12건**, 프론트 전체 726 → **738건** 초록.
        돌연변이 7종(세대 대조 제거 · 플러시가 아예 안 기다림 · 플러시가 기다리기만 함 · 초안 책을 전송 시점에 읽음 ·
        404 갈래 · 429 갈래 · `permanent`가 edit에 되살아남)이 전부 사살됐다. ⚠️ **C-1 두 시나리오는 가드
        하나만 지워선 안 죽는다** — 세대 대조와 플러시 대기가 서로 겹쳐 막기 때문이다(공허해서가 아니다:
        **둘 다 없던 원래 코드에서 실제로 빨간불이었고**, 둘을 동시에 지우면 다섯이 함께 죽는다). 번들
        `study.js` 600,258 → **600,942 bytes(+684)**, 나머지 10개 섬은 바이트 동일.
- [x] **PR-3 채점 연동 (2026-09-10)** — `NoteReference`(최근순 24,000자 = 8000×3, **장 단위** 자르기) ·
      `GET /api/study/notes/reference` · `ANALYZE_SYSTEM` 개정 + user 메시지 **블록 3개**(라벨 위조가 블록
      경계를 못 넘게) · `RecallPanel`에 「채점 기준으로 들어가는 필기」 카드. **이 기능의 목적이 여기서
      완성된다** — 구멍 판정이 「범위」 한 줄 추측에서 **필기 대조**가 된다.
- 📍 **장 단위로 자르는 것이 이 PR의 설계 결정 전부다.** 글자 단위로 자르면 정답지가 문장 중간에서
      끊겨 두 종류의 거짓이 난다 — 잘린 뒤를 「안 배운 것」으로 보는 **거짓 구멍**과, 잘린 문맥을 잘못
      이어 붙여 빠진 것을 못 잡는 **거짓 통과**. 장 단위면 잃는 것이 「이 장은 통째로 빠졌다」는
      *말할 수 있는 사실*이 되고, 그래서 화면이 그 수를 드러낸다(사용자 결정 5 — 조용한 누락 금지).
      한 장의 최대 비용이 8000 + 헤더(≤224)라 **필기가 있으면 최소 한 장은 반드시 들어간다**
      (「최근 3장은 반드시」는 아니다 — 세 장이 다 최대면 24,672자로 상한을 넘는다).
- 📍 **상한의 단위 = 「모델이 실제로 받는 글자 수」다 (리뷰 I-1 수정)** — 처음엔 본문만 셌는데, 헤더
      (`### 필기: {제목 ≤200자} ({날짜})`)가 **장마다** 붙고 장 수엔 상한이 없어 24,000이 프롬프트 크기의
      상한이 아니었다. 실측: 제목 200자·본문 1자짜리 장 1,000개면 카드는 「1,000장, 1,000자」라 말하고
      모델은 **225,000자**를 받았다(24,000개면 5,400,000자). 이 PR이 막으려던 「화면이 보여준 것과 모델이
      본 것의 어긋남」이 바로 그 자리라, `select`가 헤더까지 세고 `chars`를 `text().length()`와 **같은 값**으로
      묶었다. 같은 입력의 지금 값은 **1,000장 → 106장 포함 · 23,850자**(보고값 = 실제값).
- 📍 **정답지 조회는 상한 선점보다 앞이다** — 뒤로 가면 필기 조회가 실패한 사용자가 분석을 못 받은 채
      오늘 몫을 잃는다(환불은 어댑터 실패에만 붙어 있어 거기까지 닿지 않는다). 리포지터리 스파이로
      터뜨려 「몫이 안 깎인다」를 잰다.
- 📍 **인젝션 반경이 커져 user 메시지를 블록 3개로 갈랐다**(주제·범위 / 필기 / 오늘 쓴 글). 필기는
      길고(최대 24,000자) 사용자가 자유롭게 쓰므로 「[필기]」 같은 라벨 위조가 가능한데, **블록 분리가
      보장하는 것은 문자열 경계뿐이다** — 한 블록의 라벨이 다른 블록 내용으로는 못 넘어가지만,
      **블록 안에서의 섹션 위조는 그대로 가능하다**(필기 본문이 여러 줄로 실린다). 리뷰 실측(M-1)에서
      제목에 개행을 넣어 헤더 한 줄을 벌리는 경로가 확인돼 제목의 개행을 공백으로 접었고, 본문 쪽은
      **실효 방어가 시스템 프롬프트의 「이건 데이터다」 선언**임을 문구에서 낮춰 적었다(확정된 것과
      아닌 것을 붙여 적지 않는다). 폭발 반경은 자기 분석 결과뿐이라(툴·유출 경로 0) 이스케이프·랜덤
      구분자는 툴·유출 경로가 붙는 날의 몫이다.
- 📍 **목록 라벨의 근본 처방(PR-2에서 넘어온 숙제)** — `NoteRow.preview`(본문 첫 줄 80자)를 서버가
      실어 준다. 제목을 안 적는 것이 기본 사용법이라, 이게 없으면 목록이 거의 전부 「제목 없음」이었다.
- [x] ~~**PR-4 홈에도 탭 (2026-09-11) — 사용자 번복.**~~ **2026-09-15 탭은 걷었다 — 필기 = 홈, 백지노트 = `/study/recall`**
      (아래 「공부 홈 집중 모드」 절). 이 항목의 기록은 경위로 남긴다. 위 PR-2의 「홈엔 필기 탭 없음」을 실사용 후 뒤집었다:
      홈에서 필기에 닿는 길이 `홈 → 빠른 이동 「일정」 → /study → 날짜 클릭 → 필기 탭`이라
      **「공부 도중에 그때그때 쓴다」는 요구가 반쯤 죽었다 — 타이머가 도는 화면은 홈이다.**
      탭줄·`role="tabpanel"`·`page` 상태를 `RecallPanel`로 옮기고 `DayPanel`은 패널만 싣는다
      (홈 `RecallCard`는 한 줄도 안 고쳐도 같은 구조를 얻는다).
- 📍 **`defaultBookId` 파생도 패널 안으로 내렸다** — 「그날 일정이 가리키는 첫 책」을 두 부모가 각자 파면
      홈과 달력이 조용히 갈린다. 규칙이 한 곳이라 이제 구조적으로 같다.
- 📍 **탭↔패널 id는 `useId()`** — 고정 `study-day-page`는 더 이상 「day」 패널이 아니고, 한 화면에 패널이
      둘이면 `aria-controls`가 엉킨다. 접근성 관계(tablist → tab + `aria-controls`/`aria-selected` →
      tabpanel + `aria-labelledby`)는 그대로다.
- 📍 **홈 카드 pill 「백지복습」 → 「공부 노트」** — 카드가 탭 둘을 담는 순간 pill이 화면과 어긋난다
      (필기 탭인데 머리는 「백지복습」). 탭이 사실상 제목이므로 pill은 둘을 아우르는 이름으로 물러난다.
- 📍 **검증**: RED → GREEN, 새 테스트 3건(홈 카드 탭 렌더·전환·기본 책), 프론트 전체 **762건** 초록.
      돌연변이 2종(탭줄 통째 삭제 → 홈 3 + /study 4 사살 · 파생을 `null`로 → 홈 1 + /study 1 사살)이
      **양쪽 화면에서** 죽었다. ⚠️ 두 번째는 처음에 홈에서 **살아남았다** — 픽스처의 「오늘 일정의 책」이
      하필 서재 첫 책이라 「일정을 봤다」와 「그냥 첫 책」이 같은 값이었다(2026-09-03 원칙 「판별력을 죽이는
      것은 단정보다 데이터」의 실측 재현). 번들 `study.js` +191B · `dashboard.js` +8,865B(홈이 `NotesPanel`을
      새로 싣는다).
- ✅ **목록을 공부 바로, 편집기는 카드 폭 전체로 (2026-09-17)** — 설계 md(gitignore)는 구현 완료 후 지웠다 — 결정 D1~D10·실측 원장은 PR #1162·#1164 본문이 단일 출처다.
      홈 필기 카드의 오른쪽 목록 열(330px)을 걷고, 지난 필기로 가는 길을 셋(공부 바 「필기」 · 제목 줄 「최근 필기 ▾」 · 빈 새 필기의
      「이어 쓰기」 칩)으로 바꾼다. **편집은 홈 한 곳** — 두 화면에서 같은 장을 고치면 409 잠금으로 자기 글을 잃는다(D1).
  - [x] **PR-1 필기 화면 + 공부 바 메뉴 (2026-09-17)** — 바 「일정」 다음 「필기」(아이콘 `notes`, `RailNav.Key.NOTES`) →
        `/study/notes`(같은 셸, `StudyNotesApp.vue`). 책별 **읽기 전용** 목록이고 행 = `/?note=<id>` 링크, `?bookId=`는
        내 서재에 있을 때만 믿는다(`notesBookParam`). 이 PR만으론 홈이 `?note=`를 아직 안 읽는다 — 홈 옛 목록이 남아 길은 안 끊긴다.
  - [x] **PR-2 홈 카드 개편 (2026-09-17)** — 목록 열·책 select 제거(첫 줄은 책 이름), 제목 줄 `[제목][최근 필기 ▾][＋ 새 필기]`
        (팝오버 5장 + 「전체 필기 보기」, 닫힘 = 고르기·Esc·바깥 탭 — 아이패드 사파리 대비 `click` 아닌 `pointerdown`), 빈 새 필기에만
        「이어 쓰기」 칩 3장(`RecallEditor` `empty` 슬롯 — 절대 배치라 높이 불변), `?note=` 소비(홈이 읽고 `replaceState`로 지운다,
        다른 책의 장이면 그 책으로 옮겨 연다). `?note=`는 **한 번만** 쓴다 — 모드 전환 때 비운다(안 비우면 독서↔공부 왕복마다 그 장이
        다시 열렸다, 리뷰 I-1).
        브라우저 실측 U-1~U-9 충족(2026-09-17 로컬, 수치는 PR #1164 본문) — 13인치 가로 기준 편집기 약 716 → 1064px(1920px 실측에서
        환산). ⚠️ **남은 것: U-10 아이패드 실기기**(바깥 탭 닫기 · 칩 탭 → 키보드·캐럿 · 체감 폭) — 로컬 서버를 못 열어 ABANDON했고,
        배포 후 사용자 확인 대기다.
...
### 공부 홈 집중 모드 — 필기·백지노트 분리 + 합침 + 독서등 (PR-1 분리 ✅ · PR-2 합침 ✅ · 후속 수정 ✅)
...
- [x] **PR-1 분리 (2026-09-15)** — ①② 담당.
  - 백지노트 = 오른쪽 바 「일정」 아래 새 칸 「백지노트」(아이콘 `note`) → 전용 화면 `/study/recall`(같은 셸·같은 번들,
    `StudyRecallApp.vue`, 옛 홈 `RecallCard` 로직 이관). `?date=YYYY-MM-DD`로 그 날짜를 연다(형식 오류·없음 = 오늘).
  - 일정 화면 하루 패널 = 편집기 대신 링크 한 줄(「오늘의 백지노트」/「이 날의 백지노트」) — 같은 날 백지노트를 두 화면에서 쓰지 않는다.
  - `RecallPanel`의 [필기]/[백지노트] 탭 제거 — 백지복습 본문만 남는다.
...
- [x] **후속 수정 — 편집기 안내문구가 「1.」 위에 겹치던 것 (2026-09-16)** — 사용자 태블릿 실측 제보.
  안내문구 표시 조건이 Tiptap `isEmpty`였는데 그건 「글자가 없다」는 뜻이라, 목록 버튼이 만든 빈 항목
  (`<ol><li><p></p></li></ol>`)도 empty로 세어 안내문구가 걷히지 않고 리스트 마커와 겹쳐 그려졌다.
  빈 제목·인용·체크목록, `1. `를 손으로 친 입력규칙 경로까지 같은 뿌리. 조건을 **「글자가 없고 블록이 문단뿐」**
  으로 좁혔다 — 문단 *하나*로 좁히지 않는 것이 요점(목록을 되돌리면 `trailingNode`의 빈 문단이 남아 둘이 된다).
  `RecallEditor.test.ts` 4건(빈 편집기 양성 대조 · 목록 · 제목 · 되돌리기). 필기·백지노트 두 화면이 같이 고쳐진다.
- [x] **후속 수정 — 제목이 본문과 구별되지 않던 것 (2026-09-16)** — 사용자 태블릿 실측 제보.
  `h1 19px / h2 16px`인데 본문도 16px이라 **H2는 본문과 크기가 완전히 같았고** H1도 1.19배였다. 게다가 본문
  고운돋움은 400 단일 face라 제목의 굵게가 **합성 볼드**여서 굵기 축도 죽어 있었다. 세 축으로 갈랐다 —
  크기(26px / 19.5px = 1.63배 / 1.22배) · 위 여백(20px / 14px, 첫 자식은 0) · 서체(고운바탕 700 실 face).
  계측기 `editorHeadingScale.test.ts` 4건은 **음성 판정 전용**(「본문과 같아지지 않았다」까지만 증명).
  서체가 실제로 붙는지는 폭 실측으로 확인했다(184.5px vs 폴백 serif 203.4px — `fonts.check`는 구글 폰트의
  한글 subset 분할 때문에 로드됐는데도 false를 주므로 계측기로 쓸 수 없다).

### 측정 대상 책 「고르기 · 바꾸기 · 붙이기」 개선 (R2) — ✅ 완료·출시 (2026-09-26 — 서버·웹 라이브, 미니앱 번들 `20260925-164` 라이브)
> 책 없이 잰 측정은 종료 직후 창을 닫으면 **다시 책을 붙일 자리가 없었고**(세션 id가 stop 응답에만 있다), 잘못 붙은 책을
> 바로잡는 문도 없었다. 고치는 단위는 **측정 한 건**이다(사용자 결정 2026-09-24): 기록 화면(웹·미니앱)이 날짜를 펼치면
> 측정 한 건씩 「시각 범위 · 시간 · 책」 한 줄이 서고 거기서 [책 붙이기]/[바꾸기]를 누른다. 독서·공부 모두 같은 모양.
> 설계 요약은 PR-1(#1196) 본문. 배포 순서 제약: **서버가 먼저**(PR-3·4·5가 새 필드·새 문을 쓴다).
> 「기록에서 붙일 수 있다」는 창 힌트 문구는 그 기능이 실리는 PR(PR-3·PR-5)에만 둔다 — 사이 기간에 화면이 거짓말하지 않게.

- ✅ **PR-1 서버** — 끝난 측정 한 건의 책을 붙이고·바꾸고·떼는 별도의 문 `POST /api/sessions/{id}/book`·`/api/study/sessions/{id}/book`
  (`tag-book`·`active/book`의 가드는 한 글자도 안 건드림 — 라이브 번들이 부르는 문). 자정 분할 조각은 **양방향**으로 함께 고친다
  (이음매 = 유저 TZ 자정 · 이웃 = 고치기 전과 같은 라벨 · 수동 기록은 걷지 않음 — 과거 날짜 수동 기록은 전부 00:00 앵커라 시각 인접이 링크가 아니다).
  본문은 `{}`(키 없음 → 400)와 `{"bookId":null}`(떼기)을 가른다. `/api/history`·`/api/study/history` 일자 레코드에 `sessions[]`
  (id · 유저 TZ `HH:mm` · 초 · 책 · manual — 수동 기록은 시각 null, 실측 먼저 정렬). `TimerState`에 `wantToReadBooks` 추가.
  `ReadingSession.tagBook`에 진행 중 거부(P17 — 공부와 같은 가드 순서). README·javadoc의 「측정엔 책 필수 / 레거시」 기술 정정
- ✅ **PR-2 웹 홈** — 기본 책은 읽는 중만 · 측정 중 [책 바꾸기] · 시트 문구·시트 안 오류
  기본 칩·여백 카드가 **읽는 중 안에서만** 고른다(최근 잰 책이 다 읽은 책이면 읽는 중 첫 책, 읽는 중 0권이면 빈 상태 — `allBooksOf` 삭제).
  측정 중 패널에 [책 바꾸기] → 같은 시트의 `change` 모드 → `POST /api/sessions/active/book`(성공하면 고른 책 `pickedBook`을 비워
  종료 뒤 칩이 서버 recent를 따른다). 책 없이 재는 중엔 「책 없이」 + 누적 줄 숨김(P11). 태깅·교체 실패는 **시트 안** 오류 줄
  (`.book-sheet-error` — 페이지 알림은 딤 뒤에 가려졌다, 독서·공부 모두), 태깅 성공 뒤 대시보드 재조회(P10), 담기 실패를 삼키지 않음.
  시트 문구: start 「책을 고르면…」(P14) · tag 힌트는 중립(「나중에 정해도 괜찮아요」 삭제 — 기록 안내는 PR-3) · `assign` 모드는
  PR-3 기록 화면용으로 미리(제목이 책 유무로 갈림, 수동 기록은 `noneAllowed=false`로 「책 없이」 없음)
- ✅ **PR-3 웹 기록** — 기록에서 측정 한 건씩 책 붙이기·바꾸기(독서·공부) + 태깅 힌트 「기록에서 붙일 수 있어요」
  `/history`·`/study/history`의 날짜 줄이 `sessions[]`가 있으면 펼침 버튼이 되고, 펼치면 한 건 = 한 줄
  「시각 범위 / 길이 · 책 · [책 붙이기]|[바꾸기]」(수동 기록은 시각 대신 「직접 기록」, 60초 미만은 「1분 미만」, 책 없으면 「책 없음」).
  버튼 → 독서는 `BookPickSheet`(assign, 수동 기록엔 「책 없이 두기」 없음), 공부는 `StudyBookSheet`(assign — 공부 서재는
  화면이 마운트 때 받아 둔다) → `POST …/sessions/{id}/book` → 기록 재조회(자정 조각 두 날짜가 함께 맞는다). 실패는 시트 안.
  `history.html`에 `_csrf` 메타(없으면 POST가 조용히 403 — `HistoryControllerTest`가 토큰으로 CSRF 통과·엉뚱한 토큰 403까지 잰다).
  태깅 시트 힌트 「건너뛰어도 「독서 기록」/「공부 기록」에서 붙일 수 있어요」. `history.js` 71,272 → 84,922 B(+13.3KB — 시트 import)
- ✅ **PR-4 미니앱 홈** — 태깅·교체 후보 세 상태 · 측정 중 [책 바꾸기] 문 · 시트 오류 · 캐러셀 선택 정규화
  태깅·교체 시트 후보 = `taggableBooks`(읽는 중 → 읽고 싶어요 → 다 읽음, id 중복 제거, 행 오른쪽 상태 라벨) — 종료 게이트도
  이 수로 센다(읽고 싶어요만 가진 사람에게도 시트가 뜬다, P8). 측정 중 홈에 [책 바꾸기] 문이 둘 — 독서 「읽는 중」 카드 제목 밑,
  공부 히어로 [회당 시간 바꾸기] 옆(책 없이 재는 중에도 선다) — 둘 다 기존 교체 시트를 연다(토스트는 5초 뒤 사라져 문이 못 됐다).
  태깅 실패는 시트 안(P7), 독서 태깅 성공 뒤 대시보드 재조회(P10). `timerStartBookId`를 홈으로 옮겨 캐러셀·여백 문도 같은
  정규화를 쓴다(P6-a), 0권에서 「책 없이」를 고른 뒤 첫 책이 생기면 선택을 푼다(`homePickAfterShelf`, P6-b). 목 모드의 독서
  `tag-book`이 마지막 종료 측정의 초를 옮기고 읽고 싶어요 → 읽는 중 전환을 흉내 낸다(P22). 기록 탭 펼침·힌트 문구는 PR-5
- ✅ **PR-5 미니앱 기록** — 기록에서 측정 한 건씩 책 붙이기·바꾸기(독서·공부) + 힌트 문구
  기록 탭(독서·공부)의 날짜 줄이 `sessions[]`가 있으면 펼쳐지고(책 없는 날도 — 거기가 붙이는 자리다), 펼치면 한 건 = 한 줄
  「시각/시간 · 책 · [책 붙이기]|[바꾸기]」(`SessionLines` — 모든 줄이 한 격자를 나눠 써 칸이 줄마다 같은 자리에 선다. 수동 기록은
  「직접 기록」, 60초 미만은 「1분 미만」, 붙일 후보 0권이면 버튼 대신 「서재에 책을 담으면 붙일 수 있어요」). 옛 책별 합산 줄
  (`BookLines`)은 대체. 버튼 → 교체 시트(`ChangeBookSheet`)를 물음·부제만 바꿔 재사용(수동 기록 줄은 「책 없이」 숨김) →
  **시트를 연 순간 고정한 원장**(`assigning.study`)의 문(`/api/sessions|study/sessions/{id}/book`)으로 보낸다(두 원장은 id가 겹쳐
  서버가 못 막는다). 성공·독서/공부 종료·태깅 뒤 기록을 다시 받아(`historyEpoch`) 기록 탭에서 재고 끝내도 그 줄이 바로 선다.
  문구: 태깅 시트 힌트 「건너뛰어도 기록 탭에서 붙일 수 있어요」 · 「책 없이」 부제 「끝난 뒤 기록 탭에서 붙일 수 있어요」.
  목 모드 날짜 기록을 측정 줄에서 유도하고 두 새 문을 흉내 낸다. PR-4 리뷰 사소 지적 F1~F4(테스트 잠금 3 · 목 `active/book` 순서) 보강
- ✅ **테스트 푸시** — 미니앱 번들 `20260925-164`(PR-4+PR-5) 업로드·테스트 푸시(deploymentId `01a0d836-a5bc-78e6-bc72-8dfcb96ec045`)
- ✅ **실기기 A-9 충족** — 2026-09-25 19:59 사용자 폰: 기록 탭 9/21 펼침 8줄 「시각 범위/길이 · 전쟁과 평화 4 · 바꾸기」가 한 격자로 정렬, 넘침 없음
- ✅ **스토어 스크린샷 재촬영** — 04-history(+10 배너)만 바뀜: 잔디 + 오늘(측정 두 건 · 하나는 「책 없음」 · [책 붙이기])을 펼친 구도(사용자 결정 「잔디도 살리기」, 목 픽스처 오늘 두 건 — `miniapp/screenshots/README.md` 2026-09-25 항목)
- ✅ **후속 소PR — P9·P18·PR-3 리뷰 N1** — 웹 독서 시작이 404(다른 탭에서 지운 책)면 `conflict`로 알리고 재조회하며 고른 책을 버린다(P9) ·
  서버 `/api/sessions/start`가 책 조회 전에 진행 중 독서·공부를 먼저 봐 「재는 중 + 낡은 bookId」를 404가 아니라 409로(P18, 공부 start와 같은 순서) ·
  기록 화면 경주 가드 3곳(독서 성공·독서 catch·공부 catch)을 잠그는 테스트(N1)
- ✅ **콘솔 · 검수 · 출시** — 콘솔 스토어 스크린샷은 세로 8장뿐이라(가로 배너 칸 없음) 4번(기록)만 교체 → 비게임 체크리스트 대조 →
  2026-09-25 검수 요청 → 승인 → [출시하기](사용자) — 라이브 `20260925-164`(`bundle_get_live_version` 확인, 2026-09-26)

### 🧯 트러블슈팅 단일 파일 → 분할형 이관 (🔜 PR 올림, 2026-09-25 — `~/.claude` 검사기 `legacy:` PR 머지 후 머지)
> 810KB(LF 기준) 단일 파일이 Read 한도를 넘고 목차가 손 관리라 썩던 것을 글로벌 분할형(항목 1건 = 파일 1개 + 자동 목차 + 검사기)으로 옮긴다.
- ✅ 변환 스크립트 `scripts/migrate-troubleshooting-split.ps1` + 테스트(픽스처 · 실데이터 왕복·건수 · 돌연변이 3종)
- ✅ `claude-docs/troubleshooting/T-###.md` 253개(헤딩형 126 · 표 행형 127, 번호 보존 · 결번 T-024~026) · 허브 경로 유지(자동 목차) · 트래커 표 동결(`troubleshooting-tracker.md`)
- ✅ 커밋 게이트 `require-troubleshooting-toc.ps1` 교체(재생성 자동 add · 형식 INVALID 차단 · 옛 형식 줄 차단) · 옛 목차 스크립트 삭제
- ✅ CLAUDE.md 「🧯 트러블슈팅 활용」 절 교체
- ✅ 전역 재발 스캐너 「승격 표시 드리프트」 19건 판정 → `promoted:` 19건 소급(T-254는 리뷰에서 비근거 → 근거로 정정) · 2026-09-25
- ⏸ 30일 뒤 변환 스크립트·테스트 삭제(그때까지 리베이스·되돌리기 도구) · 트래커 44군 → `guard:`/`promoted:` 소급(사람 판정, 별건)

### 미니앱 「Soft UI」 재테마 — 연령 반영 · 부푼 면 (완료 ✅ 2026-09-24 — 번들 161 출시, 후속 163 출시)
> 「또렷한 연필」의 뒤를 잇는 재테마. 규칙: 본문 15~16px · 보조 14 · 최소 13 · 대비 4.5:1 이상 · 세이지 주색에
> 파스텔은 포인트만 · 카드는 선이 아니라 **부푼(PUFF)/눌린(DENT) 면** · 연필테는 전부 걷는다.
> **출시**: PR-1~5 전체 = 번들 `20260924-161`(2026-09-24 14:01 라이브, 스토어 스크린샷 8장 교체) ·
> 홈 목표 캡션 = 번들 `20260924-163`(같은 날 16:19 라이브, 스크린샷 1번 교체). 콘솔 `bundle_get_live_version` 실측.
> **설계 문서**: 작업용 md(`claude-docs/plans/2026-09-2{3,4}-*`, gitignore)는 메인 체크아웃의 같은 폴더에 로컬로만 남아 있다(이 PC 한정).
> 설계 요지는 각 PR 본문(`git log --grep='Soft 재테마'`, `--grep='목표 캡션'`)과 아래 항목에 흡수돼 있다.
> 시안 비교(톤 조율 A·B·C · 선택 표시 S·H·P · 목표 칸 A·B·C)는 claude.ai 디자인 캔버스에 남아 있다.
> **실기기 원장**: 각 번들을 사용자 폰으로 테스트 푸시한 뒤 사용자 지시로 머지·출시했다 — 아래 ⏳였던 항목(스크롤·iOS 합성 볼드·메달·
> 후광 판독·글자 손잡이 판독)은 **항목별 회답 없이 종결**했다(PR-5·목표 캡션 PR 원장에 `ABANDON`으로 기록). 문제가 보고되면 새 항목으로 연다.
...
- ✅ **PR-2 「홈 · 게스트 홈 · 목표 달성 메달」** — 히어로를 부푼 면(r30)으로, 머리 줄(머리말 16 + 눌린 트랙 위 모드 세그먼트 44px) ·
  눌린 게이지 트랙 + 세이지 그라데이션 · 2열 타일(버터 「남은 시간」 / 옅은 세이지 「하루 목표」 + 1.5px 「변경」) · 캐러셀 고른 칸 3px 링 +
  체크 배지(안 고른 칸 .7 — 공용 `BookCarousel`이라 서재·공부 서재에도 적용) · 계정 헤더 28/15 · 피드 탭 알약 틴트 · 목표를 **넘는 순간**(`crossedGoal`) 히어로 속이 메달 화면 —
  `.medal-pop` 1회(transform·opacity만, 감속 설정 정지) · 게스트 체험 종료 카드에 같은 메달 + 잔디 한 주. 독서등(밤) 히어로는
  등불 글로우 `!important`로 낮 그림자의 흰 테를 막는다(PR-1 리뷰 경고). 시안의 글자 올라오기(rise)는 글자 감싼 요소라 옮기지 않았다
- ⬜ **발견(작음)** — 독서등(밤) 피드의 「다 읽음」 배지가 `--adaptiveBlue800`(밤 미정의 → 낮값) 바탕에 밤 grey100 글자라 거의 안 읽힌다(PR-2 목 모드 스크린샷에서 발견 · 이 diff 밖 · 배지 정리 때 함께)
- ✅ **PR-3 「기록 · 목표 · 서재」** — 기록 스탯 타일 셋(버터 「연속」 · 부푼 면 축소판 둘) · 잔디 카드 + 오늘 칸 두 겹 링(오늘 = 판의 가장 늦은 날짜 `latestDate`) ·
  달마다 눌린 날짜 쟁반 + 16px 트랙 위 토큰 그라데이션 막대(공부 기록은 파랑) · 목표 휠 부푼 카드 + 안개를 카드 면색으로(회당 시간 시트도 함께) ·
  서재 손잡이 줄(부푼 채움 / 1.5px 실선, 높이 38 불변) · 여백 박스 부푼 면 · 관리 시트 행 `SOFT_ROW`. 월 쟁반을 부푼 카드로 다시 감싸지 않은 것은 의도(하루 막대 칸 보존)
  - PR-2에서 넘긴 판정 **결론: 2px 유지 · 옅은 면 없음** — 1.5px은 DPR 3 크롬에서 computed 1px로 떨어져 캔버스 위 「책 추가」 점선이 흐려지고 밤 카드 위에선 거의 사라졌다.
    `#F3F6F1` 면은 캔버스·카드 위에서 투명과 구별이 안 되고 밤 카드 위에선 밝은 상자가 된다(목 모드 390×844@3, 세 바탕 × 네 조합 나란히 실측)
  - ⬜ 발견(작음): 스탯 타일 두 개의 그림자는 시안 리터럴이라 밤(독서등)이면 흰 하이라이트가 새어 나온다 — 지금 닿지 않는 이유는 `lampOn`이 `tab === 'home'`일 때만 참이라서다(`App.tsx` `lampOn`. 측정 중 탭 잠금은 부차적). 독서등을 기록 탭으로 넓히는 날 함께 토큰으로. 같은 리터럴이라 **공부 모드에서도 그림자 틴트가 세이지로 남는다** — 틴트 차가 미세해 수용한 대가다(홈 2열 타일과 같은 처지)
- ✅ **PR-4 「책방 · 여백 · 대화 · 설정 · 공부 화면 · 시트」** — 연필선 9곳을 Soft로(책방 헤더 손잡이 2 · 정사각 버튼 · 신고 select → `SOFT_OUTLINE`,
  성향 카드 · 신고 패널 → 부푼 면, 대화 입력줄 → `DENT`, 공부 서재 「회독 +1」 → 독서 서재와 같은 부푼 채움 · 「관리」 `SOFT_OUTLINE` · 「검색해서 담기」 옅은 모드색 면 · 시트 행 `SOFT_ROW`) ·
  반복 행은 그림자 없이 옅은 실선 행(`UserList` · 대화함 · 둘러보기 카드 · 보관함 카드) · 말풍선은 면 색만(내 것 옅은 세이지 / 남의 것 눌린 바탕) ·
  여백 게시판 부푼 면 + 행 구분선·탭 밑줄 토큰화(배경 6색은 내용의 색이라 그대로) · 공부 일정 달력을 부푼 카드 한 장에(SE에서도 칸 45px) ·
  설정은 무변경 검수 — 위험 버튼이 Soft 버튼 규칙 키에 안 걸리는지 **규칙 매칭 자체**를 재는 가드 신설. 그림자 예산 가드 `bigShadow`(변수 경유 + blur ≥ 8 리터럴 둘 다)
  - ✅ 발견(PR-5 판정용 → **PR-5에서 반영**): 설계 완료 기준 `querySelectorAll('[style*="border-image"]').length === 0`은 **원리상 0이 안 된다** — 크롬이 `border: none`/`0` 단축 속성을
    `border-image: none;`까지 펼쳐 직렬화한다(책방 20 · 일정 66 등 전부 `none`). PR-4 화면의 실측은 `getComputedStyle(e).borderImageSource !== 'none'` 요소 수로 했고 전부 0.
    PR-5 원장(U-10)은 이 계측으로 바꾼다
  - ⬜ 발견(작음): 닉네임 상한이 30자라 긴 닉네임은 둘러보기 카드·사용자 목록에서 두 줄로 접힌다(잘림·넘침 없음 · 이 PR 전과 같은 동작)
- ✅ **PR-5 「탭바 · 연필 철거 · 스크린샷」 — 출시(번들 161)** — 탭바 부푼 알약(r999) · 높이 56 → 64 ·
  고른 칸 알약 40×28 · 잠긴 칸은 흐림 대신 16px 자물쇠 배지(게스트 셸 포함) · 타이머 원 54(측정 중은 빨강 그대로 눌린 원) ·
  토스트·스트립 부푼 면(r20) · 밤엔 `body.reading-lamp`가 `main` 밖 그림자를 따로 준다 · `PENCIL_FRAME`·`--pencil-frame*` 철거 ·
  가드(소스 전수 연필 0 · 그림자 예산 허용표 · 밤 body 흰 빛 0) · 스토어 스크린샷 9장 재촬영. 테스트 번들 158·159·160을 사용자 폰으로 확인 →
  머지 → main 번들 161 검수·콘솔 스크린샷 8장 교체(사용자 확약) → 2026-09-24 14:01 출시. 원장 U-1(스크롤)·U-4(iOS 볼드)·U-5(메달)는 항목별 회답 없이 종결(위 절 머리)
  - ✅ PR-2에서 넘긴 판정 **결론: `SOFT_OUTLINE` 1.5px 유지** — DPR 3 크롬의 computed `1px`는 크롬이 테두리 폭을 내려 맞춘 값이고(반올림
    규칙 자체는 미확인), 이 선은 카드 위 **실선**(blue700/grey100 대비 7.36)이라 1px로 그려져도 목 모드 스크린샷(홈 「변경」·서재 「관리」·
    스토어 03·07)에서 또렷했다. PR-3이 2px로 올린 것은 **점선**(캔버스 위 흐린 회색)이라 사정이 다르다. 실기기 확인은 U-4 촬영 때 곁눈으로
  - ✅ 목표 휠 선택 띠 안 밝은 줄(리뷰에서 PR-5로 편입) — 원인: TDS 휠이 안개를 높이의 42%로 잡아(180 → 75.6px) 가운데 칸이
    28.8px인데 우리 띠는 44px라, 안개 끝이 띠 위 7.6px · 아래 5.6px를 카드 면색 .45로 덮었다(PR-3이 안개를 캔버스색 → 더 밝은 카드 면색으로
    바꾸며 눈에 띄게 됐다). 수정: 안개 그라데이션을 띠 가장자리에서 투명으로 끊음(목표 화면·회당 시간 시트 둘 다, 실측 경계 일치) +
    `goal.test`가 두 화면의 높이·띠 값에서 겹침을 다시 계산해 css와 대조
  - ✅ 옛 팔레트 날 리터럴 정리(리뷰) — 서재 격자 고른 칸 테두리·점 · 검색 담긴 행·도장·칩·체크 · 돋보기 · 교체 시트 「책 없이」 점선을
    Soft 토큰으로(공부 모드에서 세이지가 남던 자리). 제품 .tsx 전수 가드 신설(`var()` 폴백 뒷값만 허용)
  - ⬜ 발견(작음): 스토어 04는 펼친 날 맨 아래 「그날 목표 30분」이 탭바 아래로 내려갔다(잔디 제목 → 그 줄 634px · 탭바 위 526px) —
    잔디를 지켰다. 다시 보여야 하면 책 두 권인 날을 펼치도록 촬영기를 고친다(`screenshots/README.md` 09-23 항목)
  - ⬜ 발견(작음): 게스트 체험 종료 카드 위에 서는 「기록을 남길지 먼저 정해 주세요」 스트립은 같은 면색 카드 위라 경계를 그림자 하나가 맡는다(읽히지만 옛 검정 그림자보다 옅다)
  - ✅ **톤 조율 A 「따뜻한 Soft」(2026-09-24, 같은 PR에 커밋 추가)** — PR-5 테스트 번들을 실기기로 본 사용자 판정 「쨍하다 · 색도 글꼴도 안 어울린다」 →
    조율안 A·B·C(디자인 캔버스 「톤 조율안」 줄) 중 A 선택. 구조(부푼 면·큰 글자·알약 탭바)는 그대로,
    색 온도를 따뜻한 오트 한 벌로 · 손글씨 제거(글꼴 두 벌) · 진한 채움은 탭바 원 하나 · 그라데이션 막대 단색. 가드 `warm-palette.test` 신설.
    PR-3 발견(스탯 타일 그림자 리터럴)의 **틴트 절반은 해소**(`--tileShadow` 토큰 — 공부 모드도 같은 따뜻한 틴트가 의도), 흰 하이라이트 리터럴은 그대로라 밤 확장 때 함께.
    실기기 색 재현(오트 바탕이 OLED에서 노랗게 뜨는지) · iOS 합성 볼드 · 스크롤 · 메달은 번들 159로 사용자가 봤고(판정 「훨씬 낫다」) 항목별 회답 없이 종결.
    진한 세이지가 남은 자리: 탭바 원(주 동작) · 목표 달성 체크 원 ·
    홈 피드 「다 읽음」 칩(`HomeFeed.tsx` `badgeStyle('solid')`, `--adaptiveBlue800` 채움 — 리뷰가 찾았다, 첫 실측은 `#3F5A3C` 일치만 세어 놓쳤다).
    (캐러셀 체크 배지는 아래 H안에서 밝은 원이 됐다)
  - ⬜ **사용자 판단 대기 — 홈 피드 「다 읽음」 칩을 옅게 할지**: 출시 후에도 진한 세이지 채움 그대로다. 출시 보고 때마다 알렸고 아직 답이 없다.
    옅게 한다면 위 PR-2 발견(밤 「다 읽음」 배지가 `--adaptiveBlue800` 낮값이라 거의 안 읽힘)과 한 번에 정리한다
  - ✅ **캐러셀 선택 표시 H 「후광」(2026-09-24, 같은 PR에 커밋 추가)** — 번들 159(톤 A) 실기기 판정 「훨씬 낫다, 다만 고른 책의 진한 초록
    테두리가 Soft와 안 어울린다」 → 시안 S(크기만)·H(후광)·P(받침)(디자인 캔버스 「책 선택 표시」 줄) 중 H 선택.
    링 = 카드색 틈 3 + Blue500 테 3(blur 0 두 겹) · 칸·표지 모서리 8(캐러셀만 — `radius` prop) · 배지 = 카드색 원 + Blue500 테 + Blue700 체크 22px ·
    `TRACK_V_PAD` +7. 게스트 홈 잔디 링·`TODAY_RING`은 뜻이 달라 그대로. 후광이 「테두리」로 안 읽히는지(설계 U-1) ·
    실제 사진 표지 위 배지 가독성(U-2)은 번들 160을 사용자가 보고 「머지해」 — 항목별 회답 없이 종결
- ✅ **홈 히어로 「남은 시간 타일 + 목표 캡션」(B안, 2026-09-24 — 번들 163 출시)** — 사용자 지적 「목표 시간을 이렇게 크게 보여줄 필요가 없고, 변경은
  중요한 버튼처럼 강조될 필요가 없다」. 목표 1시간 이상이면 「01:00:00」 + 알약이 149px 열에 안 들어 알약이 떨어지며 두 타일이 137px로
  부풀었다. 시안 A·B·C(디자인 캔버스 「하루 목표 칸」 줄) 중 B: 남은 시간만 가로 버터 타일(60px),
  목표는 그 밑 캡션 한 줄 「하루 목표 1시간 · 바꾸기 ›」(14px 회색 + 글자 손잡이, 터치 44), 초과분은 같은 줄 오른쪽, ⓘ 캐럿은 라벨 밑 41px 고정.
  히어로 363 → 320. 그림자 예산 Home 2 → 1. iOS 합성 볼드 「바꾸기」(U-1) · 글자 손잡이가 버튼으로 읽히는지(U-2)는 테스트 번들 162 푸시 후
  사용자 「머지해」로 종결(원장 `ABANDON`) — 버튼으로 안 읽힌다는 보고가 오면 같은 자리를 `SOFT_OUTLINE` 작은 알약으로 바꾼다(배치 그대로)
  - ⬜ 발견(작음, 리뷰 · 이 PR 전부터): 「남은 시간」 ⓘ 버튼이 `padding: 0`이라 터치 높이 23px — 44 기준 미달. 라벨 줄 높이를 안 바꾸고
    히트 영역만 넓히는 방법(음수 마진 + 패딩)으로 고칠 수 있다
...
- [x] **③ 「또렷한 연필」 타이포·위계 시스템 (시안 2a~2e) — 완료 ✅ 2026-08-24 (4 PR: A 토대 #944 → B 홈·피드 #946 → C 서재·기록·목표 #947 → D 탭바 #948)**
      ...
      ⚠️ **이 「개구 기준」 경고는 이제 미니앱에만 해당한다** — 웹은 2026-09-03에 같은 전환을 끝내 개구를 안 쓴다
      (아래 「웹 본문 타이포」 절). 웹의 `112.5%`만은 **개구와 무관한 사용자 확정 배율**로 남았다.
...
- ⏸ **무차별 대입 방어 보강 — 의도적 보류 (2026-09-14 결정)** — 지수 백오프 · 다중 인스턴스 대비 공유 저장소 · 앞단 WAF 레이트리밋.
      지금 해서 얻는 것이 거의 없다: 현행 `LoginAttemptService`(IP당 연속 실패 5회 → 15분 잠금, 만료 키 sweep)만으로 한 IP의
      하루 시도는 480회 안팎이고, 로그인 식별자가 비공개 `login_id`라 표적 자체를 알기 어렵다. 공유 저장소는 t3.small 한 대·컨테이너
      하나(facefit과 메모리 공유)에서 Redis를 얹는 쪽이 손해이고, WAF는 월 고정비가 붙는데 위협 신호가 없다.
      **다시 볼 트리거**(하나라도 오면 재개): ① 지수 백오프 — 로그인 실패 로그에서 같은 IP의 잠금 반복이 실제로 관측될 때
      ② 공유 저장소 — 인스턴스를 2대 이상으로 늘릴 때 ③ WAF — 분산 출처 공격이 관측되거나 트래픽이 유의미하게 커질 때.
