...
| 2026-06-09 | **밀린 부채를 헤드라인 타이머에 합산 표시 + 설정 토글(기본 ON)**(실사용 피드백) — "50분 남기고 자정 지났는데 타이머가 증가량 70분으로 리셋, 갚을 50분이 사라짐". #217 7일 윈도우 per-day의 '이월 없음'이 의도대로 동작한 것(버그 아님)이나, 사용자가 누적 부채를 원해 **헤드라인에 윈도우 미상환 부채를 합산**하도록 전환 + 설정 on/off(`ReadingTimer.debtCarryover`, V30, 기본 true — **표시 전용**, 부채 모델은 per-day 독립 유지). `DashboardModel`: ON이면 `remainingSeconds=WeeklyDebt.totalDebtSeconds()`·`carriedDebtSeconds`=floor(빠뜨린 날 합), OFF면 오늘 부채만·floor 0. 측정 차감은 `dashboard.js`가 `data-floor`에서 멈춰 서버 재계산과 일관(어제 빚은 '빠뜨린 날 채우기'로만 갚음). 설정 체크박스 + 헤드라인 보조문구 "지난 밀린 N분 포함". TDD Red→Green: ReadingTimer 2·DashboardModel 2·UserSettingsService 1·SettingsController 2·DashboardController 1 + 전체 그린. (PR #257) |
