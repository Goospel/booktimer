...
- 🔜 **스트리밍/비동기 호출 — 승격 조건이 충족됐다(2026-09-03 실측)**. 일정 생성 실호출 2회에서
  **75.7초**(3개월·주 6일 → 79일, out 10,574토큰)와 **28.9초**(1개월·주 3일 → 15일)가 나왔다.
  긴 쪽이 60s 기준을 넘고 클라이언트 타임아웃 90s와도 가깝다 — **일정만** 스트리밍이나 비동기(작업
  큐 + 폴링)로 옮기는 것을 다음 판에서 검토한다. 분석·전사는 출력이 짧아 이 문제가 없다.
  ⚠️ 타임아웃 상향은 답이 아니다(원장 U-6이 정한 규칙 — 기다리는 요청 스레드가 늘 뿐이다).
...
- [x] **1. 토스 발 미검증 이메일 계정의 구글 로그인 흡수 — 유지(동작 변경 0)** — 근거(사용자 결정): 토스가
      넘기는 이메일은 본인 확인이 끝난 토스 신원의 것이라 흡수는 「같은 사람의 두 채널 합류」이고, 반대로
      폐기하면 그 토스 사용자의 독서 기록이 사라진다. 잔여 위험은 「토스가 남의 이메일을 넘길 때」뿐이고
      수용한다(그런 사례가 확인되면 재검토). 근거를 `provision` 주석에 적고 **행동 고정 테스트 1건**으로
      못 박았다 — TOSS·이메일 미검증 기존 계정을 폐기하지 않고 그대로 반환.
