...
**합의된 방향** (사용자 결정 2026-06-05):
- **principal = login_id 전면 전환(B안)** — email은 연락/복구/OAuth 연결용 **속성**으로 강등, nickname은 공개 핸들로 유지(login_id와 별개·비공개).
- **기존 사용자 전부 wipe**(출시 전·친구뿐, 스냅샷 생략) → 백필·dual-lookup·임시값·전환 게이트 **전부 제거**(그린필드).
- **OAuth 유저는 온보딩에서 login_id 직접 선택**(자동생성 없음). 인증은 `findByLoginId`만(email 폴백 없음).
- **형식**: `a-z0-9_`, 3~20자, 소문자 정규화, 예약어 차단(표준).
- **Admin 시드**: `BOOKTIMER_ADMIN_EMAILS` → `BOOKTIMER_ADMIN_LOGIN_IDS`.
- **PR 단계**: ①스키마+도메인(V13 nullable unique) **완료 ✅ PR #146** → ②가입+OAuth 온보딩 login_id → ③인증 컷오버(wipe 선행·14곳·시드) → ④NOT NULL 강화.
  - ① 한 일: `V13__user_login_id.sql`(`login_id varchar(50)` nullable + `uk_users_login_id`), `User.loginId` 필드 + `assignLoginId`(소문자 정규화·`a-z0-9_`/3~20자·예약어 차단)·`getLoginId`. 동작 변화 없음(인증 아직 email, login_id는 null). TDD(도메인 검증 Red→Green) + FlywayMigrationTest + 전체 스위트 그린(nullable unique 다중 NULL 허용 확인).
