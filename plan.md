...
- **배너 광고(IAA) — 「여백」 두 화면** ✅ 코드 / 🔜 점등 (2026-08-25) — 세 번째 광고 포맷. 지면은 **둘**이다:
  ...
  **광고 그룹은 지면마다 따로**(`VITE_MARGIN_BANNER_AD_GROUP_ID` · `VITE_BOOK_MARGIN_BANNER_AD_GROUP_ID`) —
  ...
  `Screen`의 좌우 20px을 `margin: '0 -20px'`로 풀블리드해 되돌린다.
  **96px을 예약하고 죽으면 0으로 접는다**: 인라인이면 광고가 뜨는 성공 경로마다 게시판이 밀리고(layout shift),
  ...
  실패는 **4겹**이 전부 「자리 없음」으로 수렴한다 — 그룹 미설정·`window` 부재·`isSupported` false/동기 throw는
  렌더 전 `null`, 그 뒤 `onNoFill`·`onAdFailedToRender`·초기화 실패는 96px→0.
  SDK 초기화는 `toss.ts`의 **모듈 레벨 memoized Promise**라 두 지면이 공유해도 1회이고, 여백을 열 때만 돈다
  (App 최상위에 두면 여백을 안 여는 대부분의 세션에서도 광고 SDK가 깨어난다).
  ⏸ **점등은 후속 커밋** — 콘솔에서 배너 광고 그룹 2개를 만들어야 나오는 값이라(구글 등록 최대 2시간),
  이번 변경은 그룹 ID가 빈 채로 나가 두 지면이 꺼져 있다. 점등 시 `.env.production` 실값 2개 +
  `env-production.test.ts` 단언을 함께 넣고, 실기기에서 **card 인셋 정합**과 **탭 왕복 후 유령 슬롯 없음**을 확인한다
  (테스트 중엔 운영 ID 금지 — `ait-ad-test-banner-id`).
...
- **재참여 넛지 푸시 (7일 비활동 win-back)** ✅ 코드 완료 / 🔜 콘솔 검수·점등 대기 (2026-08-18) — **§전략 「retention
...
- **스마트 발송 현황 — 우리는 이미 절반을 쓰고 있다** ✅ 기능성 3종 운영 중 / 🔜 광고성 1종 등록 대기
  ...
  | **광고성** — 신규 유입 유도 | 🔜 이번에 도입 | **0줄**(콘솔 등록만) |
  | **광고성** — 재방문 유도 | ⏸ 보류 | 우리 7일 비활동 넛지와 대상이 겹친다. 돌리려면 서버 배치를 끄는 판단이 함께 필요 |
  ...
  📍 **소재 문안 (콘솔 등록용 · 제목 7자 · 본문 25자 · 공백 포함)** — 한 그룹에 A/B 2개까지 올릴 수 있다.

  | | 제목 | 본문 | 각도 |
  |---|---|---|---|
  | A안 | `하루 10분` (6자) | `읽은 시간을 재면 기록이 쌓여요.` (18자) | 입문자·부담 없음 — §thesis 그대로 |
  | B안 | `독서 기록` (5자) | `몇 분 읽었는지 재고 남겨요.` (16자) | 기록·성취 |

  「잔디」는 일부러 뺐다 — 앱 안에서는 쓰는 말이지만 **앱을 안 써 본 사람**이 보는 문구라 은어로 읽힌다.
  가이드가 막는 것(과장 `지금 바로`·`역대급` / 불안 조성 / 은어·밈 / 특정인 이름 / 띄어쓰기 생략 /
  「토스 + 서비스명」 조합 / 동적 변수)은 전부 피했고, 이모지도 안 넣었다(UI 규칙과 같은 선).

  ⚠️ **광고성은 기능성과 달리 알림 동의문이 없다** — 그래서 사용자가 스팸으로 체감할 여지가 크다.
  그 대신 **기존 사용자에게는 아무 푸시도 더 가지 않는다**(대상이 미사용자다). 재방문 유도를 나중에 켠다면
  그때는 위 표의 ⏸ 이유를 먼저 처리한다.

- **딥링크 착지 탭 (`?tab=`)** ✅ 코드 완료 / 🔜 콘솔 URL 반영 대기 (2026-08-25) — 푸시를 누르고 들어온 사람이
  ...
  **모르는 값은 전부 홈**이다(콘솔에 손으로 적는 URL이라 오타·옛 링크가 온다 — 그때 필요한 건 오류가 아니라 홈).
  ...
  - `CHECK:` 실기기에서 `intoss://<앱이름>/?tab=history`로 진입 / `EXPECT:` 기록 탭이 첫 화면 / `EVIDENCE:` `pending`
    — 딥링크 형식(`intoss://<앱이름>/about?name=test`)은 공유 링크 문서에 있으나 **그 쿼리가 우리 SPA의
    `location.search`까지 도달하는지는 어느 문서에도 없다**(우리는 `AppsInToss.registerApp` 파일 기반 라우팅을
    쓰지 않는 순수 Vite SPA다). 그래서 **콘솔의 이동 URL은 이 확인 전까지 바꾸지 않는다** — 파라미터가 없으면
    코드는 지금과 완전히 동일해 무해하게 잠들어 있다.
...
- **앱 아이콘 교체** ✅ 이미지 완료 / 🔜 콘솔 업로드 대기 (2026-08-14) — 「책 · 성장 · 자라남」을 축으로
...
- **기본 이모지를 제품 UI에서 걷는다** ✅ (2026-08-18, 사용자 지정)
  ...
  - 🔜 **심사 반려 — 사유가 이 변경에 안 닿는다(문의 대기)** (2026-08-18, 배포 ID `01a0140a-…`) — 반려 사유는
...
- ⬜ **웹 로그인 뒤 화면 확인** — 로그인·가입 등 비로그인 화면만 실브라우저로 훑었다. 대시보드·서재·기록·정원은
      비밀번호 입력이 필요해 미확인. 표준 테두리 64곳에 일괄 적용했으므로 대부분 따라오지만,
      **`.card`도 `border: 1px solid var(--border)`도 아닌 자체 클래스**가 있으면 그 자리만 매끈하게 남는다
      (로그인 화면의 `.auth-*`가 실제로 그랬다 — 개별 대응이 필요했다).
...
- ⬜ **실기기 확인 (남음)** — iOS WebView는 같은 px에서 데스크톱 크롬보다 굵게 그린다. 굵기를 700으로 통일했으니
      실기기에서 **너무 굵을 수 있다**. 목 모드 전 화면 + 긴 값(`10시간 30분`·긴 책 제목)까지 잘림 0으로 확인했지만,
      그건 원리상 이 클래스를 못 잡는다(T-176 계열). **다음 배포 전 체크리스트**다.
- ⬜ **스토어 스크린샷 재촬영 (남음)** — `miniapp/screenshots/` 4장이 전부 이 변경의 사정권이다.
