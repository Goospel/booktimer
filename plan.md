...
- 📍 **검증**: RED → GREEN, 새 테스트 3건(홈 카드 탭 렌더·전환·기본 책), 프론트 전체 **762건** 초록.
      ...
      **양쪽 화면에서** 죽었다. ⚠️ 두 번째는 처음에 홈에서 **살아남았다** — 픽스처의 「오늘 일정의 책」이
      ...
      새로 싣는다).
- ✅ **목록을 공부 바로, 편집기는 카드 폭 전체로 (2026-09-17)** — 설계 `claude-docs/plans/2026-09-17-study-notes-list-to-rail.md`(gitignore).
  ...
  - [x] **PR-2 홈 카드 개편 (2026-09-17)** — 목록 열·책 select 제거(첫 줄은 책 이름), 제목 줄 `[제목][최근 필기 ▾][＋ 새 필기]`
        (팝오버 5장 + 「전체 필기 보기」, 닫힘 = 고르기·Esc·바깥 클릭), 빈 새 필기에만 「이어 쓰기」 칩 3장(`RecallEditor` `empty`
        슬롯 — 절대 배치라 높이 불변), `?note=` 소비(홈이 읽고 `replaceState`로 지운다, 다른 책의 장이면 그 책으로 옮겨 연다).
        ⚠️ 로그인이 필요한 브라우저 실측(편집기 폭·칩 높이 불변·팝오버 z-order·핸드오프·아이패드)은 설계 원장 U-1~U-10으로 남아 있다.
