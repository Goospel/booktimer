...
- ✅ **PR-5 「탭바 · 연필 철거 · 스크린샷」 — 코드 완료, ⏳ 배포·실기기 대기** — 탭바 부푼 알약(r999) · 높이 56 → 64 ·
  ...
  가드(소스 전수 연필 0 · 그림자 예산 허용표 · 밤 body 흰 빛 0) · 스토어 스크린샷 9장 재촬영. **남은 것 = 머지 뒤 코디네이터 몫**:
  `deploy.sh --expect "오늘 목표를 채웠어요"` 1회 → 테스트 푸시 실기기 → 원장 U-1(스크롤)·U-4(iOS 볼드)·U-5(메달) 채움 →
  사용자 지시 후 검수 제출 → 콘솔 스크린샷 교체(사용자 확약). 그 전엔 이 절 제목의 「진행 중」을 떼지 않는다
  ...
  - ✅ **톤 조율 A 「따뜻한 Soft」(2026-09-24, 같은 PR에 커밋 추가)** — PR-5 테스트 번들을 실기기로 본 사용자 판정 「쨍하다 · 색도 글꼴도 안 어울린다」 →
    조율안 A·B·C(설계 `claude-docs/plans/2026-09-24-miniapp-retune-A.md`, gitignore) 중 A 선택. 구조(부푼 면·큰 글자·알약 탭바)는 그대로,
    ...
    ⏳ **남은 것**: 실기기 색 재현(오트 바탕이 OLED에서 노랗게 뜨는지) · iOS 합성 볼드 · 스크롤 · 메달 — 위 PR-5 원장과 함께 번들 1회로 확인.
    ...
    칩은 **안 바꿨다** — 사용자가 폰에서 보고 판단한다. (캐러셀 체크 배지는 아래 H안에서 밝은 원이 됐다)
  - ✅ **캐러셀 선택 표시 H 「후광」(2026-09-24, 같은 PR에 커밋 추가)** — 번들 159(톤 A) 실기기 판정 「훨씬 낫다, 다만 고른 책의 진한 초록
    테두리가 Soft와 안 어울린다」 → 시안 S(크기만)·H(후광)·P(받침)(설계 `claude-docs/plans/2026-09-24-miniapp-select-ring.md`, gitignore) 중 H 선택.
    링 = 카드색 틈 3 + Blue500 테 3(blur 0 두 겹) · 칸·표지 모서리 8(캐러셀만 — `radius` prop) · 배지 = 카드색 원 + Blue500 테 + Blue700 체크 22px ·
    `TRACK_V_PAD` +7. 게스트 홈 잔디 링·`TODAY_RING`은 뜻이 달라 그대로. ⏳ 남은 것: 실기기에서 후광이 「테두리」로 안 읽히는지(설계 U-1) ·
    실제 사진 표지(특히 중간 밝기 초록) 위 배지 가독성(U-2) — 다음 번들에서 확인
- ✅ **홈 히어로 「남은 시간 타일 + 목표 캡션」(B안, 2026-09-24)** — 사용자 지적 「목표 시간을 이렇게 크게 보여줄 필요가 없고, 변경은
  중요한 버튼처럼 강조될 필요가 없다」. 목표 1시간 이상이면 「01:00:00」 + 알약이 149px 열에 안 들어 알약이 떨어지며 두 타일이 137px로
  부풀었다. 시안 A·B·C(설계 `claude-docs/plans/2026-09-24-miniapp-goal-tile.md`, gitignore) 중 B: 남은 시간만 가로 버터 타일(60px),
  목표는 그 밑 캡션 한 줄 「하루 목표 1시간 · 바꾸기 ›」(14px 회색 + 글자 손잡이, 터치 44), 초과분은 같은 줄 오른쪽, ⓘ 캐럿은 라벨 밑 41px 고정.
  히어로 363 → 320. 그림자 예산 Home 2 → 1. ⏳ 남은 것: iOS 합성 볼드 「바꾸기」(U-1) · 글자 손잡이가 버튼으로 읽히는지(U-2 — 안 읽히면
  같은 자리를 `SOFT_OUTLINE` 작은 알약으로) — 다음 테스트 번들에서 실기기로
