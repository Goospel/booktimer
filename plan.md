...
- **미니앱 주소 이전 — 토스가 도메인을 옮긴다** ✅ 선반영 (2026-08-21) — 콘솔 공지로 알았다: **8/25부터
  ...
  **403 Forbidden**, 옛 오리진 200. **코드 변경은 0**: SSM `MINIAPP_ALLOWED_ORIGINS`를 **신·구 4개 병기**(v3)로
  ...
  ⬜ **옛 2개(`web`·`private-web`) 정리** — 날짜가 아니라 **라이브 번들의 SDK 버전**이 조건이다(SDK 3.0.0~3.1.0 번들이 옛 주소를
  쓴다, 콘솔 공지 53009). 2026-09-15 라이브 `20260915-151`이 SDK 3.1.1이라 **조건 충족 — 정리는 아직 안 했다**(SSM 수정 + 재배포).
  지우기 직전에 `bundle_get_live_version`의 `sdkVersion`을 다시 잰다 — 3.0.x 번들로 롤백되면 옛 주소가 다시 필요하다.
