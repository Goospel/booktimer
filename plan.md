...
- **재참여 넛지 푸시 (7일 비활동 win-back)** ✅ 코드 완료 / 🔜 콘솔 검수·점등 대기 (2026-08-18) — **§전략 「retention
  ...
  **Flyway 0건**이고, 이메일이 훗날 점등돼도 10:00(이메일) → 19:00(푸시) 순서라 같은 날은 이메일이 선점한다.
  ...
  순서 = **머지 전 SSM 2개 선생성**(⚠️ `render-env.sh` SECRET_MAP에 있는데 SSM에 없으면 배포가 `exit 1` — #722가
  같은 이유로 선생성했다) → 콘솔 템플릿 생성·검수 신청(사용자, 발송 코드 제안 `booktimer-retention-nudge`) →
  승인 후 `/booktimer/TOSS_RETENTION_{ENABLED,TEMPLATE_CODE}` 실값 갱신·재배포.
