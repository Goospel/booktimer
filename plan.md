...
- ✅ **PR-3 미니앱 (2026-09-13)** — 공부 홈의 하루 목표 게이지·「목표 정하기」·「변경 ›」·공부 새싹과 공부 목표 화면(`'studyGoal'`)을 걷었다 —
  ...
  **배포(2026-09-13)**: 번들 `20260913-148`(`01a09a8f`) 업로드 → 실기기 확인(햅틱 1회·휠 0분/6시간 초과 잠금·안드로이드 뒤로가기는 시트만, U-3) →
  검수 요청 + 스토어 스크린샷 06 교체(앱 정보 검토). 🔜 승인되면 콘솔 [출시하기](사용자).
...
- ✅ **공부 알림 동의문 분리 + 모드별 동의 카드 (2026-09-13, 설계 E20 분기)** — 콘솔 AI 검수가 공부 푸시를 독서 동의문(114526
  ...
  🔜 순서: `-148` 검수 종료(한 번에 하나만 검수) → 번들 `-149` 업로드·실기기 동의 화면 확인 → 검수·출시 → **그다음** SSM 두 값 동시
...
- ⬜ **PR-5 서버 잔재 정리** — `V92` `users.study_daily_goal_seconds` drop · `POST /api/study/goal` · `StudyState.goalSeconds` 삭제.
  착수 게이트 = PR-3 번들 라이브 확인(`bundle_get_live_version`이 `20260913-148`이어야 한다 — 2026-09-13 검수 요청 중).
