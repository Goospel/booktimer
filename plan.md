...
- ⚠️ **머지 전 확인이 남았다(I-3) — 운영 Gemini 키의 요금 티어**. `deploy-aws.md`의 「무료 티어는
      프롬프트가 모델 개선에 쓰일 수 있다」 주의문은 **6-2가 집계 수치만 보내던 시절의 위험 평가**다.
      이번 PR로 **사용자가 직접 친 자유 텍스트(주제·범위 ≤4,000자)** 가 같은 키로 나간다 — 학습 데이터가
      될 수 있는 것의 **종류가 바뀌었다.** 무료 티어면 유료(학습 제외) 전환이나 처리방침 6-2 보강이
      필요하다. 처리방침이 「Google의 데이터 처리 정책에 따릅니다」로만 적어 그 사실이 안 드러난다.
