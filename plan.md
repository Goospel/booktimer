...
| 2026-06-08 | **책BTI 견고화 — 빈 화면·지연 방어**(실사용 피드백, [[N-060]]) — "책BTI에 한 번씩 오래 걸리고, 어쩔 땐 분석이 통째로 비며, 다른 페이지 갔다 오면 다시 보임". 뿌리: `/personality`는 캐시 미스(시그니처 변동) 시점에 **요청 스레드에서 Gemini 동기 호출** 후 바로 렌더 — 느리면 지연, 실패/빈응답이면 사실만 폴백으로 빈 화면, 재진입 시 재호출 성공으로 다시 보임(세 증상 한 뿌리). 네 겹: ① **serve-stale-on-error**(재생성 실패 시 *직전 캐시(stale)* 우선 노출, 캐시를 fresh/stale/absent 3상태로) ② LLM 호출 트랜잭션 밖(`propagation=SUPPORTS`, DB 커넥션 점유 회피) ③ `RestClient` 타임아웃 5s/20s ④ Gemini 빈 출력 방어(`maxOutputTokens`+`thinkingBudget=0` — 2.5-flash thinking이 출력 예산 삼켜 빈 text 옴, 200이라 catch 못함). TDD Red→Green: stale 폴백 2·빈출력 방어 1 + 전체 그린. DB 무변경. 본문 §책BTI 갱신, N-060. (PR 예정) |
