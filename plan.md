...
- ⬜ **웹 본문 폭 재조정** — `.container` 460 기본값과 1160 페이지들이 바 여백(양쪽 124px)과 함께 어떻게 서는지 1280·1440에서
      실측한 뒤 정한다(사용자가 이번 범위 밖·후속으로 못 박음). 지금 1408px 미만 창에선 1160 페이지가 가용 폭만큼 줄어든다.
