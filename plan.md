...
### 📸 독서 스토리 — 인스타 스토리식 24h 문장 공유 (아이디어 📝, 백로그 — 밀도 신호 뒤, 2026-06-29)
...
> ⚠️ **이건 §전략 엔진 B(소셜) → "사람 밀도가 찬 뒤" 착수 (백로그).** 스토리는 **팔로워가 있어야
> 가치가 나오는 밀도 의존** 기능이다(팔로워 0 = 빈 스토리 = 0원어치). 책BTI v1 같은 "밀도 독립
> 획득 후크"와 달리 표시 대상(팔로워)이 본질이라 밀도 선행이 맞다. 아이디어로 박아두되 우선순위는
> retention 레버·밀도 신호 뒤.

> ⚠️ **구현 전 설계 먼저 (SNS 규칙 그대로)** — 코드부터 짜지 말 것. 공개범위·만료·신고 경계를
> [sns-design.md](claude-docs/sns-design.md)에 한 절로 못 박고 합의한 뒤 TDD. 노출 경계(팔로워 한정·
> 차단·비공개 누출)가 핵심이라 설계 없이 시작하면 되돌리기 어려운 결정이 굳는다.
...
- **진입점** = 책방 `/u/{loginId}`(Vue 섬 `frontend/src/profile/`)·홈(`/`) 상단. 풀스크린 뷰어는
  새 Vue 섬(`frontend/src/story/`).

**새로 필요한 것 (최소):**
- **`story` 테이블 (V53)** — `user_id`(작성자 FK)·`book_id`(nullable FK)·`text`(상한 예 500자)·
  `created_at`. 공개범위는 "팔로워 한정" 단일(전체공개 허용 시 `visibility` 컬럼). 탈퇴·책삭제 시 자식 정리(FK).
- **`Story` 엔티티 + `StoryRepository`/`StoryService` + `StoryApiController`** — 작성(`POST /api/stories`)·
  내 활성 스토리·팔로잉 피드(`GET /api/stories/feed`).
- **프론트 = 인스타 스토리 UI** — 아바타 링(미열람 강조)·풀스크린 카드·진행바·자동 넘김·좌우 탭/스와이프.
  로드순서·타이밍 얽히면 실 브라우저 검증(프론트 검증 규칙).

**열린 질문 (설계 세션에서):**
- 공개범위 = "팔로워 한정" 단일인가, "전체공개"도 허용인가(전역 카운트 철회 정신과 정합 검토).
- 조회 표시(누가 봤나 — 인스타는 작성자에게 열람자 노출)를 v1에 넣을지. 넣으면 `story_view`(V54).
- 한 번에 여러 문장/카드(인스타식 여러 장)인가 1장인가.
- 만료 스토리 물리 삭제 주기 / 미열람 추적(열람 상태 저장 위치).
...
- **페이지별 Claude Design 재스킨(롤링)** ✅ 진행 중: 입구 이후 인터랙티브 페이지를 시안대로 페이지 스코프(`body.*-page` + 전용 프리픽스 클래스)로 순차 재스킨 — `/history`·`/books`(2026-06-25)·로그인/가입(2026-06-26)·책방 헤더(2026-06-27)에 이어 **`/personality`(책BTI) 재스킨** ✅ (2026-06-27): 4상태(READY·COLD_START·FALLBACK·한도소진)를 종이톤+세이지로, 신규 `.pbti-*` + personality 전용 클래스 재스킨(공유 클래스 미접촉), 이모지→라인아이콘, 캐러셀 도트 추가, 순수함수 TDD + N-117 static-preview 검증. 상세는 [changelog.md](claude-docs/changelog.md). 공통 함정: CSS 주석 내 `*/`(N-118)·전역 `button{width:100%}` 상쇄(#286)는 매 페이지 반복 등장.
