...
#### 다음 단계 (백로그)
- 🔜 **공부 세션 자정 분할(PR-2)** — 독서는 #1008에서 저장 시점에 자정으로 쪼갠다. 공부는 아직 「시작한 날에
  전부」라 22:00~01:00 공부가 전날 3h로 선다(6h 상한이 이를 허용해 실제로 생긴다). `splitByMidnight`가 같은
  패키지의 package-private static이라 호출은 공짜다 — 저장 경로 변경이라 읽기 전용인 기록 PR과 갈라 뒀을 뿐,
  같은 릴리스에 태우는 것을 권한다.
