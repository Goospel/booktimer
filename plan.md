...
| 일자 | 내용 |
|---|---|
| 2026-06-08 | **책BTI 책방 노출 — 공개 책 기반 opt-in**(사용자 요청, 설계 Phase 6 전반부) — "내 책BTI를 책방(공개 프로필)에서 남들도 보게". 프라이버시 충돌: 기존 책BTI는 **전체 책(비공개 포함)** 기반이라 그대로 노출하면 비공개 책 취향이 샘(§7). 사용자 결정=**공개+완독 책만 재계산**(본인 것과 달라질 수 있음, 공개 더 켤수록 수렴) + **opt-in 토글**. P1 `User.personalityPublic`(V25) · P2 `publicProfileOf`(공개+완독 책 **+ 그 책 세션만**=비공개 독서시간 누출 차단) · P3 별도 테이블 `reading_personality_public`(V26)+`analyzePublicCached`(본인용과 캐시 규칙 공유=`CacheStore` 추상화)+탈퇴 정리 · P4 책방 노출(opt-in+캐시 있을 때만, **방문자는 캐시 읽기 전용=LLM 미호출**, `profile.html` 카드) · P5 `/personality` 토글+`POST /personality/visibility`(CSRF, 켤 때 즉시 생성). TDD 각 단계 Red→Green + 전체 그린. 설계 §10 Phase 6 전반부 완료. (PR #240) |
| 2026-06-08 | **책BTI "다시 분석" 일일 횟수 제한(악의적 반복 클릭 → LLM 남용 방어)**(사용자 요청) — "다시 분석 반복 클릭으로 서버 부담 → 하루 3번 제한". `POST /personality/refresh`는 매번 `force=true`로 Gemini 강제 호출이라 반복 클릭=LLM 남용. **일일 카운터를 User에 DB로**(인메모리 X — ECS 인스턴스 여럿이라도 일관·재기동 유지) + **사용자 타임존 자정 롤오버** 리셋: ① `User.tryConsumePersonalityRefresh(today)`(날짜 바뀌면 0 리셋 후 셈, 한도 3 내면 +1·true / 초과면 불변·false)+`remainingPersonalityRefreshes`(읽기) ② **V27** `personality_refresh_count`/`_date`(기존 사용자 첫 클릭 시 채워져 자연 적용) ③ 컨트롤러가 소비 시도 → 초과면 LLM 없이 redirect+flash, GET에 잔여 노출(`Clock` 주입) ④ 템플릿 남은 횟수·0이면 버튼 비활성·안내. "오늘"을 인자로 받아 테스트서 자정 경계 고정. TDD Red→Green: 도메인 3·컨트롤러 1(3번 후 차단+flash) + 전체 그린. (PR #241) |
| 2026-06-08 | **책BTI 공개/비공개 분기 폐지 → 공개 단일·항상 노출**(사용자 재결정) — "책BTI는 사용자끼리 즐기는 재미 요소. 나만 보는 비공개 성향은 재미가 아니다 → 공개 책으로만 뽑고 무조건 책방에 공개". 어제(#240) 만든 이원화(본인용 전체 책 + 공개용 공개 책 + opt-in 토글 + 별도 테이블)가 군더더기가 돼 일원화: 성향을 **공개+완독 책만**으로 단일 생성(`analyze`·`analyzeCached`→`publicProfileOf`)해 **단일 캐시 `reading_personality`**에 저장하고 **항상** 책방 노출(opt-in 제거 — 본인 `/personality`와 책방이 같은 결과). 제거: `reading_personality_public` 테이블·`PublicReadingPersonalityCache`·`analyzePublicCached`·`CacheStore` 추상화·`User.personalityPublic` 플래그·`/personality/visibility` 토글·`ProfileService` opt-in 게이트 — **V28**(테이블 drop+컬럼 drop)로 정리. 콜드스타트(공개 완독 5권 미만)면 캐시 없어 책방 카드 숨김. **프라이버시 강화**(유일 입력이 공개 책뿐 → §7 누출 차단 자명). 테스트 전부 공개 책 기준 갱신·토글 테스트 삭제·`ProfileControllerTest` 노출/숨김 2케이스 + 전체 그린. 설계 §10에 반전 기록(근거 보존). (PR #241) |
| 2026-06-08 | **책BTI 화면 안내 배치 다듬기**(실사용 UX) — 운영 확인 후 위치 2군데 조정(`personality.html`만, 로직·모델·테스트 무변경): ① "오늘 남은 횟수 N/3"을 별도 줄→**"다시 분석" 버튼 안으로**(작은 `.refresh-count`) ② 하단의 큰 "책방에 공개돼요" 카드 제거, 그 안내문을 **"내 독서 성향" 서술 바로 밑 작은 안내 톤**(`.personality-public-note`)으로 이동(성향과 묶여 읽히게, READY 상태에서만). CSS 2클래스 추가, DB·Flyway·테스트 무변경. (PR #242) |
| 2026-06-08 | **책방 — 책BTI + 공개 책장을 한 카드 2탭으로 합침**(실사용 UX) — 책방(`/u/{loginId}`)의 따로 떠 있던 책BTI·공개 책장 두 박스를 **독서 기록 카드와 동일한 CSS-only 라디오 탭**(`record-card` 재사용, JS 없음)으로 한 카드에 합침 — 탭 ① 책BTI / ② 공개 책장, 제휴 안내문은 공개 책장 탭 안으로. **기본 탭 동적**(성향 있으면 책BTI 먼저, 콜드스타트면 공개 책장 먼저 — `th:checked="${personality != null}"` — 빈 탭 착지 방지). 콜드스타트 처리는 #241의 "카드 숨김"→**안내문 empty-state**(탭 2개 대칭 유지, self/방문자 분기). CSS는 토글 선택자에 `#tab-bti`/`#tab-shelf`만 추가, 컨트롤러·모델 무변경(ProfileControllerTest 모델 단언 그대로 통과·MockMvc 뷰 렌더로 템플릿 검증, `hidesCard`→`emptyNarrative` 명칭만 정정). DB·Flyway 무변경. (PR #243) |
| 2026-06-08 | **책BTI 콜드스타트 임계 5권 → 1권**(사용자 결정) — "책이 한 권이어도 책BTI가 나오게. 정확도 낮아도 어떤 결과라도 보여주는 게 재미, 책 쌓을수록 결과 바뀌는 것도 재미". `COLD_START_MIN_BOOKS` **5→1** 한 줄 튜닝 — 완독(공개) 0권만 보류, 1권부터 서술 생성. 책 추가 시 `ProfileSignature` 변동 재생성으로 "변화의 재미"는 기존 캐시 로직 그대로(추가 코드 0). 화면 문구는 `coldStartMinBooks()` 동적 바인딩이라 "최소 1권"으로 자동, 책방 탭 empty-state도 완독 공개책 0권일 때만. TDD Red→Green: `oneBook_generatesPersonality_notColdStart`(1권→서술·캐시, min=5에선 Red)+콜드스타트 테스트 2건 0권 기준 교정 + 전체 그린. `PersonalityViewTest`는 임계 주입이라 무변경. 설계 §6·백로그 "잠정 5권"→1권 확정. DB·Flyway 무변경. (PR #244) |
| 2026-06-08 | **관리자 신고함 — 신고 내역 검토 화면 신설**(사용자 요청, SNS 5단계 후속 여지 해소) — "신고하면 어드민에서 개발자가 신고 내역을 봐야 후속 처리". 저장만 하던 `report`에 검토 UI를 문의함 패턴으로: `findAllByOrderByCreatedAtDescIdDesc`(최신순+id desc tiebreak) · `AdminReportRow` DTO(신고자·대상 LAZY 평면화=OSIV 비의존) · `ReportService.reportRows()`/`deleteByAdmin` · `AdminReportController`(GET `/admin/reports`·POST `/{id}/delete` CSRF) · `admin-reports.html`(신고자→대상·사유·상세·시각·대상책방·삭제) · admin 홈 "🚨 신고함" 카드. 부수: admin.html head `_csrf` meta로 "response committed" 세션 버그 차단(N-062 후보). 접근 경계 `/admin/**`→ADMIN 그대로. TDD Red→Green: 서비스 2+컨트롤러 5 + 전체 그린. DB·Flyway 무변경. (PR #245) |
| 2026-06-08 | **책BTI 분석 히스토리(최대 3개) + 대표 고르기**(실사용 피드백) — "책 추가 후 다시 분석했는데 성향이 안 바뀐 듯. 하루 3번뿐이라 또 못 돌림 → 과거 문장이 안 사라지고 최대 3개까지 남게, 그중 하나 골라 비교". 기존 `reading_personality` **1행/유저**라 재분석이 덮어써 비교 불가가 근본 원인. **최대 3행/유저 + `selected`(대표) 플래그**(유저당 0~1)로 전환 — 대표=공개 책방 노출+교체 보호. 결정 3: ①고른 게 **책방 대표+삭제 보호** ②재분석은 **후보로만 추가**(자동 대표 X) ③**'다시 분석' 버튼에서만** 쌓임(방문은 교체 안 함). 서비스 3진입점 `currentPersonality`(GET=대표 읽기, 비면 첫 부트스트랩, **책장 바뀌어도 재생성 안 함**)·`reanalyze`(후보 추가, 4행이면 *대표 뺀 가장 오래된 후보* 교체, LLM 실패 시 stale 유지)·`select`(대표 변경, **IDOR 가드**). 교체는 `generated_at`+id tiebreak. **V29** unique 드롭 함정(V26) 회피 위해 CREATE+INSERT…SELECT+DROP+RENAME 재생성(기존 1행→selected=true). `personality.html` 3카드(대표 뱃지·시각·책장변경 안내·선택 폼). TDD Red→Green: 도메인 10·GET 6·뷰 1·컨트롤러 3 + 전체 그린(FlywayMigrationTest로 V29 검증). 본문 §책BTI·설계문서 화해, N-063 후보. (PR #246) |
| 2026-06-08 | **책BTI 한도 소진·분석 시각 UX 3종 수정**(실사용 혼란 진단 후속) — "다시 분석해도 성향 안 바뀌고 후보도 안 뜸" 보고. 타임스탬프 대조(대표 17:43 KST < 신기능 #246 배포 ~19:00 KST)로 **버그 아님** 판정 — 오늘 3회를 신기능 배포 *전(구버전 덮어쓰기)*에 소진해 quota 0으로 처음 만난 것(자정 KST 리셋 전 막힘 → reanalyze 실행 0 → 후보 0, 대표 1개만). 0/3이 설명 안 되던 UX 3종: ① 비활성 버튼 시각화(`button:disabled`/`.btn-primary:disabled` 회색·금지 커서 — 배경색 명시 버튼이 disabled여도 눌리는 것처럼 보이던 문제) ② 한도 안내를 `refreshLimited`→`refreshRemaining==0` 상시 표시(버튼 disabled라 POST 못 가 플래시가 영영 안 뜨던 **catch-22** 해소) ③ 분석 시각 사용자 타임존 표기(`PersonalityView`에 `ZoneId zone`+`formatTime` — 그동안 `#temporals.format(Instant)`가 서버 UTC라 9시간 어긋남). 부수: 템플릿 개발 주석을 파서 수준 `<!--/* */-->`로 바꿔 출력 버퍼 비대화·주석 유출 차단(#245와 같은 'response committed' CSRF 함정 회피). TDD Red→Green(타임존 변환: 스텁 RED→구현 GREEN)+전체 그린(617). DB·Flyway 무변경. T-041·N-064 후보. (PR #247) |
| 2026-06-08 | **책BTI 책장 요약 레이아웃 + 개발 주석 유출 수정**(실사용 화면 확인) — ① **개발 주석이 화면에 노출** — #247의 파서 수준 주석 본문이 **닫힘 토큰을 글자로 인용**(`<!--/* */-->`)해 Thymeleaf가 *맨 처음 만난* 닫힘 토큰에서 조기 종료 → 뒤 텍스트가 일반 텍스트로 새던 자기참조 함정. 닫힘 토큰 인용 제거+풀어 설명. ② **책장 요약 세로 스택** — `.record-row`(가로 `space-between`)라 "자주 읽는 저자" 라벨이 긴 내용에 밀려 줄바꿈 → 라벨 위/내용 아래 스택하는 `.summary-list` 변형을 **전역 `.record-row` 안 건드리고**(타 페이지 회귀 방지) `app.css`에 추가(인라인 `<style>` 아님 = SSR 응답 비대화·버퍼 commit 함정 회피), `<ul>`에 클래스 부여. TDD Red→Green: `get_doesNotLeakDevComment`(출력에 `*/-->`·`응답 버퍼` 없음 — 유출 시 RED→수정 GREEN, `MockHttpSession` 주입으로 버퍼 commit 경계 불휘둘림=T-033). 전체 그린. 로직·DB·Flyway 무변경. (PR #249) |
