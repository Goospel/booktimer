...
- [ ] **① 재참여 알림 (최우선)** — "오늘 잔디 비었어요" 류 **일일 넛지**(푸시/이메일). 습관 앱 retention의 핵심 레버인데 현재 전무(목표 달성 알람은 *탭 열림 전제*라 이미 온 사람만 대상). ⚠️ 푸시(Web Push)·이메일 모두 인프라 선결 — 이메일은 §하드닝 「이메일 발송 인프라」와, 푸시는 VAPID/Service Worker와 겹침. **가장 ROI 높지만 인프라가 가장 무거움** → 수단(웹푸시 vs 이메일 vs 둘 다) 결정 선결.
