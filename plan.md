...
> - 🔀 **탐색(사용자 검색) IA 흡수 — A안 ✅ (2026-06-26)** — 탐색은 SNS 발견 기능인데 대시보드 QuickNav 타일로 핵심 독서기능과 동급 비중을 차지(사용자 제안: 책방 안 검색창으로 합치자). 탐색 결과물(누른 사용자→`/u/<id>`=책방)이 곧 책방이라 같은 흐름 → **대시보드 탐색 타일 제거 + 내 책방(self) 상단에 '다른 책방 찾기' 사용자 검색 + 친구 추천 흡수**(`shared/UserSearchPanel.vue`로 DRY 추출, 남의 책방엔 누수 안 됨). **A 가역성(B 여지)**: `/search` 라우트·`book-readers` 링크는 유지. **B안(보류 — A 출하·반응 평가 후 재결정)**: 책방을 소셜 허브로 승격(중립 '책방 둘러보기' 랜딩). 결정·재평가 트리거 기록 = 자동 메모리 `explore-search-into-bookshop`.
