...
### 측정 대상 책 「고르기 · 바꾸기 · 붙이기」 개선 (R2) — 🔜 진행 중 (PR-1~5 코드 ✅ — 남은 것: 미니앱 번들 배포 · 실기기 A-9 · 검수 · 스토어 스크린샷 03-history)
...
- ⬜ **배포** — 미니앱 번들 1개(PR-4+PR-5)로 테스트 푸시 → 실기기 확인(A-9: 펼친 줄 정렬·말줄임·스크롤) → 검수 요청.
  검수 직전 스토어 스크린샷 03-history 재촬영(기록 화면이 바뀌었다) · 비게임 체크리스트 대조
