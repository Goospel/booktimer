...
> ⚠️ **3차 반려(2026-08-18)** — 「스크롤하면 배경·요소는 움직이는데 결 무늬만 제자리다」. 결 레이어가 `position: fixed`라 뷰포트에 못 박혀 있었다. `absolute` + `body{position:relative}`로 종이와 함께 움직이게 하고, z-index를 9999→90으로 낮춰 떠 있는 것들(탭바·시트·모달) 아래로 넣었다(T-192). 아래 두 항목은 그대로 유효하다.
