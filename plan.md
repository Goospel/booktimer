...
### 📚✏️ 공부 서재 — 회독 기반 별도 서재 (PR-1 서버 · 완료 ✅ 2026-09-01, 미배포)

> 설계는 `claude-docs/plans/2026-09-01-study-library.md`. 미니앱 화면(PR-2)은 아래 「다음 단계」에 있다.
...
- 🔜 **공부 서재 PR-2 (미니앱 화면)** — `StudyLibrary.tsx` 신규 + `App.tsx` 분기 한 줄. 독서 `Library.tsx`는
  **바이트 불변**이 계약이다(별도 파일이라 `git diff`가 곧 증명). 「N독」 칩 + 주 손잡이 「회독 +1」 + 관리 시트.
