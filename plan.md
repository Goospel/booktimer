...
- **비주얼·기술 — 미니게임 퀄리티 로드맵 A0→B (A0 무대화 출하 ✅ 2026-06-14, PR #346)**: "이모지 격자" 인상을 **단계적 고도화 A(현 스택)→B(SPA·캔버스)**로 끌어올린다(사용자 합의 2026-06-14). **A0 무대화 = 출하** — 에셋 0·이모지 유지하되 하늘→잔디 배경·흙 화단·발밑 그림자·울타리·빈 정원 이랑으로 **순수 CSS/HTML 무대 연출**(백엔드/JS 0, 보기/편집 캔버스를 `.garden-stage`로 래핑·좌표계 불변). 2.5D perspective는 탭 좌표를 깨 비채택(깊이는 그라데이션+그림자로 암시). **후속(방향·트리거만 고정)**: A1 생명감(`@keyframes` sway·팝·파티클, `prefers-reduced-motion` 가드) → A2 핵심 식물 이모지→인라인 SVG 승격(폴백=이모지, 메타 1필드 — 여기서 백엔드 변화·정식 TDD) → A3 부드러운 드래그·핀치 줌(Pointer Events) → **B 전환**(PixiJS+Vite). **B 전환 트리거**: A3에서 ① 드래그 물리/대량 파티클이 DOM에서 실측 버벅이거나 ② 자유 픽셀 좌표 배치가 요구돼 `garden_placement`에 `x,y` 컬럼이 필요해질 때(그 전엔 DOM/CSS로 충분 — 조기 전환은 빌드 마찰만). 반응형·접근성(스크린리더 대체텍스트)은 각 단계에서 함께.
