...
> 코드 폐기는 **PR-1(스키마 무변경)**, `author_character`·`author_affection` 테이블과 `users.profile_character_code` drop은 **PR-2(V88)**로 따로 나간다 — 한 배포에 섞으면 blue-green 전환 중 이전 컨테이너가 없는 컬럼을 SELECT해 인증 요청이 500이 된다.
...
- **프로필·설정 화면 + 닉네임 변경** ✅ (2026-08-14) — 토스로 가입한 계정(운영 6명)은 닉네임이 전원 기본값
  ...
  **남는 백로그(이 작업 아님)**:
  ...
  - ❌ ~~프로필 사진(도감 작가) 선택~~ — **2026-09-09 폐기.** 웹에서 서재 캐릭터 기능을 전면 폐기하면서
    도감·스프라이트·`owned-characters` GET·select POST가 전부 사라졌다(컬럼 `profile_character_code`는 PR-2에서 drop).
