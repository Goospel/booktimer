...
- ✅ **후속 4건 마감 (2026-09-05)** — 리뷰가 「그대로 머지」로 판정하며 남긴 것들. ① **교체 404 자동 복구** —
  ...
  유일한 진입점이라(세션 id는 stop 응답에만 실린다) 닫으면 미태깅으로 굳는다. *(2026-09-25 R2 PR-3으로 해소 — 이제
...
- ✅ ~~⏸ **`/study/history` 월별 행의 책 제목**~~ — 사용자 결정(2026-09-24)으로 재개해 R2에서 풀었다: 서버가
  `Day.sessions[]`를 싣고(PR-1), 기록 날짜를 펼치면 측정 한 건씩 책 제목과 [책 붙이기]/[바꾸기]가 선다(PR-3).
...
### 측정 대상 책 「고르기 · 바꾸기 · 붙이기」 개선 (R2) — 🔜 진행 중 (PR-1 서버 ✅ · PR-2 웹 홈 ✅ · PR-3 웹 기록 ✅ · PR-4 미니앱 홈 ✅)
...
- ⬜ **배포** — 미니앱 번들 1개(PR-4+PR-5)로 검수 요청 · 스토어 스크린샷 03-history 재촬영
