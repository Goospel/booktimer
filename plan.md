...
- **남은 외부 작업(코드 아님)**: AdSense 사이트 검토 요청 → 심사 승인 → 광고 단위 생성 → `ADSENSE_CLIENT_ID`/`ADSENSE_SLOT` SSM 주입. ⚠️ 심사는 *콘텐츠 크롤링*도 하는데 본문이 로그인 뒤라 "콘텐츠 부족"으로 반려될 수 있음 → 공개 소개/랜딩 페이지가 필요할 수 있다(후속 후보). EEA/UK 트래픽이 늘면 동의 관리(CMP) 별도 검토.
