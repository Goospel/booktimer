...
- [x] **B. `POST /password/forgot` IP + 이메일 이중 레이트리밋** (IP 시간당 10 · 정규화 이메일 시간당 3) —
      이메일 키가 핵심이다: `EmailTokenService.issue`가 직전 미사용 토큰을 죽이므로, IP를 바꿔 가며 피해자
      주소로 반복 요청하면 **피해자의 재설정 링크가 열어 보기도 전에 계속 무효화**된다. 키는
      `trim().toLowerCase()`로 정규화하고, 없는 계정도 똑같이 센다(상한 반응이 갈리면 그게 열거 채널이다).
...
- 📍 **검증**: 항목마다 RED → GREEN. 신설 9건(A 2 · B 4 · C 1 · D 2) + 서비스 1건. 돌연변이 6종 전부 KILLED.
