...
| 2026-06-08 | **책BTI "내 독서 성향" 설명 문구 3줄 제거**(실사용 UX 정리) — "UI만으로 충분히 이해할 디자인이어야지 문구로 일일이 설명하면 화면이 지저분". "내 독서 성향" 헤드 밑 안내문(`다시 분석…최대 3개…⭐ 대표로 고르면 공개 책방 노출`, `.status-line muted` 한 문단) 삭제 — 대표 뱃지·후보 카드·선택 버튼·하단 공개 안내문이 이미 같은 내용을 UI로 전달해 군더더기. `personality.html` `<p>` 한 문단 제거뿐, 로직·모델·CSS·DB 무변경. 단언 테스트 없음 → 컨트롤러 렌더로 무회귀 확인. (PR #250) |
