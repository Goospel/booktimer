...
- ⬜ **PR-3 미니앱** — 종료 후 태깅 시트(「무슨 책을 공부하셨나요?」) + 측정 중 교체 시트([바꾸기]).

**타이머–책 연결 — 종료 후 태깅 시트 + 측정 중 교체 시트 ✅ (2026-09-02, PR-3)** — 독서와 같은 3단의 나머지 둘을 연다: 책 없이 끝낸 공부 측정에 **「무슨 책을 공부하셨나요?」 시트**(stop 응답 `untaggedSessionId`가 유일한 좌표, 붙일 책이 0권이면 안 연다)로 나중에 붙이고, 시작 토스트의 **[바꾸기]**로 재는 도중 대상을 갈아 끼운다(`null` = 「책 없이」로 되돌리기 — 측정은 안 멈추고 시간은 통째로 새 책에 붙는다). 시트 두 장은 **독서 것을 그대로 재사용**하고 모드가 바꾸는 것은 재료(`study.books`)와 제목뿐이다. 공부 모드에도 [바꾸기]가 서면서 그것을 가리던 게이트(`toastHasBookControls`)는 항상 참이 되어 삭제했고, 그 자리의 세이지 리터럴 2곳은 `var(--accentPill, …)` 토큰으로 바꿨다(독서 알파 .14/.16→.18 — 「독서 diff 0」의 의도된 예외 2건). 서버 문 둘(`/api/study/sessions/{id}/tag-book`·`/api/study/active/book`)은 PR-1.
