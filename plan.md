...
- 🔜 **잔액 소진 시 429 안내가 지킬 수 없는 말이 된다** — 자동 충전을 껐으므로 Gemini 잔액이 마르면 429가
      오고, 어댑터는 이를 `RATE_LIMITED`로 매핑해 「잠시 후 다시 시도해 주세요」를 보여준다. 충전 전까지
      **기다려도 안 되는** 안내다(리뷰어가 FALLBACK에서 잡았던 것과 같은 부류의 거짓말). 429 본문으로
      쿼터 소진과 레이트리밋을 가르거나, 최소한 로그에 원인을 남겨 운영자가 알아채게 한다.
      ⚠️ **사용자 몫**: GCP 예산 알림(50%·90%)을 25,000원 기준으로 걸어 두는 것 — 자동 충전을 끈 이상
      그것이 유일한 조기 경보다.
