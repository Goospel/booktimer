...
### 미니앱 디자인 개선 핸드오프 — 「또렷한 연필」 + UX 수정 2건 (진행 중 2026-08-23)
...
>
> 🚚 **배포는 ③까지 묶어 한 번에** (사용자 결정 2026-08-23) — main 머지는 미니앱 배포가 아니다(`ci.yml`은 테스트만,
> `deploy.yml`은 EC2 전용). ①은 머지됐지만 **미배포**이고, 심사가 배포마다 붙으니 ③까지 모아 `deploy.sh` 1회로 끝낸다.
> 그때 지켜야 할 두 가지:
> - **`--expect` 마커는 ②③에서 새로 생긴 UI 문구로 잡는다.** ①에는 쓸 마커가 없다 — 새 UI 문구가 0이고,
...
>   `miniapp/src` 커밋이 13개라(①의 2열 통계 + 글자색 전역 변경 포함) 이미 낡았다 — 그대로 올리면
...
- [x] **② 측정 시작 토스트 + 바꾸기 시트 (감사 3f → 시안 4b·4c) — 완료 ✅ 2026-08-24**
      ...
      책이 안 보였다.** 게다가 측정 중엔 탭 이동이 잠겨 확인하러 홈에 갈 수도 없다 — **무엇을 재는지 모른 채
      측정이 도는 상태**였다. 시작 직후 그 자리에서 말하고(토스트), 틀렸으면 그 자리에서 고치게 했다(시트).
      ...
      클라이언트가 대상을 지목할 수단 자체가 없다. (b)로 되는 건 「책 없이 시작 → 종료 시 붙이기」뿐인데
      그건 이미 있는 기능이다. → **(a) `POST /api/sessions/active/book`** 신설(Flyway 없음).
      경로에 **세션 id를 안 넣었다** — 클라에 그 값이 없고 진행 중 세션은 사용자당 최대 하나라 「내 진행 중
      세션」이면 충분하다(id를 새로 노출하는 것보다 계약이 작고, 소유 경계는 그 조회가 곧 보장한다).
      `bookId: null`이 「책 없이」로 되돌리는 유효한 목적지다 — `tag-book`이 null을 거부하는 것과 갈리는 지점이다
      (그건 빈 자리 채우기, 이건 라벨 바꾸기). 그래서 도메인 메서드도 `tagBook`과 나눠 `changeBook`을 새로 뒀다.
      ⚠️ **옛 책의 상태는 되돌리지 않는다** — 그 책이 「읽는중」이 된 출처가 이 세션 하나라는 보장이 없다.
      ⚠️ **한계(스펙 그대로)**: 「바꾸기」로 가는 문이 토스트 하나뿐이라 **5초가 지나면 종료 전엔 못 바꾼다.**
      시안이 그렇게 그렸고 그대로 갔다 — 상시 진입점이 필요하면 별건으로 판단한다.
- [ ] **③ 「또렷한 연필」 타이포·위계 시스템 (시안 2a~2e) — 마지막 ⏸**
