...
- ✅ **PR-4 「책방 · 여백 · 대화 · 설정 · 공부 화면 · 시트」** — 연필선 9곳을 Soft로(책방 헤더 손잡이 2 · 정사각 버튼 · 신고 select → `SOFT_OUTLINE`,
  ...
  - ⬜ 발견(PR-5 판정용): 설계 완료 기준 `querySelectorAll('[style*="border-image"]').length === 0`은 **원리상 0이 안 된다** — 크롬이 `border: none`/`0` 단축 속성을
...
- ⬜ **PR-5** 탭바 · 연필 철거(`PENCIL_FRAME`·`--pencil-frame*`) · 스토어 스크린샷 · 배포 1회 — 미검증 가정 원장(U-1~U-11) 전부 채움
  - PR-2에서 넘긴 판정: `SOFT_OUTLINE`의 1.5px 테두리가 DPR 3 크롬에서 computed `1px`로 나온다 — 실기기에서 1.5px로 읽히는지 판정한다
