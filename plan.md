...
- 🔜 **미해결로 머지했다 — 운영 Gemini 키의 요금 티어 확인 (I-3)**. 사용자가 2026-09-08에 두 건을 알고
      머지를 결정했다. `deploy-aws.md`의 「무료 티어는 프롬프트가 모델 개선에 쓰일 수 있다」 주의문은
      **6-2가 집계 수치만 보내던 시절의 위험 평가**다. 이 PR로 **사용자가 직접 친 자유 텍스트(주제·범위
      ≤4,000자)** 가 같은 키로 나간다 — 학습 데이터가 될 수 있는 것의 **종류가 바뀌었다.**
      무료 티어면 유료(학습 제외) 전환이나 처리방침 6-2 보강이 필요하다(지금 문구 「Google의 데이터
      처리 정책에 따릅니다」로는 안 드러난다). 확인: <https://aistudio.google.com/> → API key →
      연결된 GCP 프로젝트 결제 설정. **이 항목이 닫히기 전까지 처리방침 6-2는 「가능한 최악」을 고지하지
      않은 상태다.**
