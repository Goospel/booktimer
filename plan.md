...
- **홈 UX 정정** ✅ (2026-08-11) — 실사용 피드백 2건. ① **읽는 중인 책 탭 = 「선택」**(이전엔 탭이 곧 측정 시작이라
  여러 책을 번갈아 읽는 사용자가 시작 전에 책을 바꿀 수 없었다). 기본 선택은 웹 `BookPickForm`과 같은 이어 읽기
  규칙(`defaultBookId`: 최근 읽은 책 → 없으면 첫 책 → 0권이면 null)이고, 시작은 주 버튼 「측정 시작」이 맡는다
  (「책 없이 측정 시작」은 보조로 강등 · 0권이면 아예 안 뜬다). ② **홈 잔디가 카드 폭을 채운다** — `GrassGrid`에
  `fill` 모드(주 컬럼 `flex:1` + 칸 `aspect-ratio` 정사각)를 더해 5주 × 14px 고정 → 15주 채움. 기록 화면은 가로
  스크롤이 전제라 무변경.
  ⚠️ 하니스 사각지대(실측): 정적 렌더라 `onClick`은 마크업에 안 남아 **책 탭이 시작을 부르지 않는다**는 것을 직접
  계측할 수 없다 — 선택 강조(fill/weak)가 남는지로 간접 계측하고, 배선 수정 시엔 실기기·프리뷰 확인을 게이트로 둔다.
