...
- ✅ **2단계 — raw 색 규칙 토큰화 + 토글 점등 (2026-09-10)**
      ...
      ✅ **「라이트 픽셀 diff 0」 — 사용자 결정에 따라 채웠다 (2026-09-10)**. 처음엔 근접값 통합(설계 §4.5가
      ...
      **값이 다른 리터럴마다 제 토큰**을 준다 → 라이트 색 토큰 **60 → 119개**(계열+숫자 접미사. 다크에선
      계열 대표값 하나로 모인다), 공부 모드 토큰 **10 → 15개**. 늘어난 토큰이 중복처럼 보여 나중에 합쳐지는
      ...
      **차이 0건**. 양성 대조군 2종(`--paper-1`·`--sage-line`을 각각 1/255만 흔들면 10건·4건이 잡힌다).
- ⏸ **남은 자리 2건(의도적 보류)**
