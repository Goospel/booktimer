...
- **심사 반려 대응 — 자체 「돌아가기」 제거(네이티브 뒤로가기 일원화)** ✅ (2026-09-02 코드 · 재심사 대기) — 번들 `01a06122`(20260902-137)가 「내비게이션 바의 뒤로가기 버튼과 미니앱 자체 헤더 및 뒤로가기 버튼이 함께 노출돼요」로 반려됐다. **그 번들의 diff엔 뒤로가기가 한 줄도 없다** — 「‹ 돌아가기」 알약은 08-16(#830·#831)부터였고 그 뒤 열 번쯤 통과했다. 그래도 T-185(무변경 재제출)를 쓰지 않은 것은 이 사유가 [비게임 출시 체크리스트](https://developers-apps-in-toss.toss.im/checklist/app-nongame)의
  **필수 항목**이기 때문이다(편차가 아니라 규칙 위반). 사용자 결정 = **B안**: `apps-in-toss.config.ts`를 손대지 않고(A안 기각) **자체 뒤로가기를 전부 제거**한다. `ui.tsx` `Screen`의 `onBack`·`backDisabled` prop과 알약 블록 삭제(호출부 12곳), `Screen`에만 쓰이던 `onBack`은 prop째 삭제, `Goal.tsx` 하단 버튼은 `firstRun`의 「나중에 정할래요」만 남긴다. **`useBackClose` 배선은 무변경** — 12곳이 이미 히스토리 엔트리를 쌓아 네이티브 `<`가 하드웨어 back과 같은 경로를 탄다.
  ⚠️ **수용하는 회귀**: `backDisabled`의 「요청 중 못 나감」 잠금은 네이티브 버튼에 못 건다. 재발 가드 = 소스 가드 `miniapp/src/native-back-guard.test.ts`(제품 `.tsx`에 「돌아가기」 리터럴 0건). 승격: CLAUDE.md 「미니앱 개발 루프」에 **검수 요청 직전 체크리스트 대조** 한 줄(T-220).
