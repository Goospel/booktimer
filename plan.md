...
> - 🔜 **맞팔 DM — 서로 팔로우한 두 사람만 여는 1:1 대화** (2026-09-18 착수, 미니앱 먼저·전부 킬스위치 `booktimer.chat.enabled=false` 뒤 다크 머지) — 처음엔 랜덤채팅을 검토했으나 앱인토스 「서비스별 주의사항」이 「랜덤 매칭·모르는 사람과의 연결」을 **만남·소개팅**으로 분류하고 그 트랙은 **법인 등록이 필수**라 접었다. 대신 「지인·ID 기반 특정 사용자와 1:1」인 **채팅 서비스** 트랙으로 간다 — 상대가 나를 팔로우해야 열리는 맞팔 게이트가 재분류 트리거(공개 프로필 기반 발송)를 피하는 근거다. 4단계: PR-0 정책·콘솔(약관 「대화」 절·운영정책·푸시 템플릿·SSM 키, 사용자 몫) → ✅ **PR-1 서버**(V94 제재 컬럼·V95 방·메시지, 자격은 발송·조회마다 재검증하고 「잠김」은 저장하지 않고 파생, 본문 AES-256-GCM 컬럼 암호화, 방·수신자당 30분 1통 푸시 자리, 차단 → 방 CLOSED, 탈퇴 purge, 프로필 `dmAvailable`) → ⬜ PR-2 서버 신고·제재·관리자 대본·보존 스케줄러·처리방침(V96) → ⬜ PR-3 미니앱 대화함·방·책방 「메시지」 버튼·채널톡 제출·심사. 설계 원본은 작업용 md(gitignore)라 PR 본문이 정본이다.
