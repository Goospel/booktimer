...
| 2026-06-08 | **공개 책방 — 필터 클릭 시 책BTI 탭으로 튕기던 버그 수정**(실사용 피드백) — 책방(`/u/{loginId}`)의 책BTI/공개 책장 **CSS 라디오 탭**과 공개 책장의 상태 필터(**서버 링크 `?status=`**, 리로드)가 다른 상태 계층이라, 필터 클릭→리로드→`th:checked="${personality != null}"` 재적용으로 **책BTI 탭으로 리셋**(서버가 클라 탭 상태를 모름)→공개 책장을 다시 눌러야 하는 왕복 불편. 처방: "책장 보는 중" 신호를 URL에 실어 탭 기본값 보정 — `ProfileController`가 `tab` 받아 `shelfActive=(shelfFilter!=null)||"shelf".equals(tab)` 모델 추가, 탭 `th:checked`에 `!shelfActive`/`or shelfActive` 반영, 필터 링크 전부(전체 포함) `tab='shelf'` 실음. 첫 진입은 기존 의도(성향 있으면 책BTI) 유지. 서버 라운드트립·CSS 탭·`/books` 일관성 보존, DB·CSS 무변경. TDD Red→Green 3경계(필터·"전체"·첫진입 shelfActive) + 전체 그린. (PR #252) |
