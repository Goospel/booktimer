...
- [x] **발송 수단 = SES SMTP 확정·점등** — 프로덕션 액세스 신청 + SMTP 자격증명 발급(SSM SecureString) + `task-definition.json` 주입(`BOOKTIMER_EMAIL_ENABLED=true` → `SmtpEmailSender` 활성, transactional 실발송 ON). 딜리버러빌리티 위해 `.click`→`.app` 이전 선행(N-036 · #310/#311). (PR #312)
- [x] **발신 도메인 검증 (DKIM)** — `booktimer.app` Easy DKIM 검증 완료(SES가 Route53에 CNAME×3 자동 게시·확인). ⚠️ **SPF/DMARC는 미완** — 현재 기본 MAIL FROM(amazonses.com)이라 DKIM 정렬만 섬. 사용자 지정 MAIL FROM(`mail.booktimer.app`) + SPF·DMARC 레코드는 점등 안정화 후 딜리버러빌리티 보강으로 추가. (PR #312)
