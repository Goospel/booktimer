...
- **딥링크 착지 탭 (`?tab=`)** ✅ 코드 완료 / 🔜 콘솔 URL 반영 대기 (2026-08-25) — 푸시를 누르고 들어온 사람이
  ...
  **모르는 값은 전부 홈**이다(콘솔에 손으로 적는 URL이라 오타·옛 링크가 온다 — 그때 필요한 건 오류가 아니라 홈).
  ...
  - `CHECK:` 실기기에서 `intoss://booktimer/?tab=history`로 진입 / `EXPECT:` 기록 탭이 첫 화면 / `EVIDENCE:` `pending`
    — 콘솔이 자유 입력을 받는 것과 **그 쿼리가 우리 SPA의 `location.search`까지 도달하는 것은 별개**다(우리는
    `AppsInToss.registerApp` 파일 기반 라우팅을 쓰지 않는 순수 Vite SPA이고, 입력란 플레이스홀더가 「화면 주소」라
    경로를 기대하는 눈치다). 그래서 **콘솔의 이동 URL은 이 확인 전까지 바꾸지 않는다** — 파라미터가 없으면
    코드는 지금과 완전히 동일해 무해하게 잠들어 있다.
    ⚠️ 확인 대상은 **광고성 신규 유입이 아니라 기능성 푸시 3종**이다(신규 유입은 어차피 로그인·온보딩이 먼저라
    어느 탭을 지정하든 의미가 없어 **홈 화면**으로 등록했다).
