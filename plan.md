...
- [x] 상태 필터에 **공개여부(PUBLIC/PRIVATE) 차원** 추가 ✅ (PR #327) — 상태 × 공개여부 **직교 2차원 AND 필터**. `BookController.books`에 `visibility` 파라미터·`parseVisibility`(parseStatus 쌍둥이)·기존 status와 동일한 메모리 stream 필터, `books.html`에 🌍공개/🔒비공개 칩 줄(상태와 **상호 파라미터 보존**)·htmx 부분 swap 유지. Repository·서비스·CSS 무변경(기존 `.shelf-filter` 재사용).
