...
| 내 책장에 책이 많으면 **전부 한 번에 렌더**돼 리스트가 길어지고, 하단 네비(← 대시보드 등)까지 스크롤이 멀어짐 | 페이징/지연 표시가 없어 `myBooks` 전체를 통째로 그림 | **클라이언트 무한 스크롤**(`books.js`) — 처음 5권만 보이고 리스트 끝 센티넬이 화면에 들어오면 `IntersectionObserver`로 다음 5권 펼침. 숫자 페이지(1·2·3) 대신 스크롤(모바일 UX). JS 꺼지면 전체 표시 폴백, 표지 `loading="lazy"`. HTML/CSS/JS만(서버·DB 무변경) | ✅ PR #188 예정 |
