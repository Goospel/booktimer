...
- ⚠️ **`trailingNode`가 저장값을 오염시켰다** — 목록·구분선으로 끝나면 빈 문단이 붙어 저장값에 빈 줄이
      쌓인다(8000자 예산과 AI 프롬프트를 함께 갉는다). 경계에서 `trimEnd`하되 **밖으로 내보내는 길과
      「같은 값인가」를 비교하는 길이 같은 함수를 쓴다** — 한쪽만 다듬으면 매 입력마다 문서를 갈아치워 커서가 튄다.
...
- 📍 **검증**: RED → GREEN, 새 테스트 35건(pure 17 · 편집기 15 · 패널 2 · 프롬프트 1) · 프론트 전체 632건 ·
      돌연변이 4종(`breaks` 끄기 · 상한 경계 넓히기 · 목록 밖 Tab 무력화 · 프롬프트에서 마크다운 벗기기) 사살.
...
- ⚠️ **번들이 커진다 — 머지 전 사용자 판단이 필요한 자리다**: study.js 110,727 → **591,087 bytes**
      (gzip 40,326 → 194,046), **홈 대시보드도 같은 패널을 실어** dashboard.js 139,818 → **619,938**
      (gzip 48,785 → 202,320). 설계의 「500KB raw 초과면 보고」 선을 넘었고 대시보드 섬은 설계가 아예
