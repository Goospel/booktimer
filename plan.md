...
>
> **컨셉 전환 (2026-06-24)**: 식물 4축(TIME·GENRE·DIVERSITY·RECIPE)·소품(Decoration) 제거 → **작가(배회 캐릭터)·건물(배치)만** 남기는 마을로 전환 진행 중. **PR-1(프론트 가지치기)** 완료 — GardenGame.vue·GardenDex.vue에서 식물·소품 팔레트·섹션 제거, scene.ts에 BUILDING-only 필터 적용(고아 행 렌더·재저장 차단). PR-2(백엔드 가지치기)·재매핑(읽기 차원→오브젝트) 후속 예정. 계획: `claude-docs/plans/2026-06-24-village-prune-to-authors-buildings.md`.
>
