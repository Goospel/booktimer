...
- 🔜 **AI 호출 원가가 설계 가정의 5.5배다 (2026-09-03 실측, 미해결)**. 일정 생성 실호출에서
  **회당 약 153원**이 나왔다 — 설계가 잡은 **30원**의 5.5배이고, 최악 관측은 **455원**이었다.
  출력 토큰이 큰 것이 원인이다(79일치 일정 = out 10,574토큰). 하루 상한(PLAN 3)이 1인당 상한은
  걸지만 **사용자 수에 비례해 원가가 는다.** 선택지는 ① 출력 축소(일정 형식을 더 조밀하게)
  ② 더 싼 모델 ③ 상한 하향 ④ 감수. **정하기 전에 실사용 호출량부터 본다** — 지금은 승인자만
  쓰므로 총액이 작다. ⚠️ 위 스트리밍 승격과 **원인이 같다**(출력 토큰) — 같이 다루는 게 싸다.
