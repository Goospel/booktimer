...
- ⏸ **설정 페이지의 공부 목표 항목**(독서 관례) — PR-B가 히어로 인라인을 택했다. 서버(SSR 폼)를 건드려야 하고
  설정 페이지의 힌트 문구(「빠뜨린 날은 나중에 채워」)가 공부엔 거짓이다. 트리거: 「목표를 설정에서도 바꾸고 싶다」 제보.
...
- 🔜 **프론트 문구 후속**: `frontend/src/study/PlanForm.vue`(163행)·`RecallPanel.vue`·`pure.ts`의 「오늘 몫」·
      「내일 다시 해 주세요」가 UTC 리셋(KST 09:00)과 어긋난다 — 서버 429 문구는 「오늘 몫을 다 썼어요 —
      한국 시간 오전 9시에 다시 채워져요」로 고쳤으니 **다음 프론트 번들 재빌드 때** 화면 쪽도 맞춘다
      (이번 PR은 번들 범위를 열지 않았다).
