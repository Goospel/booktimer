...
| 일자 | 내용 |
|---|---|
| 2026-06-09 | **CLAUDE.md — PR·머지 담당 = 구현 세션 명문화**(#260 후속) — 계획↔구현 세션 분리 규칙에서 "PR·머지는 누가 하나"를 못 박음: 구현 세션이 PR 생성→머지→로컬 main 갱신·브랜치 정리까지 맡는다(PR/머지는 기계적 마무리라 저-effort 쪽 일). 예외: 회귀 리스크 큰 작업만 머지 전 독립 리뷰 opt-in. 「계획 우선」 §세션 분리 드리프트 규칙 아래 한 줄. 문서만. (PR #268) |
