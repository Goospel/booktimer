...
- [ ] (후속) 리뷰 Low 잔여 2건 — **컨테이너 비root 실행**(⚠️ `render-env.sh`가 root로 떨어뜨리는 토스 mTLS PEM 600을
      같이 chown 하지 않으면 앱은 뜨고 **토스 로그인만 조용히 죽는다**) · **CSP 없음**(⚠️ AdSense·GA가 걸려 있어
      `nonce`+`strict-dynamic`이 필요하고, 광고는 localhost에서 안 떠 **배포 후 확인이 유일한 게이트**다).
