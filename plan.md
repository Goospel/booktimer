...
- ⚠️ **주 손잡이는 채움이 아니라 파랑 틴트**다 — 「채움 주 버튼은 화면당 1개」 불변식의 계측기(`typography.test`)가
  채움이 선 파일 목록을 **셋으로 못 박고** 있어, 채움을 쓰면 기존 테스트를 고쳐야 한다(U6 위반). 독서 서재의
  「검색해서 담기」가 이미 같은 틴트 꼴이라 어휘도 어긋나지 않는다. 채움으로 승격하려면 그 목록 한 줄을 늘리면 된다.
