...
- **프로필·설정 화면 + 닉네임 변경** ✅ (2026-08-14) — 토스로 가입한 계정(운영 6명)은 닉네임이 전원 기본값
  ...
  **웹 안에서도 이미 어긋나 있어**, 40자로 온보딩한 계정은 설정에서 영영 닉네임을 못 바꿨다(온보딩 허용 폭 50→30 축소는
  ...
  미니앱 진입점은 **홈 하단 `AccountSection` 교체** — 그 자리의 "계정 관리·상세 설정은 booktimer.app에서"가 바로 그
  죽은 안내였다. **탭바는 한 줄도 안 건드렸다**(플로팅 탭바는 2026-08-12 심사 반려 대응 산물 — 5칸 확장은 재심사 노출면).
  ...
  **남는 백로그(이 작업 아님)**:
  - ⚠️ **미니앱 전용 계정은 탈퇴 경로가 0개다** — 웹 탈퇴 화면에 닿지 못하는데(로그인 불가) 미니앱엔 그 길이 없다.
    개인정보·스토어 심사 요건과 얽히므로 별도 설계 필요(파괴적 동작이라 확인 UX 포함).
