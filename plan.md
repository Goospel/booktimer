...
- ✅ **2단계 — raw 색 규칙 토큰화 + 토글 점등 (2026-09-10)**
      `app.css`의 raw 색 **158선언**을 의미 토큰 **22개**(`--on-accent`·`--surface`·`--paper-1`·`--line-1/2`·
      `--warn-*`·`--danger-*`·`--ok-*`·`--info-*`·`--scrim*` 등)로 걷어 올리고, `theme.js` 끝에 자동 마운트
      ...
      ⚠️ **「라이트 픽셀 diff 0」 게이트는 못 지켰고, 일부러 그랬다** — 근접값 통합(설계 §4.5가 테라코타
      두 벌을 합치라고 한 것의 확장)으로 리터럴 68종 중 **46종의 라이트 값이 바뀐다**. 대부분 1~6/255지만
      경고·오류 계열의 시안 잔재(테일윈드 앰버·레드 `#FCD34D`·`#B45309`·`#FECACA` 등) 12종은 Δ20~97이라
      **라이트 화면도 종이톤으로 통일된다**. 통합하지 않으려면 토큰이 68개가 된다.
- ⏸ **남은 자리 2건(의도적 보류)**
