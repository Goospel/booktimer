...
- ⬜ **공부용 하루 목표 시간 — 독서 목표와 분리**: 1차는 스톱워치·당일 누적만이다(사용자 결정: 다음 PR).
  독서의 `GoalSchedule`·부채·이월을 그대로 복제하지 말고, **공부에 필요한 최소치**부터 정하고 시작한다
  (목표를 들이면 부채·용서권·푸시까지 한 벌이 따라온다 — 그 한 벌이 정말 필요한지가 첫 질문이다).
- ⬜ **공부 모드 탭바 개편 — 「여백」 대신 달력·공부 일정**: 공부 모드에서 여백 탭을 빼고 달력/공부 일정 탭을 세워
