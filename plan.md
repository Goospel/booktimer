...
- 🔜 **2단계 — 첫 진입 붙잡기: 선택 화면 제거 + 게스트 「책 뉴스」 (설계 `claude-docs/plans/2026-09-17-first-entry-login-and-guest-news.md`)**
  ...
  - ✅ **PR-S 서버 (A) (2026-09-17)** — `/api/toss/register`가 find-or-create 결과에 `created`를 싣는다(login·link는 늘
    ...
    빈손이라 경합한다(이번엔 안 고침).
  - 🔜 **게스트 뉴스 서버 — 일반 책 뉴스로 재설계 중** — 처음 안(`/api/public/news` = 공개 완독 책 isbn의 캐시 기사)은
    리뷰 차단으로 뺐다: 아무 계정이나 제목·ISBN을 넣은 가짜 공개 완독 책으로 게스트 뉴스의 책 제목·기사 주제를 조종할 수
    있다. 사용자 결정 = **사용자 입력과 무관한 일반 책 뉴스를 새로 수집**. 원장 U-B1(운영 CORS)·U-B5는 그 설계로 옮긴다.
  - ⬜ **PR-M1 미니앱 (A)** — `beginLogin` 3갈래(register-first · `web_link`는 조회만) + 게스트 홈 맨 아래 연결 손잡이.
  - ⬜ **PR-M2 미니앱 (B)** — 게스트 피드 박스가 뉴스 탭으로 열린다(소식·여백은 흐린 알약으로 잠금 유지). 서버 재설계 뒤.
