...
- **재참여 넛지 푸시 (7일 비활동 win-back)** ✅ 코드 완료 / 🔜 콘솔 검수·점등 대기 (2026-08-18) — **§전략 「retention
  ...
  **Flyway 0건**이고, 이메일이 훗날 점등돼도 10:00(이메일) → 19:00(푸시) 순서라 같은 날은 이메일이 선점한다.
  ...
  🔜 **점등 대기**: 기존 2종과 같은 다크런치 2중 게이트(`retention-enabled` 기본 OFF + 템플릿 코드 미설정).
  ✅ **SSM 2개는 선생성 완료**(2026-08-18, `TOSS_RETENTION_ENABLED=false` · `TOSS_RETENTION_TEMPLATE_CODE=placeholder`
  — #722의 초기값 규약 그대로). ⚠️ 이 선생성이 **머지 전 필수**인 이유: `render-env.sh` SECRET_MAP에 키가 있는데
  SSM에 없으면 배포가 `exit 1`로 죽는다(누락을 침묵시키지 않는 의도적 설계).
  남은 순서 = 콘솔 템플릿 생성·검수 신청(사용자, 발송 코드 제안 `booktimer-retention-nudge`) →
  승인 후 위 2개를 실값(`true` / 콘솔 발급 코드)으로 갱신·재배포.
  ⚠️ 열린 리스크: ① **기존 알림동의문 한 장이 이 캠페인까지 커버하는지 미확인** — 안 되면 미니앱 동의 UI 재호출이
  선결로 추가된다(콘솔 템플릿 생성 시 동의문 연결 항목에서 확인). ② **검수가 리마인더성 발송을 승인할지 미확인**
  — 축하형 2종과 성격이 다르다. 반려되면 코드째 다크런치로 잠들 뿐 무해하다. ③ 점등 검증은 7일 비활동 계정을
  인위로 만들 수 없어 경계는 H2 테스트가, 실발송은 첫 배치 로그 + 자연 발생 시 실기기 수신으로 사후 확인한다.
