...
| 일자 | 내용 |
|---|---|
| 2026-06-09 | **책BTI 출간연대 신호 제거(`pubDecades` C2 완전 제거)** — 알라딘 `pubDate`는 에디션 인쇄일, 고전 재판이 "2010년대 출간작"으로 오판 → LLM 확신형 오서술. `ReadingProfile`·`Aggregator`·`Narrator`·`ProfileSignature` 완전 제거. 시그니처 변경으로 기존 캐시 1회 재생성. DB·Flyway·화면 변경 없음. TDD Red→Green. (PR #264) |
