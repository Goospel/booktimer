...
> **상태**: 비전은 캡처·일부 진행 중. **트랙 A 1단계(MVP)는 출하 완료 ✅** (PR #332), **트랙 A 수집 확장(장르 매핑 식물)도 출하 완료 ✅** (아래 「장르 매핑 출하분」), **트랙 B 메커닉(숨은 레시피 발견)도 베타 출하 완료 ✅** (아래 「트랙 B 출하분」 — 레시피 큐레이션은 커스텀 카탈로그 단계로 연기), **도감 전용 페이지(`/garden`)도 출하 완료 ✅** (아래 「도감 전용 페이지 출하분」 — 대시보드는 요약+링크로 경량화, 잔디 흡수는 후속), **작가·출판사 다양성 축도 출하 완료 ✅ — 트랙 A 매핑 3축(장르·작가·출판사) 완성** (아래 「작가·출판사 다양성 출하분」). 드래그 커스텀은 여전히 미설계(프론트 전환 스코프). "열린 질문"은 *아직 정하지 않은* 것들(트랙 A·B로 일부 해소됨 — 데이터 모델·발견 저장·파밍 가드).
>
