...
### 📸 여백 (구 「독서 스토리」) — 읽다 남긴 문장·생각 (v1 출하 ✅ 2026-07-02 · 만료 폐지·개명 ✅ 2026-08-16, 정본 [sns-design.md §13](claude-docs/sns-design.md))
...
>   (`MAX_VISIBLE_STORIES`) — 진행바가 카드 수만큼 세그먼트를 그려 무제한 노출은 UI가 못 버틴다.
>   피드 쿼리는 전체 최신순 + `Pageable` 상한(200)으로 바뀌었고, 작성자 묶기·재생순 복원은 서비스가 한다.
...
> - ⏸ **남은 사각**: 상한 20장을 넘은 옛 여백은 DB에 있지만 **볼 길이 없다**(링 → 전체화면 뷰어 구조의
>   한계). 링 UI를 목록·격자로 바꾸는 후속에서 풀린다 — 사용자와 합의해 이번 범위에서 잘라 뒀다.
...
> *(↑ 만료 부분은 2026-08-16에 폐기됐다 — 링 UI는 유지.)*
...
- **독서 성향(책BTI) 추출 — 리워드 광고 관문** ✅ 코드 완료 / 🔜 실기기·배포 대기 (2026-08-15) — 내 책방은 성향을
...
- **독서 성향 보관함 — 히스토리 비교 + 대표 되돌리기** ✅ 코드 완료 / 🔜 배포 대기 (2026-08-15) — 위 광고 관문의
...
- **「소셜」 탭 → 「책방」 탭 개편** ✅ 코드 완료 / 🔜 배포 대기 (2026-08-15) — 내 책방까지 **두 탭**이었다
