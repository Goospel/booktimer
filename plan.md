...
| 2026-06-08 | **갱신 이력 표를 plan.md → claude-docs/changelog.md로 분리**(문서 경량화) — plan.md의 48%(110KB/140행)이던 갱신 이력 표를 sibling changelog.md로 통째 이관(삭제 0·서사 100% 보존), plan.md엔 본문 + 최근 10개 발췌만 남겨 통독 가능 크기로(약 58K→30K 토큰). CLAUDE.md sweep 규칙(Git 워크플로 4·작업 종료 최신화·다중세션 공유문서)의 갱신 이력 추가 대상을 changelog.md로 갱신. 문서만. (PR 예정) |
