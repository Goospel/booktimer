...
- [x] **홈이 열릴 모드 = 마지막에 있던 바** ✅ 2026-09-16 — 활성 키가 있는 **비홈** 페이지는 그 바의 모드를
      `#side-rails[data-remember]`로 내보내고 `rail.js`가 `booktimer.timerMode`에 적는다. 그래서 공부 페이지에서
      로고를 누르면 공부 타이머로 열린다. 중립 화면(설정·피드백·차단·수동 기록·남의 책방)은 활성 키가 없어
      **안 건드린다** — 무조건 쓰면 경로상 reading이라 「공부 모드 → 설정 → 로고 → 독서 타이머」로 튄다.
      진행 중 세션이 있으면 지금처럼 서버 진실(`effectiveMode`)이 저장값을 이긴다.
