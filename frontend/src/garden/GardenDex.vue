<template>
  ...
  <p v-if="loadError" style="text-align:center;padding:1rem">⚠️ 도감을 불러오지 못했어요.</p>
  <p v-else-if="!catalog" style="text-align:center;padding:1rem">🌱 도감 불러오는 중…</p>
...
onMounted(async () => {
  try {
    ...
    console.error('[GardenDex] API fetch failed:', e);
