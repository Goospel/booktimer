<template>
  <div class="village-shell">
    ...
    <template v-else>
      <!-- 모바일 세로(portrait): 경량 돌봄 뷰 (반응형 2모드 PoC) -->
      <PortraitVillage v-if="isPortrait" :data="data" :nickname="nickname"
...
// ⚠️ N-082: Phaser(GardenGame)는 이 분기 밖. PortraitVillage는 순수 Vue(Phaser 미사용).
const PORTRAIT_MQ = '(max-width: 899px) and (orientation: portrait)';
const isPortrait = ref(false);
let portraitMq: MediaQueryList | null = null;
function onPortraitChange(e: MediaQueryListEvent) { isPortrait.value = e.matches; }
