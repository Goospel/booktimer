// StudyState 정규화 — 서버 응답·옛 픽스처를 7필드로 채우는 단 하나의 문(설계 §2.4-정규화).
// (하루 목표 goalSeconds는 2026-09-13 컨셉 전환으로 타입에서 걷었다 — 서버 필드도 2026-09-14(#1132)에 걷혔다.)
//
// 계측기 메모
...
describe('studyStateOf', () => {
    test('없는 응답(undefined·null)은 IDLE_STUDY 7필드 그대로', () => {
    ...
    test('옛 서버·옛 픽스처(3필드)도 7필드로 채워진다 — 새 필드는 부재의 기본값', () => {
        const s = studyStateOf({ hasActiveSession: true, activeStartedAt: '2026-09-05T00:00:00Z', todaySeconds: 60 });
    ...
    test('넘어온 값이 기본값을 이긴다 — 7필드 전부(spread 순서 양성 대조군)', () => {
        const s = studyStateOf({
            hasActiveSession: true, activeStartedAt: '2026-09-05T01:00:00Z', todaySeconds: 900,
