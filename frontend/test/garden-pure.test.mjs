// garden.html @free-pure-core 블록을 정규식 추출 → new Function 평가.
// 단일 출처(garden.html 안 함수 그 자체)를 돌려 확인 — 사본 없음.
// 파일 경로: frontend/test/ → repo 루트 상대로 ../../src/main/...
import { readFileSync } from 'node:fs';
import { describe, test, expect } from 'vitest';

const HTML = new URL('../../src/main/resources/templates/garden.html', import.meta.url);
const src = readFileSync(HTML, 'utf8');
const m = src.match(/@free-pure-core:start\s*\*\/([\s\S]*?)\/\*\s*@free-pure-core:end/);
if (!m) throw new Error('FAIL: @free-pure-core 마커 블록을 찾지 못함 — garden.html에 마커가 없거나 제거됨');
...
describe('garden @free-pure-core', () => {
  ...
  // cellCenter — (col,row,cols,rows) → {x,y}
  describe('cellCenter', () => {
    test('(0,0) x = 0.025', () => {
      expect(cellCenter(0, 0, COLS, ROWS).x).toBeCloseTo(0.025);
    });
    test('(0,0) y = 0.03125', () => {
      expect(cellCenter(0, 0, COLS, ROWS).y).toBeCloseTo(0.03125);
    });
    test('(10,8) x ≈ 0.525', () => {
      expect(cellCenter(10, 8, COLS, ROWS).x).toBeCloseTo(0.525);
    });
    test('(10,8) y ≈ 0.53125', () => {
      expect(cellCenter(10, 8, COLS, ROWS).y).toBeCloseTo(0.53125);
    });
    // 전수 왕복 20×16 — cellOf(cellCenter(c,r)) == (c,r)
  ...
  // normToIso / isoToNorm — 왕복(내부 점 한정, clampNorm 적용)
  describe('normToIso / isoToNorm', () => {
    // 경계 0·1은 isoToNorm의 clampNorm이 끊어 왕복 보장 안 됨 → 내부 점으로
    const pairs = [
  ...
  // normToIsoPixel / isoPixelToNorm — 픽셀 왕복(내부 점 한정)
  ...
  describe('nearestFreeCell', () => {
    ...
    test('선호 셀 점유 → 이웃 셀 반환', () => {
      ...
      expect(result.col).not.toBe(5 && result.row !== 5 ? true : false);
    });
