...
describe('기록 상단 스탯 줄', () => {
  ...
  it('식물 성장 카드의 잔재가 없다 — 서버 응답에서 필드를 걷어낼 때(2단계) 화면이 undefined를 그리면 안 된다', () => {
    expect(markup).not.toContain('🌱');
    expect(markup).not.toContain('새싹');
    expect(markup).not.toContain('돼요');
    // 진행 막대 — 픽스처 진행률 33%가 화면 어디에도 남지 않는다.
    expect(markup).not.toContain('width:33%');
  });
});
