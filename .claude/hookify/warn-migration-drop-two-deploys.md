...
pattern: (?i)(alter\s+table\s+\S+\s[^;]*?\bdrop\s+(column\s+)?(?!(index|key|foreign|constraint|primary|check)\b)\w+|drop\s+table)
---
...
- 그 창 동안 옛 이미지의 엔티티가 없어진 컬럼을 SELECT → `Unknown column` → **인증이 걸린 모든 요청 500**. readiness는 DB를 보지 않아 옛 컨테이너가 healthy로 남고 Caddy가 안 뺀다.
