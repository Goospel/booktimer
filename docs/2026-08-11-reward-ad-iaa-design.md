...
- 콘솔 광고 그룹의 보상 표기(필수 입력): 이름 "밀린 하루 지우개", 수량 1.
  > ⚠️ **실제 등록값은 「밀린 하루 지우기 / 1」이다**(콘솔 실측 2026-08-23) — 이 설계안의 표기와 다르다.
  > 이 문서는 2026-08-11 시점 스냅샷이라 그대로 두되, **콘솔 값을 인용할 땐 여기 말고 `plan.md`를 본다**
  > (이 줄을 정본으로 믿고 옮겨 적었다가 문서 3곳에 틀린 값이 퍼진 적이 있다).
