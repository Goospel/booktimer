...
4. **PR 작성** — `gh pr create` 로 작성
   ...
   - **갱신 이력 항목을 PR 브랜치 안에 포함한다 (필수)** — 그 작업을
     ...
     2026-06-08 갱신 이력 표를 changelog.md로 분리 — plan.md 경량화. 2026-06-10 plan.md의 `최근 10개` 발췌도 폐지 — changelog.md를 단일 출처로, plan.md는 본문만 둔다.)
...
1. **갱신 이력 표** — 그 작업 한 줄(일자 / 한 일·PR 번호)을 [claude-docs/changelog.md](claude-docs/changelog.md) 맨 아래에 **해당 PR 브랜치 안에** 남긴다(plan.md엔 갱신 이력을 두지 않음 — changelog.md 단일 출처).
