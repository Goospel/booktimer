...
- **effort**: 사용자가 고지한 값을 그대로 쓴다 — 척도값(`high`·`medium`…)이든 모드(`ultracode`)든. effort는 내 컨텍스트에 안 들어오니 **추측하지 않는다**. **미고지면 세션당 한 번만 묻고**, 그래도 미정이면 `effort=unknown`으로 적는다(누락보다 명시적 미상이 낫다).
- **어디에**:
