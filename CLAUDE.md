...
- **엣지케이스·TDD 계획** — 경계 테스트로 무엇을 못 박을지. **계획의 TDD 절은 평소의 RED → GREEN 순서를 명시한다(필수)** —
  구현 세션이 ① 실패 테스트 먼저 → ② Red 확인 → ③ 최소 구현 → ④ Green 확인을 밟도록 못 박는다. 테스트 가능한 로직이 있으면
...
- **PR·머지 담당 = 구현 세션**: PR 생성 → (사용자 확인) → 머지 → 로컬 `main` 갱신·브랜치 정리까지 **구현 세션**이 맡는다(방금 diff를 만든 세션이 PR body·changelog를 가장 정확·싸게 쓴다). **예외**: 회귀 리스크 큰 작업만 머지 전 **독립 리뷰 패스** opt-in. (근거·트레이드오프는 [참조](claude-docs/claude-md-reference.md).)
