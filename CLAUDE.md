...
- **PR·머지 절차 = `operator`(opus·low)**: 커밋·PR 생성·머지·브랜치 정리 같은 절차는 operator에 위임한다. 단 **PR body·changelog 문안은 컨텍스트를 쥔 코디네이터 세션이 준다**(방금 무엇을 왜 바꿨는지 아는 쪽이 가장 정확·싸게 쓴다) — operator는 판단이 필요해지면 멈추고 보고한다. (근거·트레이드오프는 [참조](claude-docs/claude-md-reference.md).)
