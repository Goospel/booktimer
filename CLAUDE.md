...
- **effort**: 사용자가 고지한 값을 그대로 쓴다 — 척도값(`high`·`medium`…)이든 모드(`ultracode`)든. effort는 내 컨텍스트에 안 들어오니 **추측하지 않는다**. **미고지면 묻는다.** ⚠️ **`unknown`으로 자동 폴백하지 않는다** — effort를 기록해야 하는 시점(커밋 trailer·계획 md 상단)에 값이 미상이면, 비워두거나 `unknown`을 박지 말고 **그 자리에서 사용자에게 다시 물어 실제 값을 받고서 기록**한다. `unknown`은 **사용자가 명시적으로 "모름/미정"이라 답한 경우에만** 적는다(미응답 시 묵시적 폴백 금지). (배경: 새 세션이 시작 시 effort 미고지면 묻지 않고 `unknown`을 박아 추적값이 비던 사례 — 2026-06-27 규칙 강화. `unknown` 폴백을 "다시 묻기"로 대체.)
- **어디에**:
...
4. **PR 작성** — `gh pr create` 로 작성
   ...
   - **갱신 이력 항목을 PR 브랜치 안에 포함한다 (필수)** — 그 작업을
     ...
     **PR 번호는 changelog.md에 넣지 않는다** — 번호를 모른 채 커밋 후 PR 생성 → 번호 반영 추가 커밋
     순서가 되면 push가 2회(= CI 2회)가 된다. 번호 추적은 PR body·git log로 충분.
...
1. **갱신 이력 표** — 그 작업 한 줄(일자 / 한 일)을 [claude-docs/changelog.md](claude-docs/changelog.md) 맨 아래에 **해당 PR 브랜치 안에** 남긴다(plan.md엔 갱신 이력을 두지 않음 — changelog.md 단일 출처). PR 번호는 넣지 않는다(이유: Git 워크플로 4번 참고).
...
> **"이 작업으로 changelog.md 갱신 이력 + plan.md 본문이 실제와 일치하는가? 안 그러면 지금 고친다."**
...
- **자기가 서 있는 워크트리(cwd)를 절대 삭제하지 않는다.** 한 작업(PR 머지)이 끝나도 `remove-worktree` 스킬·스크립트·`git worktree remove`를 **현재 작업 중인** 워크트리에 쓰지 않는다 — 자기 발밑(cwd)이 사라지면 이후 모든 셸 명령이 "no such directory"로 실패하고 세션이 마비돼 **다음 지시에 무반응**이 된다(실제 재발: 머지 후 정리 단계에서 자기 워크트리를 지워 무반응 발생, 2026-06-30).
...
`defer`/`async`/`type=module`은 스크립트의 **실행 시점**을 바꿔, 같은 소스라도 로드 순서가 달라진다. 헤드리스 재현이 라이브러리를 **동기 로드**하면 순서가 뒤바뀌어 **버그가 사라진 가짜 green**이 나온다 — 단위테스트·POC는 통과하는데 실페이지만 깨진다. 실제로 #356~#358·#364(정원 Phaser 위젯)가 이 사각으로 반복해서 새어, **Chrome 확장으로 실페이지 콘솔을 직접 읽고서야** 잡혔다. (사용자 합의: 2026-06-15)

### 어떻게

- **진단**: 막히면 추측 전에 **실 배포/로컬 페이지에 Chrome 확장으로 붙어 콘솔·네트워크를 직접 읽는다.** "UI는 멀쩡한데 한 기능만 안 됨"(부분 실패)일수록 콘솔에 답이 있다.
- **재현 하니스**: 만든다면 **production의 `defer`/로드 순서·반응성 래핑까지 충실히 복제**한다 — 동기 로드로 단순화하면 그 버그를 못 잡는다(T-053).
- **게이트**: 로드순서·타이밍·반응성이 걸린 변경은 **머지 전 실 브라우저 1회 확인**을 게이트로 둔다. 순수 로직은 빠른 단위테스트(TDD), 클라이언트 통합은 실 브라우저 — 역할 분담.
...
- **재발 카운팅 = `troubleshooting.md` 상단 「🔁 재발·승격 트래커」 표로 한다 (필수).** 위 "2회 이상" 트리거를 모델이 알아채는 운에 안 맡기게: `T-###` 를 새로 쓸 때 **같은 트랩의 재발이면 ① 항목 끝에 `N회차(이전 T-### 재발)` 를 명시하고 ② 그 트래커 표의 회차·승격상태를 갱신**한다(신규 1회 트랩은 표에 안 올리고, 2회째가 되는 순간 군으로 올린다). 표에서 **2회+인데 미승격**이 보이면 그때 승격한다 — **승격은 prose 한 줄보다 하드픽스(훅·스크립트로 트랩 자체 제거)가 우선**. 답변에서도 재발이면 "이건 N회차" 를 짚는다. (없으면 카운팅이 암묵적 상호참조에 묻혀 승격 트리거를 놓친다 — 실제로 머지 hang(T-091→T-094)이 같은 날 2회째였는데 카운팅을 명시 못 하고 지나칠 뻔함. 사용자 합의: 2026-06-25.)
