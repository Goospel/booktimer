...
각 세션은 **자기 산출물에 자기 모델을 기록**한다 — 어느 모델이 낸 결과인지 사후 추적용. (사용자 합의: 2026-06-26 · **커밋 trailer의 effort는 2026-08-13 폐지**)

- **모델**: 환경 주입값에서 자동 인식해 채운다(예: `claude-opus-5`). 세션 중 바뀌면 그 시점 값.
- **어디에**:
...
- ⚠️ **커밋 trailer의 `Session-Effort`는 폐지했다 (2026-08-13)**: 이 규칙은 "한 세션 = 한 모델 = 한 effort"라 **커밋 하나가 곧 한 세션의 산출물**일 때 성립했다. 서브에이전트 분업(designer·implementer·reviewer·operator)이 기본이 된 지금은 커밋 하나에 여러 모델·effort가 섞여, 코디네이터 세션 값을 적으면 실제 산출자를 가리키지 않는 **틀린 기록**이 된다. 게다가 effort는 세션 컨텍스트에 안 들어와 **매 커밋 사용자에게 물어야 하는 마찰**이 있었다(모델은 자동 인식이라 공짜 — 그래서 모델만 남겼다). 서브에이전트별 모델 추적은 훅 `force-subagent-model.ps1`이 벨트 로그(`~/.claude/logs/subagent-model.jsonl`)에 **이미 자동 기록**한다. 분업한 작업의 역할 분담이 남길 가치가 있으면 **PR body에 자연어로** 적는다(「설계 designer / 구현 implementer」).
...
2. **작업 + 커밋** — 의미 단위로 커밋
   ...
   - **커밋 trailer에 세션 메타(`Session-Model`)를 남긴다** — 환경 주입값에서 자동 인식. `Session-Effort`는
     2026-08-13 폐지(분업으로 "한 커밋 = 한 effort" 전제가 깨짐 + 매 커밋 사용자 확인 마찰).
     상세는 「🗺️ 계획 우선 → 세션 메타 기록」 절 참조.
...
- **브라우저는 데스크톱 크롬이 기본** (사용자 지정 2026-08-13): 화면 확인·스크린샷은 `mcp__claude-in-chrome__*`(사용자 실 크롬)로 한다 — 클로드 내부 브라우저 패널은 사용자가 패널을 열어놔야만 스크린샷·클릭이 되고, 닫혀 있으면 "not compositing frames" 타임아웃이 난다. **단 dev 서버 기동(`preview_start`)은 내부 도구가 유일 경로**라 그것만 쓰고, 확인은 크롬 탭에서 한다 — 미니앱 목 모드는 `.claude/launch.json` 의 `miniapp-mock` 항목(`npm --prefix miniapp run dev:mock`). **포트는 그 항목이 정한다** — 이 PC는 5300이다(5174는 OS 예약 구간에 걸렸다, T-197). ⚠️ **`launch.json` 은 gitignore 대상(머신 로컬)이라 새 워크트리·새 PC엔 없다** — 없으면 그 항목을 다시 넣고 시작한다. ⚠️ vite는 `::1`에만 바인딩 — `127.0.0.1:<포트>`는 안 열리니 `localhost:<포트>`로.
- ⚠️ **로그인이 필요 없는 화면(목 모드·로컬)의 스크린샷·기기 모드는 `chrome-devtools` MCP가 1순위다 (2026-08-21 승격, T-199).**
  확장(`claude-in-chrome`)과 **다른 크롬·다른 CDP 연결**이라 확장 탭이 상해도(T-184) 내부 패널이 닫혀 있어도(T-154)
  영향을 안 받는다 — 두 경로가 동시에 막혀 그림을 끝내 못 얻었던 T-184 9회차의 그 화면을 **첫 시도에** 찍었다.
  **기기 모드는 F12 없이 명령으로 건다**(사용자에게 `Ctrl+Shift+M`을 부탁하던 절차는 폐지):
  `emulate` 에 `viewport: "390x844x3,mobile,touch"` → `navigate_page` → `take_snapshot`(uid를 준다) →
  `click`·`take_screenshot`. ⚠️ **`emulate`를 `navigate_page` 앞에 건다** — 뒤에 걸면 첫 페인트가 데스크톱으로 뜬다.
  ⚠️ **한계 — 이 크롬은 빈 프로필이라 아무 데도 로그인이 안 돼 있다.** 그래서 **인증이 걸린 화면(운영·실계정)엔 못 쓴다**:
  사용자 실 크롬(확장)이 그대로 기본이고, 스토어 스크린샷의 「실데이터는 사용자 폰이 유일 경로」 규칙도 그대로다.
  사용자와 **같은 화면을 보며** 이야기할 때도 확장 쪽이다.
- **진단**: 막히면 추측 전에 **실 배포/로컬 페이지에 Chrome 확장으로 붙어 콘솔·네트워크를 직접 읽는다.** "UI는 멀쩡한데 한 기능만 안 됨"일수록 콘솔에 답이 있다.
- ⚠️ **검증은 전용 탭을 새로 열어 시작한다 (3회 재발 승격, T-184).** 오래 재사용한 탭은 **CDP 세션만 상해서**
  `computer`의 클릭·스크린샷이 `Input.dispatchMouseEvent`/`Page.captureScreenshot` **30초 타임아웃**으로 죽는다
  (T-154 내부 패널 미개방 · T-179 크롬 기기 모드 · T-184 탭 장기 재사용 — 유발 조건은 매번 달랐다).
  에러가 「The renderer may be frozen」이라 앱 결함으로 오인하기 쉬운데, **같은 탭에서 `javascript_tool`의 JS와
  콘솔 읽기는 멀쩡히 돈다** — 죽는 건 CDP를 타는 경로뿐이라 판별 기준은 「읽기냐 입력이냐」가 아니라 **「CDP를
  타느냐」**다. 앱 코드를 뒤지기 전에 **`tabs_create_mcp`로 탭을 갈아타 1회 재시도**하는 것이 가장 싼 감별이다.
  ⚠️ **새 탭으로 시작해도 면역이 아니다 (4·5회차 실측)** — 세션 도중에도 상한다. 그래서 순서를 바꾼다:
  **판정에 필요한 수치는 `javascript_tool`로 먼저 뽑고 스크린샷은 나중에 찍는다.** 스크린샷은 사람에게 보여 주는
  용도이지 판정의 유일한 근거가 아니라, 순서만 바꿔도 탭이 상했을 때 잃는 것이 「그림」뿐이 된다.
  ⚠️ **갈아타도 안 풀릴 수 있다 (9회차 실측, 2026-08-20)** — 새 탭의 **첫 스크린샷부터** 죽었고, 내부 브라우저로
  넘어가니 그쪽은 T-154(패널 미개방)로 막혀 **두 경로가 동시에** 그림을 못 줬다. 그러니 규칙은 「갈아타면 풀린다」가
  아니라 **「한 번 갈아타 보고, 안 되면 다른 크롬으로 간다」**다 — 세 번째·네 번째 탭을 여는 것은 이 군에서 값을 한 적이 없다.
  ⚠️ **「포기」 앞에 한 계단이 생겼다 (2026-08-21, T-199)** — 위 `chrome-devtools` MCP는 별개 크롬·별개 CDP라 이 군에
  면역이다. 순서는 **확장에서 1회 탭 갈아타기 → `chrome-devtools` MCP → 그래도 안 되면 수치로 마감**. 9회차에서
  「그림을 포기한다」로 끝냈던 것은 이 계단을 몰라서였다. 그림 없이 끝낼 땐 **PR body에 수치 표를 넣고 「스크린샷 없음」을 명시**한다.
- ⚠️ **브라우저에서 이상값을 만나면 앱보다 먼저 「관측 경로」를 의심한다 (T-196·T-194·T-184 공통).** 특히
  **트랜지션을 건 속성은 판정 근거가 못 된다** — 배경으로 밀린 탭은 렌더링이 스로틀돼 진행 중인 CSS 트랜지션이
  얼고, `getComputedStyle`은 최종값이 아니라 **얼어붙은 보간값**을 돌려준다(클래스는 붙었는데 색은 옛것). 감별
  신호는 **값이 「일부만」 낡았다**는 비대칭이다 — 규칙이 안 먹은 것이면 전부 낡아야 하는데, 트랜지션을 안 타는
  커스텀 프로퍼티·`classList`는 새것이다. 스크린샷 1회로 페인트를 강제한 뒤 다시 재거나, 애초에 트랜지션 없는
  값으로 판정한다.
